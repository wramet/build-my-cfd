...
<details>
<summary>1. functionObject แต่ละตัวสามารถทำงานด้วยความถี่ที่แตกต่างกันได้อย่างไร (เช่น ตัวหนึ่งรันทุก Time Step แต่อีกตัวรันทุก 100 Time Steps)?</summary>

**คำตอบ:** เพราะ `functionObject` แต่ละตัวจะเก็บรักษาออบเจกต์ **`timeControl`** ของตัวเองเอาไว้ ซึ่งถูกจัดการโดย `functionObjectList` โดยก่อนที่จะเรียกเมธอด `execute()` ระบบจะทำการตรวจสอบเงื่อนไขเวลาของ `timeControl` ประจำตัวนั้นๆ ก่อนเสมอ
</details>

<details>
<summary>2. ในเมธอด `execute()` ของ functionObject เราจะเข้าถึงข้อมูลสนาม (Fields) เช่น ความเร็วหรือความดัน ได้อย่างปลอดภัยได้อย่างไร?</summary>

**คำตอบ:** โดยการใช้เมธอด **`mesh_.lookupObject<Type>(name)`** เพื่อดึง Reference ของ Field นั้นมาจากระบบ Registry กลาง และควรตรวจสอบก่อนว่ามี Field นั้นอยู่จริงหรือไม่ด้วยคำสั่ง **`mesh_.foundObject<Type>(name)`** เพื่อป้องกันโปรแกรมพัง (Crash)
</details>

## 📚 เอกสารที่เกี่ยวข้อง (Related Documents)

*   **ก่อนหน้า:** [03_Dynamic_Library_Loading.md](03_Dynamic_Library_Loading.md) - กลไกภายใน: การโหลดไลบรารีแบบไดนามิก
*   **ถัดไป:** [05_Design_Patterns.md](05_Design_Patterns.md) - รูปแบบการออกแบบ: เบื้องหลังความสามารถในการขยายของ OpenFOAM