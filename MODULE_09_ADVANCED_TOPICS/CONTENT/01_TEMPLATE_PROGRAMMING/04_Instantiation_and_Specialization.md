...
```mermaid
graph TD
    A[Generic Template<br/>magSqr&lt;Type&gt;] --> B{Type Check}
    B -->|scalar| C[Specialization&lt;scalar&gt;<br/>return s * s]
    B -->|vector| D[Specialization&lt;vector&gt;<br/>return v.x*v.x + v.y*v.y + v.z*v.z]
    B -->|tensor| E[Specialization&lt;tensor&gt;<br/>return sum of all components squared]

    C --> F[Optimized FPU Instruction]
    D --> G[SIMD Vectorization]
    E --> H[Cache-friendly Matrix Op]
```
