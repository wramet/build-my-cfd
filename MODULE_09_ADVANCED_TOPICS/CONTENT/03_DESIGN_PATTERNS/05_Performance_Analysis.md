...
```mermaid
graph TD
    A[Sequential Time] --> B[Parallel Execution]
    B --> C[Compute Time Processor 1]
    B --> D[Compute Time Processor 2]
    B --> E[Compute Time Processor N]
    C --> F[Max Compute Time]
    D --> F
    E --> F
    F --> G[Communication Overhead]
    G --> H[Actual Parallel Time]
    A --> I[Actual Speedup Calculation]
    H --> I
```
