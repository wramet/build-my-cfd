...
> [!TIP] **Physical Analogy: The Chef's Seasoning (การปิดรสชาติอาหาร)**
>
> การเลือก **Turbulent Dispersion Model** ก็เหมือนเชฟเลือก "เครื่องปรุง" (Seasoning) เพื่อกระจายรสชาติ (อนุภาค) ให้ทั่วหม้อ:
>
> 1.  **Burns Model:** เหมือน **"เกลือแกง" (Table Salt)** - สูตรมาตรฐาน (Standard), ใช้ง่าย (Simple), เข้าได้กับทุกเมนู (Robust), เป็นตัวเลือกแรกที่เชฟหยิบมาใช้
> 2.  **Bertodano Model:** เหมือน **"น้ำปลาพริก" (Seasoning with chunks)** - มีความเข้มข้นเฉพาะจุด (Local phase fraction dependency) เหมาะกับคนที่ชอบรสจัดจ้านในบางคำ (High void fraction gradients)
> 3.  **Simonin Model:** เหมือน **"เครื่องเทศเฉพาะทาง" (Specialty Spices)** - ซับซ้อนและละเอียดอ่อน ต้องเข้าใจวัตถุดิบ (Kinetic Theory) เหมาะกับเมนูพิเศษอย่างซุปข้นคลั่ก (Dense/Fluidized Beds)
>
> ถ้าเลือกผิด รสชาติ (การกระจายตัว) อาจจะจืดชืด (Under-predicted) หรือเค็มปี๋ (Over-predicted) จนกินไม่ได้!
...
| สามารถจับผลกระทบที่ไม่ใช่เฉพาะที่ (non-local effects) และผลของหน่วยความจำ (memory effects) | ต้องการวิธีการเชิงตัวเลขที่ซับซับซ้อน |
...
## 🧠 12. Concept Check (ทดสอบความเข้าใจ)

1.  **ความแตกต่างหลักระหว่างโมเดล "Burns" และ "Simonin" คืออะไร?**
    <details>
    <summary>เฉลย</summary>
    - **Burns:** เป็นโมเดล Gradient Diffusion พื้นฐานที่ใช้ $\mu_t$ และ $\alpha_{max}$ เหมาะสำหรับงานทั่วไป (General purpose).
    - **Simonin:** อิงตาม **Kinetic Theory** ของ Granular flow ใช้ Length scale และ $k$ เหมาะสำหรับระบบที่มีอนุภาคหนาแน่นหรือมีการชนกันของอนุภาค (Dense/Fluidized beds).
    </details>

2.  **ทำไมโมเดล Bertodano ถึงมีเทอม "Asymmetric" ($\frac{\alpha_k \alpha_l}{\max(\alpha_k, \alpha_l)}$)?**
    <details>
    <summary>เฉลย</summary>
    เพื่อจัดการกับความแตกต่างของพฤติกรรมระหว่างบริเวณที่เฟสหนึ่งมีมากเทียบกับอีกเฟสหนึ่ง (Phase dominance) ทำให้แรงกระจายตัวตอบสนองต่อการเปลี่ยนแปลงของ Void fraction ได้ดีกว่า โดยเฉพาะในระบบ Gas-Liquid ที่มีฟองอากาศแทรกตัว
    </details>

3.  **ถ้าผลการจำลองพบว่า "อนุภาคตกตะกอนเร็วเกินไปและไม่กระจายตัวออกด้านข้าง" (Under-predicted spreading) เราควรปรับค่า $C_{TD}$ หรือ $\sigma_{TD}$ อย่างไร?**
    <details>
    <summary>เฉลย</summary>
    ควร **เพิ่ม $C_{TD}$** (เพิ่มแรง) หรือ **ลด $\sigma_{TD}$** (เพิ่ม Diffusivity เพราะ $D \propto 1/\sigma$) เพื่อให้อนุภาคกระจายตัวมากขึ้นตามกระแส Turbulent eddies
    </details>