...
> [!TIP] **Physical Analogy: The Negotiators (การเจรจาต่อรอง)**
>
> ลองเปรียบเทียบ PIMPLE Loop เหมือนการเจรจาระหว่างสองฝ่ายเพื่อหาข้อตกลง:
>
> 1.  **Momentum Predictor (ฝ่ายขาย):** "ฉันอยากขายของราคานี้ (Velocity)" (คำนวณความเร็วจากแรงต่างๆ)
> 2.  **Pressure Corrector (ฝ่ายบัญชี):** "ไม่ได้! เงินไม่พอ (Mass Conservation Violation)" (แก้ความดันเพื่อให้มวลไหลเข้าเท่ากับไหลออก)
> 3.  **Iterate (การต่อรอง):** ฝ่ายขายปรับราคาใหม่ -> ฝ่ายบัญชีตรวจสอบใหม่ -> ทำซ้ำไปเรื่อยๆ
> 4.  **Convergence (ข้อตกลง):** เมื่อทั้งสองฝ่ายพอใจ (Residuals ต่ำพอ) ก็เซ็นสัญญาแล้วขยับไปเดือนถัดไป (Next Time Step)
...
1. **ทำไมเราถึงต้องใช้ `fvm::Sp` แทน `fvm::Su` ในบางครั้ง?**
   <details>
   <summary>เฉลย</summary>
   `fvm::Sp` คือ Implicit Source Term ซึ่งจะไปเพิ่มค่าในแนวทแยงมุม (Diagonal) ของ Matrix ทำให้ระบมี "Diagonal Dominance" มากขึ้น ซึ่งช่วยให้ Matrix Solver ลู่เข้า (Converge) ได้ง่ายขึ้นและเสถียรขึ้น โดยเฉพาะเมื่อเทอม Source มีค่าเป็นลบ
   </details>

2. **ค่า Courant Number (Co) > 1 ใน multiphaseEulerFoam ยอมรับได้หรือไม่?**
   <details>
   <summary>เฉลย</summary>
   โดยทั่วไป **ไม่แนะนำ** แม้ PIMPLE จะรองรับ Co > 1 ได้ในทางทฤษฎี แต่สำหรับ Multiphase Flow ที่มีความซับซ้อนของ Interface และ Coupling ที่รุนแรง การใช้ Co > 0.5-0.8 มักจะทำให้ผลลัพธ์แกว่งหรือไม่แม่นยำ (Instability) โดยเฉพาะสมการ Alpha ที่ไวมาก
   </details>

3. **อะไรคือความแตกต่างระหว่าง `fixedValue` และ `fixedMean` ที่ boundary?**
   <details>
   <summary>เฉลย</summary>
   - **fixedValue:** บังคับค่าที่ขอบให้เป็นค่านั้นเป๊ะๆ ตลอดเวลา (Hard constraint) แต่อาจเกิดการสะท้อนของคลื่นความดันกลับเข้ามา
   - **fixedMean:** ยอมให้ค่าที่ขอบแกว่งไปมาได้ แต่รักษาค่าเฉลี่ยให้ได้ตามที่กำหนด (Soft constraint) ช่วยลดการสะท้อนกลับ (Wave reflection) ที่ทางออกได้ดีกว่า
   </details>