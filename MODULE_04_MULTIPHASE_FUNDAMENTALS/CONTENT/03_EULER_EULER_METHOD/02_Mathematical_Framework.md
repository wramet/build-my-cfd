...
> [!TIP] **Physical Analogy: The Mathematical Blender (เครื่องปั่นทางคณิตศาสตร์)**
>
> ลองนึกภาพคุณกำลังทำน้ำปั่นผลไม้รวม (Fruit Smoothie):
>
> 1.  **Microscopic (ความจริง):** ก่อนปั่น คุณมีชิ้นกล้วย (Phase A) และสตรอว์เบอร์รี (Phase B) เป็นชิ้นๆ แยกกันชัดเจน
> 2.  **Averaging (การปั่น):** เมื่อคุณกดปั่น (Volume Averaging) คุณกำลังเปลี่ยนชิ้นผลไม้ให้กลายเป็น "เนื้อเดียว" (Continuum)
> 3.  **Result (Eulerian Field):** ในแก้วน้ำปั่น คุณไม่สามารถชี้ได้ว่า "ตรงนี้คือกล้วย 100%" แต่คุณบอกได้ว่า "ณ จุดนี้ มีความเข้มข้นของกล้วย 40% และสตรอว์เบอร์รี 60%" ($\alpha_{banana}=0.4, \alpha_{strawberry}=0.6$)
>
> E-E Model คือการมองของไหลแบบ "ปั่นละเอียด" แล้ว แต่ยังคงแยกชนิด (Species) ของน้ำผลไม้ได้อยู่ ไม่ได้รวมเป็นเนื้อเดียวสมบูรณ์เหมือนสารละลาย (Solution)
...
1. **ทำไมสมการโมเมนตัมถึงซับซ้อนกว่า Single Phase มาก?**
   <details>
   <summary>เฉลย</summary>
   เพราะมีเทอม $\mathbf{M}_k$ (Interfacial Transfer) ที่ต้องคำนวณแรงที่เฟสหนึ่งกระทำต่ออีกเฟสหนึ่ง (Drag, Lift, Virtual Mass) ซึ่งต้องใช้ Empirical Correlation (สูตรจากการทดลอง) มาช่วยปิดสมการ และเทอมนี้มักจะทำให้สมการ Unstable ได้ง่าย
   </details>

2. **MULES (Multidimensional Universal Limiter with Explicit Solution) มีไว้ทำไม?**
   <details>
   <summary>เฉลย</summary>
   มีไว้เพื่อ "บังคับ" ให้ค่า Volume Fraction ($\alpha$) อยู่ในช่วง 0 ถึง 1 เสมอ (Boundedness) เพราะการคำนวณแบบปกติอาจเกิด Numerical Error ทำให้ค่าหลุดช่วง (เช่น -0.1 หรือ 1.2) ซึ่งเป็นไปไม่ได้ทางฟิสิกส์
   </details>

3. **เทอม "Virtual Mass Force" คืออะไร? จำเป็นต้องใช้เมื่อไหร่?**
   <details>
   <summary>เฉลย</summary>
   คือแรงต้านที่เกิดจากการที่อนุภาคต้อง "เบียด" ของไหลรอบๆ ให้พ้นทางเมื่อมันเร่งความเร็ว (เหมือนเรารู้สึกหนักขึ้นเมื่อเดินลุยน้ำ) จำเป็นต้องใช้เมื่อ Density ของสองเฟสต่างกันไม่มาก (เช่น น้ำในน้ำมัน) หรือเมื่อมีการเร่งความเร็วสูงๆ (Unsteady flows) แต่ถ้าเป็น อากาศในน้ำ (Density ต่างกันมาก) มักจะละเลยได้ในบางกรณีเพราะผลน้อยเมื่อเทียบกับ Drag
   </details>