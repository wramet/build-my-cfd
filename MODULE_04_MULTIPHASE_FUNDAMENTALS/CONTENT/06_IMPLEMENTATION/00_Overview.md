...
> [!TIP] **Physical Analogy: The Orchestra Conductor (วาทยากรวงออร์เคสตรา)**
>
> ลองนึกภาพ Solver `multiphaseEulerFoam` เป็นเหมือน **"ผู้ควบคุมการก่อสร้าง"** (Construction Manager):
>
> 1.  **Phase Managers (`phaseModel`):** หัวหน้าคนงานแต่ละทีม (น้ำ, อากาศ) คอยดูแลงบประมาณและทรัพยากร (Mass, Momentum, Energy) ของทีมตัวเอง
> 2.  **Central Hub (`phaseSystem`):** ห้องบัญชาการกลาง ที่คอยประสานงานระหว่างทีม "เฮ้ย! ทีมน้ำส่งปูน (Momentum Transfer) ให้ทีมอากาศหน่อย"
> 3.  **Solver Algorithm (`PIMPLE`):** ตารางงานรายวัน (Daily Schedule) ที่วนลูปตรวจสอบงาน:
>     -   *Predictor:* "วางแผนคร่าวๆ ว่าวันนี้จะสร้างถึงไหน"
>     -   *Corrector:* "ตรวจสอบหน้างานจริง ถ้าไม่ตรงแก้ใหม่" (แก้ Pressure/Velocity)
>     -   *Iterate:* "ทำซ้ำจนกว่าจะเป๊ะตามแบบ" (Convergence)
...
> 
> **แนวคิดสำคัญ:** การแยกส่วนการตั้งค่า (initialization) ออกจากการ logic หลักของ solver ทำให้โค้ดเป็นระเบียบและอ่านง่าย
...
1.  **คลาส `phaseSystem` มีบทบาทสำคัญอย่างไรในสถาปัตยกรรมนี้?**
    <details>
    <summary>เฉลย</summary>
    เป็น **Central Hub** ที่ทำหน้าที่จัดการภาพรวมของระบบหลายเฟส เช่น เก็บรายการของเฟสทั้งหมด, จัดการตาราง Interfacial Transfer (Drag, Heat Transfer), และประสานงานการอัปเดตค่าต่างๆ ระหว่างเฟส
    </details>

2.  **ทำไม Multiphase Solver ถึงนิยมใช้ PIMPLE (Hybrid PISO-SIMPLE) มากกว่า PISO ปกติ?**
    <details>
    <summary>เฉลย</summary>
    เพราะระบบ Multiphase มีการ Coupling ระหว่างเฟสที่ซับซ้อนและรุนแรงมาก การใช้ **PIMPLE** ช่วยให้เราสามารถวนลูปแก้สมการ (Outer Correctors) และใช้ Under-relaxation เพื่อเพิ่มความเสถียร (Stability) ทำให้สามารถใช้ Time step ที่ใหญ่ขึ้นได้
    </details>

3.  **ถ้าต้องการเพิ่มโมเดล Drag ใหม่ ต้องไปดูที่ส่วนไหนของ Source Code?**
    <details>
    <summary>เฉลย</summary>
    ดูที่ `src/phaseSystem/interfacialModels/dragModels` (สำหรับ OpenFOAM รุ่นใหม่ๆ) หรือ `src/transportModels/multiphase/interfaceModels` โดยต้องสร้างคลาสใหม่ที่สืบทอดมาจาก base class `dragModel` และ implement ฟังก์ชัน `K()` หรือ `F()`
    </details>
