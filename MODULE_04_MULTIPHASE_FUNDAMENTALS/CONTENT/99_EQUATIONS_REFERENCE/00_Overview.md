...
> [!TIP] **Physical Analogy: The Rules of the Traffic (กฎจราจร)**
>
> สมการเหล่านี้เหมือนกับ "กฎจราจร" ที่ควบคุมการเคลื่อนที่ของรถยนต์ (Fluid Parcels):
>
> 1.  **Mass Conservation (ห้ามวาร์ป):** รถยนต์ห้ามจู่ๆ โผล่มาหรือหายไป ถ้ามีรถเข้าทางด่วน 100 คัน ก็ต้องออก 100 คัน (เว้นแต่จะเปลี่ยนเลน/Phase Change)
> 2.  **Momentum Conservation (F=ma):** รถจะเร่งหรือเบรกได้ต้องมีแรงกระทำ (Engine/Pressure, Brakes/Drag, Collision/Interfacial Force)
>     -   ใน Multiphase, ถ้า "รถบรรทุก" (Water) ชน "มอเตอร์ไซค์" (Air) จะเกิดแรงกระแทกระหว่างกัน ($\mathbf{M}_k$)
> 3.  **Volume Fraction ($\sum \alpha = 1$):** ถนนต้องเต็มไปด้วยรถหรืออากาศเสมอ ห้ามมี "หลุมดำ" (Vacuum) บนถนน
...
1.  **ในสมการ Momentum ของ Multiphase มีเทอม $\mathbf{M}_k$ เพิ่มขึ้นมาจาก Single-phase เทอมนี้คืออะไร?**
    <details>
    <summary>เฉลย</summary>
    คือ **Interfacial Momentum Transfer** ซึ่งรวมแรงต่างๆ ที่เฟสหนึ่งกระทำต่ออีกเฟสหนึ่ง เช่น Drag, Lift, Virtual Mass, และ Turbulent Dispersion แรงนี้เป็นหัวใจสำคัญที่ทำให้เฟสต่างๆ แลกเปลี่ยนพลังงานกัน
    </details>

2.  **ในสมการ VOF (Eq. 42) เทอม $\nabla \cdot (\alpha (1-\alpha) \mathbf{u}_r)$ มีหน้าที่อะไร?**
    <details>
    <summary>เฉลย</summary>
    นี่คือเทอม **Interface Compression** ซึ่งเป็นเอกลักษณ์ของ OpenFOAM (MULES solver) เทอมนี้จะทำงานเฉพาะบริเวณ Interface ($0 < \alpha < 1$) เพื่อ "บีบ" ให้ Interface คมชัดขึ้น ต้านทานการเบลอ (Numerical Diffusion)
    </details>

3.  **ถ้า $\alpha_k \to 0$ (เฟสหายไปในบางเซลล์) จะเกิดปัญหาอะไรกับสมการ Momentum?**
    <details>
    <summary>เฉลย</summary>
    สมการจะกลายเป็น Singular (หารด้วยศูนย์) เพราะเทอม $\alpha_k$ เป็นตัวคูณอยู่หน้าเทอมเวลาและเทอม Convection ในทาง Numerical เรามักเลี่ยงปัญหานี้ด้วยการแก้สมการแบบ "Phase-Intensive" หรือการกำหนดค่าต่ำสุด ($\alpha_{min}$) เพื่อไม่ให้เป็นศูนย์สนิท
    </details>