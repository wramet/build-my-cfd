...
> [!TIP] **Physical Analogy: The Digital Photo Resolution (ความละเอียดภาพถ่ายดิจิทัล)**
>
> ลองเปรียบเทียบ Mesh กับ **Pixles** ของรูปภาพ:
>
> 1.  **Coarse Mesh (Low-Res):** ภาพแตกเป็นสี่เหลี่ยมเบลอๆ (Pixelated)
>     - เราพอดูออกว่าเป็น "แมว" แต่ไม่เห็นหนวด หรือลายขน
>     - *ผลลัพธ์ไม่แม่นยำ รายละเอียดหายไปเยอะ*
>
> 2.  **Fine Mesh (High-Res):** ภาพคมชัดระดับ HD (1080p)
>     - เห็นหนวดแมวชัดเจน
>     - *ผลลัพธ์แม่นยำ จับ Physics ได้ครบถ้วน*
>
> 3.  **Mesh Independence (Ultra HD vs 8K):**
>     - ถามว่า **"จำเป็นต้องอัพเกรดเป็น 8K ไหม?"**
>     - ถ้าตาเราแยกความแตกต่างระหว่าง 4K กับ 8K ไม่ได้แล้ว (ผลลัพธ์ไม่เปลี่ยน) ก็ไม่มีประโยชน์ที่จะเสียเงินซื้อกล้อง 8K (เปลือง Cost/Compute Time โดยใช่เหตุ)
>     - จุดที่ **"เพิ่มความละเอียดไป ผลลัพธ์ก็เท่าเดิม"** นั่นแหละคือ **Mesh Independence!**
...
**หัวข้อถัดไป**: [การตรวจสอบความถูกต้องโดยการทดลอง](./03_Experimental_Validation.md)

---

## 🧠 5. Concept Check (ทดสอบความเข้าใจ)

1. **ทำไมเราถึงต้องใช้ Mesh อย่างน้อย 3 ระดับ (Coarse, Medium, Fine)? ใช้แค่ 2 ระดับได้ไหม?**
   <details>
   <summary>เฉลย</summary>
   **ไม่ได้** ถ้าจะคำนวณ GCI อย่างถูกต้อง เพราะเราต้องหา **Order of Convergence ($p$)** ซึ่งเป็นตัวแปรที่ไม่รู้ค่า (Unknown) ต้องใช้สมการจากผลต่างของ 3 จุด (Fine-Medium และ Medium-Coarse) ในการหาค่า $p$ ถ้าใช้แค่ 2 จุด เราต้อง "เดา" ค่า $p$ เอาเอง (เช่น สมมติว่าเป็น 2) ซึ่งอาจจะไม่ตรงกับความจริง
   </details>

2. **ถ้า GCI สูง (เช่น 10%) แปลว่าผลลัพธ์ของเราผิด (Error) ใช่หรือไม่?**
   <details>
   <summary>เฉลย</summary>
   **ไม่ใช่ว่าผิด (Correction error)** แต่แปลว่ามีความ **ไม่แน่นอน (Uncertainty)** สูงจากการแบ่ง Mesh หมายความว่าถ้าเราทำ Mesh ให้ละเอียดขึ้นอีก ผลลัพธ์ก็มีโอกาสเปลี่ยนไปได้อีกถึง 10% คือยังเชื่อถือไม่ได้เต็มร้อย ยังไม่ถึงจุด Mesh Independence
   </details>

3. **ทำไม Asymptotic Range ถึงสำคัญ? (ratio $\approx$ 1)**
   <details>
   <summary>เฉลย</summary>
   เพื่อยืนยันว่า Solver ของเรากำลังเข้าสู่ช่วงที่ความคลาดเคลื่อนลดลงอย่างเป็นระเบียบตามทฤษฎี (Asymptotic convergence) ถ้า Ratio กระโดดไปมา หรือห่างจาก 1 มากๆ แปลว่า Mesh ของเราอาจจะยังหยาบเกินไป หรืออยู่ในช่วงที่พฤติกรรมของ Flow เปลี่ยนแปลงแบบคาดเดาไม่ได้ (Pre-asymptotic) ผล GCI ที่คำนวณได้อาจจะเชื่อถือไม่ได้
   </details>
