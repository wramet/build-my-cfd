...
- **Calculate** and interpret error metrics ($L_1$,1$L_2$,1$R^2$)
...
Where:
-1$f_{exp}1= experimental measurement (ค่าที่วัดได้จากการทดลอง)
-1$f_{CFD}1= CFD prediction (ค่าที่ทำนายได้จาก CFD)
-1$\varepsilon_{model}1= model error (ความคลาดเคลื่อนของโมเดล)
...
- `UPrime2Mean` — Reynolds stress tensor1$\overline{u'_i u'_j}$
- `pMean` — Mean pressure
...
####1$L_11Norm (Mean Absolute Error)
...
**Interpretation:**
-1$R^2 = 1$: Perfect correlation
-1$R^2 = 0$: No better than mean
-1$R^2 < 0$: Model is worse than mean
...
**Caution:** Avoid when1$y_{exp} \approx 0$
...
| Application |1$R^21| MAPE |1$L_21(normalized) | Notes |
|-------------|-------|------|--------------------|-------|
...
<details>
<summary><b>1.1$L_11กับ1$L_\infty1norm บอกอะไรต่างกัน?</b></summary>

**คำตอบ:**

- **$L_11(Mean Absolute Error)**: ค่าเฉลี่ยความคลาดเคลื่อน "โดยรวม"
  - ดู performance เฉลี่ยของทั้งกราฟ
  - ไม่ sensitive ต่อ outliers
  - เหมาะสำหรับ general performance assessment

- **$L_\infty1(Maximum Error)**: ค่าความคลาดเคลื่อน "สูงสุด" ที่จุดใดจุดหนึ่ง
  - ดู worst-case scenario
  - **สำคัญมาก** สำหรับ safety-critical applications
  - บอกว่ามีจุดที่ผิดพลาดมากแค่ไหน

**ตัวอย่าง:** ถ้า1$L_1 = 0.021แต่1$L_\infty = 0.151→ เฉลี่ยดี แต่มีจุดที่ผิดพลาดมาก
</details>
...
-1$\bar{U}1= mean component (สิ่งที่เปรียบเทียบได้)
-1$u'1= fluctuating component (เปลี่ยนไปตามเวลา)
...
|-------------|------------------|
| Preliminary design |1$R^2 > 0.85$, MAPE < 20% |
| Engineering design |1$R^2 > 0.90$, MAPE < 15% |
| Research publication |1$R^2 > 0.95$, MAPE < 10% |
...
4. **Validate on multiple metrics**
   - Not just1$R^21→ check shape, gradients, extrema
