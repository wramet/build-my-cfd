...
- **ตรวจสอบความละเอียดของ mesh บนผนัง** ด้วยค่า $y^+$ ที่เหมาะสม
...
**1. Run Three Meshes:**
- Fine mesh: $N_1$ cells
- Medium mesh: $N_2 = N_1/r^3$
- Coarse mesh: $N_3 = N_2/r^3$
...
- $r$ = refinement ratio (typically $\sqrt[3]{2}$ for 3D)
- $f$ = key variable (e.g., drag coefficient, Nusselt number)
...
> **⚠️ Common Pitfall:** ไม่ตรวจสอบ asymptotic range — ให้แน่ใจว่า $p$ ค่าใกล้เคียง theoretical order (2 for second-order schemes)
...
**Where:**
- $y$ = distance to first cell center
...
| Model | Target $y^+$ | Near-Wall Treatment |
|-------|-------------|---------------------|
...
| `postProcess -func yPlus` | Wall resolution check | $y^+$ field |
| `postProcess -func wallShearStress` | Wall shear stress | $\tau_w$ field |
...
| **Channel flow** | Wall turbulence | `incompressible/pisoFoam/channel395` | $u^+$ vs $y^+$ |
...
✅ **$y^+$ เป็นตัวชี้วัดความละเอียดผนัง:**
...
$L_2$ ให้น้ำหนักกับ errors ขนาดใหญ่มากกว่า ทำให้เห็นจุดที่มีปัญหาชัดเจนกว่า และมีความสัมพันธ์กับ energy norm ในหลายระบบ ทำให้เหมาะสำหรับการตรวจสอบความแม่นยำของ solutions
</details>
...
<details>
<summary><b>4. $y^+$ คืออะไร และทำไมต้องเช็ค?</b></summary>

$y^+$ คือ dimensionless distance จากผนังถึงจุดกลางของเซลล์แรก ต้องตรวจสอบเพื่อให้แน่ใจว่า mesh ละเอียดพอสำหรับ turbulence model ที่ใช้ — ถือผิดจาก target range จะทำให้ผลลัพธ์ไม่แม่นยำ
</details>
