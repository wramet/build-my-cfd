...
โดยที่:
-1$|u|1= ความเร็วสูงสุดใน domain
-1$\Delta t1= time step
-1$\Delta x1= cell size ที่เล็กที่สุด
...
ทุก equation ใน CFD ถูกแปลงเป็น **linear system**1$Ax=b1— `fvSolution` บอกว่าใช้ solver ไหนแกะมัน
...
|--------|------|------|----------|
| **GAMG** |1$O(N)1— very fast for large meshes | Memory overhead | Large meshes (>100k cells), steady-state |
| PCG | Robust, simple |1$O(N^{1.5})1— slower | Small meshes, debugging |
...
$k$,1$\epsilon$,1$\omega1— สมการ scalar ที่มี source terms แรง และ convection-dominated
...
- Pressure correction1$\Delta p1มากเกินไป → velocity เปลี่ยนแรงเกิน → diverge
...
**GAMG:** Multigrid — แก้ low-frequency errors บน coarse grids (where they look high-frequency) → ลด global errors ได้เร็วมาก →1$O(N)1vs1$O(N^{1.5})$
