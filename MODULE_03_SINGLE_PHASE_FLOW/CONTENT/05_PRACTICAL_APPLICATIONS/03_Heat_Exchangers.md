...
- **Temperature continuity:** $T_{fluid} = T_{solid}$ ที่ interface
- **Heat flux continuity:** $q_{fluid} = q_{solid}$ ที่ interface
- **Conservation of energy:** $\dot{Q}_{hot} = \dot{Q}_{cold}$ (ใน steady state)
...
โดยที่:
- $C_{min} = \min(\dot{m}_h c_{p,h}, \dot{m}_c c_{p,c})$ — minimum heat capacity rate
- $\varepsilon \in [0, 1]$ — 1 คือ ideal heat exchanger
...
1. **Conjugate Heat Transfer:** Heat conduction ใน solid walls ร่วมกับ convection ใน fluid
   - Wall conduction resistance: $R_{wall} = \frac{t_w}{k_w A_w}$
   - Fluid convection resistance: $R_{fluid} = \frac{1}{h A}$
...
- **Temperature matching:** $T_{fluid} = T_{solid}$ ที่ interface
- **Heat flux continuity:** $q = -k_{fluid}\nabla T_{fluid} = -k_{solid}\nabla T_{solid}$
...
- $C_F$ = inertial resistance coefficient
- **Darcy coefficient (linear):** $D = \frac{\mu}{\alpha}$
- **Forchheimer coefficient (quadratic):** $F = \frac{C_F \rho}{\sqrt{\alpha}}$

**Calibration Procedure:**

1. **Measure pressure drop vs flow rate** จาก experiment หรือ detailed CFD
2. **Fit to quadratic equation:** $\Delta p = A \dot{m} + B \dot{m}^2$
3. **Extract coefficients:**
   - $D = \frac{A \cdot A_{flow}}{\mu}$
   - $F = \frac{B \cdot A_{flow}^2}{\rho}$

เมื่อ $A_{flow}$ = cross-sectional area
...
เมื่อ:
- $\dot{Q}_{hot} = \dot{m}_h c_{p,h} (T_{h,in} - T_{h,out})$  — Heat lost by hot fluid
- $\dot{Q}_{cold} = \dot{m}_c c_{p,c} (T_{c,out} - T_{c,in})$  — Heat gained by cold fluid
...
เมื่อ:
- $h_h, h_c$ = heat transfer coefficients สำหรับ hot/cold fluids
- $t_w$ = wall thickness
- $k_w$ = wall thermal conductivity
- $A_h, A_w, A_c$ = surface areas

**Procedure:**
1. Extract $h$ จาก CFD: $h = \frac{q_{wall}}{T_{wall} - T_{bulk}}$
2. Calculate $U$ จาก equation ข้างบน
3. Compare $Q_{theory} = U A \Delta T_{LMTD}$ กับ $Q_{CFD}$
...
เมื่อ:
- $f = (0.79 \ln Re - 1.64)^{-2}$ — friction factor
...
- **Boundary layer:** Resolve viscous sublayer ด้วย $y^+ \approx 1$
...
- **NTU Method:** Effectiveness-NTU method เป็นเครื่องมือสำคัญในการประเมิน performance: $\varepsilon = Q_{actual}/Q_{max}$, $NTU = UA/C_{min}$ โดยไม่ต้องทราบ outlet temperature ล่วงหน้า ทำให้เหมาะสำหรับ design และ optimization โดยมีความสัมพันธ์กับ flow arrangement (counter-flow, parallel-flow, cross-flow)
...
- **Validation Criteria:** การตรวจสอบความถูกต้องต้องผ่าน 3 เกณฑ์: (1) Energy balance error < 5% — heat ที่เสียไปจาก hot fluid ต้องเท่ากับ heat ที่ cold fluid ได้รับ, (2) LMTD method error < 10% — compare $Q_{CFD}$ กับ $Q_{theory} = U A \Delta T_{LMTD}$, (3) Nusselt number ต้อง match กับ empirical correlations (Dittus-Boelter, Gnielinski) ภายใน 20%

- **Mesh Requirements:** $y^+ \approx 1$ สำคัญมากสำหรับ heat transfer applications เพื่อ capture temperature gradient ใน viscous sublayer ควรมี 5-10 layer cells ที่ fluid-solid interface และ cell size ratio ระหว่าง regions < 5:1 เพื่อ prevent interpolation errors และ maintain flux continuity
...
- **Heat Exchanger Performance:** Effectiveness ($\varepsilon$) และ NTU เป็น parameters สำคัญในการประเมิน performance โดย NTU = UA/C_min แสดงถึง size ของ heat exchanger และ $\varepsilon$ แสดงถึงประสิทธิภาพในการถ่ายเทความร้อน เมื่อเปรียบเทียบกับ maximum possible heat transfer ($Q_{max} = C_{min} \Delta T_{max}$)
...
2. **Heat flux continuity:**
   $$q_{fluid} = q_{solid}$$
   $$-k_{fluid} \left(\frac{\partial T}{\partial n}\right)_{fluid} = -k_{solid} \left(\frac{\partial T}{\partial n}\right)_{solid}$$
...
<details>
<summary><b>3. ทำไม $y^+$ ใกล้ 1 จึงสำคัญสำหรับ heat transfer simulations?</b></summary>

Temperature gradient สูงสุดอยู่ใน **viscous sublayer** (buffer layer) ใกล้ผนัง:

- **Physics:** สำหรับ flow over heated wall: Temperature หายไปอย่างรวดเร็วจาก $T_w$ ถึง $T_{\infty}$ ในบริเวณนี้
- **Numerical issue:** ถ้า $y^+$ สูงเกินไป (mesh หยาบเกินไป): จะ miss gradient นี้ → underestimate heat flux
- **Consequence:** Heat transfer coefficient จะผิดพลาด 20-50% ถ้า $y^+ > 30$
...
**2. LMTD Method (design validation):**
   Compare $Q_{CFD}$ กับ $Q_{theory} = U A \Delta T_{LMTD}$
   $$Error = \left|\frac{Q_{CFD} - Q_{theory}}{Q_{theory}}\right| < 10\%$$
   - Calculate $U$ จาก: $\frac{1}{UA} = \frac{1}{h_h A_h} + \frac{t_w}{k_w A_w} + \frac{1}{h_c A_c}$
   - Extract $h$ จาก CFD: $h = \frac{q_{wall}}{T_{wall} - T_{bulk}}$

**3. Nusselt Number (physics validation):**
   Compare $Nu_{CFD} = \frac{h L}{k}$ กับ correlations:
   - Dittus-Boelter: $Nu = 0.023 Re^{0.8} Pr^{0.4}$ (heating)
   - Gnielinski: $Nu = \frac{(f/8)(Re - 1000)Pr}{1 + 12.7\sqrt{f/8}(Pr^{2/3} - 1)}$ (more accurate)
   $$\left|\frac{Nu_{CFD} - Nu_{corr}}{Nu_{corr}}\right| < 20\%$$
...
เมื่อ $C_r = C_{min}/C_{max}$ — heat capacity ratio
...
2. **Compare กับ theory:**
   - Calculate NTU จาก geometry ($U$, $A$)
   - Verify $\varepsilon$ matches theoretical value ภายใน 10%

3. **Optimization:**
   - Vary geometry และ monitor $\varepsilon$
   - Target: maximize $\varepsilon$ สำหรับ given pressure drop
