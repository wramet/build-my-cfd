...
> - รู้ **engineering metrics** ที่สำคัญ —1$C_D$,1$\Delta p$,1$Nu$
...
- Key Metrics:1$\Delta p$, friction factor1$f$, Reynolds number
...
- Key Metrics: Nusselt number1$Nu$, heat transfer coefficient1$h$
...
|-------------|-------------------|------------------|----------------|
| **Vehicle aerodynamics** | `simpleFoam` | k-ω SST |1$y^+1≈ 1-30, boundary layers สำคัญ |
| **Pipe flow** | `simpleFoam` | k-ε |1$y^+1≈ 30-300, wall functions ใช้ได้ |
...
| Application |1$y^+1Target | Mesh Type | Boundary Layers | Refinement Zones |
|-------------|--------------|-----------|-----------------|------------------|
| **External Aerodynamics** | 1-30 (resolved) หรือ 30-100 (wall functions) | snappyHexMesh | ใช่ (ถ้า1$y^+1≈ 1) | รอบ body, wake region |
...
| High1$C_D1| Wrong turbulence model | ลอง k-ω SST แทน k-ε |
...
| Heat transfer ต่่าเกินไป |1$y^+1สูงเกินไป | Add boundary layers ให้1$y^+1≈ 1 |
...
>
> 2. **Engineering Metrics Drive Design**:1$C_D$,1$\Delta p$,1$Nu$, CoV — ใช้ function objects คำนวณระหว่าง simulation ไม่ต้อง post-process ทีหลัง
>
> 3. **Mesh Quality Matters Most**:1$y^+1target ต่างกันไปตาม application — aerodynamics (1-30), pipe flow (30-300), CHT (1-5)
>
...
<details>
<summary><b>2. Force coefficients ($C_D$,1$C_L$) ใช้ทำอะไร?</b></summary>
...
คำนวณค่าสำคัญ **ระหว่างการจำลอง** โดยอัตโนมัติ ไม่ต้อง post-process ทีหลัง — เช่น1$C_D$,1$\Delta p$, heat flux, scalar statistics ช่วย monitor convergence, validate ระหว่างรัน, และ save time ในการวิเคราะห์
</details>

<details>
<summary><b>4.1$y^+1คืออะไร และทำไมสำคัญ?</b></summary>

$y^+1เป็นค่าไร้มิติของระยะห่างจากผนัง บอกว่า mesh ใกล้ผนังพอที่จะ resolve boundary layer หรือไม่ ถ้า1$y^+1≈ 1 จะ resolve viscous sublayer ได้แม่นยำ ถ้า1$y^+1= 30-100 ใช้ wall functions ได้ ค่าที่ผิดทำให้ drag/heat transfer ผิดพลาด
</details>
...
- **Mesh Quality:** [MODULE_02_MESHING_AND_CASE_SETUP/CONTENT/05_MESH_QUALITY_AND_MANIPULATION](../../MODULE_02_MESHING_AND_CASE_SETUP/CONTENT/05_MESH_QUALITY_AND_MANIPULATION) — mesh criteria,1$y^+1calculation