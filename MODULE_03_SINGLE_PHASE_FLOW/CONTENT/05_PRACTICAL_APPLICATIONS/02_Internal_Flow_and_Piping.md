...
- Pressure drop สัมพันธ์โดยตรงกับ **pump power requirement**:1$P = \dot{m} \Delta p / \rho$
...
ที่ไหน:
-1$D_h1= hydraulic diameter ($4A/P1for non-circular ducts)
-1$U1= average velocity ($\dot{V}/A$)

### Flow Regime Criteria

|1$Re_D1Range | Regime | Characteristics | Velocity Profile |
|--------------|--------|-----------------|------------------|
| < 2300 | **Laminar** | Viscous forces dominant, streamline flow | Parabolic:1$u(r) = 2U_{avg}[1-(r/R)^2]1|
...
- Centerline velocity:1$u_{center} = 2U_{avg}$
- Mean velocity:1$U_{avg} = 0.5 u_{center}$
- Wall shear stress:1$\tau_w = \frac{8\mu U_{avg}}{D}$

**Turbulent Profile (Law of the Wall):**
$$u^+ = \frac{1}{\kappa}\ln y^+ + B$$

ที่ไหน:
-1$u^+ = u/u_\tau1(dimensionless velocity)
-1$y^+ = y u_\tau/\nu1(dimensionless wall distance)
-1$\kappa \approx 0.411(von Kármán constant)
-1$B \approx 5.01(intercept constant)

- Centerline velocity:1$u_{center} \approx 1.2-1.3 U_{avg}1(depends on Re)
...
แต่ละ component:
-1$\Delta p1= pressure drop (Pa)
-1$f1= Darcy friction factor
-1$L1= pipe length (m)
-1$D1= pipe diameter (m)
-1$\rho1= fluid density (kg/m³)
-1$U1= average velocity (m/s)

**Pump Power Requirement:**
$$P_{pump} = \frac{\dot{m} \Delta p_{total}}{\rho \eta} = \frac{Q \Delta p_{total}}{\eta}$$

ที่ไหน:
-1$\dot{m}1= mass flow rate (kg/s)
-1$Q1= volumetric flow rate (m³/s)
-1$\eta1= pump efficiency (typically 0.6-0.8)
...
ที่ไหน1$\epsilon1= absolute roughness height (m)

**Typical Roughness Values:**
| Material |1$\epsilon1(mm) |
|----------|-----------------|
| PVC/plastic | 0.0015 |
| Steel (new) | 0.045 |
...
| Fitting Type | Loss Coefficient1$K1|
|--------------|---------------------|
...
| Flow Regime | Entry Length1$L_e1| Practical Implication |
|-------------|-------------------|----------------------|
| Laminar |1$L_e \approx 0.06 \cdot D \cdot Re_D1| Can be very long:1$L_e = 60D1at Re=1000 |
| Turbulent |1$L_e \approx 4.4 \cdot D \cdot Re_D^{1/6}1| Shorter:1$L_e \approx 20-30D1for most cases |

**Example Calculation:**
- Water flow ($D=501mm,1$U=11m/s,1$\nu=10^{-6}1m²/s)
-1$Re = (1 \times 0.05)/10^{-6} = 50,0001(turbulent)
-1$L_e = 4.4 \times 0.05 \times (50000)^{1/6} \approx 1.11m ($22D$)
...
- Directly related to friction factor:1$f = \frac{8\tau_w}{\rho U^2}$
...
-1$Re = 50,000$,1$f \approx 0.021$
-1$u_\tau = 1 \times \sqrt{0.021/8} = 0.0511m/s
-1$\Delta y = 50 \times 10^{-6} / 0.051 = 0.981mm
...
**Key Concepts:**
- Convective heat transfer coefficient1$h1depends on flow regime
- Turbulent flow → better mixing → higher1$h$
...
ที่ไหน:
-1$n = 0.41for heating (wall hotter than fluid)
-1$n = 0.31for cooling (wall cooler than fluid)

**Heat Transfer Coefficient:**
$$h = \frac{Nu \cdot k}{D}$$

ที่ไหน:
-1$k1= thermal conductivity of fluid [W/(m·K)]
-1$D1= pipe diameter [m]
...
**Given:**
- Water:1$\rho = 9981kg/m³,1$\mu = 0.0011Pa·s
- Pipe:1$D = 501mm,1$L = 101m, steel ($\epsilon = 0.0451mm)
...
- Wall shear stress:1$\tau_w = \Delta p \cdot D / (4L) = 10.51Pa
- Friction velocity:1$u_\tau = \sqrt{\tau_w/\rho} = 0.1031m/s
- y+ for first cell (0.5 mm):1$y^+ = 0.0005 \times 0.103 / 10^{-6} = 51.51✓
...
### Critical Thresholds
- **Laminar flow:** Re < 2,300 → use parabolic inlet,1$f = 64/Re$
- **Turbulent flow:** Re > 4,000 → use turbulent inlet or uniform profile, Colebrook-White for1$f$
- **Transitional:** 2,300 < Re < 4,000 → avoid, unstable region

### Entry Length Formulas
- **Laminar:**1$L_e \approx 0.06 \cdot D \cdot Re1(can be > 60D)
- **Turbulent:**1$L_e \approx 4.4 \cdot D \cdot Re^{1/6}1(typically 20-30D)
...
<details>
<summary><b>1. ทำไม1$\Delta p \propto U^21ในช่วง Turbulent แต่1$\Delta p \propto U1ในช่วง Laminar?</b></summary>

**Laminar flow:** Viscous forces dominate → shear stress1$\tau_w \propto \mu (du/dy) \propto U1
ดังนั้น1$\Delta p \propto \tau_w \propto U$

**Turbulent flow:** Inertial forces dominate → shear stress สัมพันธ์กับ Reynolds stress1$\tau_{turb} \sim \rho u'^2 \propto \rho U^21
และ friction factor1$f \approx1constant (weak function of Re)  
ดังนั้น1$\Delta p = f \frac{L}{D} \frac{\rho U^2}{2} \propto U^2$
...
เมื่อ1$\phi1คือ field variable (U, p, k, etc.) และ1$n1คือทิศทางตั้งฉากกับ boundary
