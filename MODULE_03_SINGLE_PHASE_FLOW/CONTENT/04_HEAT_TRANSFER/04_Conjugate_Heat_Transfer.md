...
| `value` | Initial guess (ใช้ $internalField) |
...
- $h_h$ = Hot side convection coefficient
- $t_w$ = Wall thickness [m]
- $k_w$ = Wall thermal conductivity [W/m·K]
- $h_c$ = Cold side convection coefficient
...
| Boundary Layer | Resolve thermal BL | $y^+ < 1$ for conjugate problems |
...
**Guidelines:**
- Solid typically has much larger $\tau$ than fluid
- Use adaptive time stepping based on max Courant number
- Consider quasi-steady approach if $\tau_{solid} \gg \tau_{fluid}$
...
ไม่มี convection term: $(\mathbf{u} \cdot \nabla T)$
</details>
