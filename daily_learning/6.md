...
> [!TIP] **Summary of Section 1**
> Section 1 establishes the mathematical foundation for Finite Volume Method (FVM) discretization of transport equations, covering the integral form of conservation laws and appropriate schemes for convection, diffusion, and source terms. The critical expansion source term $\nabla \cdot \mathbf{U} = \dot{m}(1/\rho_v - 1/\rho_l)$ for phase change flows is introduced, along with design decisions that prioritize structured grids and pre-tabulated properties for performance over OpenFOAM's flexibility.
...
> [!TIP] **Summary of Section 4**
> Section 4 provides complete C++ implementation code for the FVM solver, including header files with class declarations and implementation files with discretization logic for convection, diffusion, and pressure equations with phase change expansion sources. The code demonstrates structured grid indexing, upwind convection schemes, central differencing for diffusion, Jacobi linear solver, boundary condition handling, and critical implementation details for avoiding divergence in two-phase flows with large density ratios.
...
- **Previous**: [[5]] - Thermodynamics and Property Tables
- **Next**: [[7]] - Pressure-Velocity Coupling and SIMPLE Algorithm
- **See also**: 
  - [[90_day_roadmap]] - Complete 90-day development plan
  - [[2]] - Governing Equations for Two-Phase Flow
  - [[8]] - Linear Solvers and Convergence Acceleration
