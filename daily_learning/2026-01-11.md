...
3.  **Phase Availability:** อัตราการถ่ายโอนมวลยังเป็นสัดส่วนกับปริมาณของเฟสต้นทาง (source phase) ที่มีอยู่ ผ่าน `$\alpha$_l` หรือ `$\alpha$_v` และความหนาแน่น `$\rho$_l` หรือ `$\rho$_v`
...
*   `$\dot{m}$_lv`: อัตราการสร้างมวลของไอ (vapor generation rate) ต่อหน่วยปริมาตร (`kg/m³·s`) **Positive** เมื่อ `T > T_{\text{sat}}`.
*   `$r_{\text{coeff}}$`: สัมประสิทธิ์ความเข้มข้นของการถ่ายโอนมวล (mass transfer intensity coefficient) ควบคุมว่าเฟสตอบสนองต่อ driving force เร็วเพียงใด
*   `$\alpha$_l $\rho$_l`: แสดงถึง "มวลของของเหลวที่พร้อมจะเปลี่ยนสถานะได้" ต่อหน่วยปริมาตรในเซลล์นั้นๆ หากไม่มีของเหลว (`$\alpha$_l = 0`) ก็จะไม่มีการระเหยเกิดขึ้น
...
*   `max(..., 0)`: ฟังก์ชันนี้ทำหน้าที่เป็น **activation function** รับประกันว่า `$\dot{m}$_lv` จะมีค่าเป็นบวก (การระเหย) เฉพาะเมื่อ `T > T_{\text{sat}}` เท่านั้น หาก `T $\le$ T_{\text{sat}}` ค่านี้จะเป็นศูนย์
...
**Physical Interpretation:**
*   `$\dot{m}$_vl`: อัตราการสูญเสียมวลของไอ (vapor condensation rate) ต่อหน่วยปริมาตร (`kg/m³·s`) **Positive** เมื่อ `T < T_{\text{sat}}`.
*   `$\alpha$_v $\rho$_v`: แสดงถึง "มวลของไอที่พร้อมจะควบแน่นได้" ต่อหน่วยปริมาตร
*   `(T_{\text{sat}} - T)/T_{\text{sat}}`: dimensionless driving force สำหรับการควบแน่น
*   `max(..., 0)`: รับประกันว่า `$\dot{m}$_vl` จะมีค่าเป็นบวกเฉพาะเมื่อ `T < T_{\text{sat}}`
...
*   `S_m,l`: source term สำหรับสมการมวลของของเหลว (liquid continuity equation) มีหน่วย `kg/m³·s`
    *   **เครื่องหมายลบ** หน้า `$\dot{m}$_lv`: เมื่อมีการระเหย (`$\dot{m}$_lv > 0`) มวลของเหลวจะ **ลดลง**
    *   **เครื่องหมายบวก** หน้า `$\dot{m}$_vl`: เมื่อมีการควบแน่น (`$\dot{m}$_vl > 0`) มวลของเหลวจะ **เพิ่มขึ้น**
*   `S_m,v`: source term สำหรับสมการมวลของไอ (vapor continuity equation)
    *   **เครื่องหมายบวก** หน้า `$\dot{m}$_lv`: เมื่อมีการระเหย มวลของไอจะ **เพิ่มขึ้น**
    *   **เครื่องหมายลบ** หน้า `$\dot{m}$_vl`: เมื่อมีการควบแน่น มวลของไอจะ **ลดลง**
...
| :--- | :--- | :--- | :--- | :--- |
| `$\dot{m}$_lv`, `$\dot{m}$_vl` | Mass transfer rate | `kg/m³·s` | อัตราการเปลี่ยนมวลระหว่างเฟสต่อหน่วยปริมาตร | Positive scalar field; ขึ้นกับ local T และ $\alpha$ |
...
| `$\alpha$_l`, `$\alpha$_v` | Volume fraction | dimensionless | สัดส่วนโดยปริมาตรของของเหลวและไอในเซลล์ควบคุม | `$\alpha$_l + $\alpha$_v = 1`; 0 $\le$ $\alpha$ $\le$ 1 |
| `$\rho$_l`, `$\rho$_v` | Density | `kg/m³` | ความหนาแน่นของของเหลวและไอ | ค่าคงที่หรือเป็นฟังก์ชันของ T, P; โดยทั่วไป `$\rho$_l >> $\rho$_v` |
...
ใน finite volume method (FVM) การจัดการกับ source terms `S($\phi$)` ที่เป็นฟังก์ชันของตัวแปรสนาม (field variable) `$\phi$` (เช่น `$\alpha$_l`, `T`) ต้องกระทำด้วยความระมัดระวังเป็นพิเศษ เพื่อรักษา **ความเสถียรเชิงตัวเลข (numerical stability)** และ **อัตราการลู่เข้า (convergence rate)** ของ iterative solver เทคนิคมาตรฐานคือการทำให้ source term เป็นเชิงเส้น (linearize) รอบค่าปัจจุบันของ `$\phi$` ในระหว่างการ iteration
...
*   `$\phi$` คือตัวแปรสนามที่กำลังถูก solve (เช่น `$\alpha$_l`, `T`)
...
จาก Lee Model, source term ในสมการของ `$\alpha$_l` (หรือสมการ VOF) สามารถเขียนได้จาก `S_m,l` โดยคำนึงถึงความสัมพันธ์ระหว่างมวลและ volume fraction:
`มวลของเหลวในเซลล์ = $\alpha$_l $\rho$_l V_cell`
ดังนั้น source term สำหรับสมการ `$\alpha$_l` (หน่วย `1/s`) คือ:
$$
S_{\alpha,l} = \frac{S_{m,l}}{\rho_l} = \frac{-\dot{m}_{lv} + \dot{m}_{vl}}{\rho_l}
$$
ให้ `$\phi$ = $\alpha$_l` เป็นตัวแปรที่เราต้องการ solve
*   **สำหรับ Evaporation Term (`-$\dot{m}$_lv/$\rho$_l`):**
    จาก `$\dot{m}$_lv = $r_{\text{coeff}}$ $\alpha$_l $\rho$_l (T-T_{\text{sat}})/T_{\text{sat}}` (เมื่อ `T > T_{\text{sat}}`)
    ดังนั้น `-$\dot{m}$_lv/$\rho$_l = -$r_{\text{coeff}}$ $\alpha$_l (T-T_{\text{sat}})/T_{\text{sat}}`
...
*   **สำหรับ Condensation Term (`+$\dot{m}$_vl/$\rho$_l`):**
    จาก `$\dot{m}$_vl = $r_{\text{coeff}}$ $\alpha$_v $\rho$_v (T_{\text{sat}}-T)/T_{\text{sat}}` (เมื่อ `T < T_{\text{sat}}`) และ `$\alpha$_v = 1 - $\alpha$_l`
    ดังนั้น `+$\dot{m}$_vl/$\rho$_l = +$r_{\text{coeff}}$ (1 - $\alpha$_l) ($\rho$_v/$\rho$_l) (T_{\text{sat}}-T)/T_{\text{sat}}`
    ...
    **หมายเหตุสำคัญ:** เนื่องจาก `(T_{\text{sat}}-T) > 0` สำหรับการควบแน่น และ `$\rho$_v/$\rho$_l > 0` ดังนั้น `S_p^{cond} < 0` เช่นกัน

**สรุป Linearized Form สำหรับ `$\alpha$_l` Equation:**
$$
...
Foam::phaseChangeTwoPhaseMixtureLee::mDot1() const
{
    ...
    // ใน Lee Model: $\dot{m}$_lv = $r_{\text{coeff}}$ * $\alpha$_l * $\rho$_l * max(T - T_{\text{sat}}, 0) / T_{\text{sat}}
...
Foam::phaseChangeTwoPhaseMixtureLee::mDot2() const
{
    ...
    // ใน Lee Model: $\dot{m}$_vl = $r_{\text{coeff}}$ * $\alpha$_v * $\rho$_v * max(T_{\text{sat}} - T, 0) / T_{\text{sat}}
...
Foam::phaseChangeTwoPhaseMixtureLee::SuSp1() const
{
    ...
    // d(alpha1)/dt + ... = -$\dot{m}$_lv/$\rho$_l + $\dot{m}$_vl/$\rho$_l
    ...
    (
        ...
        (
            ...
            // $\dot{m}$_vl/$\rho$_l (positive for condensation)
            mDotC()/rho1
        )
    );
    ...
    (
        ...
        (
            ...
            // -$\dot{m}$_lv/($\rho$_l * alpha1) แต่ต้องรักษาให้ S_p $\le$ 0
...
|--------|----------------------------------|-----------------------------|----------------|
| **Source Term Linearization** | ใช้ simple linearization: `S_p = -|$\dot{m}$|/($\rho$*$\alpha$)` | ใช้ adaptive linearization based on local phase fraction และ temperature gradient | ป้องกัน numerical instability ใน regions ที่ $\alpha$ → 0 หรือ $\alpha$ → 1 |
...
graph TB
    ...
    subgraph "Phase Change Module ([[Day 11: Phase Change Theory (Lee Model & Linearization)|Day 11]])"
        ...
        LeeModel --> |"$\dot{m}$, S_u_$\alpha$, S_p_$\alpha$"| AlphaEqn[Alpha Equation]
        LeeModel --> |"$\dot{m}$"| PressureEqn[Pressure Eqn]
...
1.  **การคำนวณ `mDot`:** Implement สมการจาก Theory Section 11.1 อย่างเคร่งครัด โดยต้องใช้ `max(T - TSat_, deltaT_)` เพื่อป้องกันการคำนวณค่าลบและเพิ่ม Numerical Stability รอบๆ ค่า `T_{\text{sat}}`
    ...
    ```
2.  **การทำ Linearization:** เมธอด `linearizeSource` เป็นหัวใจสำคัญ รับ `sourceValue` (เช่น ค่า `$\dot{m}$` ที่คำนวณได้) และฟิลด์ `phiField` (เช่น `alpha`) แล้วคำนวณ `Su` และ `Sp` โดยใช้เทคนิค "Negative Slope Linearization" เพื่อให้ `Sp $\le$ 0` เสมอ:
    ```cpp
...
Description
    ...
    หน้าที่หลัก:
    1. คำนวณ mass transfer rate ($\dot{m}$) ด้วย Lee Model
    ...
    - ต้องรักษา S_p $\le$ 0 เสมอเพื่อ diagonal dominance
...
class PhaseChangeSolver
{
    // Private Data
        ...
        //- Mass transfer intensity coefficient ($r_{\text{coeff}}$)
        ...
        //- Mass transfer rate field ($\dot{m}$)
        volScalarField mDot_;
...
)
{
    ...
    // $\dot{m}$_lv = $r_{\text{coeff}}$ * $\alpha$_l * $\rho$_l * max(T - T_{\text{sat}}, 0) / T_{\text{sat}}
    ...
    // $\dot{m}$_vl = $r_{\text{coeff}}$ * $\alpha$_v * $\rho$_v * max(T_{\text{sat}} - T, 0) / T_{\text{sat}}
    // โดย $\alpha$_v = 1 - $\alpha$_l
    ...
    // Net mass transfer rate: $\dot{m}$ = $\dot{m}$_lv - $\dot{m}$_vl
...
)
{
    ...
    // ∂($\alpha$_l)/∂t + $\nabla$·($\alpha$_l U) = S_$\alpha$
    // โดย S_$\alpha$ = ($\dot{m}$_vl - $\dot{m}$_lv)/$\rho$_l = -$\dot{m}$/$\rho$_l
    
    // Linearized form: S_$\alpha$ = S_u,$\alpha$ + S_p,$\alpha$ * $\alpha$_l
    
    // คำนวณ derivatives ของ mass transfer rates ตาม $\alpha$_l
    ...
    // Total derivative: dS_$\alpha$/d$\alpha$_l = (d$\dot{m}$_vl/d$\alpha$_l - d$\dot{m}$_lv/d$\alpha$_l)/$\rho$_l
    ...
    // S_$\alpha$($\alpha$) ≈ S_$\alpha$($\alpha$_old) + (dS_$\alpha$/d$\alpha$)_old * ($\alpha$ - $\alpha$_old)
    //         = [S_$\alpha$($\alpha$_old) - (dS_$\alpha$/d$\alpha$)_old * $\alpha$_old] + (dS_$\alpha$/d$\alpha$)_old * $\alpha$
...
TEST_F(SourceTermLinearizationTest, EvaporationSourceLinearization) {
    // สร้าง evaporation source term: S = $r_{\text{coeff}}$ * alpha * rho * (T-T_{\text{sat}})/T_{\text{sat}}
    // สำหรับ alpha = 0.5, T-T_{\text{sat}} = 10K, $r_{\text{coeff}}$ = 0.1, rho = 1000, T_{\text{sat}} = 373
    scalar alpha = 0.5;
    scalar deltaT = 10.0;
    scalar rCoeff = 0.1;
    scalar rho = 1000.0;
    ...
    // เราต้องการ S_p $\le$ 0 เพื่อรักษา diagonal dominance
    scalar Su, Sp;
...
> >     *   `S_p * ϕ`: Implicit Part หรือ Linear Part ที่ `ϕ` คือตัวแปรที่กำลังแก้ (เช่น `$\alpha$_l`, `T`) **ส่วนนี้จะถูกเพิ่มเข้าไปในแนวทแยงมุมของเมทริกซ์ `A`** (หากใช้ `fvm::Sp(Sp, phi)` ใน OpenFOAM) การเลือก `S_p` ให้มีค่า **น้อยกว่าหรือเท่ากับศูนย์ (`S_p $\le$ 0`)** จะทำให้ค่าสัมบูรณ์ในแนวทแยงมุม (`|A_ii|`) เพิ่มขึ้น ส่งผลให้เมทริกซ์มี Diagonal Dominance ที่แข็งแกร่งขึ้น และเพิ่ม Numerical Stability อย่างมีนัยสำคัญ
> > 3.  **ผลกระทบของการใช้ Explicit Source เท่านั้น:** ในบริเวณ Interface ที่มีการเปลี่ยนเฟสรุนแรง (High `$r_{\text{coeff}}$` หรือ `ΔT` ใหญ่) Mass Transfer Rate `$\dot{m}$` จะมีค่าสูงมาก Source Term `S_u` ที่คำนวณจากค่า `ϕ` จาก Iteration ที่แล้ว (Explicit) จะมีขนาดใหญ่และเปลี่ยนแปลงรวดเร็ว การอัพเดทค่า `ϕ` ใน Iteration ใหม่อาจทำให้เกิดการแกว่งกวัด (Oscillation) ของ Solution ได้ง่าย เนื่องจากระบบขาด "แรงดึงกลับ" (Restoring Force) จาก Diagonal Term ที่จะ Stabilize การเปลี่ยนแปลงนี้ นำไปสู่ Divergence ของ Solver ในที่สุด
...
> >     Sp[i] += Sp_alpha[i];          // เพิ่ม Implicit part (ต้อง $\le$ 0)
...
> >     *   **Scale Analysis:** `$r_{\text{coeff}}$` ควรสัมพันธ์กับ Time Scale ของปรากฏการณ์ที่สนใจ เช่น สำหรับ Boiling Bubble Growth อาจพิจารณาจาก `$r_{\text{coeff}}$ ~ 1 / τ_growth` โดย `τ_growth` คือ Characteristic Time ของการขยายตัวของฟอง
> >     *   **Relation to Numerical Parameters:** ค่า `$r_{\text{coeff}}$` ควรสัมพันธ์กับ Time Step (`Δt`) ที่ใช้ หลักการคือ `$r_{\text{coeff}}$ * Δt` ควรมีค่าอยู่ในลำดับ `O(1)` หรือน้อยกว่าเพื่อ Stability ถ้า `$r_{\text{coeff}}$ * Δt >> 1` แสดงว่าในหนึ่ง Time Step ระบบพยายามจะเปลี่ยนเฟสในปริมาณที่มากเกินไป ซึ่งไม่ Stable
...
> >     *   **คำนึงถึง Density Ratio:** สำหรับของไหลที่มี Density Ratio สูงมาก (เช่น R410A, `$\rho$_l/$\rho$_v ~ 22`) การเปลี่ยนเฟสเพียงเล็กน้อยทำให้ Volume เปลี่ยนแปลงมหาศาล (จาก Expansion Term) ควรเริ่มด้วย `$r_{\text{coeff}}$` ที่ต่ำกว่าเพื่อลด Numerical Stiffness จาก Pressure Equation
> [!QUESTION] Concept Check 3: คำถามที่ 3: การรักษา Mass Conservation ระหว่าง Phase ในการ Implement Lee Model
>
> > [!SUCCESS]- เฉลย (คลิกเพื่ออ่านคำตอบ)
> > **คำถาม:** ในสมการ Continuity ของแต่ละ Phase: `∂($\alpha$_l $\rho$_l)/∂t + $\nabla$·($\alpha$_l $\rho$_l U) = -$\dot{m}$_lv + $\dot{m}$_vl` และ `∂($\alpha$_v $\rho$_v)/∂t + $\nabla$·($\alpha$_v $\rho$_v U) = +$\dot{m}$_lv - $\dot{m}$_vl` การ Implement ต้องรับประกันว่า Total Mass ของระบบ `∫_V ($\alpha$_l $\rho$_l + $\alpha$_v $\rho$_v) dV` ถูกอนุรักษ์อย่างไร? ข้อผิดพลาดใดที่พบบ่อยที่ทำให้เกิด Mass Error และจะป้องกันได้อย่างไร?
> >
> > **คำตอบเชิงลึก:**
> > 1.  **หลักการ Conservation ที่ต้องรักษา:** Net Mass Transfer ระหว่าง Phase ต้องเป็นศูนย์เมื่อพิจารณาทั้งระบบ กล่าวคือ **`$\dot{m}$_lv` และ `$\dot{m}$_vl` ที่คำนวณสำหรับ Cell เดียวกัน ต้องมีขนาดเท่ากันทุกประการ แต่มีเครื่องหมายตรงข้ามกัน** เมื่อรวม Source Terms เข้าด้วยกันสำหรับสมการ Mixture (`$\alpha$_l+$\alpha$_v=1`) จะได้ว่า `S_m,mixture = S_m,l + S_m,v = (-$\dot{m}$_lv+$\dot{m}$_vl) + (+$\dot{m}$_lv-$\dot{m}$_vl) = 0` ซึ่งสอดคล้องกับสมการ Continuity ของ Mixture ที่ไม่มี Source Term
> > 2.  **ข้อผิดพลาด Implement ที่พบบ่อย:**
> >     *   **การใช้ Conditional Statement แยกกัน:** การคำนวณ `$\dot{m}$_lv` และ `$\dot{m}$_vl` ด้วย `if-else` แยกกัน เช่น `if(T > T_{\text{sat}}) { $\dot{m}$_lv = $r_{\text{coeff}}$*$\alpha$_l*$\rho$_l*(T-T_{\text{sat}})/T_{\text{sat}}; $\dot{m}$_vl = 0; } else { $\dot{m}$_lv = 0; $\dot{m}$_vl = $r_{\text{coeff}}$*$\alpha$_v*$\rho$_v*(T_{\text{sat}}-T)/T_{\text{sat}}; }` วิธีนี้ดูเหมือนถูกต้อง แต่ปัญหาอยู่ที่ **บริเวณ Interface ที่ `T ≈ T_{\text{sat}}`** เนื่องมาจาก Numerical Error เล็กน้อย `T` อาจกระโดดข้าม `T_{\text{sat}}` ระหว่าง Iterations ทำให้ `$\dot{m}$_lv` และ `$\dot{m}$_vl` ถูกคำนวณสลับกันและมีค่าไม่เท่ากันพอดี ส่งผลให้เกิด Local Mass Imbalance
...
> >     *   **การอัพเดท Properties ไม่พร้อมกัน:** การคำนวณ `$\dot{m}$` โดยใช้ `$\rho$_l` และ `$\rho$_v` จาก Time Step หรือ Iteration ที่ต่างกัน (หาก Properties ถูกอัพเดทในขั้นตอนอื่น) อาจทำให้ค่า Density ที่ใช้ไม่สอดคล้องกัน
> > 3.  **กลยุทธ์การ Implement เพื่อรับประกัน Conservation:**
> >     *   **วิธีที่ 1: คำนวณ Net Mass Transfer Rate เดียว:** กำหนด `$\dot{m}$_net = $r_{\text{coeff}}$ * ($\alpha$_l $\rho$_l * (T - T_{\text{sat}}) / T_{\text{sat}})` โดยอนุญาตให้ `$\dot{m}$_net` เป็นได้ทั้งบวกและลบ (บวก = Evaporation, ลบ = Condensation) จากนั้นกำหนด:
> >         ```cpp
> >         $\dot{m}$_lv = max($\dot{m}$_net, 0.0); // Evaporation rate (>=0)
> >         $\dot{m}$_vl = max(-$\dot{m}$_net, 0.0); // Condensation rate (>=0)
> >         S_m,l = -$\dot{m}$_lv + $\dot{m}$_vl; // = -$\dot{m}$_net
> >         S_m,v = +$\dot{m}$_lv - $\dot{m}$_vl; // = +$\dot{m}$_net
> >         ```
...
> >         scalar coeff = $r_{\text{coeff}}$_ / T_{\text{sat}};
> >         if(drivingForce > 0) {
> >             // Evaporation
> >             $\dot{m}$_lv = coeff * alpha_l * rho_l * drivingForce;
> >             $\dot{m}$_vl = 0.0;
> >         } else {
> >             // Condensation
> >             $\dot{m}$_lv = 0.0;
> >             $\dot{m}$_vl = coeff * alpha_v * rho_v * (-drivingForce); // ใช้ค่า drivingForce เดียวกันแต่กลับเครื่องหมาย
> >         }
> >         // S_m,l และ S_m,v ยังคงคำนวณจาก $\dot{m}$_lv และ $\dot{m}$_vl ข้างต้น
> >         ```
> >         ต้องมั่นใจว่า `coeff` และ `drivingForce` เป็นตัวแปรเดียวกันที่ใช้คำนวณทั้งสองกรณี
> >     *   **วิธีที่ 3: Post-Processing Enforcement:** หลังจากคำนวณ `$\dot{m}$_lv` และ `$\dot{m}$_vl` แล้ว ให้ทำการ Normalize หรือ Average เพื่อบังคับให้ Sum เป็นศูนย์ในแต่ละ Cell เช่น `$\dot{m}$_avg = ($\dot{m}$_lv - $\dot{m}$_vl)/2; $\dot{m}$_lv = $\dot{m}$_avg; $\dot{m}$_vl = -$\dot{m}$_avg;` แต่วิธีนี้อาจทำให้ Physical Accuracy ลดลง
...
>
> > [!SUCCESS]- เฉลย (คลิกเพื่ออ่านคำตอบ)
> > **คำถาม:** จาก Expansion Term `$\nabla$·U = $\dot{m}$ (1/$\rho$_v - 1/$\rho$_l)` ที่ได้จาก [[Day 01: Governing Equations|Day 01]] จะถูก Integrate เข้ากับ Pressure-Velocity Coupling Algorithm (เช่น PISO/SIMPLE จาก [[Day 09: Pressure-Velocity Coupling (SIMPLE, PISO, Rhie-Chow)|Day 09]]) อย่างไรโดยเฉพาะ? Source Term นี้ทำให้ Pressure Equation `$\nabla$·(rAU $\nabla$p) = $\nabla$·(U*) - S_p` เปลี่ยนแปลงไปอย่างไร และการมีอยู่ของ Term นี้เพิ่มความท้าทายด้าน Numerical Stability อย่างไร?
> >
> > **คำตอบเชิงลึก:**
> > 1.  **ที่มาของ Expansion Term ใน Pressure Equation:** เริ่มจากสมการ Momentum ที่ Discrete แล้วได้ความสัมพันธ์ระหว่าง Velocity Correction และ Pressure Gradient: `U = U* - rAU $\nabla$p` (โดย `U*` คือ Predicted Velocity, `rAU = 1/A_p` จาก Momentum Equation) นำ Divergence ของสมการนี้:
> >     `$\nabla$·U = $\nabla$·U* - $\nabla$·(rAU $\nabla$p)`
> >     แต่จากสมการ Continuity ที่มี Phase Change เรารู้ว่า `$\nabla$·U = $\dot{m}$ (1/$\rho$_v - 1/$\rho$_l) = S_p` (กำหนดให้ `S_p` คือ Source Term ใน Pressure Equation)
> >     ดังนั้นจะได้ **Pressure Equation with Phase Change**:
> >     ```
> >     $\nabla$·(rAU $\nabla$p) = $\nabla$·(U*) - S_p
> >     ```
> >     เปรียบเทียบกับ Pressure Equation แบบไม่มี Phase Change (`$\nabla$·(rAU $\nabla$p) = $\nabla$·(U*)`) จะเห็นว่ามี Source Term `-S_p` เพิ่มเข้ามาทางด้านขวา
> > 2.  **Physical Interpretation:** Term `S_p = $\dot{m}$ (1/$\rho$_v - 1/$\rho$_l)` แสดงถึง **Rate of Volume Generation/Consumption** เนื่องจาก Phase Change เมื่อของเหลวระเหยเป็นไอ (`$\dot{m}$ > 0`) และ `$\rho$_v << $\rho$_l` ดังนั้น `(1/$\rho$_v - 1/$\rho$_l) > 0` ทำให้ `S_p > 0` ส่งผลให้ด้านขวาของ Pressure Equation `$\nabla$·U* - S_p` มีค่าน้อยลง (หรือเป็นลบ) ซึ่งหมายความว่า Pressure Field ต้องปรับตัวเพื่อสร้าง Velocity Div
...
1.  **Lee, W. H. (1980). *A Pressure Iteration Scheme for Two-Phase Flow Modeling.***
    ...
    *   **หัวใจสำคัญ:** แนะนำรูปแบบ `$\dot{m}$ ∝ (T - T_{\text{sat}})` สำหรับการคำนวณอัตราการเปลี่ยนมวล ซึ่งเป็นพื้นฐานของ implementation ในบทเรียนนี้
...
5.  **"Thermophysical Properties of Refrigerants" (ASHRAE Handbook - Fundamentals)**
    *   **ความเกี่ยวข้อง:** แหล่งข้อมูลสำหรับ saturation temperature (`T_{\text{sat}}`), latent heat (`h_lv`), และคุณสมบัติของ liquid/vapor (`$\rho$_l`, `$\rho$_v`, `c_p`) สำหรับสารทำงานต่างๆ เช่น R134a, R410A, น้ำ
...
*   **Day 01: Governing Equations**
    *   **Connection:** **Hero Concept: Expansion Term** `$\nabla$·U = $\dot{m}$(1/$\rho$_v - 1/$\rho$_l)` ที่ derive ใน [[Day 01: Governing Equations|Day 01]] กลายเป็น source term `S_p` ใน pressure equation (Section 11.4) โดยตรง
...
*   **Day 09: Pressure-Velocity Coupling**
    *   **Connection:** **Key Link** การมี source term `S_p = $\dot{m}$(1/$\rho$_v - 1/$\rho$_l)` ใน pressure equation (Section 11.4) เป็นการปรับเปลี่ยนขั้นพื้นฐานของอัลกอริทึม `PISO`/`SIMPLE` จาก Day 9
...
*   **Day 10: Two-Phase Fundamentals (VOF)**
    *   **Connection:** **Most Direct Predecessor** Lee Model ใช้ `$\alpha$` (volume fraction) จาก [[Day 10: Two-Phase Fundamentals (VOF & MULES)|Day 10]] เป็นตัวแปรสำคัญในการคำนวณ `$\dot{m}$` (`$\dot{m}$ ∝ $\alpha$_l` หรือ `$\alpha$_v`)
