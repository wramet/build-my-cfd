...
### 11.1 Lee Model สำหรับอัตราการเปลี่ยนมวล (Lee Model for Mass Transfer Rate)
...
#### 11.1.1 Physical Basis และ Assumptions ของ Lee Model
...
3.  **Phase Availability:** อัตราการถ่ายโอนมวลยังเป็นสัดส่วนกับปริมาณของเฟสต้นทาง (source phase) ที่มีอยู่ ผ่าน `α_l` หรือ `α_v` และความหนาแน่น `ρ_l` หรือ `ρ_v`
4.  **Empirical Coefficient:** ความรุนแรง (intensity) ของการถ่ายโอนมวลถูกควบคุมโดยสัมประสิทธิ์เชิงประจักษ์ `r_coeff` (หน่วย `1/s`) ซึ่งต้องถูกปรับค่า (calibrate) ให้สอดคล้องกับข้อมูลการทดลองหรือผลลัพธ์จากแบบจำลองที่ละเอียดกว่า

#### 11.1.2 Governing Equations ของ Lee Model
...
$$
\boxed{\dot{m}_{lv} = r_{coeff} \, \alpha_l \rho_l \frac{\max(T - T_{\text{sat}}, 0)}{T_{\text{sat}}}}
$$

**Physical Interpretation:**
*   `ṁ_lv`: อัตราการสร้างมวลของไอ (vapor generation rate) ต่อหน่วยปริมาตร (`kg/m³·s`) **Positive** เมื่อ `T > T_{\text{sat}}`.
*   `r_coeff`: สัมประสิทธิ์ความเข้มข้นของการถ่ายโอนมวล (mass transfer intensity coefficient) ควบคุมว่าเฟสตอบสนองต่อ driving force เร็วเพียงใด
*   `α_l ρ_l`: แสดงถึง "มวลของของเหลวที่พร้อมจะเปลี่ยนสถานะได้" ต่อหน่วยปริมาตรในเซลล์นั้นๆ หากไม่มีของเหลว (`α_l = 0`) ก็จะไม่มีการระเหยเกิดขึ้น
*   `(T - T_{\text{sat}})/T_{\text{sat}}`: เป็น dimensionless driving force ใช้ `T_{\text{sat}}` เป็นตัวทำให้เป็นไร้มิติ (normalization) เพื่อให้ได้ scaling ที่เหมาะสม
*   `max(..., 0)`: ฟังก์ชันนี้ทำหน้าที่เป็น **activation function** รับประกันว่า `ṁ_lv` จะมีค่าเป็นบวก (การระเหย) เฉพาะเมื่อ `T > T_{\text{sat}}` เท่านั้น หาก `T ≤ T_{\text{sat}}` ค่านี้จะเป็นศูนย์
...
**Physical Interpretation:**
*   `ṁ_vl`: อัตราการสูญเสียมวลของไอ (vapor condensation rate) ต่อหน่วยปริมาตร (`kg/m³·s`) **Positive** เมื่อ `T < T_{\text{sat}}`.
*   `α_v ρ_v`: แสดงถึง "มวลของไอที่พร้อมจะควบแน่นได้" ต่อหน่วยปริมาตร
*   `(T_{\text{sat}} - T)/T_{\text{sat}}`: dimensionless driving force สำหรับการควบแน่น
*   `max(..., 0)`: รับประกันว่า `ṁ_vl` จะมีค่าเป็นบวกเฉพาะเมื่อ `T < T_{\text{sat}}`
...
*   `S_m,l`: source term สำหรับสมการมวลของของเหลว (liquid continuity equation) มีหน่วย `kg/m³·s`
    *   **เครื่องหมายลบ** หน้า `ṁ_lv`: เมื่อมีการระเหย (`ṁ_lv > 0`) มวลของเหลวจะ **ลดลง**
    *   **เครื่องหมายบวก** หน้า `ṁ_vl`: เมื่อมีการควบแน่น (`ṁ_vl > 0`) มวลของเหลวจะ **เพิ่มขึ้น**
*   `S_m,v`: source term สำหรับสมการมวลของไอ (vapor continuity equation)
    *   **เครื่องหมายบวก** หน้า `ṁ_lv`: เมื่อมีการระเหย มวลของไอจะ **เพิ่มขึ้น**
    *   **เครื่องหมายลบ** หน้า `ṁ_vl`: เมื่อมีการควบแน่น มวลของไอจะ **ลดลง**
*   **กฎการอนุรักษ์มวลรวม (Total Mass Conservation):** สังเกตว่า `S_m,l + S_m,v = 0` เสมอ นี่คือเงื่อนไขบังคับ (mandatory condition) ที่ต้องรักษาไว้ใน implementation มวลที่สูญเสียจากเฟสหนึ่งจะต้องเพิ่มขึ้นในอีกเฟสหนึ่งพอดี

#### 11.1.3 ตารางสรุปตัวแปรและพารามิเตอร์ของ Lee Model

| สัญลักษณ์ (Symbol) | ชื่อ (Name) | หน่วย (Unit) | บทบาททางฟิสิกส์ (Physical Role) | Typical Values / Notes |
| :--- | :--- | :--- | :--- | :--- |
| `ṁ_lv`, `ṁ_vl` | Mass transfer rate | `kg/m³·s` | อัตราการเปลี่ยนมวลระหว่างเฟสต่อหน่วยปริมาตร | Positive scalar field; ขึ้นกับ local T และ α |
| `r_coeff` | Mass transfer intensity coefficient | `1/s` | ควบคุมความเร็วของการตอบสนองต่อ driving force | **Tunable parameter**; Range: 0.1 - 100 `1/s`; ขึ้นกับ fluid และ scale |
| `α_l`, `α_v` | Volume fraction | dimensionless | สัดส่วนโดยปริมาตรของของเหลวและไอในเซลล์ควบคุม | `α_l + α_v = 1`; 0 ≤ α ≤ 1 |
| `ρ_l`, `ρ_v` | Density | `kg/m³` | ความหนาแน่นของของเหลวและไอ | ค่าคงที่หรือเป็นฟังก์ชันของ T, P; โดยทั่วไป `ρ_l >> ρ_v` |
...
#### 11.1.4 การเลือกค่า `r_coeff` และข้อควรระวัง (Critical Tuning and Warnings)

ค่าของ `r_coeff` เป็นหัวใจของการทำให้แบบจำลองทำงานได้อย่างถูกต้องทั้งในเชิงฟิสิกส์และความเสถียรเชิงตัวเลข

**1. ผลกระทบของ `r_coeff` ที่มีค่าสูงเกินไป (Too High):**
...
**2. ผลกระทบของ `r_coeff` ที่มีค่าต่ำเกินไป (Too Low):**
...
3.  **Consider Scale and Time Constant:** `r_coeff` มีมิติเป็นส่วนกลับของเวลา (`1/s`) ดังนั้นควรพิจารณาให้สัมพันธ์กับ time scale ที่สนใจของปัญหา เช่น ถ้า time scale ของ flow เป็น `0.01 s` `r_coeff` ที่ `100 1/s` จะทำให้เฟสตอบสนองในเวลาที่สั้นมากเมื่อเทียบกับ flow time scale

**⚠️ WARNING: Numerical Stability Consideration**
การ implement `ṁ` ต้องคำนึงถึงการทำให้เป็นเชิงเส้น (linearization) ของ source term เสมอ โดยเฉพาะเมื่อ `r_coeff` มีค่าสูง การใช้ explicit source term (`S_u` เพียงอย่างเดียว) จะนำไปสู่ conditional stability ที่รุนแรง (severe conditional stability) ซึ่งจะถูกอธิบายอย่างละเอียดในหัวข้อถัดไป (Section 11.2)

### 11.2 การทำให้ Source Term เป็นเชิงเส้น S_u, S_p (Source Term Linearization)

ใน finite volume method (FVM) การจัดการกับ source terms `S(φ)` ที่เป็นฟังก์ชันของตัวแปรสนาม (field variable) `φ` (เช่น `α_l`, `T`) ต้องกระทำด้วยความระมัดระวังเป็นพิเศษ เพื่อรักษา **ความเสถียรเชิงตัวเลข (numerical stability)** และ **อัตราการลู่เข้า (convergence rate)** ของ iterative solver เทคนิคมาตรฐานคือการทำให้ source term เป็นเชิงเส้น (linearize) รอบค่าปัจจุบันของ `φ` ในระหว่างการ iteration

#### 11.2.1 รูปแบบมาตรฐานของการทำให้เป็นเชิงเส้น (Standard Linearized Form)

source term ใดๆ ที่เป็นฟังก์ชันของ `φ` สามารถถูกประมาณเชิงเส้น (linearized) ได้ในรูปแบบ:
...
*   `φ` คือตัวแปรสนามที่กำลังถูก solve (เช่น `α_l`, `T`)

**ที่มาทางคณิตศาสตร์ (Mathematical Derivation):**
พิจารณา source term `S(φ)` โดยทั่วไป เราสามารถทำ Taylor expansion รอบค่า `φ^*` (ค่า guess หรือค่าจาก iteration ก่อนหน้า) ได้ดังนี้:
$$
...
$$
นี่คือที่มาของรูปแบบ `S_u + S_p φ`

#### 11.2.2 การประยุกต์กับ Lee Model Source Terms

เราจะทำ linearization ของ source terms ที่มาจาก Lee Model สำหรับสมการ volume fraction (`α` equation) และสมการพลังงาน (`T` equation)

**1. Linearization สำหรับสมการ Volume Fraction (α Equation)**

จาก Lee Model, source term ในสมการของ `α_l` (หรือสมการ VOF) สามารถเขียนได้จาก `S_m,l` โดยคำนึงถึงความสัมพันธ์ระหว่างมวลและ volume fraction:
`มวลของเหลวในเซลล์ = α_l ρ_l V_cell`
ดังนั้น source term สำหรับสมการ `α_l` (หน่วย `1/s`) คือ:
$$
S_{\alpha,l} = \frac{S_{m,l}}{\rho_l} = \frac{-\dot{m}_{lv} + \dot{m}_{vl}}{\rho_l}
$$
ให้ `φ = α_l` เป็นตัวแปรที่เราต้องการ solve
*   **สำหรับ Evaporation Term (`-ṁ_lv/ρ_l`):**
    จาก `ṁ_lv = r_coeff α_l ρ_l (T-T_{\text{sat}})/T_{\text{sat}}` (เมื่อ `T > T_{\text{sat}}`)
    ดังนั้น `-ṁ_lv/ρ_l = -r_coeff α_l (T-T_{\text{sat}})/T_{\text{sat}}`
    เทียบกับรูปแบบ `S_u + S_p φ`:
    *   `S_u^{evap} = 0`
    *   `S_p^{evap} = -r_coeff (T-T_{\text{sat}})/T_{\text{sat}}`
    **หมายเหตุสำคัญ:** เนื่องจาก `(T-T_{\text{sat}}) > 0` สำหรับการระเหย ดังนั้น `S_p^{evap} < 0` ซึ่งเป็นไปตามกฎ `S_p ≤ 0`

*   **สำหรับ Condensation Term (`+ṁ_vl/ρ_l`):**
    จาก `ṁ_vl = r_coeff α_v ρ_v (T_{\text{sat}}-T)/T_{\text{sat}}` (เมื่อ `T < T_{\text{sat}}`) และ `α_v = 1 - α_l`
    ดังนั้น `+ṁ_vl/ρ_l = +r_coeff (1 - α_l) (ρ_v/ρ_l) (T_{\text{sat}}-T)/T_{\text{sat}}`
    จัดรูปใหม่เป็น `S_u + S_p φ`:
    *   `S_u^{cond} = +r_coeff (\rho_v/\rho_l) (T_{\text{sat}}-T)/T_{\text{sat}}`
    *   `S_p^{cond} = -r_coeff (\rho_v/\rho_l) (T_{\text{sat}}-T)/T_{\text{sat}}`
    **หมายเหตุสำคัญ:** เนื่องจาก `(T_{\text{sat}}-T) > 0` สำหรับการควบแน่น และ `ρ_v/ρ_l > 0` ดังนั้น `S_p^{cond} < 0` เช่นกัน

**สรุป Linearized Form สำหรับ `α_l` Equation:**
$$
...
## 3.1 Class Analysis: `phaseChangeTwoPhaseMixture`
...
### 3.1.1 Header File Analysis (`phaseChangeTwoPhaseMixture.H`)
...
### 3.1.2 Implementation Analysis (`phaseChangeTwoPhaseMixture.C`)
...
### 3.1.3 Lee Model Implementation (`phaseChangeTwoPhaseMixtureLee.C`)
...
Foam::phaseChangeTwoPhaseMixtureLee::mDot1() const
{
    ...
    // ใน Lee Model: ṁ_lv = r_coeff * α_l * ρ_l * max(T - T_{\text{sat}}, 0) / T_{\text{sat}}
...
Foam::phaseChangeTwoPhaseMixtureLee::mDot2() const
{
    ...
    // ใน Lee Model: ṁ_vl = r_coeff * α_v * ρ_v * max(T_{\text{sat}} - T, 0) / T_{\text{sat}}
...
Foam::phaseChangeTwoPhaseMixtureLee::SuSp1() const
{
    ...
    (
        ...
        (
            ...
            // ṁ_vl/ρ_l (positive for condensation)
            mDotC()/rho1
        )
    );
    ...
    (
        ...
        (
            ...
            // -ṁ_lv/(ρ_l * alpha1) แต่ต้องรักษาให้ S_p ≤ 0
...
### 3.1.4 What We Do DIFFERENTLY: Enhanced Phase Change Model

| Aspect | Standard OpenFOAM Implementation | Our Enhanced Implementation | Why It Matters |
|--------|----------------------------------|-----------------------------|----------------|
| **Source Term Linearization** | ใช้ simple linearization: `S_p = -|ṁ|/(ρ*α)` | ใช้ adaptive linearization based on local phase fraction และ temperature gradient | ป้องกัน numerical instability ใน regions ที่ α → 0 หรือ α → 1 |
| **Driving Force Calculation** | `max(T - T_{\text{sat}}, 0)/T_{\text{sat}}` | ใช้ smooth function: `tanh((T - T_{\text{sat}})/ΔT)/T_{\text{sat}}` ด้วย small ΔT | ลด numerical oscillations รอบ interface โดยยังรักษา sharp transition |
| **Coefficient Bounding** | ใช้ fixed bounds สำหรับ r_coeff | Dynamic bounding based on local CFL number และ phase fraction | อัตโนมัติปรับความเข้มของ phase change ตาม stability requirements |
...
## 3.2 Class Analysis: `fvScalarMatrix`
...
### 3.2.1 Matrix Structure and Storage
...
### 3.2.2 Source Term Handling in `fvMatrix.C`
...
graph TB
    ...
    subgraph "Phase Change Module ([[Day 11: Phase Change Theory (Lee Model & Linearization)|Day 11]])"
        PC_Solver[PhaseChangeSolver] --> LeeModel
        PC_Solver --> ThermalCoupler

        LeeModel --> |"ṁ, S_u_α, S_p_α"| AlphaEqn[Alpha Equation]
        LeeModel --> |"ṁ"| PressureEqn[Pressure Eqn]

        ThermalCoupler --> |"S_u_T, S_p_T"| TEqn[Energy Equation]
        ThermalCoupler --> |"T_{\text{sat}}, h_lv"| LeeModel
    end

    subgraph "Two-Phase Foundation ([[Day 10: Two-Phase Fundamentals (VOF & MULES)|Day 10]])"
        AlphaEqn --> VOF[VOF Transport]
        VOF --> MixtureProps[Mixture Properties<br/>ρ, μ, cp, k]
...
**คำอธิบายสถาปัตยกรรม:**
1.  **PhaseChangeSolver** เป็น **Orchestrator Class** ที่ควบคุมลำดับการคำนวณทั้งหมด รับข้อมูลจากฟิลด์ปัจจุบัน (T, α, p) และเรียกใช้โมดูลย่อยเพื่อคำนวณ Source Terms ที่เหมาะสมสำหรับแต่ละสมการ
2.  **LeeModel** เป็นคลาสที่ encapsulate การคำนวณอัตราการเปลี่ยนมวล (ṁ) ตามสมการใน Theory Section 11.1 และทำ Linearization ของ Source Term สำหรับสมการปริมาตร (α)
...
4.  การไหลของข้อมูลเป็นสองทาง (Bidirectional Coupling): `T` และ `α` จากฟิลด์ปัจจุบันถูกใช้คำนวณ `ṁ` → `ṁ` ถูกใช้สร้าง Source Term ในสมการ `α`, `T`, และ `p` → การแก้สมการได้ฟิลด์ `α`, `T`, `p` ใหม่ → วนลูป
...
1.  **การคำนวณ `mDot`:** Implement สมการจาก Theory Section 11.1 อย่างเคร่งครัด โดยต้องใช้ `max(T - TSat_, deltaT_)` เพื่อป้องกันการคำนวณค่าลบและเพิ่ม Numerical Stability รอบๆ ค่า `T_{\text{sat}}`
    ...
    ```
2.  **การทำ Linearization:** เมธอด `linearizeSource` เป็นหัวใจสำคัญ รับ `sourceValue` (เช่น ค่า `ṁ` ที่คำนวณได้) และฟิลด์ `phiField` (เช่น `alpha`) แล้วคำนวณ `Su` และ `Sp` โดยใช้เทคนิค "Negative Slope Linearization" เพื่อให้ `Sp ≤ 0` เสมอ:
    ```cpp
...
public:
    ...
    // Member Functions
        ...
        //- Convert temperature to enthalpy (if using enthalpy
# 5. Section 4: Implementation
## 5.1 ไฟล์ Header: `PhaseChangeSolver.H`
...
Description
    ...
    หน้าที่หลัก:
    1. คำนวณ mass transfer rate (ṁ) ด้วย Lee Model
    2. Linearize source terms เป็นรูปแบบ S_u + S_p*ϕ
...
// ************************************************************************* //
```text
## 5.2 ไฟล์ Implementation: `PhaseChangeSolver.C`
...
)
{
    ...
    // Evaporation: เมื่อ T > T_{\text{sat}} (liquid → vapor)
    // ṁ_lv = r_coeff * α_l * ρ_l * max(T - T_{\text{sat}}, 0) / T_{\text{sat}}
    ...
    // Condensation: เมื่อ T < T_{\text{sat}} (vapor → liquid)
    // ṁ_vl = r_coeff * α_v * ρ_v * max(T_{\text{sat}} - T, 0) / T_{\text{sat}}
    // โดย α_v = 1 - α_l
    ...
    // Net mass transfer rate: ṁ = ṁ_lv - ṁ_vl
...
)
{
    ...
    // ∂(α_l)/∂t + ∇·(α_l U) = S_α
    // โดย S_α = (ṁ_vl - ṁ_lv)/ρ_l = -ṁ/ρ_l
    
    // Linearized form: S_α = S_u,α + S_p,α * α_l
    
    // คำนวณ derivatives ของ mass transfer rates ตาม α_l
    ...
    // Total derivative: dS_α/dα_l = (dṁ_vl/dα_l - dṁ_lv/dα_l)/ρ_l
    ...
    // S_α(α) ≈ S_α(α_old) + (dS_α/dα)_old * (α - α_old)
    //         = [S_α(α_old) - (dS_α/dα)_old * α_old] + (dS_α/dα)_old * α
    //         = S_u,α + S_p,α * α
...
## 6.1 การสร้าง Build System ด้วย CMake
...
```cmake# CMakeLists.txt - Root Level
...
```cmake# src/CMakeLists.txt# สร้าง shared library สำหรับ phase change components
...
```cmake# apps/CMakeLists.txt# สร้าง executable สำหรับ phase change solver
...
### 6.1.4 การ Configure และ Build

สร้าง build script สำหรับ convenience:

```bash
#!/bin/bash# build.sh - Build script for Phase Change Solver
...
### 6.2.1 Test Suite Structure

```cmake# test/CMakeLists.txt# Enable testing
enable_testing()
...
### 6.2.2 Unit Test สำหรับ Lee Model
...
// Test 1: Evaporation เมื่อ T > T_{\text{sat}}
TEST_F(LeeModelTest, EvaporationCalculation) {
    // ตั้งค่า T > T_{\text{sat}} สำหรับ evaporation
...
// Test 2: Condensation เมื่อ T < T_{\text{sat}}
...
// Test 3: No phase change เมื่อ T = T_{\text{sat}}
TEST_F(LeeModelTest, NoPhaseChangeAtSaturation) {
    volScalarField T(leeModel->mesh().lookupObject<volScalarField>("T"));
    T = dimensionedScalar("T", dimTemperature, 373.15);  // Exactly T_{\text{sat}}
...
### 6.2.3 Unit Test สำหรับ Source Term Linearization
...
TEST_F(SourceTermLinearizationTest, EvaporationSourceLinearization) {
    // สร้าง evaporation source term: S = r_coeff * alpha * rho * (T-T_{\text{sat}})/T_{\text{sat}}
    // สำหรับ alpha = 0.5, T-T_{\text{sat}} = 10K, r_coeff = 0.1, rho = 1000, T_{\text{sat}} = 373
    scalar alpha = 0.5;
    scalar deltaT = 10.0;
    scalar rCoeff = 0.1;
    scalar rho = 1000.0;
...
TEST_F(SourceTermLinearizationTest, CondensationSourceLinearization) {
    ...
    scalar deltaT = -5.0;  // T < T_{\text{sat}}
    scalar rCoeff = 0.1;
...
> > **คำถาม:** ทำไมใน Finite Volume Method โดยเฉพาะในกรณีที่มี Phase Change ที่มี Source Term ขนาดใหญ่ ถึงจำเป็นต้องทำ Linearization เป็นรูปแบบ `S_u + S_p*ϕ`? การใช้ Explicit Source Term (`S_u` เพียงอย่างเดียว) จะส่งผลเสียต่อ Numerical Stability อย่างไร?
> >
> > **คำตอบเชิงลึก:**
> > 1.  **หลักการของ Diagonal Dominance:** ระบบสมการพีชคณิต (ระบบเมทริกซ์ `A x = b`) จาก FVM จะมี Numerical Stability สูงเมื่อเมทริกซ์ `A` มีคุณสมบัติ **Diagonally Dominant** กล่าวคือ ค่าสัมบูรณ์ของสมาชิกในแนวทแยงมุม (Diagonal) ของแต่ละแถว ต้องมากกว่าหรือเท่ากับผลรวมของค่าสัมบูรณ์ของสมาชิกนอกแนวทแยงในแถวเดียวกัน (`|A_ii| ≥ Σ_{j≠i} |A_ij|`) การเพิ่ม Explicit Source Term (`S_u`) ลงในเวกเตอร์ด้านขวา (`b`) เท่านั้น **ไม่ส่งผลใดๆ ต่อค่าสัมประสิทธิ์ในแนวทแยงมุมของเมทริกซ์ `A`** ทำให้คุณสมบัติ Diagonal Dominance ของระบบไม่เปลี่ยนแปลงและอาจอ่อนลงได้หาก Source Term มีขนาดใหญ่
> > 2.  **กลไกของ Implicit Contribution:** รูปแบบ Linearized `S(ϕ) ≈ S_u + S_p * ϕ` ช่วยให้เราสามารถแยก Source Term ออกเป็นสองส่วน:
> >     *   `S_u`: Constant Part หรือ Explicit Part ที่ถูกเพิ่มเข้าไปใน Source Vector `b`
> >     *   `S_p * ϕ`: Implicit Part หรือ Linear Part ที่ `ϕ` คือตัวแปรที่กำลังแก้ (เช่น `α_l`, `T`) **ส่วนนี้จะถูกเพิ่มเข้าไปในแนวทแยงมุมของเมทริกซ์ `A`** (หากใช้ `fvm::Sp(Sp, phi)` ใน OpenFOAM) การเลือก `S_p` ให้มีค่า **น้อยกว่าหรือเท่ากับศูนย์ (`S_p ≤ 0`)** จะทำให้ค่าสัมบูรณ์ในแนวทแยงมุม (`|A_ii|`) เพิ่มขึ้น ส่งผลให้เมทริกซ์มี Diagonal Dominance ที่แข็งแกร่งขึ้น และเพิ่ม Numerical Stability อย่างมีนัยสำคัญ
> > 3.  **ผลกระทบของการใช้ Explicit Source เท่านั้น:** ในบริเวณ Interface ที่มีการเปลี่ยนเฟสรุนแรง (High `r_coeff` หรือ `ΔT` ใหญ่) Mass Transfer Rate `ṁ` จะมีค่าสูงมาก Source Term `S_u` ที่คำนวณจากค่า `ϕ` จาก Iteration ที่แล้ว (Explicit) จะมีขนาดใหญ่และเปลี่ยนแปลงรวดเร็ว การอัพเดทค่า `ϕ` ใน Iteration ใหม่อาจทำให้เกิดการแกว่งกวัด (Oscillation) ของ Solution ได้ง่าย เนื่องจากระบบขาด "แรงดึงกลับ" (Restoring Force) จาก Diagonal Term ที่จะ Stabilize การเปลี่ยนแปลงนี้ นำไปสู่ Divergence ของ Solver ในที่สุด
> > 4.  **Physical Analogy:** เปรียบเทียบได้กับระบบ Spring-Mass-Damper การใช้ Explicit Source เท่านั้นคือการกระตุ้นระบบด้วยแรงภายนอก (`S_u`) โดยไม่มี Spring Constant (`S_p`) ที่จะต้านการเคลื่อนที่มากเกินไป ทำให้ระบบตอบสนองรุนแรงและไม่เสถียร การเพิ่ม Implicit Part (`S_p*ϕ`) ก็เหมือนการเพิ่ม Spring Constant ที่เหมาะสมเพื่อควบคุมการตอบสนองของระบบให้อยู่ในขอบเขตที่ Stable
> >
...
> > }
> > ```text
> [!QUESTION] Concept Check 2: คำถามที่ 2: การเลือกค่า `r_coeff` ใน Lee Model และผลกระทบต่อ Simulation
>
> > [!SUCCESS]- เฉลย (คลิกเพื่ออ่านคำตอบ)
> > **คำถาม:** Empirical Coefficient `r_coeff` ใน Lee Model มีบทบาทอย่างไร? การเลือกค่าที่ "สูงเกินไป" หรือ "ต่ำเกินไป" จะส่งผลต่อ Dynamics ของ Interface, Numerical Stability และ Physical Accuracy อย่างไร? มีหลักการหรือ Rule of Thumb ในการเลือกค่าเริ่มต้นหรือไม่?
> >
> > **คำตอบเชิงลึก:**
> > 1.  **บทบาททางฟิสิกส์ของ `r_coeff`:** `r_coeff` (หน่วย `s⁻¹`) ทำหน้าที่เป็น **Relaxation Time Constant** หรือ **Mass Transfer Intensity Factor** ในสมการ `ṁ = r_coeff * α ρ * (T - T_{\text{sat}})/T_{\text{sat}}` มันควบคุมอัตราการตอบสนอง (Response Rate) ของระบบต่อ Driving Force `(T - T_{\text{sat}})` ค่าที่สูงหมายความว่า Interface จะตอบสนองต่อความแตกต่างของอุณหภูมิได้รวดเร็วมาก (Phase Change รวดเร็ว) ค่าต่ำหมายความว่าต้องใช้เวลานานกว่ามวลจะถูกถ่ายโอน
> > 2.  **ผลของ `r_coeff` สูงเกินไป (Over-damped System):**
> >     *   **Numerical:** Source Term `ṁ` จะมีขนาดใหญ่มาก ทำให้เกิด Numerical Stiffness สูง แม้จะทำ Linearization แล้ว เมทริกซ์ก็ยังอาจมี Eigenvalues ที่แตกต่างกันมาก (High Condition Number) ทำให้ Iterative Solver ทำงานช้า หรือ Divergence ได้ง่าย
...
> >     *   **Manifestation:** อาจเห็น Oscillation ของ `T` และ `α` รอบ Interface, Time Step ลดลงมากจน Simulation ช้า, หรือ Solver ล้มเหลว
> > 3.  **ผลของ `r_coeff` ต่ำเกินไป (Under-damped System):**
...
> >     *   **Scale Analysis:** `r_coeff` ควรสัมพันธ์กับ Time Scale ของปรากฏการณ์ที่สนใจ เช่น สำหรับ Boiling Bubble Growth อาจพิจารณาจาก `r_coeff ~ 1 / τ_growth` โดย `τ_growth` คือ Characteristic Time ของการขยายตัวของฟอง
> >     *   **Relation to Numerical Parameters:** ค่า `r_coeff` ควรสัมพันธ์กับ Time Step (`Δt`) ที่ใช้ หลักการคือ `r_coeff * Δt` ควรมีค่าอยู่ในลำดับ `O(1)` หรือน้อยกว่าเพื่อ Stability ถ้า `r_coeff * Δt >> 1` แสดงว่าในหนึ่ง Time Step ระบบพยายามจะเปลี่ยนเฟสในปริมาณที่มากเกินไป ซึ่งไม่ Stable
> >     *   **Typical Range:** สำหรับการประยุกต์ทั่วไปกับของไหลเช่นน้ำหรือสารทำความเย็น ค่า `r_coeff` มักอยู่ในช่วง **0.1 ถึง 100 `s⁻¹`** การเริ่มต้นด้วยค่าปานกลาง (~1-10 `s⁻¹`) แล้วปรับตามผลลัพธ์เป็นวิธีที่ปลอดภัย
> >     *   **Sensitivity Study:** ควรทำ Parametric Study โดยรัน Simulation สั้นๆ ด้วยค่า `r_coeff` ที่แตกต่างกัน (เช่น 0.1, 1, 10, 100) เพื่อดูผลต่อ Interface Velocity และ Stability ของ Solver แล้วเลือกค่าที่ให้ผลสมเหตุสมผลและ Stable
> >     *   **คำนึงถึง Density Ratio:** สำหรับของไหลที่มี Density Ratio สูงมาก (เช่น R410A, `ρ_l/ρ_v ~ 22`) การเปลี่ยนเฟสเพียงเล็กน้อยทำให้ Volume เปลี่ยนแปลงมหาศาล (จาก Expansion Term) ควรเริ่มด้วย `r_coeff` ที่ต่ำกว่าเพื่อลด Numerical Stiffness จาก Pressure Equation
> [!QUESTION] Concept Check 3: คำถามที่ 3: การรักษา Mass Conservation ระหว่าง Phase ในการ Implement Lee Model
>
> > [!SUCCESS]- เฉลย (คลิกเพื่ออ่านคำตอบ)
> > **คำถาม:** ในสมการ Continuity ของแต่ละ Phase: `∂(α_l ρ_l)/∂t + ∇·(α_l ρ_l U) = -ṁ_lv + ṁ_vl` และ `∂(α_v ρ_v)/∂t + ∇·(α_v ρ_v U) = +ṁ_lv - ṁ_vl` การ Implement ต้องรับประกันว่า Total Mass ของระบบ `∫_V (α_l ρ_l + α_v ρ_v) dV` ถูกอนุรักษ์อย่างไร? ข้อผิดพลาดใดที่พบบ่อยที่ทำให้เกิด Mass Error และจะป้องกันได้อย่างไร?
> >
> > **คำตอบเชิงลึก:**
> > 1.  **หลักการ Conservation ที่ต้องรักษา:** Net Mass Transfer ระหว่าง Phase ต้องเป็นศูนย์เมื่อพิจารณาทั้งระบบ กล่าวคือ **`ṁ_lv` และ `ṁ_vl` ที่คำนวณสำหรับ Cell เดียวกัน ต้องมีขนาดเท่ากันทุกประการ แต่มีเครื่องหมายตรงข้ามกัน** เมื่อรวม Source Terms เข้าด้วยกันสำหรับสมการ Mixture (`α_l+α_v=1`) จะได้ว่า `S_m,mixture = S_m,l + S_m,v = (-ṁ_lv+ṁ_vl) + (+ṁ_lv-ṁ_vl) = 0` ซึ่งสอดคล้องกับสมการ Continuity ของ Mixture ที่ไม่มี Source Term
> > 2.  **ข้อผิดพลาด Implement ที่พบบ่อย:**
> >     *   **การใช้ Conditional Statement แยกกัน:** การคำนวณ `ṁ_lv` และ `ṁ_vl` ด้วย `if-else` แยกกัน เช่น `if(T > T_{\text{sat}}) { ṁ_lv = r_coeff*α_l*ρ_l*(T-T_{\text{sat}})/T_{\text{sat}}; ṁ_vl = 0; } else { ṁ_lv = 0; ṁ_vl = r_coeff*α_v*ρ_v*(T_{\text{sat}}-T)/T_{\text{sat}}; }` วิธีนี้ดูเหมือนถูกต้อง แต่ปัญหาอยู่ที่ **บริเวณ Interface ที่ `T ≈ T_{\text{sat}}`** เนื่องมาจาก Numerical Error เล็กน้อย `T` อาจกระโดดข้าม `T_{\text{sat}}` ระหว่าง Iterations ทำให้ `ṁ_lv` และ `ṁ_vl` ถูกคำนวณสลับกันและมีค่าไม่เท่ากันพอดี ส่งผลให้เกิด Local Mass Imbalance
> >     *   **การคำนวณ Driving Force แยกกัน:** การใช้ `max(T-T_{\text{sat}},0)` และ `max(T_{\text{sat}}-T,0)` แยกกันสำหรับ Evaporation และ Condensation อาจทำให้ผลลัพธ์จากฟังก์ชัน `max` ไม่ Symmetric อย่างสมบูรณ์แบบเนื่องจาก Round-off Error
> >     *   **การอัพเดท Properties ไม่พร้อมกัน:** การคำนวณ `ṁ` โดยใช้ `ρ_l` และ `ρ_v` จาก Time Step หรือ Iteration ที่ต่างกัน (หาก Properties ถูกอัพเดทในขั้นตอนอื่น) อาจทำให้ค่า Density ที่ใช้ไม่สอดคล้องกัน
> > 3.  **กลยุทธ์การ Implement เพื่อรับประกัน Conservation:**
> >     *   **วิธีที่ 1: คำนวณ Net Mass Transfer Rate เดียว:** กำหนด `ṁ_net = r_coeff * (α_l ρ_l * (T - T_{\text{sat}}) / T_{\text{sat}})` โดยอนุญาตให้ `ṁ_net` เป็นได้ทั้งบวกและลบ (บวก = Evaporation, ลบ = Condensation) จากนั้นกำหนด:
> >         ```cpp
> >         ṁ_lv = max(ṁ_net, 0.0); // Evaporation rate (>=0)
> >         ṁ_vl = max(-ṁ_net, 0.0); // Condensation rate (>=0)
> >         S_m,l = -ṁ_lv + ṁ_vl; // = -ṁ_net
> >         S_m,v = +ṁ_lv - ṁ_vl; // = +ṁ_net
> >         ```
> >         วิธีนี้รับประกันว่า `S_m,l + S_m,v = 0` โดย construction
> >     *   **วิธีที่ 2: ใช้ Symmetric Conditional:** ใช้ฟังก์ชัน `sign()` หรือการตรวจสอบที่แน่นอนเดียวกันสำหรับทั้งสอง Phase:
> >         ```cpp
> >         scalar drivingForce = T - T_{\text{sat}};
> >         scalar coeff = r_coeff_ / T_{\text{sat}};
> >         if(drivingForce > 0) {
> >             // Evaporation
> >             ṁ_lv = coeff * alpha_l * rho_l * drivingForce;
> >             ṁ_vl = 0.0;
> >         } else {
> >             // Condensation
> >             ṁ_lv = 0.0;
> >             ṁ_vl = coeff * alpha_v * rho_v * (-drivingForce); // ใช้ค่า drivingForce เดียวกันแต่กลับเครื่องหมาย
> >         }
> >         // S_m,l และ S_m,v ยังคงคำนวณจาก ṁ_lv และ ṁ_vl ข้างต้น
> >         ```
> >         ต้องมั่นใจว่า `coeff` และ `drivingForce` เป็นตัวแปรเดียวกันที่ใช้คำนวณทั้งสองกรณี
> >     *   **วิธีที่ 3: Post-Processing Enforcement:** หลังจากคำนวณ `ṁ_lv` และ `ṁ_vl` แล้ว ให้ทำการ Normalize หรือ Average เพื่อบังคับให้ Sum เป็นศูนย์ในแต่ละ Cell เช่น `ṁ_avg = (ṁ_lv - ṁ_vl)/2; ṁ_lv = ṁ_avg; ṁ_vl = -ṁ_avg;` แต่วิธีนี้อาจทำให้ Physical Accuracy ลดลง
...
>
> > [!SUCCESS]- เฉลย (คลิกเพื่ออ่านคำตอบ)
> > **คำถาม:** จาก Expansion Term `∇·U = ṁ (1/ρ_v - 1/ρ_l)` ที่ได้จาก [[Day 01: Governing Equations|Day 01]] จะถูก Integrate เข้ากับ Pressure-Velocity Coupling Algorithm (เช่น PISO/SIMPLE จาก [[Day 09: Pressure-Velocity Coupling (SIMPLE, PISO, Rhie-Chow)|Day 09]]) อย่างไรโดยเฉพาะ? Source Term นี้ทำให้ Pressure Equation `∇·(rAU ∇p) = ∇·(U*) - S_p` เปลี่ยนแปลงไปอย่างไร และการมีอยู่ของ Term นี้เพิ่มความท้าทายด้าน Numerical Stability อย่างไร?
> >
> > **คำตอบเชิงลึก:**
> > 1.  **ที่มาของ Expansion Term ใน Pressure Equation:** เริ่มจากสมการ Momentum ที่ Discrete แล้วได้ความสัมพันธ์ระหว่าง Velocity Correction และ Pressure Gradient: `U = U* - rAU ∇p` (โดย `U*` คือ Predicted Velocity, `rAU = 1/A_p` จาก Momentum Equation) นำ Divergence ของสมการนี้:
> >     `∇·U = ∇·U* - ∇·(rAU ∇p)`
> >     แต่จากสมการ Continuity ที่มี Phase Change เรารู้ว่า `∇·U = ṁ (1/ρ_v - 1/ρ_l) = S_p` (กำหนดให้ `S_p` คือ Source Term ใน Pressure Equation)
> >     ดังนั้นจะได้ **Pressure Equation with Phase Change**:
> >     ```
> >     ∇·(rAU ∇p) = ∇·(U*) - S_p
> >     ```
> >     เปรียบเทียบกับ Pressure Equation แบบไม่มี Phase Change (`∇·(rAU ∇p) = ∇·(U*)`) จะเห็นว่ามี Source Term `-S_p` เพิ่มเข้ามาทางด้านขวา
> > 2.  **Physical Interpretation:** Term `S_p = ṁ (1/ρ_v - 1/ρ_l)` แสดงถึง **Rate of Volume Generation/Consumption** เนื่องจาก Phase Change เมื่อของเหลวระเหยเป็นไอ (`ṁ > 0`) และ `ρ_v << ρ_l` ดังนั้น `(1/ρ_v - 1/ρ_l) > 0` ทำให้ `S_p > 0` ส่งผลให้ด้านขวาของ Pressure Equation `∇·U* - S_p` มีค่าน้อยลง (หรือเป็นลบ) ซึ่งหมายความว่า Pressure Field ต้องปรับตัวเพื่อสร้าง Velocity Div
...
1.  **Lee, W. H. (1980). *A Pressure Iteration Scheme for Two-Phase Flow Modeling.***
    ...
    *   **หัวใจสำคัญ:** แนะนำรูปแบบ `ṁ ∝ (T - T_{\text{sat}})` สำหรับการคำนวณอัตราการเปลี่ยนมวล ซึ่งเป็นพื้นฐานของ implementation ในบทเรียนนี้
...
3.  **Jasak, H. (1996). *Error Analysis and Estimation for the Finite Volume Method with Applications to Fluid Flows.***
    ...
    *   **หัวใจสำคัญ:** ให้เหตุผลทางคณิตศาสตร์ที่ชัดเจนว่าทำไมเราต้องเขียน source term ในรูป `S_u + S_pφ` และทำไม `S_p` ต้องน้อยกว่าหรือเท่ากับศูนย์เพื่อรักษา stability
...
4.  **Rusche, H. (2002). *Computational Fluid Dynamics of Dispersed Two-Phase Flows at High Phase Fractions.***
    *   **ความเกี่ยวข้อง:** Derivation ของ governing equations สำหรับ two-phase flow รวมถึงการบัญชีสำหรับ phase change และการได้มาซึ่ง *expansion term* `∇·U = ṁ(1/ρ_v - 1/ρ_l)`
...
5.  **"Thermophysical Properties of Refrigerants" (ASHRAE Handbook - Fundamentals)**
    *   **ความเกี่ยวข้อง:** แหล่งข้อมูลสำหรับ saturation temperature (`T_{\text{sat}}`), latent heat (`h_lv`), และคุณสมบัติของ liquid/vapor (`ρ_l`, `ρ_v`, `c_p`) สำหรับสารทำงานต่างๆ เช่น R134a, R410A, น้ำ
...
*   **Day 01: Governing Equations**
    *   **Connection:** **Hero Concept: Expansion Term** `∇·U = ṁ(1/ρ_v - 1/ρ_l)` ที่ derive ใน [[Day 01: Governing Equations|Day 01]] กลายเป็น source term `S_p` ใน pressure equation (Section 11.4) โดยตรง
...
*   **Day 02: FVM Basics & Day 03: Spatial Discretization**
    *   **Connection:** การคำนวณ `ṁ` ใน Lee Model ใช้ค่า `α`, `ρ`, `T` ที่ cell center (เป็น `volScalarField`) จากนั้น source term นี้ต้องถูก *integrate* ลงในสมการผ่านกระบวนการ Finite Volume Method
...
*   **Day 05: Mesh Topology & Day 07: Linear Algebra (LDU)**
    *   **Connection:** เมื่อเราเพิ่ม linearized source term `S_p*φ` ลงในสมการ เรากำลังแก้ไขค่า *diagonal* (`D`) ของ `lduMatrix` โดยตรง
    *   **Flow of Logic:** Day 05/07 สอนโครงสร้าง `lduMatrix` (L, D, U) → Day 11 ใช้ความรู้นั้นเพื่อเข้าใจว่า `S_p ≤ 0` ช่วยเพิ่ม *diagonal dominance* ของ matrix `A` อย่างไร ทำให้ iterative solver ([[Day 08: Iterative Solvers (PCG & PBiCGStab)|Day 08]]) ทำงานได้ `เสถียร` ขึ้น
    *   **Engineering Thai Context:** การที่เราบังคับ `S_p ≤ 0` ใน implementation ไม่ใช่กฎไร้เหตุผล แต่เป็น requirement โดยตรงจากทฤษฎี matrix stability ที่เรียนใน [[Day 07: Linear Algebra (LDU)|Day 07]]
...
*   **Day 06: Boundary Conditions**
    ...
    *   **Flow of Logic:** Day 06 สอนวิธีกำหนด `fixedValue` หรือ `fixedGradient` สำหรับ `T` → Day 11 ค่า `T` ที่ boundary นี้จะส่งผลต่อการคำนวณ `ṁ` ใน cells ที่ติดกับผนัง
...
*   **Day 09: Pressure-Velocity Coupling**
    *   **Connection:** **Key Link** การมี source term `S_p = ṁ(1/ρ_v - 1/ρ_l)` ใน pressure equation (Section 11.4) เป็นการปรับเปลี่ยนขั้นพื้นฐานของอัลกอริทึม `PISO`/`SIMPLE` จาก Day 9
    *   **Flow of Logic:** Day 09 สอน `pEqn: laplacian(rAU, p) = div(phi)` → Day 11 ปรับเป็น `pEqn: laplacian(rAU, p) = div(phi) - S_p` เพื่อรับมือกับ volume expansion จาก phase change
...
*   **Day 10: Two-Phase Fundamentals (VOF)**
    *   **Connection:** **Most Direct Predecessor** Lee Model ใช้ `α` (volume fraction) จาก [[Day 10: Two-Phase Fundamentals (VOF & MULES)|Day 10]] เป็นตัวแปรสำคัญในการคำนวณ `ṁ` (`ṁ ∝ α_l` หรือ `α_v`)
    *   **Flow of Logic:** Day 10 สอนวิธี transport `α` → Day 11 เพิ่ม source term `S_α` ลงในสมการ transport ของ `α` นั้นเอง เพื่อให้ interface เคลื่อนที่จากผลของ phase change ไม่ใช่แค่ fluid advection
...
### 8.3 บทเรียนต่อไป (Forward Look: Day 12)
