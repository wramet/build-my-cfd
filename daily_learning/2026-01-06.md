...
- **Scalar Fields (T, α, etc.):** สำหรับสนามสเกลาร์อื่นๆ เช่น อุณหภูมิ $T$ หรือ volume fraction $\alpha$ เราสามารถกำหนดค่าได้ตามสภาวะทางเข้า เช่น $T_b = T_{\text{in}}$ หรือ $\alpha_b = 0$ (ทางเข้าคือไอ)
...
### 3.1.1 Header File Analysis (`GeometricBoundaryField.H`)
...
### 3.1.2 Source File Analysis (`GeometricBoundaryField.C`)
...
### 3.1.3 การทำงานของ `GeometricBoundaryField` ใน Practice
...
### 3.1.4 What We Do DIFFERENTLY ใน CFD Engine ของเรา
...
### 3.2.1 Base Class Analysis (`fvPatchField.H`)
...
### 3.2.2 การทำงานของ `manipulateMatrix()` Method
...
### 3.2.3 Derived Classes ที่สำคัญ
...
2. **`zeroGradientFvPatchField`** (Neumann with zero gradient):
   ...
   // Key feature: ∂φ/∂n = 0 โดย implicit ผ่าน matrix coefficients
   ```
...
### 3.2.4 What We Do DIFFERENTLY ใน CFD Engine ของเรา
...
### 3.3.1 Geometric Information ใน `fvPatch`
...
### 3.3.2 Implementation ของ Geometric Field Calculations
...
### 3.3.4 What We Do DIFFERENTLY ใน CFD Engine ของเรา
...
มาดูการ Implement คลาสลูกสำหรับเงื่อนไขพื้นฐานที่สุด: **Dirichlet** หรือ **Fixed Value** (`φ_b = φ_fixed`)
 ...
 * 
 * Mathematical Form: φ_b = value
 * Matrix Implementation: For an owner cell P adjacent to boundary face f,
 * we want the equation for cell P to implicitly satisfy φ_b = value.
 ...
 * This effectively makes the equation for cell P: A_pp * φ_p ≈ largeNumber * value,
 * forcing φ_p (and by extension, φ_b) towards the specified value.
 * 
...
private:
    FieldType value_;   ///< The fixed value (φ_fixed).
...
```
A_pp * φ_p + Σ A_pN * φ_N = b_p
```
เมื่อเราเพิ่ม `penalty_` เข้าไปใน `A_pp` และเพิ่ม `penalty_ * value` เข้าไปใน `b_p` สมการจะกลายเป็น:
```
(A_pp + penalty_) * φ_p + Σ A_pN * φ_N = b_p + penalty_ * value
```
หาก `penalty_` มีค่ามากมาก (เช่น 1e30) เทอม `A_pp` และ `b_p` เดิมจะถูกลบล้าง สมการมีผลเท่ากับ:
```
penalty_ * φ_p ≈ penalty_ * value
```
ซึ่งบังคับให้ `φ_p ≈ value` และเนื่องจาก `φ_b` มักได้มาจากการ extrapolate จาก `φ_p` (ขึ้นกับ numerical scheme) ก็จะได้ `φ_b ≈ value` ตามที่ต้องการ วิธีนี้รักษา Sparsity Pattern ของเมทริกซ์ไว้ได้ (ไม่เพิ่ม non-zero entries) แต่ต้องระวังเรื่อง **Condition Number** ของเมทริกซ์ที่อาจแย่ลงได้

### 3.2.3 คลาส `ZeroGradientBC` (Neumann Condition with g=0)

เงื่อนไข **Zero Gradient** (`∂φ/∂n = 0`) เป็น Neumann condition ที่พบได้บ่อยที่สุด
 ...
 * 
 * Mathematical Form: ∂φ/∂n = 0
 * Matrix Implementation: For a boundary face f with owner cell P,
 * the flux contribution from the face is approximated using the cell value φ_p.
 * Effectively, we treat φ_f = φ_p. This is often the default "do nothing" condition
...
- ในทางปฏิบัติของ Finite Volume Method สำหรับเทอมเช่น `∇·(Uφ)` หรือ `∇·(Γ∇φ)` การใช้ `zeroGradient` มักหมายความว่าค่า `φ` ที่หน้า boundary `f` จะถูกประมาณด้วยค่า `φ` ของ owner cell `P` (`φ_f = φ_P`) ซึ่งเป็นไปตามสมมติฐาน gradient เป็นศูนย์ในทิศ normal
- ดังนั้น ในขั้นตอน `applyToMatrix` เราอาจ **ไม่ต้องทำอะไร** หากการประกอบเมทริกซ์ของเราได้คำนวณ flux ที่หน้าขอบเขตโดยใช้ `φ_P` อยู่แล้ว
...
### 5.1.1 Base Class: `BoundaryCondition`
...
### 5.1.2 Concrete BC Class: `FixedValueBoundaryCondition`
...
TEST(WallFunctionTest, AppliesShearStressToMatrix) {
    ...
    for (int i = 0; i < patch.size(); ++i) {
        ...
        // Wall shear stress: τ_w = ρ * uτ²
        // ใน momentum equation: source += τ_w * area / volume
...
> > ในสมการ Navier-Stokes สำหรับของไหลแบบ incompressible (`∇·U = 0`) ค่า pressure `p` ที่ปรากฏในสมการ momentum คือ **pressure gradient** (`∇p`) เท่านั้น ไม่ใช่ค่า absolute pressure เอง นี่หมายความว่าถ้าเราบวกค่าคงที่ `C` ใดๆ เข้าไปใน pressure field `p` ทั้งหมดในโดเมน สมการ momentum จะยังคงเหมือนเดิมเพราะ gradient ของค่าคงที่เป็นศูนย์ (`∇C = 0`) ดังนั้น **pressure field จึงถูกกำหนดได้เพียงแค่ up to an additive constant** เราต้องการ reference point หนึ่งจุดเพื่อ "pin down" ค่าคงที่นี้ การกำหนด pressure ที่ outlet (เช่น `p_out = 0` หรือค่าบรรยากาศ) คือการตั้ง reference นั้น
> >
...
> >
> > **ทางเลือกอื่น:**
> > - **Zero Gradient (Neumann):** `∂p/∂n = 0` ที่ outlet ก็ใช้ได้เช่นกัน แต่ต้องมั่นใจว่ามี pressure reference ที่อื่น (เช่น ที่ inlet) มิฉะนั้นระบบยังคง indeterminate
...
> >
> >             // Iterative solution สำหรับ u_τ และ y+
...
4.  **Wilcox, D. C. (2006). *Turbulence Modeling for CFD* (3rd ed.). DCW Industries.**
    *   **ส่วนที่เกี่ยวข้อง:** Chapter 4 - Near-Wall Treatments. เป็นตำราคลาสสิกสำหรับทำความเข้าใจ wall functions, log-law, และการคำนวณ `y+`, `u+`, `u_τ` อย่างละเอียด สำคัญสำหรับการ implement wall boundary conditions สำหรับ turbulent flows
...
### 8.1.2 เอกสารอ้างอิงจาก OpenFOAM (OpenFOAM-Specific References)
...
### 8.1.3 บทความวิจัยและเอกสารเทคนิค (Research Papers & Technical Notes)
...
3.  **Launder, B. E., & Spalding, D. B. (1974). *The numerical computation of turbulent flows*. Computer Methods in Applied Mechanics and Engineering, 3(2), 269-289.**
    *   **ส่วนที่เกี่ยวข้อง:** บทความ seminal ที่แนะนำ standard wall functions (`u+ = (1/κ) ln(y+) + C`) ซึ่งยังคงใช้กันอย่างแพร่หลายใน CFD จนถึงปัจจุบัน
    *   **Link:** [https://doi.org/10.1016/0045-7825(74)90029-2](https://doi.org/10.1016/0045-7825(74)90029-2)
## 8.2 วันก่อนหน้าและวันถัดไปที่เกี่ยวข้อง (Related Previous & Next Days)

### 8.2.1 การเชื่อมโยงกับวันก่อนหน้า (Connection to Previous Days)
...
*   **Day 01: Governing Equations**
    ...
    *   **ตัวอย่าง:** Expansion term `∇·U = ṁ(1/ρ_v - 1/ρ_l)` ต้องมี BCs ที่สอดคล้องกัน เช่น ที่ผนังของ boiling surface อาจต้องใช้ fixed temperature หรือ fixed heat flux ร่วมกับ no-slip condition
...
*   **Day 02: FVM Basics & Day 03: Spatial Discretization**
    ...
    *   **ตัวอย่าง:** การคำนวณ convective flux `(U·S_f) φ_f` ที่ inlet face ถ้าใช้ upwind scheme และ `U` ไหลเข้า domain ค่า `φ_f` จะถูกดึงมาจาก boundary value (`φ_b`) ที่กำหนดโดย fixedValue BC
...
*   **Day 05: Mesh Topology (Owner-Neighbor Addressing)**
    ...
    *   **ตัวอย่าง:** เมื่อ enforce Dirichlet condition `φ_b = value` สำหรับ cell `P` ที่มี boundary face การปรับค่าจะทำที่ `A_P` และ `b_P` ของ cell `P` โดยตรง โดยอาจ set `A_P = largeNumber`, `b_P = largeNumber * value` และ zero out off-diagonal coefficients ที่เชื่อมโยงกับ face นั้น

### 8.2.2 การเชื่อมโยงกับวันถัดไป (Connection to Next Days)
...
*   **Day 08: Iterative Solvers (CG, BiCGStab)**
    ...
    *   **ตัวอย่าง:** การใช้ wall functions ซึ่งสร้าง source terms ที่ไม่เป็นเชิงเส้น (non-linear) และขึ้นกับ solution เอง (`u_τ`) อาจต้องการ outer iteration (ระหว่าง momentum solve และ BC update) เพื่อให้ระบบลู่เข้า ซึ่งเกี่ยวข้องกับ algorithm ใน [[Day 08: Iterative Solvers (PCG & PBiCGStab)|Day 08]]
...
*   **Day 10: Two-Phase Fundamentals (VOF, α)**
    *   **ความเชื่อมโยง:** ใน two-phase flows, boundary conditions สำหรับ volume fraction field (`α`) มีความซับซ้อน เช่น ที่ inlet อาจมีทั้ง liquid และ gas เข้ามาพร้อมกัน (ใช้ `inletOutlet` หรือ `fixedValue` ผสม) ที่ wall อาจต้องพิจารณา contact angle (ใช้ `alphaContactAngle` boundary condition)
    *   **ตัวอย่าง:** การ implement `BoundaryCondition` class ในวันนี้ควรออกแบบให้รองรับ field types ต่างๆ ได้ รวมถึง `volScalarField` สำหรับ `α` ซึ่งจะถูกใช้เต็มที่ใน [[Day 10: Two-Phase Fundamentals (VOF & MULES)|Day 10]]

*   **Day 11: Phase Change Theory (Lee Model, Expansion Term)**
    *   **ความเชื่อมโยง:** กระบวนการ phase change (การเดือด/การกลั่นตัว) มักเกิดขึ้นที่ขอบเขต (wall) หรือ interface การกำหนด boundary conditions ที่ผนังสำหรับ temperature (`T`) และ coupling กับ mass transfer rate (`ṁ`) จาก Lee model เป็นเรื่องสำคัญ
    *   **ตัวอย่าง:** ที่ผนังร้อน (heating wall) อาจต้องใช้ fixed heat flux (`q_w = constant`) หรือ fixed temperature (`T_w = constant`) BC ซึ่งจะเป็น driving force สำหรับการคำนวณ `ṁ` ใน [[Day 11: Phase Change Theory (Lee Model & Linearization)|Day 11]] และส่งผลต่อ expansion term ใน continuity equation
## 8.3 โมดูลที่เกี่ยวข้องในหลักสูตร (Related Course Modules)
