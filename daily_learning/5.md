...
> **สรุป Section 1:** Mesh topology is the foundation of CFD simulations, defining how points, faces, and cells connect to represent the computational domain. For evaporator flows with phase change, proper mesh design is critical because it directly impacts accuracy, stability, and conservation properties. The owner-neighbor convention and face-based storage are essential design choices that enable efficient flux calculations and consistent discretization.
...
> **สรุป Section 4:** This section implemented a complete mesh topology system with Point, Face, Cell, Mesh, BoundaryPatch, and MeshQuality classes. The implementation uses face-based storage with owner-neighbor addressing, computes geometry via Gauss's theorem, and includes comprehensive validation and quality checking. Unit tests verify correctness for hexahedral and tetrahedral meshes, ensuring the foundation is solid for building the finite volume solver.
...
### Question 1: Why does the face normal ALWAYS point from owner to neighbor, and what happens if you get this wrong?

> **Answer:** The owner-neighbor convention is critical for flux consistency in finite volume methods. When you compute flux through a face, the sign depends on direction: positive flux = owner→neighbor. If the face normal points the wrong way, your flux signs will be inverted, causing immediate mass imbalance and solver divergence.
>
> **For evaporator simulation:** This is especially dangerous because the expansion term $\nabla \cdot \mathbf{U} = \dot{m}''(1/\rho_v - 1/\rho_l)$ is already large (density ratio ~1000). If your face normals are inconsistent, you'll have TWO sources of mass imbalance instead of one, and the pressure equation will never converge.
>
> **Implementation check:** After building connectivity, verify that for every internal face: $\mathbf{n}_f \cdot (\mathbf{x}_N - \mathbf{x}_P) > 0$. For boundary faces, normals should point OUT of the domain.

### Question 2: Why use Gauss's theorem ($V = \frac{1}{3}\sum \mathbf{S}_f \cdot \mathbf{x}_f$) instead of shape-specific formulas for cell volume?

> **Answer:** Gauss's theorem works for ANY polyhedral cell without needing shape-specific logic. This makes your code more flexible and maintainable. Shape-specific formulas (like hexahedron volume from edge vectors) require you to detect cell type and branch, which adds complexity and potential bugs.
>
> **Trade-off:** Gauss's theorem requires face centroids to be computed first, but this is a one-time cost for static meshes. The benefit is that your volume calculation works for hex, tet, and even polyhedral cells without modification.
>
> **For evaporator simulation:** You might start with hex/tet meshes, but as you refine near the liquid-vapor interface, you may generate polyhedral cells. Gauss's theorem handles this automatically.

### Question 3: Why does OpenFOAM use face-based storage instead of cell-based storage, and why should you do the same?

> **Answer:** Finite volume methods compute FLUXES through faces, not cell values. Face-based storage means you loop over faces once per iteration, computing fluxes directly. Cell-based storage would require you to find which faces belong to each cell (O(N_cells × N_faces_per_cell)), which is much slower.
>
> **Owner-neighbor addressing** gives you O(1) access to both cells connected to a face. This is critical for performance because flux calculation is the inner loop of your solver.
>
> **For evaporator simulation:** You'll compute mass flux, heat flux, and momentum flux through every face at every time step. Face-based storage makes this efficient. The expansion term in the continuity equation also requires face fluxes.

### Question 4: What is non-orthogonality, why does it matter for evaporator flows, and how do you handle it?

> **Answer:** Non-orthogonality is the angle between the face normal and the line connecting cell centroids. For orthogonal meshes, this angle is 0°, and gradient calculation is straightforward. For non-orthogonal meshes (common in tube bends), you need correction terms.
>
> **Why it matters:** High non-orthogonality (>70°) causes:
> 1. Incorrect gradient reconstruction (wrong heat flux prediction)
> 2. Pressure-velocity coupling instability (SIMPLE diverges)
> 3. Spurious oscillations in temperature field
>
> **For evaporator simulation:** Tube bends create non-orthogonal meshes. You MUST either:
> 1. Use non-orthogonal correction in gradient calculation
> 2. Use under-relaxation (0.7-0.9) for pressure equation
> 3. Improve mesh quality (limit non-orthogonality to <50° if possible)
>
> **Implementation:** The `MeshQuality` class checks this for you. If `maxNonOrthogonality > 70°`, warn the user before starting the simulation.

### Question 5: Why must you check for negative cell volumes immediately after mesh generation, and what causes them?

> **Answer:** Negative volume means the face normals are inconsistent (some point inward, some outward). This causes the Gauss's theorem sum to be negative, which is physically impossible. Your solver will crash immediately with NaN values.
>
> **Common causes:**
> 1. Wrong point ordering in face definition (clockwise vs counter-clockwise)
> 2. Inverted face normals (not pointing owner→neighbor)
> 3. Degenerate cells (points nearly coplanar)
> 4. Mesh generation errors (especially for complex geometries)
>
> **For evaporator simulation:** You'll likely use external meshing tools (gmsh, snappyHexMesh). These tools can generate invalid meshes if geometry has defects. ALWAYS validate mesh before running:
> ```cpp
> for (const Cell& cell : cells_) {
>     if (cell.getVolume() <= 0.0) {
>         throw std::runtime_error("Cell " + id + " has negative volume!");
>     }
> }
> ```
>
> **Recovery:** If you find negative volumes, check:
> 1. Face point ordering (use right-hand rule)
> 2. Owner-neighbor assignment (first cell = owner)
> 3. Mesh geometry (are points coplanar?)
...
- Previous: [[4]]
- Next: [[6]]
- See also: [[90_day_roadmap]], [[finite_volume_method]], [[boundary_conditions]]
