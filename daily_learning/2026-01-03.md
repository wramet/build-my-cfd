...
### 5.1 Task 1: Implement a Custom Pressure-Velocity Coupling Scheme

**Objective**: Create a custom pressure-velocity coupling algorithm that implements a simplified SIMPLE-C (Consistent) scheme to improve convergence for steady-state simulations.

**Background**: The SIMPLE-C algorithm modifies the standard SIMPLE approach by using a more consistent velocity-pressure coupling, reducing the under-relaxation dependence.

**Steps**:

1. **Create a new solution control class** `simplecControl.H`:

```cpp
#ifndef simplecControl_H
#define simplecControl_H
...
public:
    ...
    // Member Functions
        ...
        virtual bool read()
        {
            if (solutionControl::read())
            {
                const dictionary& solutionDict = dict_;
                
                solutionDict.readIfPresent("maxCorr", maxCorr_);
                solutionDict.readIfPresent("tolerance", tolerance_);
        ...
        //- Calculate momentum coefficient for pressure equation
        //  This is the key difference from SIMPLE - uses consistent coefficients
        tmp<volScalarField> momentumCoefficient(const fvVectorMatrix& UEqn) const
        {
            const volScalarField& rAU = UEqn.A();
            
            // SIMPLE-C uses the reciprocal of diagonal coefficients
            // but with additional consistency terms
            tmp<volScalarField> tMomentumCoeff
            (
                new volScalarField
                (
                    IOobject
                    (
                        "momentumCoeff",
                        UEqn.psi().mesh().time().timeName(),
                        UEqn.psi().mesh(),
                        IOobject::NO_READ,
                        IOobject::NO_WRITE
                    ),
                    1.0 / max(rAU, dimensionedScalar("small", dimless, SMALL))
                )
            );
            
            return tMomentumCoeff;
        }
};
...
2. **Create the solver implementation** `mySimplecFoam.C`:

```cpp
#include "fvCFD.H"
#include "singlePhaseTransportModel.H"
#include "turbulentTransportModel.H"
#include "simplecControl.H"
#include "fvOptions.H"
...
int main(int argc, char *argv[])
{
    ...
    while (simplec.loop())
    {
...

        {
            ...
            // Pressure equation using SIMPLEC consistent coefficients
            tmp<volScalarField> rAU = simplec.momentumCoefficient(UEqn);
            volVectorField HbyA(constrainHbyA(rAU()*UEqn.H(), U, p));
            ...
            // Correct velocity using consistent SIMPLEC formulation
            U = HbyA - rAU()*fvc::grad(p);
            U.correctBoundaryConditions();
        }
...
3. **Create the momentum equation file** `UEqn.H`:
...
EXE = $(FOAM_USER_APPBIN)/mySimplecFoam
EOF
...
# Test on tutorial case
cd $FOAM_TUTORIALS/incompressible/simpleFoam/airFoil2D
cp -r . $FOAM_RUN/test_simplec
cd $FOAM_RUN/test_simplec
mySimplecFoam
```

> [!TIP] **Key Differences from SIMPLE**
> - **Consistent coefficients**: Uses $1/a_P$ directly without additional relaxation
> - **Better convergence**: Typically requires fewer iterations for same tolerance
> - **Reduced relaxation dependence**: Can use higher relaxation factors
> - **Implementation**: The `momentumCoefficient()` method ensures consistency between momentum and pressure equations
> 
> (ข้อดี: การลู่เข้าที่ดีกว่าและลดการพึ่งพาการผ่อนคลาย)

---

### 5.2 Task 2: Implement Adaptive Under-Relaxation

**Objective**: Create an adaptive under-relaxation strategy that automatically adjusts relaxation factors based on residual behavior to optimize convergence.

**Background**: Fixed under-relaxation factors can be suboptimal - too conservative (slow convergence) or too aggressive (instability). Adaptive relaxation adjusts factors dynamically based on solution behavior.

**Steps**:

1. **Create adaptive relaxation class** `adaptiveRelaxation.H`:

```cpp
#ifndef adaptiveRelaxation_H
#define adaptiveRelaxation_H
...
class adaptiveRelaxation
{
    // Private Data
        ...
        //- Residual history (last 3 iterations)
        scalarField residualHistory_;

        //- Increase factor (when residuals decreasing)
        scalar increaseFactor_;

        //- Decrease factor (when residuals increasing)
        scalar decreaseFactor_;
...
public:
    ...
    // Constructors
        ...
        //- Construct from dictionary and field name
        adaptiveRelaxation
        (
            const word& fieldName,
            const dictionary& dict
        ...
        :
            ...
            initialAlpha_(dict.lookupOrDefault<scalar>("initialAlpha", 0.5)),
            minAlpha_(dict.lookupOrDefault<scalar>("minAlpha", 0.1)),
            maxAlpha_(dict.lookupOrDefault<scalar>("maxAlpha", 0.9)),
            residualHistory_(3, GREAT),
            increaseFactor_(dict.lookupOrDefault<scalar>("increaseFactor", 1.1)),
            decreaseFactor_(dict.lookupOrDefault<scalar>("decreaseFactor", 0.8)),
            currentAlpha_(initialAlpha_)
        {}

    // Member Functions

        //- Update relaxation factor based on residual
        scalar update(const scalar currentResidual)
        {
            // Shift history
            for (int i = 0; i < 2; i++)
            {
                residualHistory_[i] = residualHistory_[i+1];
            }
            residualHistory_[2] = currentResidual;

            // Check if we have enough history
            if (residualHistory_[0] >= GREAT)
            {
                return currentAlpha_;
            }

            // Calculate residual trend
            scalar trend = (residualHistory_[2] - residualHistory_[0]) / 
                          max(residualHistory_[0], SMALL);

            // Adjust relaxation factor
            if (trend < -0.1)  // Residuals decreasing well
            {
                currentAlpha_ = min(maxAlpha_, currentAlpha_ * increaseFactor_);
            }
            else if (trend > 0.1)  // Residuals increasing
            {
                currentAlpha_ = max(minAlpha_, currentAlpha_ * decreaseFactor_);
            }
            // If trend is stable, keep current alpha

            return currentAlpha_;
        }

        //- Return current relaxation factor
        scalar alpha() const
        {
            return currentAlpha_;
        }
...
2. **Integrate into solver** `myAdaptiveFoam.C`:

```cpp
#include "fvCFD.H"
#include "singlePhaseTransportModel.H"
#include "turbulentTransportModel.H"
#include "adaptiveRelaxation.H"
#include "fvOptions.H"
...
int main(int argc, char *argv[])
{
    ...
    // Read adaptive relaxation settings
    IOdictionary adaptiveDict
    (
        IOobject
        (
            "adaptiveRelaxationDict",
            runTime.system(),
            mesh,
            IOobject::MUST_READ,
            IOobject::NO_WRITE
    ...
    // Create adaptive relaxation objects
    adaptiveRelaxation pRelax("p", adaptiveDict.subDict("p"));
    adaptiveRelaxation URelax("U", adaptiveDict.subDict("U"));
    ...
    while (simple.loop())
    {
        ...
        // --- Pressure-velocity SIMPLE corrector with adaptive relaxation

        {
            #include "UEqn.H"

            // Get adaptive relaxation factors
            scalar alphaP = pRelax.alpha();
            scalar alphaU = URelax.alpha();

            Info<< "Adaptive relaxation: p = " << alphaP 
                << ", U = " << alphaU << endl;
            ...
            while (simple.correctNonOrthogonal())
            {
                ...
                // Solve and get initial residual
                SolverPerformance<scalar> solverPerf = pEqn.solve();

                // Update adaptive relaxation based on residual
                if (simple.finalNonOrthogonalIter())
                {
                    scalar pResidual = solverPerf.initialResidual();
                    alphaP = pRelax.update(pResidual);
                    
                    phi = phiHbyA + pEqn.flux();
                }
            }

            // Correct velocity with adaptive relaxation
            U = HbyA - rAU()*fvc::grad(p);
            
            // Apply adaptive under-relaxation to velocity
            U = U*alphaU + U.oldTime()*(1.0 - alphaU);
            
            U.correctBoundaryConditions();

            // Update velocity relaxation based on momentum residual
            scalar UResidual = max(mag(U) - mag(U.oldTime())).average().value();
            alphaU = URelax.update(UResidual);
        }
...
3. **Create configuration dictionary** `system/adaptiveRelaxationDict`:

```cpp
// Pressure adaptive relaxation settings
p
{
    initialAlpha    0.3;    // Starting relaxation factor
    minAlpha        0.1;    // Minimum allowed
    maxAlpha        0.7;    // Maximum allowed
    increaseFactor  1.15;   // How fast to increase (15% per step)
    decreaseFactor  0.75;   // How fast to decrease (25% per step)
}

// Velocity adaptive relaxation settings
U
{
    initialAlpha    0.5;
    minAlpha        0.2;
...
EXE = $(FOAM_USER_APPBIN)/myAdaptiveFoam
EOF

# Create Make/options (same as Task 1)
cat > Make/options << EOF
EXE_INC = \
    -I\$(LIB_SRC)/finiteVolume/lnInclude \
    -I\$(LIB_SRC)/meshTools/lnInclude \
...
# Test and monitor adaptive behavior
cd $FOAM_TUTORIALS/incompressible/simpleFoam/pitzDaily
cp -r . $FOAM_RUN/test_adaptive
cd $FOAM_RUN/test_adaptive

# Add adaptiveRelaxationDict to system/
# (copy the dictionary from step 3 above)

# Run solver and monitor relaxation changes
myAdaptiveFoam 2>&1 | tee log.myAdaptiveFoam

# Check how relaxation factors changed
grep "Adaptive relaxation" log.myAdaptiveFoam
```

> [!INFO] **Adaptive Relaxation Benefits**
> - **Faster convergence**: Increases relaxation when residuals are decreasing
> - **Better stability**: Decreases relaxation when residuals oscillate
> - **Automatic tuning**: No manual adjustment needed during simulation
> - **Robustness**: Handles different flow regimes automatically
> 
> (ประโยชน์: การปรI need to truncate the last callout block that was cut off. Let me fix that:

daily_learning/2026-01-03.md
````markdown
<<<<<<< SEARCH
> [!INFO] **Adaptive Relaxation Benefits**
> - **Faster convergence**: Increases relaxation when residuals are decreasing
