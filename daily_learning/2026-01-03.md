---
tags: [openfoam, cfd, hardcore, day-03]
date: 2026-01-03
aliases: [Pressure-Velocity-Coupling]
difficulty: hardcore
topic: Pressure-Velocity-Coupling
---

# Pressure-Velocity-Coupling
## HARDCORE Level - 2026-01-03

---

## Table of Contents
- [1. Theory](#1-theory-core-equations--physics)
- [2. Class Hierarchy](#2-openfoam-class-hierarchy--implementation)
- [3. Code Walkthrough](#3-code-walkthrough)
- [4. Dictionary Analysis](#4-dictionary-analysis--configuration)
- [5. Practical Tasks](#5-hands-on-practical-tasks--coding)
- [6. Concept Checks](#6-concept-checks)

---

## 1. Theory: Core Equations & Physics {#1-theory-core-equations--physics}

### 1.1 The Fundamental Challenge

The pressure-velocity coupling problem arises because the **momentum equation** contains pressure gradients, but for incompressible flows, there is **no explicit equation for pressure**. Pressure acts as a Lagrange multiplier that enforces the continuity constraint (mass conservation).

> [!INFO] **Why is this difficult?**
> The pressure field is coupled to velocity through the momentum equation, but velocity must satisfy the continuity equation. This creates a **circular dependency**:
> - Pressure gradient → drives velocity
> - Velocity → must satisfy continuity
> - Continuity → determines pressure

(ปัญหานี้เกิดจากการที่สมการโมเมนตัมมีความดัน แต่สำหรับการไหลแบบอัดตัวไม่ได ไม่มีสมการชัดเจนสำหรับความดัน ความดันทำหน้าที่เป็นตัวคูณ Lagrange ที่บังคับให้เกิดการอนุรักษ์มวล)

---

### 1.2 Governing Equations

#### 1.2.1 Continuity Equation (Mass Conservation)

For incompressible flow ($\rho = \text{constant}$):

$$\nabla \cdot \mathbf{U} = 0$$

Where:
- $\mathbf{U}$ = velocity vector field $[m/s]$
- $\nabla \cdot$ = divergence operator
- This equation states that the **net volume flux** through any control volume is zero

(สมการต่อเนื่อง: อัตราการไหลเข้าและออกต้องสมดุล ไม่มีการสะสมมวล)

#### 1.2.2 Momentum Equation (Navier-Stokes)

$$\frac{\partial \mathbf{U}}{\partial t} + \nabla \cdot (\mathbf{U}\mathbf{U}) = -\frac{1}{\rho}\nabla p + \nu \nabla^2 \mathbf{U} + \mathbf{g}$$

**Term-by-term explanation:**

| Term | Mathematical Form | Physical Meaning |
|------|-------------------|------------------|
| **Unsteady** | $\frac{\partial \mathbf{U}}{\partial t}$ | Local acceleration (rate of change of velocity) |
| **Convection** | $\nabla \cdot (\mathbf{U}\mathbf{U})$ | Nonlinear inertial transport (velocity transporting itself) |
| **Pressure Gradient** | $-\frac{1}{\rho}\nabla p$ | Force driving flow from high to low pressure |
| **Diffusion** | $\nu \nabla^2 \mathbf{U}$ | Viscous forces (momentum diffusion) |
| **Body Force** | $\mathbf{g}$ | Gravitational acceleration |

Where:
- $p$ = pressure $[Pa]$
- $\rho$ = density $[kg/m^3]$
- $\nu$ = kinematic viscosity $[m^2/s]$
- $\mathbf{g}$ = gravitational acceleration $[m/s^2]$

(สมการโมเมนตัม: อธิบายการเปลี่ยนแปลงของโมเมนตัมตามกฎข้อที่สองของนิวตัน)

---

### 1.3 The Pressure-Velocity Coupling Problem

When we discretize the momentum equation to solve for velocity:

$$\mathbf{U}^{n+1} = \mathbf{U}^n + \Delta t \left[ -\nabla \cdot (\mathbf{U}\mathbf{U}) - \frac{1}{\rho}\nabla p + \nu \nabla^2 \mathbf{U} + \mathbf{g} \right]$$

**The critical issue:** The velocity field $\mathbf{U}^{n+1}$ computed from this equation will **NOT** satisfy $\nabla \cdot \mathbf{U}^{n+1} = 0$ unless the pressure field is correct.

> [!WARNING] **The Chicken-and-Egg Problem**
> - To get correct velocity → need correct pressure
> - To get correct pressure → need correct velocity (satisfying continuity)
> - Neither is known initially!

(ปัญหาไก่กับไข่: ต้องการความดันเพื่อหาความเร็ว แต่ต้องการความเร็วที่ต่อเนื่องเพื่อหาความดัน)

---

### 1.4 Solution Approaches

#### 1.4.1 Pressure Poisson Equation

Taking the divergence of the momentum equation and enforcing $\nabla \cdot \mathbf{U} = 0$:

$$\nabla^2 p = \rho \nabla \cdot \left[ -\nabla \cdot (\mathbf{U}\mathbf{U}) + \nu \nabla^2 \mathbf{U} + \mathbf{g} \right] - \frac{\rho}{\Delta t} \nabla \cdot \mathbf{U}^*$$

Where $\mathbf{U}^*$ is the intermediate velocity field.

**Boundary conditions for pressure:**
- Neumann BC: $\frac{\partial p}{\partial n} = 0$ (at walls)
- Dirichlet BC: $p = p_{\text{specified}}$ (at inlets/outlets)

(สมการ Poisson สำหรับความดัน: ได้จากการเอา divergence ของสมการโมเมนตัมและบังคับใช้ continuity)

#### 1.4.2 Operator Splitting Methods

**Projection Method (Chorin's Method):**

```mermaid
graph TD
    A[Start: U^n, p^n] --> B[Solve Momentum Eq for U*]
    B --> C[U* does NOT satisfy continuity]
    C --> D[Solve Pressure Poisson Eq]
    D --> E[Correct velocity: U^n+1 = U* - grad p]
    E --> F[U^n+1 satisfies continuity]
    F --> G[Next time step]
```

**Mathematical formulation:**

1. **Predictor step** (intermediate velocity):
   $$\frac{\mathbf{U}^* - \mathbf{U}^n}{\Delta t} = -\nabla \cdot (\mathbf{U}^n\mathbf{U}^n) + \nu \nabla^2 \mathbf{U}^n + \mathbf{g}$$

2. **Corrector step** (pressure projection):
   $$\mathbf{U}^{n+1} = \mathbf{U}^* - \frac{\Delta t}{\rho} \nabla p^{n+1}$$

3. **Pressure equation** (enforcing continuity):
   $$\nabla^2 p^{n+1} = \frac{\rho}{\Delta t} \nabla \cdot \mathbf{U}^*$$

(วิธี Projection: แบ่งเป็นขั้นตอนการทำนายและการแก้ไข เพื่อให้ความเร็วตอบสนองต่อสมการต่อเนื่อง)

---

### 1.5 Key Numerical Challenges

> [!TIP] **Staggered Grid Approach**
> On a **collocated grid** (all variables at same cell centers), pressure-velocity coupling can lead to **checkerboard oscillations**. The solution:
> - Store pressure at cell centers
> - Store velocity components at cell faces
> - This naturally enforces continuity coupling

(ตาข่าย Staggered: เก็บความดันที่จุดศูนย์กลางเซลล์ และความเร็วที่ผนังเซลล์ เพื่อป้องกันปัญหา checkerboard)

#### 1.5.1 The Rhie-Chow Interpolation

For collocated arrangements, the **Rhie-Chow interpolation** prevents pressure-velocity decoupling:

$$\mathbf{U}_f = \overline{\mathbf{U}}_f - \Delta_f \left( \overline{\frac{\nabla p}{\rho}}_f - \frac{\nabla p_f}{\rho} \right)$$

Where:
- $\mathbf{U}_f$ = face velocity
- $\overline{\mathbf{U}}_f$ = linearly interpolated cell-centered velocity
- $\Delta_f$ = geometric coefficient
- This adds a **pressure difference term** to couple adjacent pressure nodes

(การแทรก Rhie-Chow: เพิ่มเทอมความต่างความดันเพื่อเชื่อมโยงโหนดความดันที่ติดกัน)

#### 1.5.2 Under-Relaxation Factors

For steady-state solutions using iterative methods:

$$\phi^{n+1} = \phi^n + \alpha_\phi (\phi^* - \phi^n)$$

Typical values:
- $\alpha_U = 0.5 - 0.7$ (velocity under-relaxation)
- $\alpha_p = 0.3 - 0.5$ (pressure under-relaxation)

> [!WARNING] **Too high relaxation** → solution divergence
> **Too low relaxation** → very slow convergence

(ปัจจัย under-relaxation: ควบคุมอัตราการเปลี่ยนแปลงของตัวแปรในแต่ละรอบการวนซ้ำ)

---

### 1.6 Summary of Key Equations

| Equation | Form | Purpose |
|----------|------|---------|
| **Continuity** | $\nabla \cdot \mathbf{U} = 0$ | Mass conservation constraint |
| **Momentum** | $\frac{\partial \mathbf{U}}{\partial t} + \nabla \cdot (\mathbf{U}\mathbf{U}) = -\frac{1}{\rho}\nabla p + \nu \nabla^2 \mathbf{U} + \mathbf{g}$ | Newton's 2nd law for fluid |
| **Pressure Poisson** | $\nabla^2 p = \frac{\rho}{\Delta t} \nabla \cdot \mathbf{U}^*$ | Enforces continuity |
| **Velocity Correction** | $\mathbf{U}^{n+1} = \mathbf{U}^* - \frac{\Delta t}{\rho} \nabla p^{n+1}$ | Projects onto divergence-free field |

(สรุปสมการสำคัญ: การอนุรักษ์มวล สมการโมเมนตัม สมการความดัน Poisson และการแก้ไขความเร็ว)

---

## 2. OpenFOAM Class Hierarchy & Implementation {#2-openfoam-class-hierarchy--implementation}

### 2.1 Core Class Hierarchy

OpenFOAM's pressure-velocity coupling implementation centers around the **finite volume discretization** framework and **solution algorithms**. The key classes form a hierarchy that handles:

1. **Linear equation systems** (matrix representation)
2. **Solution algorithms** (iterative solvers)
3. **Pressure-velocity coupling** (PISO, SIMPLE, PIMPLE)
4. **Boundary conditions** (fvPatchFields)

```mermaid
classDiagram
    class fvSolution {
        +dictionary
        +solverPerformance
        +solve()
    }
    
    class solution {
        -dict_
        +solve()
        +scalableSolver()
    }
    
    class fvMatrix {
        -lduMatrix_
        -source_
        -psi_
        +solve()
        +residual()
    }
    
    class lduMatrix {
        -lower_
        -upper_
        -diag_
        +solve()
    }
    
    class solver {
        +solve()
        +read()
    }
    
    class PBiCGStab {
        +solve()
    }
    
    class GAMG {
        +solve()
    }
    
    class smoothSolver {
        +solve()
    }
    
    fvSolution --> solution
    fvMatrix --> lduMatrix
    solver --> PBiCGStab
    solver --> GAMG
    solver --> smoothSolver
    lduMatrix --> solver
```

> [!INFO] **Class Organization**
> The `fvSolution` class is the main entry point for solving finite volume equations. It manages solver settings and controls the solution loop for pressure-velocity coupling algorithms.

(โครงสร้างคลาส: `fvSolution` เป็นจุดเริ่มต้นหลักในการแก้สมการ จัดการการตั้งค่า solver และควบคุมวงจรการแก้ปัญหา)

---

### 2.2 Pressure-Velocity Coupling Algorithms

OpenFOAM implements three primary algorithms for handling pressure-velocity coupling:

#### 2.2.1 PISO (Pressure Implicit with Splitting of Operators)

**Location:** `$FOAM_SRC/finiteVolume/fvSolution/fvSolution.C`
> **Reference:** `$FOAM_SRC/finiteVolume/fvSolution/fvSolution.H`
> **Reference:** `$FOAM_SRC/finiteVolume/fvSolution/fvSolution.H`

```cpp
// PISO algorithm structure (simplified)
while (piso.correct())
{
    // 1. Solve momentum equation
    solve(fvm::ddt(U) + fvm::div(phi, U) - fvm::laplacian(nu, U) == -fvc::grad(p));
    
    // 2. Correct boundary conditions
    U.correctBoundaryConditions();
    
    // 3. Solve pressure equation
    solve(fvm::laplacian(rAU, p) == fvc::div(phi));
    
    // 4. Correct flux
    phi -= fvc::flux(rAU * fvc::grad(p));
    
    // 5. Correct velocity
    U -= rAU * fvc::grad(p);
    U.correctBoundaryConditions();
}
```

**Key Classes:**
- `pisoControl`: `$FOAM_SRC/finiteVolume/fvSolution/pisoControl.H`
- `pisoLoop`: Controls the PISO correction loop
> **Reference:** `$FOAM_SRC/finiteVolume/fvSolution/pisoControl.C`

(อัลกอริทึม PISO: แบ่งการดำเนินการเป็นขั้นตอนการทำนายและแก้ไข ใช้สำหรับการไหลแบบไม่สมมาตร)

#### 2.2.2 SIMPLE (Semi-Implicit Method for Pressure-Linked Equations)

**Location:** `$FOAM_SRC/finiteVolume/fvSolution/fvSolution.C`

```cpp
// SIMPLE algorithm structure (simplified)
while (simple.correct())
{
    // 1. Solve momentum equation (under-relaxed)
    solve(fvm::ddt(U) + fvm::div(phi, U) - fvm::laplacian(nu, U) == -fvc::grad(p));
    
    // 2. Under-relax velocity
    U.relax();
    
    // 3. Solve pressure equation
    solve(fvm::laplacian(rAU, p) == fvc::div(phi));
    
    // 4. Under-relax pressure
    p.relax();
    
    // 5. Correct flux
    phi -= fvc::flux(rAU * fvc::grad(p));
    
    // 6. Correct velocity
    U -= rAU * fvc::grad(p);
    U.correctBoundaryConditions();
}
```

**Key Classes:**
- `simpleControl`: `$FOAM_SRC/finiteVolume/fvSolution/simpleControl.H`
- `simpleLoop`: Controls the SIMPLE iteration loop
> **Reference:** `$FOAM_SRC/finiteVolume/fvSolution/simpleControl.C`

(อัลกอริทึม SIMPLE: ใช้ under-relaxation เพื่อความเสถียรในการแก้ปัญหาสถานะคงที่)

#### 2.2.3 PIMPLE (PISO + SIMPLE)

**Location:** `$FOAM_SRC/finiteVolume/fvSolution/pimpleControl.H`
> **Reference:** `$FOAM_SRC/finiteVolume/fvSolution/pimpleControl.C`

```cpp
// PIMPLE combines PISO and SIMPLE
while (pimple.loop())
{
    // Outer time loop
    #include "UEqn.H"
    
    // Pressure-velocity correction
    while (pimple.correct())
    {
        // PISO-like corrections
        // Can also include SIMPLE under-relaxation
    }
}
```

**Key Classes:**
- `pimpleControl`: `$FOAM_SRC/finiteVolume/fvSolution/pimpleControl.H`
- `pimpleNoLoopControl`: For steady-state cases

(อัลกอริทึม PIMPLE: รวม PISO และ SIMPLE เข้าด้วยกัน ยืดหยุ่นสำหรับทั้ง steady และ unsteady)

---

### 2.3 Finite Volume Matrix Classes

#### 2.3.1 fvMatrix Class

**Location:** `$FOAM_SRC/finiteVolume/finiteVolume/fvMatrix/fvMatrix.H`
> **Reference:** `$FOAM_SRC/finiteVolume/finiteVolume/fvMatrix/fvMatrix.C`

The `fvMatrix` class represents a discretized equation in the finite volume method:

```cpp
template<class Type>
class fvMatrix
:
    public lduMatrix,
    public refCount
{
    // Private data
    
        //- Field to solve for
        GeometricField<Type, fvPatchField, volMesh>& psi_;
        
        //- Source term
        Field<Type> source_;
        
        //- Reference to mesh
        const fvMesh& mesh_;
        
    
    // Member functions
        
        //- Solve the matrix
        solverPerformance solve
        (
            const dictionary&
        );
        
        //- Return residual
        tmp<Field<Type>> residual() const;
};
```

> [!TIP] **fvMatrix Operations**
> OpenFOAM uses operator overloading for intuitive equation construction:
> - `fvm::ddt(U)` - Time derivative (implicit)
> - `fvm::div(phi, U)` - Convection term (implicit)
> - `fvc::grad(p)` - Pressure gradient (explicit)
> - `fvm::laplacian(nu, U)` - Diffusion term (implicit)

(การดำเนินการ fvMatrix: ใช้ operator overloading เพื่อให้เขียนสมการได้ใกล้เคียงรูปแบบคณิตศาสตร์)

#### 2.3.2 fvm vs fvc

| Aspect | fvm (finite volume method) | fvc (finite volume calculus) |
|--------|---------------------------|------------------------------|
| **Treatment** | Implicit (matrix coefficients) | Explicit (evaluated field) |
| **Usage** | Time derivatives, diffusion, convection | Gradients, divergences, interpolations |
| **Result** | Adds to matrix diagonal/off-diagonal | Creates source term |
| **Example** | `fvm::ddt(U)`, `fvm::laplacian(nu, U)` | `fvc::grad(p)`, `fvc::div(phi)` |

(ความแตกต่าง fvm กับ fvc: fvm สร้างเมทริกซ์ implicit ส่วน fvc คำนวณ explicit)

---

### 2.4 Linear Solver Hierarchy

**Location:** `$FOAM_SRC/OpenFOAM/matrices/lduMatrix/solvers/`
> **Reference:** `$FOAM_SRC/OpenFOAM/matrices/lduMatrix/lduMatrix.H`

OpenFOAM provides a hierarchy of linear solvers for solving the pressure Poisson equation:

```mermaid
graph TD
    A[lduMatrix::solver] --> B[Krylov Subspace Methods]
    A --> C[Multigrid Methods]
    A --> D[Split Methods]
    
    B --> B1[PBiCGStab]
    B --> B2[CG]
    B --> B3[BiCGStab]
    
    C --> C1[GAMG]
    C --> C2[PBiCGStab with GAMG]
    
    D --> D1[smoothSolver]
    
    style A fill:#e1f5ff
    style C1 fill:#ffe1e1
```

#### 2.4.1 Key Solver Classes

**PBiCGStab (Preconditioned Bi-Conjugate Gradient Stabilized):**
```cpp
// Location: $FOAM_SRC/OpenFOAM/matrices/lduMatrix/solvers/PBiCGStab/PBiCGStab.H
> **Reference:** `$FOAM_SRC/OpenFOAM/matrices/lduMatrix/solvers/PBiCGStab/PBiCGStab.C`
template<class Type, class DType, class LUType>
class PBiCGStab
:
    public LduMatrix<Type, DType, LUType>::solver
{
    // - Stabilized bi-conjugate gradient
    // - Suitable for non-symmetric matrices
    // - Often used for momentum equations
};
```

**GAMG (Geometric-Algebraic Multi-Grid):**
```cpp
// Location: $FOAM_SRC/OpenFOAM/matrices/lduMatrix/solvers/GAMG/GAMG.H
> **Reference:** `$FOAM_SRC/OpenFOAM/matrices/lduMatrix/solvers/GAMG/GAMG.C`
template<class Type, class DType, class LUType>
class GAMG
:
    public LduMatrix<Type, DType, LUType>::solver
{
    // - Multi-grid acceleration
    // - Very fast for Poisson-type equations
    // - Default for pressure equation
};
```

> [!INFO] **Solver Selection**
> - **Pressure equation**: GAMG (fast convergence for elliptic equations)
> - **Momentum equation**: PBiCGStab (handles non-symmetric convection)
> - **Scalar transport**: smoothSolver (simple, robust)

(การเลือก Solver: GAMG สำหรับสมการ Poisson, PBiCGStab สำหรับโมเมนตัม)

---

### 2.5 Boundary Condition Classes

#### 2.5.1 Pressure Boundary Conditions

**Location:** `$FOAM_SRC/finiteVolume/fields/fvPatchFields/`
> **Reference:** `$FOAM_SRC/finiteVolume/fields/fvPatchFields/fvPatchField.H`

| Boundary Condition | Class | Description |
|-------------------|-------|-------------|
| **Fixed value** | `fixedValueFvPatchField` | Dirichlet BC: $p = p_{\text{specified}}$ |
| **Zero gradient** | `zeroGradientFvPatchField` | Neumann BC: $\frac{\partial p}{\partial n} = 0$ |
| **Fixed flux** | `fixedFluxPressureFvPatchField` | Adjusts pressure to match specified flux |
| **Total pressure** | `totalPressureFvPatchField` | $p_0 = p + \frac{1}{2}\rho|\mathbf{U}|^2$ |

(เงื่อนไขขอบเขตความดัน: มีหลายประเภทขึ้นกับสภาพการไหล)

#### 2.5.2 Velocity Boundary Conditions

| Boundary Condition | Class | Description |
|-------------------|-------|-------------|
| **No-slip** | `noSlip` | $\mathbf{U} = 0$ at walls |
| **Free stream** | `freestreamFvPatchField` | Fixed value in far-field |
| **Inlet outlet** | `inletOutletFvPatchField` | Switches between inlet/outlet based on flow direction |
| **Wall function** | `wallFunction` | Uses wall functions for near-wall treatment |

(เงื่อนไขขอบเขตความเร็ว: กำหนดพฤติกรรมความเร็วที่ผนังและขอบเขต)

---

### 2.6 Source File Reference Map

```
$FOAM_SRC/
├── finiteVolume/
│   ├── fvSolution/
│   │   ├── fvSolution.H/.C           # Main solution control
│   │   ├── pisoControl.H/.C          # PISO algorithm
│   │   ├── simpleControl.H/.C        # SIMPLE algorithm
│   │   └── pimpleControl.H/.C        # PIMPLE algorithm
│   ├── fvMatrix/
│   │   └── fvMatrix.H/.C             # Finite volume matrix
│   ├── fvm/
│   │   ├── ddt/
│   │   ├── div/
│   │   └── laplacian/                # Implicit operators
│   └── fvc/
│       ├── grad/
│       ├── div/
│       └── snGrad/                   # Explicit operators
├── OpenFOAM/
│   └── matrices/
│       └── lduMatrix/
│           ├── solvers/
│           │   ├── PBiCGStab/
│           │   ├── GAMG/
│           │   └── smoothSolver/
│           └── lduMatrix.H/.C        # Base matrix class
└── db/
    └── runTimeSelection/
        └── runTimeSelectionTables.H  # Runtime selection mechanism
```

> [!TIP] **Runtime Selection**
> OpenFOAM uses a runtime selection mechanism that allows solvers and algorithms to be chosen at runtime through dictionary files (`fvSolution`, `fvSchemes`), without recompilation.

(กลไก Runtime Selection: ให้เลือก solver และอัลกอริทึมได้ขณะรันโดยไม่ต้องคอมไพล์ใหม่)

---

### 2.7 Key Class Relationships Summary

| Class | Responsibility | Source Location |
|-------|----------------|-----------------|
| **fvSolution** | Top-level solution control | `finiteVolume/fvSolution/` |
| **pisoControl** | PISO algorithm management | `finiteVolume/fvSolution/` |
| **simpleControl** | SIMPLE algorithm management | `finiteVolume/fvSolution/` |
| **pimpleControl** | PIMPLE algorithm management | `finiteVolume/fvSolution/` |
| **fvMatrix** | Discretized equation representation | `finiteVolume/fvMatrix/` |
| **lduMatrix** | Linear matrix operations | `OpenFOAM/matrices/lduMatrix/` |
| **solver** | Linear solver base class | `OpenFOAM/matrices/lduMatrix/solvers/` |
| **GAMG** | Multi-grid solver | `OpenFOAM/matrices/lduMatrix/solvers/GAMG/` |
| **PBiCGStab** | Krylov subspace solver | `OpenFOAM/matrices/lduMatrix/solvers/PBiCGStab/` |

(สรุปความสัมพันธ์คลาส: แต่ละคลาสมีหน้าที่เฉพาะในกระบวนการแก้ปัญหา pressure-velocity coupling)

---

## 3. Code Walkthrough {#3-code-walkthrough}

### 3.1 UEqn.H

The `UEqn.H` file constructs the momentum equation for incompressible flow. It is typically included in the main solver loop before the pressure-velocity correction.

> **Reference:** This file is typically located in the solver directory, e.g., `$FOAM_SOLVERS/incompressible/simpleFoam/UEqn.H`

**Typical UEqn.H structure:**

```cpp
// Solve the Momentum equation

tmp<fvVectorMatrix> UEqn
(
    fvm::ddt(U)
  + fvm::div(phi, U)
  - fvm::laplacian(nu, U)
 ==
    fvOptions(U)
);

UEqn.relax();

fvOptions.constrain(UEqn);

if (pimple.momentumPredictor())
{
    solve(UEqn == -fvc::grad(p));
}
```

**Explanation:**

1. **Matrix construction** - Creates a temporary `fvVectorMatrix` containing:
   - `fvm::ddt(U)` - Unsteady term (implicit)
   - `fvm::div(phi, U)` - Convection term (implicit)
   - `fvm::laplacian(nu, U)` - Diffusion term (implicit)
   - `fvOptions(U)` - Source terms from fvOptions (e.g., porosity, momentum sources)

2. **Under-relaxation** - `UEqn.relax()` applies under-relaxation to stabilize convergence for steady-state or transient cases

3. **Constrain sources** - `fvOptions.constrain()` applies additional constraints from fvOptions

4. **Momentum predictor** - If enabled, solves the momentum equation with the pressure gradient as an explicit source term (`-fvc::grad(p)`)

> [!TIP] **Implicit vs Explicit**
> - `fvm::` operators add coefficients to the matrix (implicit treatment)
> - `fvc::` operators evaluate the field directly (explicit treatment)
> - Pressure gradient is explicit because pressure is unknown at this stage

(UEqn.H: สร้างสมการโมเมนตัมโดยใช้ fvm สำหรับเทอม implicit และ fvc สำหรับเทอม explicit ความดัน)

#### 3.1.1 Memory Layout: fvMatrix

The `fvMatrix` class represents the discretized equation in sparse matrix format. Understanding its memory layout helps explain how OpenFOAM efficiently solves large systems.

```cpp
// Simplified memory layout of fvMatrix<Type>
// Inherits from lduMatrix (Lower-Diagonal-Upper matrix format)

fvMatrix<Type>
{
    // === lduMatrix base class (sparse storage) ===
    
    // Diagonal coefficients [nCells]
    scalarField diag_;              // A_P in discretization
    // [A_0, A_1, A_2, ..., A_N]
    //  ^^^^  (contiguous, cache-friendly)

    // Upper coefficients (owner -> neighbor) [nInternalFaces]
    scalarField upper_;             // A_N for internal faces
    // [A_0->1, A_0->2, ..., A_i->j, ...]
    //  ^^^^^^^^  (face-based, sparse)

    // Lower coefficients (neighbor -> owner) [nInternalFaces]
    scalarField lower_;             // A_P contribution from neighbor
    // [A_1->0, A_2->0, ..., A_j->i, ...]
    //  ^^^^^^^^  (face-based, sparse)

    // Addressing arrays (from mesh, not stored in matrix)
    // owner_[faceI]    = owner cell index
    // neighbor_[faceI] = neighbor cell index

    // === fvMatrix-specific data ===

    // Source term [nCells]
    Field<Type> source_;            // b in Ax = b
    // [b_0, b_1, b_2, ..., b_N]

    // Reference to field being solved for
    GeometricField<Type, ...>& psi_;  // The unknown field (e.g., U)

    // Boundary conditions
    FieldField<Field, Type> internalCoeffs_;  // Implicit BC contribution
    FieldField<Field, Type> boundaryCoeffs_;  // Explicit BC contribution

    // === Matrix assembly example ===
    // For each cell i, the discretized equation is:
    // A_P * U_P + sum(A_NB * U_NB) = source
    //
    // Stored as:
    // diag_[i]     = A_P
    // upper_[face] = A_NB (if owner[face] == i)
    // lower_[face] = A_NB (if neighbor[face] == i)
    // source_[i]   = RHS (including pressure gradient, etc.)
};

// Memory access pattern during matrix-vector multiply:
// result[i] = diag_[i] * x[i] 
//           + sum(upper_[face] * x[neighbor[face]])  // owner faces
//           + sum(lower_[face] * x[owner[face]])      // neighbor faces
```

> [!INFO] **Sparse Matrix Efficiency**
> - **Storage**: O(nCells + nInternalFaces) instead of O(nCells²)
> - **Assembly**: Face-based loop naturally fills upper/lower arrays
> - **Solution**: Iterative solvers (GAMG, PBiCGStab) work directly on this format
> - **Cache behavior**: Diagonal access is contiguous; off-diagonal is indirect but still efficient

> [!TIP] **Matrix Coefficient Naming**
> In finite volume discretization:
> - **A_P**: Coefficient for the cell center (diagonal)
> - **A_NB**: Coefficients for neighboring cells (off-diagonal, upper/lower)
> - **Source**: Right-hand side term (explicit contributions)

---

### 3.2 pEqn.H

The `pEqn.H` file constructs and solves the pressure equation to enforce continuity (mass conservation). This is the core of the pressure-velocity coupling algorithm.

> **Reference:** This file is typically located in the solver directory, e.g., `$FOAM_SOLVERS/incompressible/simpleFoam/pEqn.H`

**Typical pEqn.H structure:**

```cpp
// Solve the Pressure equation

volScalarField rAU(1.0/UEqn.A());
volVectorField HbyA(constrainHbyA(rAU*UEqn.H(), U, p));
surfaceScalarField phiHbyA("phiHbyA", fvc::flux(HbyA));

// Adjust flux for non-orthogonal correction
MRF.makeRelative(fvc::flux(HbyA));

// Pressure reference (for incompressible)
if (p.needReference())
{
    p += dimensionedScalar("pRef", p.dimensions(), pRefValue);
    p.correctBoundaryConditions();
}

// PISO loop
while (piso.correct())
{
    // Pressure equation
    fvScalarMatrix pEqn
    (
        fvm::laplacian(rAU, p) == fvc::div(phiHbyA)
    );

    pEqn.setReference(pRefCell, pRefValue);
    pEqn.solve();

    // Correct flux
    phi = phiHbyA - pEqn.flux();

    // Correct velocity
    U = HbyA - rAU*fvc::grad(p);
    U.correctBoundaryConditions();
}
```

**Explanation:**

1. **Matrix coefficients** - `rAU` stores the inverse diagonal coefficients from the momentum equation (`1.0/UEqn.A()`), representing the influence of velocity on pressure

2. **HbyA construction** - Creates the intermediate velocity field `HbyA = H/U`, where `H` contains all explicit terms from the momentum equation except the pressure gradient

3. **Flux calculation** - `phiHbyA` computes the face flux based on the intermediate velocity field

4. **Pressure reference** - For incompressible flows, pressure is relative; a reference cell and value are needed to ensure a unique solution

5. **PISO loop** - Multiple corrections per time step:
   - Solves the pressure Poisson equation: `∇²(p) = ∇·(HbyA)`
   - Corrects the flux: `φ = φ_HbyA - ∇p`
   - Corrects the velocity: `U = HbyA - rAU·∇p`

> [!INFO] **Why Multiple Corrections?**
> The PISO loop performs 2-4 corrections per time step to tighten the pressure-velocity coupling. Each correction improves mass conservation, especially important for:
> - Large time steps
> - High Courant numbers
> - Complex geometries with non-orthogonal meshes

(pEqn.H: แก้สมการความดัน Poisson เพื่อบังคับใช้การอนุรักษ์มวล และแก้ไขความเร็วให้สอดคล้องกับความดันใหม่)

#### 3.2.1 Memory Layout: fvMesh and Face-Based Data

The `fvMesh` stores the mesh topology and geometry. Understanding its layout is essential for efficient flux calculations and pressure-velocity coupling.

```cpp
// Simplified memory layout of fvMesh
// (inherits from polyMesh)

fvMesh
{
    // === Point data (vertices) ===
    pointField points_;             // [nPoints]
    // [(x_0, y_0, z_0), (x_1, y_1, z_1), ..., (x_N, y_N, z_N)]
    //  ^^^^^^^^^^^^^^^^  ^^^^^^^^^^^^^^^^       ^^^^^^^^^^^^^^^^
    //       Point 0           Point 1              Point N

    // === Face data ===
    faceList faces_;                // [nFaces]
    // [
    //   [0, 1, 2, 3],           // Face 0: quad (4 points)
    //   [4, 5, 6],              // Face 1: triangle (3 points)
    //   ...
    // ]

    // === Cell data ===
    cellList cells_;                // [nCells]
    // [
    //   [0, 1, 2, 3, 4, 5],     // Cell 0: 6 faces
    //   [6, 7, 8, 9, 10],       // Cell 1: 5 faces
    //   ...
    // ]

    // === Connectivity (addressing) ===
    
    // Owner-neighbor addressing [nInternalFaces]
    labelList owner_;               // Owner cell for each internal face
    // [0, 0, 1, 1, 2, ...]
    //  ^  ^  ^  ^  ^
    //  |  |  |  |  +-- Face 4 owned by cell 2
    //  |  |  |  +----- Face 3 owned by cell 1
    //  |  |  +-------- Face 2 owned by cell 1
    //  |  +----------- Face 1 owned by cell 0
    //  +-------------- Face 0 owned by cell 0

    labelList neighbor_;            // Neighbor cell for each internal face
    // [1, 2, 2, 3, 3, ...]
    //  ^  ^  ^  ^  ^
    //  |  |  |  |  +-- Face 4 neighbors cell 3
    //  |  |  |  +----- Face 3 neighbors cell 3
    //  |  |  +-------- Face 2 neighbors cell 2
    //  |  +----------- Face 1 neighbors cell 2
    //  +-------------- Face 0 neighbors cell 1

    // === Geometry (computed on demand) ===
    
    // Face area vectors [nFaces]
    surfaceVectorField Sf_;         // Area vector (magnitude = area, direction = normal)
    // [(Sx_0, Sy_0, Sz_0), (Sx_1, Sy_1, Sz_1), ...]
    //  ^^^^^^^^^^^^^^^^^^  ^^^^^^^^^^^^^^^^^^
    //      Face 0 normal        Face 1 normal

    // Face centers [nFaces]
    surfaceVectorField Cf_;         // Face center coordinates

    // Cell centers [nCells]
    volVectorField C_;              // Cell center coordinates

    // Cell volumes [nCells]
    volScalarField V_;              // Cell volumes
    // [V_0, V_1, V_2, ..., V_N]
};

// === Flux field memory layout ===
surfaceScalarField phi             // Volumetric flux [nFaces]
{
    // Internal faces [0 .. nInternalFaces-1]
    // [phi_0, phi_1, phi_2, ..., phi_nInt-1]
    //  ^^^^^^  (flux = U · Sf at each face)

    // Boundary faces [nInternalFaces .. nFaces-1]
    // Organized by patches:
    // - Patch 0 (inlet):  [phi_0, phi_1, ..., phi_M]
    // - Patch 1 (outlet): [phi_0, phi_1, ..., phi_N]
    // - ...
};

// === Access pattern for flux calculation ===
// For each internal face:
//   phi[faceI] = (U[owner[faceI]] + U[neighbor[faceI]])/2 · Sf[faceI]
//
// For boundary face:
//   phi[faceI] = U_boundary · Sf[faceI]
```

> [!TIP] **Mesh Storage Efficiency**
> - **Face-based storage**: Fluxes, gradients computed once per face
> - **Owner-neighbor**: Enables efficient matrix assembly (upper/lower coefficients)
> - **Patch grouping**: Boundary faces grouped by patch for efficient BC application
> - **Lazy evaluation**: Geometry (Sf, Cf, C, V) computed on demand and cached

> [!INFO] **Pressure-Velocity Data Flow**
> ```
> U (cell-centered) → interpolate → Uf (face) → flux = Uf · Sf → phi
>                                                    ↓
>                                            continuity check
>                                                    ↓
> p (cell-centered) → grad(p) → correct flux → phi_new
#                                                    ↓
#                                            correct U → U_new
# ```

---

### 3.3 createFields.H

The `createFields.H` file is responsible for creating and initializing all the field variables required for the simulation. This file is typically included at the beginning of the solver's `main()` function.

> **Reference:** This file is typically located in the solver directory, e.g., `$FOAM_SOLVERS/incompressible/simpleFoam/createFields.H`

**Typical createFields.H structure:**

```cpp
// Create velocity field
Info<< "Reading field U\n" << endl;
volVectorField U
(
    IOobject
    (
        "U",
        runTime.timeName(),
        mesh,
        IOobject::MUST_READ,
        IOobject::AUTO_WRITE
    ),
    mesh
);

// Create pressure field
Info<< "Reading field p\n" << endl;
volScalarField p
(
    IOobject
    (
        "p",
        runTime.timeName(),
        mesh,
        IOobject::MUST_READ,
        IOobject::AUTO_WRITE
    ),
    mesh
);

// Create transport properties
IOdictionary transportProperties
(
    IOobject
    (
        "transportProperties",
        runTime.constant(),
        mesh,
        IOobject::MUST_READ_IF_MODIFIED,
        IOobject::NO_WRITE
    )
);

dimensionedScalar nu
(
    "nu",
    dimViscosity,
    transportProperties
);

// Create flux field
surfaceScalarField phi
(
    IOobject
    (
        "phi",
        runTime.timeName(),
        mesh,
        IOobject::READ_IF_PRESENT,
        IOobject::AUTO_WRITE
    ),
    fvc::flux(U)
);
```

**Explanation:**

1. **Field creation** - Each field is created using the `IOobject` class that specifies:
   - Field name (e.g., "U", "p")
   - Time directory (from `runTime.timeName()`)
   - Mesh reference
   - Read/write flags (`MUST_READ`, `AUTO_WRITE`)

2. **Velocity field** - `volVectorField U` reads the velocity field from the time directory (e.g., `0/U`)

3. **Pressure field** - `volScalarField p` reads the pressure field from the time directory (e.g., `0/p`)

4. **Transport properties** - `IOdictionary` reads physical properties from `constant/transportProperties`, including kinematic viscosity (`nu`)

5. **Flux field** - `surfaceScalarField phi` represents the volumetric flux ($\phi = \mathbf{U} \cdot \mathbf{S}_f$) at cell faces, computed from the velocity field using `fvc::flux(U)`

> [!TIP] **Field Registration**
> All fields are automatically registered with the mesh database. This allows OpenFOAM to:
> - Manage field I/O operations
> - Track field dependencies
> - Apply boundary conditions consistently
> - Handle field interpolation between cell centers and faces

(createFields.H: สร้างและเริ่มต้นฟิลด์ต่างๆ ที่จำเป็นสำหรับการจำลอง เช่น ความเร็ว ความดัน และคุณสมบัติการขนส่ง)

#### 3.3.1 Memory Layout: GeometricField

The `volVectorField` and `volScalarField` are typedefs for `GeometricField`, which stores field data on the mesh. Understanding its memory layout is crucial for performance.

```cpp
// Simplified memory layout of GeometricField
GeometricField<Type, PatchField, GeoMesh>
{
    // Field data (dimensioned: value + units)
    DimensionedField<Type, GeoMesh>  // Internal field data
    {
        Field<Type>                  // Actual data array [nCells]
        {
            Type* data_;             // Pointer to contiguous memory
            // Example for U (volVectorField):
            // [Ux_0, Uy_0, Uz_0, Ux_1, Uy_1, Uz_1, ..., Ux_N, Uy_N, Uz_N]
            //  ^^^^^^^^^^^^^^^^  ^^^^^^^^^^^^^^^^       ^^^^^^^^^^^^^^^^
            //      Cell 0            Cell 1                  Cell N
        }
    }

    // Boundary field data
    PatchField<Field<Type>>         // Boundary patches
    {
        List<PatchField<Type>>      // Array of patches
        {
            [0]                     // Patch 0 (e.g., inlet)
            {
                Field<Type>         // Face values on this patch
                // [Ux_0, Uy_0, Uz_0, ..., Ux_M, Uy_M, Uz_M]
                //  ^^^^^^^^^^^^^^^^         ^^^^^^^^^^^^^^^^
                //    Face 0 on patch           Face M on patch
            }
            [1]                     // Patch 1 (e.g., outlet)
            [2]                     // Patch 2 (e.g., walls)
            ...
        }
    }

    // Reference to mesh (shared pointer)
    const fvMesh& mesh_;
};
```

> [!TIP] **Memory Access Pattern**
> - **Internal field**: Contiguous array, cache-friendly for cell-based operations
> - **Boundary patches**: Separate arrays, accessed via patch indices
> - **Face access**: Requires interpolation from cell centers (uses `fvc::interpolate`)

---

## 4. Dictionary Analysis & Configuration {#4-dictionary-analysis--configuration}

### 4.1 fvSchemes Analysis

The `fvSchemes` dictionary in the `system/` directory specifies the discretization schemes used for the finite volume method. It controls how temporal derivatives, spatial gradients, divergences, and Laplacians are approximated.

#### 4.1.1 ddtSchemes (Time Derivative Schemes)

**Purpose:** Discretization of the unsteady term $\frac{\partial \phi}{\partial t}$

**Common schemes:**
- **Euler** - First-order explicit: $\frac{\phi^{n+1} - \phi^n}{\Delta t}$
- **backward** - Second-order implicit: $\frac{3\phi^{n+1} - 4\phi^n + \phi^{n-1}}{2\Delta t}$
- **CrankNicolson** - Second-order implicit: $\frac{\phi^{n+1} - \phi^n}{\Delta t} = \theta \mathcal{L}(\phi^{n+1}) + (1-\theta)\mathcal{L}(\phi^n)$

**Example configuration:**
```cpp
ddtSchemes
{
    default         backward;
}
```

> [!TIP] **Scheme Selection**
> - **Euler**: Fast but less accurate, suitable for initial testing
> - **backward**: Better accuracy for transient simulations, requires more storage
> - **CrankNicolson**: Most accurate (second-order), but can cause oscillations if $\theta < 1$

(รูปแบบการเปลี่ยนแปลงตามเวลา: Euler ใช้สำหรับการทดสอบ backward ให้ความแม่นยำดีขึ้น CrankNicolson แม่นยำที่สุดแต่อาจเกิดการสั่น)

#### 4.1.2 gradSchemes (Gradient Schemes)

**Purpose:** Discretization of the gradient $\nabla \phi$ at cell centers and faces

**Common schemes:**
- **Gauss linear** - Second-order using Gauss theorem with linear interpolation
- **Gauss upwind** - First-order upwind-biased
- **leastSquares** - Least-squares reconstruction (second-order on arbitrary meshes)
- **fourthOrder** - Fourth-order accurate (requires structured meshes)

**Example configuration:**
```cpp
gradSchemes
{
    default         Gauss linear;
    grad(p)         Gauss linear;
    grad(U)         Gauss linear;
}
```

> [!INFO] **Gauss Theorem Application**
> The Gauss scheme applies the divergence theorem:
> $$\int_V \nabla \phi \, dV = \oint_S \phi \, \mathbf{dS}$$
> The face values $\phi_f$ are obtained by interpolation from cell centers.

(รูปแบบการคำนวณเกรเดียนต์: Gauss linear ใช้ทฤษฎีเรียกว่า Gauss กับการแทรกแบบเส้นตรง)

#### 4.1.3 divSchemes (Divergence Schemes)

**Purpose:** Discretization of the convection term $\nabla \cdot (\mathbf{U} \phi)$

**Common schemes:**
- **Gauss upwind** - First-order, unconditionally stable but diffusive
- **Gauss linear** - Second-order central differencing (unstable for high Re)
- **Gauss linearUpwind** - Second-order upwind (more stable than linear)
- **Gauss vanLeer** - TVD scheme with flux limiter (good compromise)
- **Gauss QUICK** - Quadratic upwind (third-order on uniform grids)

**Example configuration:**
```cpp
divSchemes
{
    default         none;
    div(phi,U)      Gauss linearUpwind grad(U);
    div(phi,k)      Gauss upwind;
    div(phi,epsilon) Gauss upwind;
}
```

> [!WARNING] **Stability vs Accuracy Trade-off**
> - **Upwind**: Very stable but numerically diffusive (smears sharp gradients)
> - **Linear**: Accurate but can cause unphysical oscillations (wiggles)
> - **TVD schemes** (vanLeer, limitedLinear): Best compromise for most flows

(รูปแบบการคำนวณเทอม convection: upwind เสถียรแต่ diffusive linear แม่นยำแต่อาจสั่น TVD คือคอมโพรมีส์ที่ดี)

#### 4.1.4 laplacianSchemes (Laplacian Schemes)

**Purpose:** Discretization of the diffusion term $\nabla \cdot (\Gamma \nabla \phi)$

**Common schemes:**
- **Gauss linear corrected** - Second-order with non-orthogonal correction
- **Gauss linear uncorrected** - Second-order without correction (faster but less accurate)
- **Gauss fourthOrder** - Fourth-order accurate (structured meshes only)

**Example configuration:**
```cpp
laplacianSchemes
{
    default         Gauss linear corrected;
    laplacian(nu,U) Gauss linear corrected;
    laplacian((1|A(U)),p) Gauss linear corrected;
}
```

> [!TIP] **Non-Orthogonal Correction**
> For non-orthogonal meshes, the `corrected` scheme adds an explicit correction term:
> $$\nabla \phi \cdot \mathbf{S}_f = |\mathbf{S}_f| \left( \frac{\phi_P - \phi_N}{d_{PN}} + \text{correction} \right)$$
> This improves accuracy on skewed meshes but may require under-relaxation.

(รูปแบบการคำนวณเทอม diffusion: corrected ใช้สำหรับ mesh ที่ไม่ตั้งฉาก เพิ่มความแม่นยำ)

#### 4.1.5 interpolationSchemes

**Purpose:** Interpolation of cell-centered values to face centers

**Common schemes:**
- **linear** - Linear interpolation (second-order)
- **linearUpwind** - Upwind-biased linear interpolation
- **cubic** - Cubic polynomial interpolation (fourth-order)

**Example configuration:**
```cpp
interpolationSchemes
{
    default         linear;
}
```

#### 4.1.6 snGradSchemes (Surface Normal Gradient Schemes)

**Purpose:** Calculation of the surface-normal gradient $\nabla \phi \cdot \mathbf{n}$ at cell faces

**Common schemes:**
- **corrected** - Includes non-orthogonal correction
- **uncorrected** - Simple orthogonal difference

**Example configuration:**
```cpp
snGradSchemes
{
    default         corrected;
}
```

> [!INFO] **Complete fvSchemes Example**
> ```cpp
> FoamFile
> {
>     version     2.0;
>     format      ascii;
>     class       dictionary;
>     location    "system";
>     object      fvSchemes;
> }
> // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //
> 
> ddtSchemes
> {
>     default         backward;
> }
> 
> gradSchemes
> {
>     default         Gauss linear;
> }
> 
> divSchemes
> {
>     default         none;
>     div(phi,U)      Gauss linearUpwind grad(U);
>     div(phi,k)      Gauss upwind;
>     div(phi,epsilon) Gauss upwind;
> }
> 
> laplacianSchemes
> {
>     default         Gauss linear corrected;
> }
> 
> interpolationSchemes
> {
>     default         linear;
> }
> 
> snGradSchemes
> {
>     default         corrected;
> }
> ```

### 4.2 fvSolution Analysis

The `fvSolution` dictionary in the `system/` directory controls the solution algorithms, linear solvers, and relaxation factors used during the simulation. It defines how the pressure-velocity coupling is handled and how equations are solved iteratively.

#### 4.2.1 solvers Sub-dictionary

**Purpose:** Specifies linear solvers and tolerances for each variable

**Example configuration:**
```cpp
solvers
{
    p
    {
        solver          GAMG;
        tolerance       1e-06;
        relTol          0.01;
        smoother        GaussSeidel;
    }

    pFinal
    {
        solver          GAMG;
        tolerance       1e-06;
        relTol          0;
    }

    U
    {
        solver          PBiCGStab;
        preconditioner  DILU;
        tolerance       1e-05;
        relTol          0.1;
    }
}
```

**Key parameters:**

| Parameter | Description |
|-----------|-------------|
| **solver** | Linear solver algorithm (GAMG, PBiCGStab, smoothSolver, CG) |
| **tolerance** | Absolute residual tolerance (stop when residual < tolerance) |
| **relTol** | Relative tolerance (stop when residual < relTol × initial residual) |
| **preconditioner** | Preconditioning method (DILU, DIC, GAMG) |
| **smoother** | Smoother for multigrid (GaussSeidel, symGaussSeidel) |

> [!INFO] **Solver Selection Strategy**
> - **Pressure (p)**: Use GAMG for fast convergence on elliptic equations
> - **Velocity (U)**: Use PBiCGStab for non-symmetric convection-dominated equations
> - **pFinal**: Set `relTol 0` to solve to absolute tolerance in final PISO loop

(การเลือก Solver: ใช้ GAMG สำหรับสมการความดันที่เป็น elliptic และ PBiCGStab สำหรับสมการโมเมนตัมที่มี convection)

#### 4.2.2 relaxationFactors Sub-dictionary

**Purpose:** Controls under-relaxation factors for steady-state or transient simulations

**Example configuration:**
```cpp
relaxationFactors
{
    fields
    {
        p               0.3;
        rho             1;
    }

    equations
    {
        U               0.7;
        k               0.7;
        epsilon         0.7;
    }
}
```

**Key parameters:**

| Factor | Typical Range | Purpose |
|--------|---------------|---------|
| **p (pressure)** | 0.2 - 0.5 | Prevents pressure oscillations |
| **U (velocity)** | 0.5 - 0.7 | Stabilizes momentum convergence |
| **k, ε (turbulence)** | 0.7 - 0.9 | Moderate relaxation for turbulence |

> [!WARNING] **Relaxation Factor Guidelines**
> - **Lower values** (0.2 - 0.4): More stable but slower convergence
> - **Higher values** (0.7 - 0.9): Faster convergence but risk of divergence
> - **Set to 1.0**: No under-relaxation (use with caution)

(ปัจจัยผ่อนคลาย: ค่าต่ำให้เสถียรแต่ช้า ค่าสูงเร็วแต่เสี่ยงต่อการแตกต่าง)

#### 4.2.3 PISO/SIMPLE/PIMPLE Controls

**Purpose:** Configures the pressure-velocity coupling algorithm

**PISO configuration:**
```cpp
PISO
{
    nCorrectors     2;
    nNonOrthogonalCorrectors 0;
    pRefCell        0;
    pRefValue       0;
}
```

**SIMPLE configuration:**
```cpp
SIMPLE
{
    nNonOrthogonalCorrectors 0;
    pRefCell        0;
    pRefValue       0;

    residualControl
    {
        p               1e-4;
        U               1e-5;
        "(k|epsilon|omega)" 1e-4;
    }
}
```

**PIMPLE configuration:**
```cpp
PIMPLE
{
    correctPhi      no;
    nCorrectors     2;
    nNonOrthogonalCorrectors 0;
    nOuterCorrectors 1;
    pRefCell        0;
    pRefValue       0;

    residualControl
    {
        p               1e-4;
        U               1e-5;
    }
}
```

**Key parameters:**

| Parameter | Description |
|-----------|-------------|
| **nCorrectors** | Number of PISO corrections per time step (typically 2-4) |
| **nOuterCorrectors** | Number of outer iterations for PIMPLE (combines PISO + SIMPLE) |
| **nNonOrthogonalCorrectors** | Corrections for non-orthogonal meshes (0-3) |
| **pRefCell** | Cell index for pressure reference (incompressible flows) |
| **pRefValue** | Reference pressure value (typically 0) |
| **residualControl** | Convergence criteria for each variable |

> [!TIP] **Algorithm Selection**
> - **PISO**: For transient simulations with small time steps
> - **SIMPLE**: For steady-state simulations only
> - **PIMPLE**: For transient simulations with large time steps (combines both)

(การเลือกอัลกอริทึม: PISO สำหรับ transient SIMPLE สำหรับ steady-state PIMPLE ผสมทั้งสอง)

#### 4.2.4 Complete fvSolution Example

```cpp
FoamFile
{
    version     2.0;
    format      ascii;
    class       dictionary;
    location    "system";
    object      fvSolution;
}
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //

solvers
{
    p
    {
        solver          GAMG;
        tolerance       1e-06;
        relTol          0.01;
        smoother        GaussSeidel;
        nCellsInCoarsestLevel 10;
        agglomerator    faceAreaPair;
        mergeLevels     1;
    }

    pFinal
    {
        solver          GAMG;
        tolerance       1e-06;
        relTol          0;
        smoother        GaussSeidel;
        nCellsInCoarsestLevel 10;
        agglomerator    faceAreaPair;
        mergeLevels     1;
    }

    U
    {
        solver          PBiCGStab;
        preconditioner  DILU;
        tolerance       1e-05;
        relTol          0.1;
    }

    k
    {
        solver          PBiCGStab;
        preconditioner  DILU;
        tolerance       1e-05;
        relTol          0.1;
    }

    epsilon
    {
        solver          PBiCGStab;
        preconditioner  DILU;
        tolerance       1e-05;
        relTol          0.1;
    }
}

relaxationFactors
{
    fields
    {
        p               0.3;
    }

    equations
    {
        U               0.7;
        k               0.7;
        epsilon         0.7;
    }
}

PISO
{
    nCorrectors     2;
    nNonOrthogonalCorrectors 0;
    pRefCell        0;
    pRefValue       0;
}

// ************************************************************************* //
```

> [!INFO] **GAMG-Specific Parameters**
> - **nCellsInCoarsestLevel**: Minimum cells on coarsest grid (10-50)
> - **agglomerator**: Method to coarsen grid (faceAreaPair, cellPair)
> - **mergeLevels**: How many levels to merge per coarsening step (1-2)

(พารามิเตอร์ GAMG: ควบคุมการสร้างเกริดหยาบสำหรับ multigrid acceleration)

---

## 5. Hands-on: Practical Tasks & Coding {#5-hands-on-practical-tasks--coding}

### Task 1: Implement a Simple PISO Correction Loop

**Objective:** Create a simplified PISO correction function that demonstrates the pressure-velocity coupling mechanism.

**Description:** Implement a function that performs one iteration of the PISO algorithm, including momentum prediction, pressure solution, and velocity correction.

```cpp
// File: pisoCorrection.H

#ifndef pisoCorrection_H
#define pisoCorrection_H

#include "fvMesh.H"
#include "volFields.H"
#include "surfaceFields.H"

namespace Foam
{

//- Perform one PISO correction iteration
void pisoCorrection
(
    const fvMesh& mesh,
    volVectorField& U,
    volScalarField& p,
    surfaceScalarField& phi,
    const dimensionedScalar& nu,
    const int nCorr
)
{
    // Step 1: Construct momentum equation matrix
    tmp<fvVectorMatrix> UEqn
    (
        fvm::ddt(U)
      + fvm::div(phi, U)
      - fvm::laplacian(nu, U)
    );

    // Step 2: Store diagonal coefficients for pressure equation
    volScalarField rAU(1.0/UEqn().A());

    // Step 3: Calculate intermediate velocity (HbyA)
    volVectorField HbyA(constrainHbyA(rAU*UEqn().H(), U, p));

    // Step 4: Calculate intermediate flux
    surfaceScalarField phiHbyA("phiHbyA", fvc::flux(HbyA));

    // Step 5: PISO correction loop
    for (int corr = 0; corr < nCorr; corr++)
    {
        // Solve pressure Poisson equation: laplacian(rAU, p) == div(phiHbyA)
        fvScalarMatrix pEqn
        (
            fvm::laplacian(rAU, p) == fvc::div(phiHbyA)
        );

        pEqn.solve();

        // Correct flux: phi = phiHbyA - flux(rAU * grad(p))
        phi = phiHbyA - pEqn.flux();

        // Correct velocity: U = HbyA - rAU * grad(p)
        U = HbyA - rAU*fvc::grad(p);
        U.correctBoundaryConditions();
    }
}

} // End namespace Foam

#endif
```

**Solution Explanation:**
1. **Momentum Equation:** Constructs the implicit matrix for velocity without pressure gradient
2. **rAU Field:** Stores inverse diagonal coefficients representing velocity's influence on pressure
3. **HbyA:** Intermediate velocity field excluding pressure effects
4. **PISO Loop:** Iteratively solves pressure equation and corrects velocity/flux to enforce continuity

> [!TIP] **Key Insight**
> The pressure equation is derived by taking the divergence of the momentum equation and enforcing $\nabla \cdot \mathbf{U} = 0$. Multiple corrections tighten the coupling.

---

### Task 2: Implement Rhie-Chow Interpolation for Face Fluxes

**Objective:** Implement Rhie-Chow interpolation to prevent pressure-velocity decoupling (checkerboard oscillations) on collocated grids.

**Description:** Create a function that computes face fluxes using Rhie-Chow interpolation, which adds a pressure difference term to couple adjacent pressure nodes.

```cpp
// File: rhieChowInterpolation.H

#ifndef rhieChowInterpolation_H
#define rhieChowInterpolation_H

#include "fvMesh.H"
#include "volFields.H"
#include "surfaceFields.H"

namespace Foam
{

//- Calculate face flux using Rhie-Chow interpolation
surfaceScalarField rhieChowInterpolate
(
    const volScalarField& p,
    const volVectorField& U,
    const volScalarField& rAU,
    const surfaceScalarField& gamma
)
{
    const fvMesh& mesh = p.mesh();

    // Step 1: Linearly interpolate velocity to faces
    surfaceVectorField Uf(fvc::interpolate(U));

    // Step 2: Calculate interpolated pressure gradient
    surfaceVectorField gradPf(fvc::interpolate(fvc::grad(p)));

    // Step 3: Calculate face pressure gradient using direct difference
    surfaceScalarField pOwn(fvc::interpolate(p, own()));
    surfaceScalarField pNei(fvc::interpolate(p, nei()));
    surfaceVectorField delta(mesh.Sf()/mesh.magSf());
    surfaceVectorField gradPDirect(delta * (pNei - pOwn));

    // Step 4: Rhie-Chow interpolation formula
    // Uf = Uf_interpolated - (1/Af) * (gradP_interpolated - gradP_direct)
    surfaceVectorField UfRC
    (
        Uf - gamma * (gradPf - gradPDirect)
    );

    // Step 5: Calculate flux = UfRC · Sf
    surfaceScalarField phiRC(fvc::flux(UfRC));

    return phiRC;
}

//- Alternative implementation using OpenFOAM's built-in methods
surfaceScalarField rhieChowFlux
(
    const volVectorField& U,
    const volScalarField& p,
    const volScalarField& rAU
)
{
    const fvMesh& mesh = U.mesh();

    // Calculate HbyA (intermediate velocity without pressure)
    tmp<volVectorField> tHbyA = U - rAU * fvc::grad(p);
    const volVectorField& HbyA = tHbyA();

    // Interpolate HbyA to faces
    surfaceScalarField phiHbyA(fvc::flux(HbyA));

    // Add pressure correction term
    // This couples adjacent pressure nodes to prevent checkerboarding
    surfaceScalarField rAUf("rAUf", fvc::interpolate(rAU));
    surfaceScalarField pCorr
    (
        fvc::flux(rAUf * fvc::snGrad(p) * mesh.magSf())
    );

    return phiHbyA - pCorr;
}

} // End namespace Foam

#endif
```

**Solution Explanation:**
1. **Interpolated Velocity:** Standard linear interpolation of cell-centered velocity to faces
2. **Interpolated Gradient:** Pressure gradient interpolated from cell centers
3. **Direct Gradient:** Pressure gradient computed directly from face neighbor values
4. **Rhie-Chow Correction:** Difference between interpolated and direct gradients adds coupling
5. **Flux Calculation:** Final flux computed from corrected face velocity

> [!WARNING] **Why This Matters**
> Without Rhie-Chow interpolation, collocated arrangements suffer from checkerboard pressure fields where adjacent cells oscillate between high/low values without affecting velocity.

---

### Task 3: Create a Custom Pressure Boundary Condition

**Objective:** Implement a custom pressure boundary condition that adjusts pressure to match a specified mass flow rate at an outlet.

**Description:** Create a `fixedFlowRatePressure` boundary condition that calculates the required pressure to achieve a target mass flux.

```cpp
// File: fixedFlowRatePressureFvPatchScalarField.H

#ifndef fixedFlowRatePressureFvPatchScalarField_H
#define fixedFlowRatePressureFvPatchScalarField_H

#include "fvPatchFields.H"
#include "fixedValueFvPatchFields.H"

namespace Foam
{

class fixedFlowRatePressureFvPatchScalarField
:
    public fixedValueFvPatchScalarField
{
    // Private data

        //- Target mass flow rate [kg/s]
        scalar targetFlowRate_;

        //- Reference density for incompressible flow
        scalar rho_;

        //- Current patch area
        scalar patchArea_;


public:

    //- Runtime type information
    TypeName("fixedFlowRatePressure");


    // Constructors

        //- Construct from patch and internal field
        fixedFlowRatePressureFvPatchScalarField
        (
            const fvPatch&,
            const DimensionedField<scalar, volMesh>&
        );

        //- Construct from patch, internal field and dictionary
        fixedFlowRatePressureFvPatchScalarField
        (
            const fvPatch&,
            const DimensionedField<scalar, volMesh>&,
            const dictionary&
        );


    // Member functions

        //- Update the coefficients associated with the patch field
        virtual void updateCoeffs();

        //- Write
        virtual void write(Ostream&) const;
};

} // End namespace Foam

#endif
```

```cpp
// File: fixedFlowRatePressureFvPatchScalarField.C

#include "fixedFlowRatePressureFvPatchScalarField.H"
#include "addToRunTimeSelectionTable.H"
#include "fvPatchFieldMapper.H"
#include "volFields.H"
#include "surfaceFields.H"

// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //

namespace Foam
{

// * * * * * * * * * * * * * * * * Constructors * * * * * * * * * * * * * * //

fixedFlowRatePressureFvPatchScalarField::
fixedFlowRatePressureFvPatchScalarField
(
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF
)
:
    fixedValueFvPatchScalarField(p, iF),
    targetFlowRate_(0.0),
    rho_(1.0),
    patchArea_(0.0)
{}


fixedFlowRatePressureFvPatchScalarField::
fixedFlowRatePressureFvPatchScalarField
(
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF,
    const dictionary& dict
)
:
    fixedValueFvPatchScalarField(p, iF, dict),
    targetFlowRate_(dict.lookupOrDefault<scalar>("targetFlowRate", 0.0)),
    rho_(dict.lookupOrDefault<scalar>("rho", 1.0)),
    patchArea_(gSum(patch().magSf()))
{
    // Initialize with zero gradient
    if (dict.found("value"))
    {
        fvPatchScalarField::operator=
        (
            scalarField("value", dict, p.size())
        );
    }
    else
    {
        fvPatchScalarField::operator=(0.0);
    }
}


// * * * * * * * * * * * * * * * Member Functions * * * * * * * * * * * * * //

void fixedFlowRatePressureFvPatchScalarField::updateCoeffs()
{
    if (updated())
    {
        return;
    }

    // Get the flux field
    const surfaceScalarField& phi =
        db().lookupObject<surfaceScalarField>("phi");

    // Calculate current flow rate through this patch
    scalarField currentFlux = phi.boundaryField()[patch().index()];
    scalar currentFlowRate = gSum(currentFlux);

    // Calculate flow rate error
    scalar flowRateError = targetFlowRate_ - currentFlowRate;

    // Calculate pressure correction needed
    // Using simple proportional control: deltaP = Kp * error
    scalar Kp = 1000.0;  // Proportional gain (tune as needed)
    scalar pressureCorrection = Kp * flowRateError / patchArea_;

    // Get internal pressure values
    const scalarField& pInternal = patchInternalField();

    // Apply correction to boundary values
    operator==(pInternal + pressureCorrection);

    fixedValueFvPatchScalarField::updateCoeffs();
}


void fixedFlowRatePressureFvPatchScalarField::write(Ostream& os) const
{
    fvPatchScalarField::write(os);
    os.writeKeyword("targetFlowRate") << targetFlowRate_
        << token::END_STATEMENT << nl;
    os.writeKeyword("rho") << rho_
        << token::END_STATEMENT << nl;
    writeEntry("value", os);
}


// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //

makePatchTypeField
(
    fvPatchScalarField,
    fixedFlowRatePressureFvPatchScalarField
);

// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //

} // End namespace Foam
```

**Usage in boundary conditions:**

```cpp
outlet
{
    type            fixedFlowRatePressure;
    targetFlowRate  0.5;      // Target mass flow rate [kg/s]
    rho             1.0;      // Reference density [kg/m^3]
    value           uniform 0; // Initial pressure value
}
```

**Solution Explanation:**
1. **Flow Rate Calculation:** Computes current mass flow rate by summing flux over the patch
2. **Error Calculation:** Determines difference between current and target flow rates
3. **Pressure Correction:** Uses proportional control to adjust pressure
4. **Boundary Update:** Sets boundary pressure values to drive flow toward target
5. **Runtime Selection:** Registers the boundary condition for use in case files

> [!TIP] **Applications**
> This boundary condition is useful for:
> - Outlet conditions where flow rate is known but pressure is unknown
> - Matching experimental mass flow rates
> - Multi-domain simulations where mass conservation is critical

---

### Summary of Practical Skills

| Task | Key Concept | OpenFOAM Classes Used |
|------|-------------|----------------------|
| **PISO Loop** | Pressure-velocity coupling algorithm | `fvMatrix`, `fvm::laplacian`, `fvc::grad` |
| **Rhie-Chow** | Prevent checkerboard oscillations | `fvc::interpolate`, `fvc::flux`, `fvc::snGrad` |
| **Custom BC** | Extending boundary conditions | `fixedValueFvPatchField`, `runtimeSelection` |

> [!SUCCESS] **Next Steps**
> 1. Compile and test these implementations in a simple cavity case
> 2. Compare results with standard OpenFOAM boundary conditions
> 3. Experiment with different relaxation factors and solver settings

---

## 6. Concept Checks {#6-concept-checks}

### Question 1: Why is there no explicit equation for pressure in incompressible flow?

> **Answer:** In incompressible flows, density is constant, so the continuity equation becomes $\nabla \cdot \mathbf{U} = 0$ (a constraint on velocity only). The pressure field acts as a Lagrange multiplier that enforces this constraint, but it doesn't have its own governing equation. Instead, pressure must be derived from the momentum equation and the continuity constraint, typically through a Pressure Poisson equation obtained by taking the divergence of the momentum equation.

---

### Question 2: What is the "chicken-and-egg" problem in pressure-velocity coupling?

> **Answer:** The momentum equation requires pressure to compute velocity ($\mathbf{U}^{n+1} = \mathbf{U}^n - \frac{\Delta t}{\rho}\nabla p + \text{other terms}$), but the pressure equation requires velocity to enforce continuity ($\nabla^2 p = \frac{\rho}{\Delta t}\nabla \cdot \mathbf{U}^*$). Neither field is known initially, creating a circular dependency. This is solved using iterative algorithms like PISO, SIMPLE, or PIMPLE that alternate between solving for momentum and pressure until both equations are satisfied.

---

### Question 3: What is the difference between PISO and SIMPLE algorithms?

> **Answer:** **PISO** (Pressure Implicit with Splitting of Operators) is designed for transient simulations with small time steps. It performs multiple pressure-velocity corrections (typically 2-4) per time step to tighten the coupling. **SIMPLE** (Semi-Implicit Method for Pressure-Linked Equations) is for steady-state simulations and uses under-relaxation factors to prevent divergence. **PIMPLE** combines both: it can use outer iterations with under-relaxation (like SIMPLE) and inner PISO corrections, making it suitable for transient cases with large time steps.

---

### Question 4: What are checkerboard oscillations and how does Rhie-Chow interpolation prevent them?

> **Answer:** Checkerboard oscillations are unphysical pressure-velocity decoupling that occurs on collocated grids (where all variables are stored at cell centers). Adjacent cells can oscillate between high/low pressure values without affecting the velocity field, because the pressure gradient is calculated using values two cells apart. **Rhie-Chow interpolation** prevents this by adding a pressure difference term to the face velocity calculation: $\mathbf{U}_f = \overline{\mathbf{U}}_f - \Delta_f (\overline{\nabla p}_f - \nabla p_f)$, which couples adjacent pressure nodes and eliminates the oscillation mode.

---

### Question 5: Why is GAMG typically used for the pressure equation while PBiCGStab is used for momentum?

> **Answer:** The pressure Poisson equation ($\nabla^2 p = \text{source}$) is an **elliptic** equation where information propagates in all directions. **GAMG** (Geometric-Algebraic Multi-Grid) is extremely efficient for such equations because it uses multiple grid levels to quickly eliminate low-frequency errors. The momentum equation contains convection terms ($\nabla \cdot (\mathbf{U}\mathbf{U})$) that make it **non-symmetric** and convection-dominated. **PBiCGStab** (Preconditioned Bi-Conjugate Gradient Stabilized) is designed for non-symmetric matrices and handles the convective effects better than standard conjugate gradient methods.

---

## Recommended Reading

- OpenFOAM User Guide: https://www.openfoam.com/documentation/user-guide
- OpenFOAM Programmer's Guide: https://doc.openfoam.com/
- CFD Online Forum: https://www.cfd-online.com/Forums/openfoam/

