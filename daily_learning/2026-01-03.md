...
| $\mathbf{U}_f$ | Face velocity | m/s | สำหรับ incompressible flow มักคำนวณจาก pressure-velocity coupling (`Rhie-Chow`) |
...
) const
{
    ...
    forAll(mesh.owner(), facei)
    {
        ...
        {
            WarningInFunction
                << "High Peclet number (Pe = " << Pe << ") detected at face " 
                << facei << ". Central scheme may be unstable." << endl;
        }
    }
    return true;
}
...
> โค้ดด้านบนเป็น simplified version สำหรับการศึกษา สำหรับ implementation เต็มรูปแบบ โปรดดูที่ OpenFOAM source code
...
### 3.2 คลาสหลัก: SpatialDiscretizer
...
### 3.3 คลาสพื้นฐาน: BaseScheme และ Hierarchy
...
#### 3.3.1 BaseScheme (Abstract Base Class)
```cpp
/**
 * @class BaseScheme
...
#### 3.3.2 UpwindScheme (First-order Upwind)
```cpp
/**
 * @class UpwindScheme
...
#### 3.3.3 CentralScheme (Linear Interpolation)
```cpp
/**
 * @class CentralScheme
...
### 5.1.1 โครงสร้าง CMake สำหรับ Library และ Unit Tests
...
### 5.1.2 CMake Configuration สำหรับ Core Library
...
### 5.1.3 CMake สำหรับ Scheme Implementations
แต่ละ discretization scheme ควร compile เป็น separate module:

```cmake
# src/schemes/CMakeLists.txt
...
endforeach()
```
### 5.2 การเขียน Unit Tests แบบ Comprehensive

#### 5.2.1 Test Framework Setup ด้วย GoogleTest
...
#### 5.2.2 Unit Test สำหรับ Upwind Scheme
...
#### 5.2.3 Unit Test สำหรับ TVD Limiters
...
TEST_F(VanLeerLimiterTest, TVDRegionCompliance) {
    ...
    for (scalar r : testRatios) {
        ...
        // Check TVD condition: $0 \le \psi(r) \le \min(2r, 2)$
        EXPECT_GE(psi, 0.0);
...
#### 5.2.4 Unit Test สำหรับ SuperBee Limiter (Compressive)
...
TEST_F(SuperBeeLimiterTest, TVDCompliance) {
    ...
    for (scalar r = 0.0; r <= 5.0; r += 0.1) {
        ...
        // TVD condition: $0 \le \psi(r) \le \min(2r, 2)$
...
> > >
> > > ```
> > > $\phi_{\text{exact}}(x_f) = \phi_P + (\nabla\phi)_P \cdot (x_f - x_P) + O(\Delta x^2)$  [expansion จาก P]
> > > $\phi_{\text{exact}}(x_f) = \phi_N + (\nabla\phi)_N \cdot (x_f - x_N) + O(\Delta x^2)$  [expansion จาก N]
> > > ```
> > >
...
> > >
> > > $Error_{\text{UDS}} = \phi_{\text{exact}} - \phi_{\text{UDS}} = \pm(1/2)\Delta x\cdot(\partial\phi/\partial x) + O(\Delta x^2)$
> > >
> > > เมื่อแทน error นี้กลับเข้าไปใน convection term `∇·(Uφ)` เราจะได้ term เพิ่มเติมที่มีรูปทรงเหมือน diffusion:
> > >
> > > ```
> > > $\nabla\cdot(\mathbf{U} \phi_{\text{UDS}}) = \nabla\cdot(\mathbf{U} \phi_{\text{exact}}) - \nabla\cdot[\mathbf{U}\cdot(1/2)\Delta x\cdot(\nabla\phi)] + ...$
> > > ```
> > >
> > > Term ที่สอง `∇·[U·(1/2)Δx·(∇φ)]` มีรูปแบบ `∇·(Γ_num ∇φ)` โดยที่ `$\Gamma_{\text{num}} = (1/2)|\mathbf{U}|\Delta x$` คือ **numerical diffusion coefficient**.
> > >
...
> > > // ตรวจสอบได้โดยคำนวณ interface thickness `1.0/(maxGradAlpha*deltaX)`:
...
> > > โดยที่:
> > > $a_W = \Gamma/\Delta x^2 + \rho U/(2\Delta x)$
> > > $a_E = \Gamma/\Delta x^2 - \rho U/(2\Delta x)$
> > > $a_P = a_W + a_E = 2\Gamma/\Delta x^2$
> > > ```
> > >
> > > สังเกตว่า coefficient `a_E` จะเป็น **ลบ** เมื่อ:
> > >
> > > ```
> > > $\Gamma/\Delta x^2 - \rho U/(2\Delta x) < 0$
> > > => $\rho U\Delta x/\Gamma > 2$
> > > => Pe > 2
> > > ```
> > >
...
> > >
> > > ```
> > > $|a_P| \ge \sum|a_{nb}|$ สำหรับทุก cell
> > > ```
> > >
> > > **ผลกระทบเมื่อ Pe > 2:**
> > >
...
> > >
> > > ```
> > > $0 \le \psi(r) \le \min(2r, 2)$ สำหรับ r ≥ 0
> > > $\psi(r) = 0$ สำหรับ r ≤ 0
> > > ```
> > >
> > > **ตัวอย่างการทำงานของ van Leer limiter:**
> > > ```
> > > $\psi_{\text{vanLeer}}(r) = (r + |r|) / (1 + r)$
> > > ```
> > >
...
> > > Total Variation ถูกนิยามเป็น: `$TV(\phi) = \Sigma|\phi_{i+1} - \phi_i|$`
...
> > > - เหตุผล: Pressure Poisson equation `$\nabla \cdot (1/\rho \nabla p) = ...$` เป็น elliptic → diffusion-dominated เสมอ ($Pe \approx 0$)
> > > - ต้องการ accurate gradient calculation สำหรับ `$\nabla p$` ใน momentum source term
> > > - Implementation:
> > > ```cpp
> > > gradSchemes
> > > {
...
> > > - ต้องรักษา boundedness ($0 \le \alpha \le 1$) เสมอ
> > >
...
> > > CFL (Courant-Friedrichs-Lewy) condition: `$Co = U\Delta t/\Delta x \le 1$` สำหรับ explicit schemes
> > >
> > > สำหรับ **upwind scheme**:
> > > - Numerical diffusion coefficient: `$\Gamma_{num} = (1/2)U\Delta x(1 - Co)$`
> > > - สังเกตว่าเมื่อ `$Co \to 1$` (Δt ใหญ่สุดที่อนุญาต), `$\Gamma_{num} \to 0$`
> > > - นี่คือเหตุผลที่เราต้องการ `$Co \approx 1$` สำหรับ pure convection problems เพื่อลด numerical diffusion
> > >
...
> > > พิจารณา VOF equation: `$\partial\alpha/\partial t + \nabla \cdot (U\alpha) = 0$`
> > >
...
> > > - Error รวม: `$O(\Delta t) + O(\Delta x)$` → interface smears ตามทั้ง space และ time
> > > - Interface thickness เพิ่มเป็น: `$\delta \sim \sqrt{\Gamma_{num} t} \sim \sqrt{U\Delta x t}$`
> > >
...
*   **Day 09: Pressure-Velocity Coupling (SIMPLE/PISO)**
    *   **ความเชื่อมโยง:** Algorithm เช่น SIMPLE และ PISO ต้องแก้สมการ momentum (สำหรับ U) และ pressure correction (สำหรับ p) ซ้ำๆ การเลือก spatial scheme สำหรับ term `∇·(ρUU)` ใน momentum equation และ `∇·(ρU)` ใน continuity equation มีผล `สำคัญมาก` ต่อความเร็วในการลู่เข้าและความเสถียรของ algorithm
...
### 7.3 บทเรียนที่เกี่ยวข้องใน Phase ต่อไป (Connection to Future Phases)
...
### 7.4 แหล่งข้อมูลเพิ่มเติม (Additional Resources)
