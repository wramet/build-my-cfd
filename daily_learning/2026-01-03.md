...
> [!INFO] **สรุป Section 1**
> Section 1 covers the mathematical foundations of finite volume discretization for convection-diffusion equations, including upwind, central differencing, and TVD schemes. Key design decisions emphasize boundedness for phase fraction (using TVD limiters like van Leer or MUSCL), harmonic mean for discontinuous diffusion coefficients at liquid-vapor interfaces, and proper handling of the expansion term $\nabla \cdot \mathbf{U} \neq 0$ during phase change. The section establishes critical pitfalls to avoid: unbounded schemes for phase fraction cause crashes, arithmetic mean at interfaces gives wrong heat flux, and ignoring the expansion term violates mass conservation.
...
> [!INFO] **สรุป Section 4**
> Section 4 provides complete C++ implementation of the spatial discretization framework, including SurfaceInterpolation class with multiple schemes (UPWIND, CENTRAL, VAN_LEER, MUSCL, SUPERBEE), DivergenceOperator for convection terms with phase change expansion source, GradientOperator supporting Green-Gauss and least-squares methods with non-orthogonal correction, and LaplacianOperator using harmonic mean for discontinuous properties. The implementation includes critical safeguards: explicit clipping of phase fraction to [0,1], division-by-zero protection in limiters, and proper matrix assembly with owner-neighbor contributions. A SchemeManager class provides automatic scheme selection based on field physics, preventing common errors like using central differencing for phase fraction.
...
- Previous: [[daily_learning/2026-01-02|Day 2: Governing Equations and Thermodynamics]]
- Next: [[daily_learning/2026-01-04|Day 4: Time Integration and Solver Methods]]
- See also: 
  - [[daily_learning/2026-01-01|Day 1: Project Setup and Requirements]]
  - [[daily_learning/2026-01-10|Day 10: VOF Method Implementation]]
  - [[daily_learning/2026-01-15|Day 15: Phase Change Models]]
  - [[90_day_roadmap|90-Day CFD Engine Development Roadmap]]
