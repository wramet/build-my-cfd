...
The `fvMatrix` class stores the discretized equation in a sparse LDU (Lower-Diagonal-Upper) format optimized for finite volume meshes. Below is the memory layout representation:
...
> [!INFO] **Memory Efficiency (ประสิทธิภาพหน่วยความจำ)**
...
### Question 1: Why is there no explicit equation for pressure in the Navier-Stokes equations for incompressible flow?

> **Answer:** In incompressible flows, pressure is not governed by an equation of state (unlike compressible flows where pressure relates to density and temperature via an equation of state). Instead, pressure acts as a Lagrange multiplier that enforces the continuity constraint (mass conservation). The pressure field must adjust itself instantaneously to ensure that the velocity field remains divergence-free (∇ · U = 0). This is why we need to derive a Pressure Poisson Equation from the momentum equation and continuity constraint to solve for pressure.

### Question 2: What is the "checkerboard" pressure problem, and how does the Rhie-Chow interpolation scheme prevent it?

> **Answer:** The checkerboard problem is a numerical artifact that occurs on collocated grids (where pressure and velocity are stored at the same cell centers). It causes pressure oscillations in a checkerboard pattern because the pressure gradient calculation can couple only alternating cells, decoupling the pressure-velocity relationship. The Rhie-Chow interpolation scheme prevents this by adding a correction term that involves the difference between the face-interpolated pressure gradient and the cell-centered pressure gradient. This introduces a small amount of numerical dissipation that couples all neighboring cells and eliminates the spurious oscillation modes.

### Question 3: What are the key differences between the SIMPLE and PISO algorithms in OpenFOAM?

> **Answer:** 
> - **SIMPLE (Semi-Implicit Method for Pressure-Linked Equations):** Designed for steady-state simulations. It uses under-relaxation factors (typically 0.3 for pressure, 0.7 for velocity) to prevent divergence. The algorithm performs one pressure correction per outer iteration and relies on gradual convergence to steady state.
> - **PISO (Pressure-Implicit with Splitting of Operators):** Designed for transient simulations. It performs multiple corrector loops (typically 2-4) per time step to achieve tighter pressure-velocity coupling within each time step. PISO generally does not use under-relaxation since the time step itself provides stability.
> 
> In OpenFOAM, these are controlled by `SIMPLE` and `PISO` dictionaries in `system/fvSolution`, with PISO using `nCorrectors` and SIMPLE using `relaxationFactors`.

### Question 4: Why is GAMG (Geometric-Algebraic Multi-Grid) the recommended solver for the pressure equation in OpenFOAM?

> **Answer:** The pressure Poisson equation (∇²p = f) is an elliptic partial differential equation, which means pressure information propagates instantaneously throughout the entire domain (global coupling). Standard iterative solvers (like conjugate gradient) are slow to eliminate low-frequency errors that span large distances. GAMG addresses this by using a hierarchy of coarser grids: low-frequency errors appear as high-frequency errors on coarser grids and can be eliminated quickly. This gives GAMG optimal O(N) scaling, making it much faster than other solvers for large meshes. The coarsening is typically done using the `faceAreaPair` agglomerator, which groups cells based on face area connectivity.

### Question 5: What is the physical meaning of the under-relaxation formula: ψ_new = α·ψ_calculated + (1-α)·ψ_old?

> **Answer:** Under-relaxation is a stability technique used in iterative algorithms, particularly for steady-state simulations (SIMPLE). The formula means that the new value of a variable is a weighted blend of the newly calculated value and the previous iteration's value. The relaxation factor α (0 < α ≤ 1) controls how much of the new solution is accepted:
> - **Lower α (e.g., 0.2-0.3 for pressure):** Strong damping, more stable but slower convergence. Used for pressure because it's highly sensitive to velocity changes.
> - **Higher α (e.g., 0.7-0.8 for velocity):** Less damping, faster convergence but risk of oscillation. Used for velocity because it's more strongly coupled to the momentum equation.
> 
> Without under-relaxation, the tight coupling between pressure and velocity can cause the solution to oscillate or diverge, especially in the early iterations when the initial guess is far from the solution.
