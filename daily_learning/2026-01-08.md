...
EnhancedPCG::EnhancedPCG(...) : PCG(...)
{
    ...
    if (!lower.empty() && !upper.empty())
    {
        ...
        {
            WarningInFunction
                << "Matrix asymmetry metric = " << asymmetry
...
classDiagram
    ...
    class lduMatrix {
        ...
        -scalarField diag_
        -scalarField upper_
        +Amul(Field~Type~&, const Field~Type~&) const
        +Tmul(Field~Type~&, const Field~Type~&) const
    }
    ...
    class lduMatrixSolver {
        ...
        #scalarNormProduct(const Field~Type~&, const Field~Type~&) const
        #stop(solverPerformance&) const
    }
    ...
    class PCG {
        ...
        -precondition(Field~Type~&, const Field~Type~&) const
    }
    ...
    class lduMatrixPreconditioner {
        ...
        +preconditionT(Field~Type~& w, const Field~Type~& r) const
    }
    ...
    class DICPreconditioner {
        ...
        +calcReciprocalD(scalarField& rD)
        +precondition(Field~Type~& w, const Field~Type~& r) const override
    }
...
## 4.1 ไฟล์ Header: `IterativeSolverDriver.H`
...
// ************************************************************************* //
```text
## 4.2 ไฟล์ Implementation: `IterativeSolverDriver.C`
...
```cmake# File: src/CMakeLists.txt# สร้าง static library สำหรับ iterative solvers
...
```cmake# File: apps/CMakeLists.txt# Test driver สำหรับทดสอบ solvers กับเมทริกซ์ตัวอย่าง
...
```cmake# File: tests/CMakeLists.txt# ใช้ Google Test framework
...
TEST(PreconditionerTest, DICForSymmetricMatrix) {
    ...
    double residualNorm = Foam::gSumMag(residual) /
...
> >
> > **ผลกระทบในทางปฏิบัติ:**
> > 1.  **Algorithmic Breakdown**: สูตรการคำนวณ `β_k = (r_{k+1}^T r_{k+1}) / (r_k^T r_k)` ใน CG อาศัยสมบัติ `A^T = A` เพื่อให้ `p_k` และ `p_{k+1}` เป็น A-orthogonal (conjugate) หาก A ไม่สมมาตร การค้นหา direction จะ fail และ algorithm อาจ diverge
> > 2.  **Loss of Optimality**: CG ถูกออกแบบให้หา solution ใน subspace `K_k = span{r_0, A r_0, ..., A^{k-1} r_0}` โดยที่ error norm `||e||_A = e^T A e` ลดลงเร็วที่สุด หาก A ไม่ใช่ SPD, norm นี้ไม่มีความหมายทางกายภาพ และ convergence จะช้าลงมากหรือไม่เกิดขึ้นเลย
...
> > ---
> >
> [!QUESTION] 7.3 คำถามที่ 3: BiCGStab ต้องการ "Shadow Residual" `r̃_0` เพื่ออะไร? เกิดอะไรขึ้นถ้าเราเลือก `r̃_0` ผิด?
>
> > [!SUCCESS]- เฉลย (คลิกเพื่ออ่านคำตอบ)
> >
> > **คำตอบเชิงลึก:**
> > `r̃_0` (shadow residual หรือ dual residual) เป็น **key differentiator** ของ BiCG family (BiCG, BiCGStab, QMR) จาก CG family
> >
> > **หน้าที่ของ `r̃_0`:**
> > 1.  **สร้าง Biorthogonal Basis**: BiCGStab สร้างสอง sequence ของ vectors:
> >     - **Primary sequence**: `r_k` (residual), `p_k` (search direction)
> >     - **Dual sequence**: `r̃_k`, `p̃_k` ที่สร้างจาก `r̃_0`
> >     โดยที่ `r_i^T r̃_j = 0` สำหรับ `i ≠ j` (biorthogonality)
> > 2.  **รับมือกับ Asymmetry**: สำหรับเมทริกซ์ asymmetric `A`, เราต้องการ operator อื่นมาช่วยสร้าง orthogonal basis `r̃_0` ทำหน้าที่เป็น initial vector สำหรับลำดับ dual ซึ่งมักเลือกเป็น `r̃_0 = r_0`
> > 3.  **คำนวณ Inner Product**: ใน algorithm จะเห็น `ρ_k = r̃_0^T r_k` ซึ่งใช้ตรวจสอบ "projection" ของ residual ปัจจุบัน onto shadow space
> >
> > **ผลหากเลือก `r̃_0` ผิด:**
> > 1.  **Breakdown ของ Algorithm**: หาก `ρ_k = r̃_0^T r_k = 0` เกิดขึ้นก่อน convergence (เรียกว่า "serious breakdown") จะทำให้ `α_k = ρ_k / (r̃_0^T v_k)` หาค่าไม่ได้ (division by zero)
> > 2.  **Slow Convergence**: หาก `r̃_0` ไม่สอดคล้องกับ `r_0` ทาง numerical (เช่น orthogonal กันโดยบังเอิญ) การสร้าง biorthogonal basis จะไม่มีประสิทธิภาพ
> > 3.  **Numerical Instability**: ค่า `ρ_k` ที่แกว่งมากระหว่าง iteration บวก-ลบ ส่งสัญญาณว่า algorithm ใกล้ breakdown
> >
...
> > Type rhoOld = rho;
> > ```text
> > **Restart Mechanism**: หากตรวจพบว่า `|ρ_k|` ต่ำมาก (ใกล้ศูนย์) หรือ residual ไม่ลดลงหลาย iteration บาง implementation จะทำ "restart" โดยตั้ง `r̃_0 = r_k` (ปัจจุบัน) ใหม่ เพื่อหลีกเลี่ยง breakdown ซึ่ง OpenFOAM ทำโดยอัตโนมัติผ่าน convergence monitoring
> >
> > **คำแนะนำ**: สำหรับปัญหาทั่วไป การตั้ง `r̃_0 = r_0` ทำงานได้ดี การต้องปรับเปลี่ยน `r̃_0` มักเกิดในปัญหาพิเศษเท่านั้น (เช่น มี symmetry บางรูปแบบที่ทำให้ `r̃_0^T A r_0 = 0`)
> >
> > ---
> >
...
> > ---
> >
> [!QUESTION] 7.5 คำถามที่ 5: Matrix-Vector Product `A·p` ใน Iterative Solver ถูกคำนวณอย่างไรสำหรับ LDU Format? จงอธิบาย Algorithm
>
> > [!SUCCESS]- เฉลย (คลิกเพื่ออ่านคำตอบ)
> >
> > **คำตอบเชิงลึก:**
> > การคำนวณ `w = A·p` สำหรับ sparse matrix ใน LDU format มีประสิทธิภาพสูงเพราะใช้เฉพาะ non-zero entries โดย OpenFOAM ใช้ function template `lduMatrix::Amul` ซึ่ง implement face-based operation
> >
...
> >    - Apsi[neigh] += lower[f] * psi[owner]  // contribution from owner to neighbor
...
> > - **PCG**: เรียก 1 ครั้งต่อ iteration (สำหรับ `A·p_k`)
> > - **BiCGStab**: เรียก 2 ครั้งต่อ iteration (สำหรับ `A·p_k` และ `A·s`)
> >
...
> > 4.  **Change in Matrix Properties**: ใน nonlinear problems ที่ matrix properties เปลี่ยนระหว่าง iterations (แต่ใน linear solver ใช้กับ fixed
