...
date: 2026-01-09
difficulty: "hardcore"
tags: ["CFD", "OpenFOAM", "SIMPLE", "PISO", "Rhie-Chow", "Pressure-Velocity Coupling"]
...
2.  **วิเคราะห์ (Analyze) หลักการและที่มาของ Rhie-Chow Interpolation**
    ...
    *   อธิบายความหมายทางฟิสิกส์ของ **Pressure Gradient Correction Term** ในสูตร Rhie-Chow: `- (1/aP)_f * (∇⟂p_f - interpolate(∇p)_f)`
...
# Day 09: Pressure-Velocity Coupling (SIMPLE, PISO) - Section 1: Theory
...
## 3.1 Core Class: `fvVectorMatrix` - The Equation Container

### 3.1.1 Header Analysis (`src/finiteVolume/fvMatrices/fvMatrix/fvMatrix.H`)
...
### 3.1.2 Critical Methods สำหรับ Pressure-Velocity Coupling
...
```
## 3.2 Control Class: `pimpleControl` - The Algorithm Orchestrator

### 3.2.1 Architecture Analysis (`src/finiteVolume/cfdTools/general/solutionControl/pimpleControl/`)
...
### 3.2.2 Critical Method: `correct()` - The Main Algorithm Loop
...
```
## 3.3 Interpolation System: `surfaceInterpolation` และ Rhie-Chow Implementation

### 3.3.1 `surfaceInterpolation` Class Analysis
...
### 3.3.2 Rhie-Chow Implementation ใน OpenFOAM
...
### 3.3.3 Our `RhieChowInterpolator` Implementation
...
{
    // สูตร: φ_corr = (1/Ap)_f * (∇⟂p_f - interpolate(∇p)) · S_f
...
```
## 4.2 Class Specification รายละเอียด

### 4.2.1 Class: `PressureVelocityCoupler`
...
### 4.2.2 Class: `RhieChowInterpolator`
...
### 4.2.3 Class: `PressureEquationAssembler`
...
### 4.2.4 Class: `UnderRelaxationManager`
...
## 5.1 ภาพรวมการ Implement Pressure-Velocity Coupling
...
## 5.2 Header File: `RhieChowInterpolator.H`
...
// ************************************************************************* //
```
## 5.3 Implementation File: `RhieChowInterpolator.C`
...
## 6.1 การตั้งค่า CMake และการ Build Project
...
### 6.1.1 โครงสร้างไฟล์ CMakeLists.txt หลัก (Root CMakeLists.txt)
...
```cmake
# ============================================# CMakeLists.txt - Root Level# Pressure-Velocity Coupling Module ([[Day 09: Pressure-Velocity Coupling (SIMPLE, PISO, Rhie-Chow)|Day 09]])# ============================================
...
### 6.1.2 CMakeLists.txt สำหรับ Source Library (src/CMakeLists.txt)

ไฟล์นี้จะจัดการการ compile และสร้าง shared library จาก source code ของเรา

```cmake
# ============================================# src/CMakeLists.txt# Library Implementation for Pressure-Velocity Coupling# ============================================
...
### 6.1.3 CMakeLists.txt สำหรับ Applications (applications/CMakeLists.txt)

ไฟล์นี้จะสร้าง executable solvers ที่ใช้ library ของเรา

```cmake
# ============================================# applications/CMakeLists.txt# Custom Solver Implementations# ============================================
...
### 6.2.1 CMakeLists.txt สำหรับ Test Suite (tests/CMakeLists.txt)

```cmake
# ============================================# tests/CMakeLists.txt# Unit Test Suite for Pressure-Velocity Coupling# ============================================
...
### 6.2.2 ตัวอย่าง Unit Test: RhieChowInterpolationTest.C
...
> [!QUESTION] 7.1 คำถามที่ 1: ทำไมต้องใช้ Rhie-Chow interpolation ใน collocated grid? จะเกิดอะไรขึ้นถ้าไม่ใช้?
>
...
> > ---
> >
> [!QUESTION] 7.2 คำถามที่ 2: อะไรคือความแตกต่างหลักระหว่าง SIMPLE และ PISO algorithms? และ PIMPLE คืออะไร?
>
> > [!SUCCESS]- เฉลย (คลิกเพื่ออ่านคำตอบ)
> >
> > **คำตอบเชิงลึก:**
...
> > ---
> >
> [!QUESTION] 7.3 คำถามที่ 3: การเลือกค่า Under-Relaxation Factors ใน SIMPLE มีหลักการอย่างไร? ค่าไหนที่ "ดีที่สุด"?
>
> > [!SUCCESS]- เฉลย (คลิกเพื่ออ่านคำตอบ)
> >
> > **คำตอบเชิงลึก:**
...
> >     ```
> >
> > 4.  **ได้สมการ Pressure Correction:** จัด
...
### 8.1.2 OpenFOAM-Specific References
...
### 8.1.3 บทความและแหล่งข้อมูลออนไลน์ (Online Articles & Resources)
...
### 8.2.1 ความเชื่อมโยงกับวันที่ผ่านมา (Backward Connections)
...
### 8.2.2 ความเชื่อมโยงกับวันที่กำลังจะมาถึง (Forward Connections)
...
## 8.3 ความเชื่อมโยงกับ Phase อื่นๆ (Connections to Other Phases)
...
## 8.4 แหล่งข้อมูลเพิ่มเติมสำหรับการศึกษาเชิงลึก (Additional Resources for Deep Dive)
