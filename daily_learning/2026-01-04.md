...
> **สรุป Section 1:** Section 1 covers the mathematical foundation of temporal discretization schemes for two-phase evaporating flows. Key concepts include implicit Euler for stability, adaptive time-stepping based on CFL and phase change rate, and the critical expansion source term in the pressure equation. Common pitfalls such as ignoring the expansion term or using fixed timesteps are identified with prevention strategies.
...
> **สรุป Section 4:** Section 4 provides complete C++ implementations for the TimeController, ImplicitEuler, AdaptiveStepper, and PressureSolver classes. The code includes proper memory management with RAII, safety validations, and explicit handling of the expansion source term. Unit tests demonstrate CFL-based time-stepping, implicit integration accuracy, and pressure equation solving with expansion effects. Integration guidelines show how temporal discretization connects to momentum, VOF, and energy solvers in the main time loop.
...
### Question 1: Why must the expansion term $\nabla \cdot \mathbf{U} = \dot{m} \left( \frac{1}{\rho_v} - \frac{1}{\rho_l} \right)$ be included in the pressure equation for evaporating flows? What happens if you omit it?

> **Answer:** 
> 
> During evaporation, liquid converts to vapor. Since $\rho_v \ll \rho_l$ (for R410A: $\rho_v \approx 50$ kg/m³, $\rho_l \approx 500$ kg/m³), the same mass occupies ~10× more volume. This creates a volumetric source that violates the incompressibility assumption $\nabla \cdot \mathbf{U} = 0$.
> 
> **If you omit the expansion term:**
> - The pressure equation becomes $\nabla^2 p = \nabla \cdot \mathbf{U}_{hat}$ (standard incompressible form)
> - Mass conservation fails: vapor generation creates "missing" volume
> - Pressure drifts unphysically (typically becomes negative)
> - Solver diverges within a few timesteps
> 
> **Correct implementation:**
> $$
> \nabla \cdot \left( \frac{1}{a_U} \nabla p \right) = \nabla \cdot \mathbf{U}_{hat} - \dot{m} \left( \frac{1}{\rho_v} - \frac{1}{\rho_l} \right)
> $$
> 
> Note the **minus sign**—expansion is a source term that must be subtracted from RHS. This is the #1 reason custom phase-change solvers fail.

---

### Question 2: You implemented `ImplicitEuler::computeDerivative()` but it's actually explicit (it uses `rhs[i]` directly without iteration). Why is this wrong for implicit schemes, and what would a correct implementation look like?

> **Answer:**
> 
> **The bug:** The current implementation computes:
> ```cpp
> phiNew[i] = oldField_[i] + dt * rhs[i];
> ```
> This is **explicit Euler**, not implicit. It evaluates RHS at the old time level.
> 
> **Why implicit matters for phase change:**
> - Explicit Euler requires CFL < 1 for stability
> - During rapid evaporation, interface velocity spikes → requires $\Delta t \sim 10^{-7}$ s
> - Implicit Euler is **unconditionally stable**—can take $\Delta t \sim 10^{-4}$ s
> 
> **Correct implicit implementation:**
> The implicit equation $\phi^{n+1} = \phi^n + \Delta t \cdot \text{RHS}(\phi^{n+1})$ is nonlinear because RHS depends on $\phi^{n+1}$. You must iterate:
> 
> ```cpp
> // Pseudo-code for correct implicit Euler
> double* phiNew = new double[nCells];
> 
> // Initialize with old time values
> for (int i = 0; i < nCells; ++i) {
>     phiNew[i] = oldField_[i];
> }
> 
> // Newton iteration to solve: phi - dt*RHS(phi) = phi_old
> for (int iter = 0; iter < maxIter; ++iter) {
>     // 1. Evaluate RHS at current guess (phiNew)
>     double* rhs = evaluateRHS(phiNew);
>     
>     // 2. Compute residual: R = phiNew - dt*rhs - phi_old
>     double maxResidual = 0.0;
>     for (int i = 0; i < nCells; ++i) {
>         double residual = phiNew[i] - dt * rhs[i] - oldField_[i];
>         maxResidual = std::max(maxResidual, std::abs(residual));
>     }
>     
>     if (maxResidual < tolerance) break;
>     
>     // 3. Update phiNew (simplified—real Newton needs Jacobian)
>     for (int i = 0; i < nCells; ++i) {
>         phiNew[i] = oldField_[i] + dt * rhs[i];
>     }
>     
...
> ```
> 
> In practice, you don't implement this directly—instead, the outer PISO/SIMPLE loop handles the iteration. The key insight: **implicit means coupling equations at the new time level**.

---

### Question 3: In `AdaptiveStepper::computeTimeStep()`, you compute three timesteps (CFL-based, phase-change-based, interface-based) and take the minimum. Why not just use CFL? What physical phenomenon does each criterion protect against?

> **Answer:**
> 
> Each timestep criterion guards against a different instability mechanism:
> 
> | Criterion | Formula | Physical Threat | Example Failure Mode |
|-----------|---------|-----------------|---------------------|
| **CFL-based** | $\Delta t = \text{Co} \cdot \Delta x / |\mathbf{U}|_{max}$ | Convective instability | Information travels faster than numerical scheme can capture |
| **Phase-change-based** | $\Delta t = \text{threshold} / \dot{m}_{max}$ | Stiffness from rapid evaporation | Nucleation event causes $\dot{m}$ to spike 100×, solver diverges |
| **Interface-based** | $\Delta t = \Delta x / |\mathbf{U}_{interface}|$ | Interface smearing | Interface moves >1 cell/timestep, VOF becomes unbounded |
> 
> **Why CFL alone is insufficient:**
> 
> Consider an evaporating droplet with quiescent vapor ($|\mathbf{U}| \approx 0$):
> - CFL criterion: $\Delta t \to \infty$ (no velocity)
> - But phase change is active: $\dot{m} \approx 100$ kg/m³/s
> - Phase-change criterion: $\Delta t \approx 0.001$ s
> - **If you use CFL only:** You'd take huge timesteps, mass transfer would be wrong, pressure would blow up
> 
> **Concrete example from R410A evaporator:**
> - At startup: wall is hot, liquid is cold → rapid boiling
> - $\dot{m}$ spikes from 0 to 500 kg/m³/s in milliseconds
> - CFL-based $\Delta t$ might be 0.01 s (velocity is low)
> - Phase-change $\Delta t$ drops to 0.0001 s
> - **Without phase-change criterion:** Solver crashes during startup transient
> 
> **Design insight:** Adaptive time-stepping must consider **all active timescales**, not just convection. For evaporating flows, the phase-change timescale $\tau_{pc} \sim \rho / \dot{m}$ is often much shorter than convective timescale $\tau_{conv} \sim \Delta x / U$.

---

### Question 4: Your `PressureSolver` uses Jacobi iteration, which converges slowly. For a 1M cell mesh, you'd need ~10,000 iterations. Why is Jacobi acceptable for a learning engine, and what would you replace it with for production?

> **Answer:**
> 
> **Why Jacobi is acceptable NOW:**
> - **Correctness first:** A slow correct solver is better than a fast wrong one
> - **Debugging:** Jacobi is trivial to verify—each iteration is explicit
> - **Mesh size:** Learning engine uses <10K cells (2D test cases)
> - **Code clarity:** No complex preconditioners to understand
> 
> **Why Jacobi fails for production:**
> - Convergence rate: $O(N^{-2})$ where N = grid points per dimension
> - For 1M cells (100×100×100): ~10,000 iterations needed
> - Each iteration is $O(N)$ → total solve time is prohibitive
> - For transient simulations with 100K timesteps: impossible
> 
> **Production alternatives (in order of complexity):**
> 
> 1. **Gauss-Seidel (GS):** 2× faster than Jacobi, still trivial
>    ```cpp
>    // GS uses updated values immediately
>    p[i] = (rhs[i] - sum) / aii;  // p[i] updates in-place
>    ```
> 
> 2. **Conjugate Gradient (CG):** $O(\sqrt{\kappa})$ convergence
>    - $\kappa$ = condition number (~h⁻² for Laplacian)
>    - For 1M cells: ~100 iterations (100× faster than Jacobi)
>    - Requires only matrix-vector multiplication
> 
> 3. **Geometric Multigrid (GMG):** $O(N)$ total work
>    - Optimal for elliptic equations (Poisson)
>    - For 1M cells: ~10-20 iterations (1000× faster than Jacobi)
>    - **This is what OpenFOAM uses** (GAMG solver)
> 
> **Implementation strategy:**
> ```cpp
> // Start with Jacobi (current)
> bool solve(double tol, int maxIter) {
>     for (int iter = 0; iter < maxIter; ++iter) {
>         jacobiSweep();  // O(N)
>         if (residual < tol) return true;
>     }
> }
> 
> // Upgrade to Gauss-Seidel (easy)
> void jacobiSweep() {
>     for (int i = 0; i < nCells; ++i) {
>         pNew[i] = computeUpdate(i);  // Write to separate array
>     }
>     std::swap(p, pNew);
> }
> 
> // Upgrade to CG (medium difficulty)
> bool solve(double tol, int maxIter) {
>     return conjugateGradient(*pressureMatrix_, p, rhs, tol, maxIter);
> }
> 
> // Upgrade to Multigrid (hard—use library)
> bool solve(double tol, int maxIter) {
>     return hypreBoomerAMG(*pressureMatrix_, p, rhs, tol, maxIter);
> }
> ```
> 
> **Key insight:** The pressure equation structure doesn't change—only the iterative method. Design your `PressureSolver` class so the solver is pluggable (strategy pattern).

---

### Question 5: In the pressure equation, you fix the pressure at a reference cell (`refCell_`) to `refValue_`. Why is this necessary? What happens if you don't do this?

> **Answer:**
> 
> **The mathematical reason:**
> The pressure Poisson equation $\nabla^2 p = f$ has a **singular coefficient matrix**. The nullspace is constant vectors: if $p$ is a solution, then $p + C$ is also a solution for any constant $C$.
> 
> **Why the matrix is singular:**
> - Pure Neumann boundary conditions (∂p/∂n specified everywhere)
> - No Dirichlet boundary (fixed p value anywhere)
> - Continuity equation only defines pressure **up to a constant**
> 
> **What happens without reference pressure:**
> - The matrix has a zero eigenvalue
> - Iterative solvers (Jacobi, CG) **fail to converge**—residuals stagnate
> - Direct solvers (LU) produce numerical garbage (division by zero)
> - Pressure drifts arbitrarily: $p \to p + 1000$ in one timestep, $p \to p - 5000$ in next
> 
> **How the reference cell fixes this:**
> ```cpp
> void PressureSolver::applyReferencePressure() {
>     // Zero out entire row
>     for (int j = 0; j < nCols; ++j) {
...
> }
> ```
> This replaces the singular row with:
> $$
> 1 \cdot p_{refCell} = refValue
> $$
> Now the matrix is non-singular (all eigenvalues > 0).
> 
> **Physical interpretation:**
> - Pressure is a thermodynamic potential, not an absolute quantity
> - Only pressure **gradients** affect velocity (∇p in momentum equation)
> - Fixing $p = 101325$ Pa at one cell is like choosing a gauge origin
> - Similar to choosing voltage ground in circuit analysis
> 
> **Common pitfalls:**
> 
> 1. **Wrong reference cell location:** Choose cell far from boundaries, not in a corner
>    - Bad: refCell at domain corner (boundary layers affect it)
>    - Good: refCell at domain center
> 
> 2. **Changing reference value during simulation:** Causes pressure jumps
>    - Keep `refValue` constant throughout simulation
> 
> 3. **Multiple reference cells:** Over-constrains the system
>    - Only ONE reference cell needed
> 
> 4. **Forgetting to call `applyReferencePressure()`:** Most common bug
>    - Solver appears to work but gives wrong pressures
>    - Symptoms: pressure drifts, mass imbalance, slow convergence
> 
> **OpenFOAM comparison:**
> OpenFOAM reads `pRefCell` and `pRefValue` from `system/fvSolution`:
> ```cpp
> pRefCell     0;
> pRefValue    0;
> ```
> This is exactly what your engine does—just hardcoded instead of dictionary-driven.
...
- **Previous**: [[daily_learning/2026-01-03|Day 3: Phase Change Models]] - Lee model implementation and mass transfer computation
- **Next**: [[daily_learning/2026-01-05|Day 5: Spatial Discretization]] - Gradient, divergence, and Laplacian operators for finite volume method
- **See also**: 
  - [[90_day_roadmap|90-Day CFD Engine Roadmap]] - Full curriculum overview
  - [[daily_learning/2026-01-02|Day 2: Mesh and Data Structures]] - Field and Mesh classes used by temporal schemes
  - [[daily_learning/2026-01-08|Day 8: VOF Advection]] - Interface tracking coupled with time integration
