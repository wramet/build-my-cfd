...
### 4.1 พื้นฐานการอินทิเกรตตามเวลา (Time Integration Fundamentals)
...
### 4.2 วิธีออยเลอร์: Explicit กับ Implicit (Euler Methods: Explicit vs Implicit)
...
#### 4.2.1 Explicit Euler (Forward Euler)
...
#### 4.2.2 Implicit Euler (Backward Euler)
...
*   **ข้อดี:** **Unconditionally Stable (A-Stable)** สำหรับปัญหาเชิงเส้นจำนวนมาก หมายความว่า 至少在ทางทฤษฎี เรา可以选择 $\Delta t$ ใหญ่เท่าใดก็ได้โดยที่การคำนวณจะไม่ลู่ออก (แต่ความถูกต้องจะแย่ลง) ทำให้เหมาะกับปัญหาที่ต้องการขั้นเวลาขนาดใหญ่
...
### 4.3 วิธีแครงค์-นิโคลสัน (Crank-Nicolson Method)
...
### 4.4 เงื่อนไข CFL สำหรับการไหลแบบบีบอัดพร้อมการเปลี่ยนสถานะ (CFL Condition for Compressible Flow with Phase Change)
...
## 3.1 การวิเคราะห์ Source Code: `ddtScheme` Class

### 3.1.1 Abstract Base Class: `ddtScheme<Type>`
...
### 3.1.2 Derived Class: `EulerDdtScheme<Type>`
...
### 3.1.3 Derived Class: `CrankNicolsonDdtScheme<Type>`
...
## 3.2 การวิเคราะห์ Source Code: `CourantNo` Functions

### 3.2.1 Header File Analysis: `CourantNo.H`
...
### 3.2.2 Implementation สำหรับ Phase Change CFL
...
## 4.3.1 ภาพรวมสถาปัตยกรรม (Architecture Overview)
...
classDiagram
    ...
    class ddtScheme {
        <<abstract>>
        #tmp~GeometricField~ fvmDdt(GeometricField&)
        #tmp~GeometricField~ fvcDdt(GeometricField&)
    }
...
## 4.3.2 รายละเอียดคลาส `TimeIntegrator`
...
## 4.3.3 รายละเอียดคลาส `CFLCalculator`
...
## 5.1 โครงสร้างไฟล์ Implementation
...
## 5.2 Header File: `TimeIntegrator.H`
...
## 5.3 Implementation File: `TimeIntegrator.C`
...
1.  **Root Cause - Eigenvalue ของ Diffusion Operator:**
    สมการ diffusion อย่างง่ายใน 1D: `∂φ/∂t = ν ∂²φ/∂x²` เมื่อ discretize ด้วย explicit Euler จะได้:
    ```
    φ_i^{n+1} = φ_i^n + (ν Δt / Δx²) * (φ_{i+1}^n - 2φ_i^n + φ_{i-1}^n)
    ```
    เราสามารถวิเคราะห์ความเสถียรด้วย **Von Neumann Stability Analysis** โดยแทนที่คำตอบด้วย wave component: `φ_i^n = ξ^n * e^{I k i Δx}` (เมื่อ I คือหน่วยจินตภาพ) จะได้ **amplification factor** `g(ξ)`:
    ```
    ξ = 1 - (4ν Δt / Δx²) * sin²(k Δx / 2)
    ```
    เงื่อนไขความเสถียร `|ξ| ≤ 1` สำหรับทุก wave number `k` นำไปสู่:
    ```
    Δt ≤ Δx² / (2ν)
    ```
    นี่คือ **Diffusion Number (Fourier Number) Condition** `Fo = ν Δt / Δx² ≤ 0.5`
...
1.  **Derivation of Interface Velocity:**
    ...
    ```
    ∇·U = ṁ * (1/ρ_v - 1/ρ_l) = ṁ * ( (ρ_l - ρ_v) / (ρ_l ρ_v) )
    ```
    ...
    ```
    ∂α/∂t + ∇·(αU) + ∇·(U_interface α(1-α)) = ... (compression term ถูกละไว้)
    ```
    ...
    ```
    U_interface ≈ ṁ / ρ_l  (บนด้าน liquid) หรือ ṁ / ρ_v (บนด้าน vapor)
    ```
...
2.  **Extended CFL Condition:**
    ...
    ```
    CFL_total = max( CFL_convection, CFL_diffusion, CFL_interface ) < Co_max
    ```
...
2.  **Recommended Scheme Strategy:**
    ...
    | **Pressure (p)** | เชิงนัยผ่าน **SIMPLE/PISO loop** | Pressure equation (Poisson-type) มักแก้ด้วย implicit method อยู่แล้ว (ผ่าน linear solver) การ discretize ในเวลาไม่สำคัญเท่ากับการ couple กับ velocity |
    | **Volume Fraction (α)** | **Bounded Implicit Euler** หรือ **MULES (Explicit)** | ต้องรักษา boundedness (0≤α≤1) เป็นสิ่งสำคัญสูงสุด Bounded scheme หรือ explicit limiter-based scheme จำเป็น |
    | **Temperature (T)** | **Crank-Nicolson (blended)** | Thermal diffusion มักไม่ stiff เท่า viscous diffusion และการมี second-order accuracy ช่วยลด numerical diffusion ของ thermal front |
...
### 8.3 การเชื่อมโยงกับโมดูลอื่นในโครงการ (Cross-Module Connections)
...
### 8.4 แหล่งข้อมูลออนไลน์และชุมชน (Online Resources & Community)
