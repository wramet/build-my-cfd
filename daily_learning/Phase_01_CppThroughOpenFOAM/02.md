...
```cpp
// Compile: g++ -std=c++17 -O2 -Wall ptraits_demo.cpp -o ptraits_demo
...
// Tensor3 type (simplified)
struct Tensor3 {
    double xx, xy, xz, yx, yy, yz, zx, zy, zz;
    Tensor3() : xx(0),xy(0),xz(0), yx(0),yy(0),yz(0), zx(0),zy(0),zz(0) {}
    Tensor3(double xx_, double xy_, double xz_, double yx_, double yy_,
            double yz_, double zx_, double zy_, double zz_)
        : xx(xx_),xy(xy_),xz(xz_), yx(yx_),yy(yy_),yz(yz_), zx(zx_),zy(zy_),zz(zz_) {}
    Tensor3& operator+=(const Tensor3& r) {
        xx+=r.xx; xy+=r.xy; xz+=r.xz; yx+=r.yx; yy+=r.yy; yz+=r.yz;
        zx+=r.zx; zy+=r.zy; zz+=r.zz; return *this;
    }
...
template<> struct pTraits<Tensor3> {
...
public:
    ...
    Field<double> magField() const {
        ...
        for (int i = 0; i < size(); ++i)
            result[i] = mag(data_[i]);  // overload resolution
        return result;
    }
};
...
int main() {
    ...
    // pTraits demo
    std::cout << "scalar: nComponents=" << pTraits<double>::nComponents << "\n";
    std::cout << "vector: nComponents=" << pTraits<Vec3>::nComponents << "\n";
    std::cout << "tensor: nComponents=" << pTraits<Tensor3>::nComponents << "\n";

    // mag() dispatch
    std::cout << "\nmag(-3.14) = " << mag(-3.14) << "\n";
    std::cout << "mag(Vec3(3,4,0)) = " << mag(Vec3(3,4,0)) << "\n";
    ...
    std::cout << "\nvectorField sum: " << vf.sum() << "\n";

    auto mags = vf.magField();
    std::cout << "magnitudes: " << mags[0] << ", " << mags[1] << ", " << mags[2] << "\n";
...
scalar: nComponents=1
vector: nComponents=3
tensor: nComponents=9

mag(-3.14) = 3.14
mag(Vec3(3,4,0)) = 5

vectorField sum: (1 2 3)
magnitudes: 1, 2, 3
```
...
### Exercise 1: Adding `SymmTensor` Specialization

**Question:** OpenFOAM has `symmTensor` (6 components for symmetric tensors). Write `pTraits<SymmTensor>`.

**Solution:**

```cpp
struct SymmTensor {
    double xx, xy, xz, yy, yz, zz;  // 6 unique components
    SymmTensor() : xx(0),xy(0),xz(0),yy(0),yz(0),zz(0) {}
    SymmTensor& operator+=(const SymmTensor& r) {
        xx+=r.xx; xy+=r.xy; xz+=r.xz; yy+=r.yy; yz+=r.yz; zz+=r.zz;
        return *this;
    }
};
...
const SymmTensor pTraits<SymmTensor>::zero = SymmTensor();
...
**Key insight:** Once `pTraits<SymmTensor>` exists, `Field<SymmTensor>` works immediately — no need to modify `Field` class!

---

### Exercise 2: Compile-Time Type Safety

**Question:** Use `static_assert` to verify `pTraits<Type>::nComponents > 0` at compile time.
...
class Field {
    static_assert(pTraits<Type>::nComponents > 0,
        "Field<Type> requires pTraits specialization with nComponents > 0");
    // ...
};

// Field<std::string> bad;  // Compile error! No pTraits<string> specialization
```

This catches type errors at compile time with clear messages, unlike runtime checks.

---

### Exercise 3: Partial Specialization for Pointers

**Question:** Write a partial specialization `pTraits<T*>` for all pointer types.
...
// Now Field<double*> works if needed
// pTraits<int*>::zero == nullptr ✓
```

Partial specialization matches **any** pointer type (`double*`, `int*`, `Vec3*`), providing a default implementation for all pointers.
