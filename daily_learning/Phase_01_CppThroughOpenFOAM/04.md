# Day 04: CRTP — Static Polymorphism & Modern Alternatives

**Phase:** 1 — C++ Through OpenFOAM (Days 01–14)
**Previous:** Day 02 — C++20 Concepts & Constraints: Type-Safe Templates
**Next:** Day 05 — Lambda Expressions & Functional Algorithms
...
## Part 6: Modern C++ Context — CRTP vs C++20 Concepts

### CRTP Still Has Its Place

While C++20 **Concepts** (Day 02) provide a cleaner way to constrain templates, **CRTP remains valuable** for:

1. **Static polymorphism** — When you need the base class to call derived methods
2. **Mixin patterns** — Adding functionality through inheritance
3. **Compile-time duck typing** — Concepts check requirements, CRTP enables behavior

### CRTP vs Concepts — When to Use Which

| Scenario | Use | Why |
|----------|-----|-----|
| Constrain template parameters | **Concepts** (Day 02) | Cleaner syntax, better errors |
| Enable base-to-derived calls | **CRTP** | Concepts can't do this |
| Mixin functionality | **CRTP** | Inheritance-based composition |
| Type requirements | **Concepts** | Declarative constraints |

### Example: CRTP for Interface, Concepts for Constraints

```cpp
// Modern C++20: Combine both approaches

// Concept constrains the type
template<typename T>
concept Interpolated = requires(T t, double w) {
    { t.interpolate(w) } -> std::convertible_to<double>;
};

// CRTP base provides interface
template<Interpolated Derived>
class InterpolationScheme {
public:
    double weighted(double w) const {
        // CRTP: Call derived's method
        return static_cast<const Derived*>(this)->interpolate(w);
    }
};

// Derived class implements interpolation
class LinearScheme : public InterpolationScheme<LinearScheme> {
public:
    double interpolate(double w) const {
        return w;  // Linear interpolation
    }
};

// Both the Concept and CRTP work together
auto scheme = LinearScheme{};
static_assert(Interpolated<LinearScheme>);  // ✅ Concept check
double result = scheme.weighted(0.5);       // ✅ CRTP call
```

**Takeaway:** CRTP and Concepts are complementary, not competing. Use Concepts to **express requirements**, CRTP to **enable static polymorphism**.
...
**Deliverable:** A CRTP-based `InterpolationScheme<Derived>` base class with two concrete implementations (`UpwindScheme`, `LinearScheme`). Static dispatch confirmed — no virtual call overhead, verified with `-O2 -S` assembly output showing inlined calls.
