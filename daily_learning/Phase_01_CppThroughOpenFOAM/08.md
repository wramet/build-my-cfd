...
**Next:** Day 09 — Expression Templates (see Day 03 for modern C++20 Ranges)

> **⚠️ Historical Note:** This file covers OpenFOAM's pre-C++11 move semantics workarounds. For modern C++ move semantics, see **Day 07**.
...
**Deliverable:** A `make_field()` factory using perfect forwarding (`std::forward<Args>(args)...`) to construct `Field<T>` in-place. Unit test confirming no extra copies using a `CopyCounter` move-tracking type.
