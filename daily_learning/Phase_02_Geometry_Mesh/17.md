...
## Section 5: R410A O-Grid Meshing Example for Evaporator Tube | ส่วนที่ 5: ตัวอย่าง O-Grid Meshing สำหรับท่องอ Evaporator ของ R410A

นี่คือตัวอย่างการออกแบบ O-Grid Mesh ที่ละเอียดสำหรับการจำลองการเปลี่ยนเฟสของ R410A ในท่งอ ตัวอย่างนี้แสดงวิธีการสร้าง mesh คุณภาพสูงสำหรับการเปลี่ยนจากของเหลวเป็นไอภายในท่งอที่มีเส้นผ่านศูนย์กลาง 5 mm

### R410A O-Grid Design for Evaporator Tube | การออกแบบ O-Grid สำหรับท่งอ Evaporator ของ R410A

#### Geometry Specifications | ข้อมูลเชิงเรขาคณิต

**Target:** R410A evaporator tube
- **Tube ID:** 5mm (radius = 2.5mm)
- **Tube Length:** 100mm (for validation)
- **Operating Conditions:** 10°C, 1.0 MPa
- **Mesh Type:** 2D O-Grid (extrudable to 3D)

**Design Goals:** | เป้าหมายในการออกแบบ:
- First cell: 2.5 μm (for y+ < 1 with liquid R410A)
- Boundary layers: 20 cells
- Center block: 50% of diameter
- Aspect ratio: < 5
- Orthogonality: > 80°

### Complete Topology | โทโพโลยีที่สมบูรณ์

**O-Grid Strategy:** | กลยุทธ์ O-Grid:
```
                    Top Wall
         ┌─────────────────────────────────┐
         │                                 │
...
**2D Cross-Section (z = 0):** | ขวางแนวตั้ง 2D (z = 0):

**Vertices (9 total for center + O-grid):** | จุดยอด (9 จุดสำหรับศูนย์กลาง + O-grid):

```python
# Center
(0 0 0)                  # v0: Center

# Inner square (1.25mm from center, 50% of radius)
(0.00125 0 0)            # v1: East (1/4 radius)
(0 0.00125 0)            # v2: North
(-0.00125 0 0)           # v3: West
(0 -0.00125 0)           # v4: South

# Mid-radius (2.5mm from center, full radius)
(0.0025 0 0)             # v5: East (1/2 radius)
(0 0.0025 0)             # v6: North
(-0.0025 0 0)            # v7: West
(0 -0.0025 0)            # v8: South

# Wall corners (2.5mm radius, 45°)
(0.0025 0.0025 0)        # v9: Northeast
(-0.0025 0.0025 0)       # v10: Northwest
(-0.0025 -0.0025 0)      # v11: Southwest
(0.0025 -0.0025 0)       # v12: Southeast
```

**Block Structure (9 blocks):** | โครงสร้างบล็อก (9 บล็อก):
```
Block 0: Center (hex)
Block 1: Quadrant 1 (NE) - Radial grading
Block 2: Quadrant 2 (NW) - Radial grading
Block 3: Quadrant 3 (SW) - Radial grading
Block 4: Quadrant 4 (SE) - Radial grading
Blocks 5-8: Corner blocks (optional, can merge with quadrants)
```

### Boundary Layer Grading Calculation | การคำนวณการเกรดของชั้นเบาะเบาะ

**For y+ < 1 with R410A liquid:** | สำหรับ y+ < 1 กับของเหลว R410A:

Given: | สมมติฐาน:
- First cell: y₀ = 2.5 μm = 2.5e-6 m
- Last BL cell: y_N = 250 μm = 250e-6 m
- Number of layers: N = 20

**Growth Ratio:** | อัตราการเพิ่มขึ้น:
$$ r = \left(\frac{y_N}{y_0}\right)^{1/N} = \left(\frac{250}{2.5}\right)^{1/20} = 100^{1/20} \approx 1.26 $$

**Verification:** | การตรวจสอบ:
$$ y_{19} = y_0 \times r^{19} = 2.5 \times 10^{-6} \times 1.26^{19} \approx 248 \times 10^{-6} \text{ m} \approx 248 \text{ μm} $$

### Complete blockMeshDict | ไฟล์ blockMeshDict ที่สมบูรณ์
...
(
    ...
    // Wall corners (2.5mm radius)
    (0.0025 0.0025 0)          // 9: northeast
    (-0.0025 0.0025 0)         // 10: northwest
    (-0.0025 -0.0025 0)        // 11: southwest
    (0.0025 -0.0025 0)         // 12: southeast

    // Z-direction (extrude 100mm)
    // Duplicate all with z = 0.1
    (0 0 0.1)                  // 13
    (0.00125 0 0.1)            // 14
    (0 0.00125 0.1)            // 15
    (-0.00125 0 0.1)           // 16
    ...
    (-0.0025 0 0.1)            // 20
    (0 -0.0025 0.1)            // 21
    (0.0025 0.0025 0.1)        // 22
    (-0.0025 0.0025 0.1)       // 23
    (-0.0025 -0.0025 0.1)      // 24
    (0.0025 -0.0025 0.1)       // 25
);

blocks
(
    // Center block (no grading)
    hex (0 1 2 0 13 14 15 13) (20 20 100) simpleGrading (1 1 1)

    // Quadrant 1 (NE): radial grading in x and y
    hex (1 5 9 6 14 18 22 19) (20 20 100)
    edgeGrading (1 1 100 1 1 100 1 1 1 1 1 1)

    // Quadrant 2 (NW)
    hex (2 6 10 7 15 19 23 20) (20 20 100)
    edgeGrading (1 1 100 1 1 100 1 1 1 1 1 1)

    // Quadrant 3 (SW)
    hex (0 7 11 8 13 20 24 21) (20 20 100)
    edgeGrading (1 1 100 1 1 100 1 1 1 1 1 1)

    // Quadrant 4 (SE)
    hex (0 8 12 5 13 21 25 18) (20 20 100)
    edgeGrading (1 1 100 1 1 100 1 1 1 1 1 1)
);
...
(
    inlet
    {
        ...
        faces
        (
            (0 2 15 13)      // Center + north
            (2 7 20 15)      // North quadrant
            (7 0 13 20)      // West quadrant
        );
    }
    ...
    outlet
    {
        ...
        faces
        (
            (5 9 22 18)      // East quadrant
            (9 6 19 22)      // Northeast corner
            (6 5 18 19)      // North-east edge
        );
    }
    ...
    walls
    {
        ...
        faces
        (
            (9 10 23 22)     // North wall
            (10 11 24 23)    // West wall
            (11 12 25 24)    // South wall
            (12 9 22 25)     // East wall
        );
    }
...
### Quality Metrics Comparison | การเปรียบเทียบตัวชี้วัดคุณภาพ

**O-Grid vs. Quad Mesh for R410A:** | O-Grid กับ Quad Mesh สำหรับ R410A:

| Metric | O-Grid | Quad Mesh | R410A Impact | ผลกระทบต่อ R410A |
|--------|--------|-----------|--------------|-------------------|
| **Orthogonality** | 85° | 70° | Better for pressure solver with property jumps | ดีกว่าสำหรับ pressure solver ที่มีการกระโดดคุณสมบัติ |
| **Aspect Ratio** | 3 | 8 | Critical for vapor phase stability | สำคัญสำหรับเสถียรภาพของเฟสไอ |
| **Skewness** | 0.3 | 1.2 | Affects VOF interface sharpness | ส่งผลต่อความคมชัดของ interface VOF |
| **Non-orthogonality** | 5° | 20° | Pressure solver convergence | การลู่เข้าของ pressure solver |
| **Cell Count** | 400K | 200K | 2× more cells but much better quality | 2× มากกว่า cells แต่คุณภาพดีกว่ามาก |
| **Setup Time** | 2 hours | 30 min | O-Grid more complex | O-Grid ซับซ้อนกว่า |
| **y+ Control** | Excellent (2.5 μm) | Poor (10 μm) | Critical for R410A evaporation | สำคัญสำหรับการ蒸發ของ R410A |

**Recommendation:** Use O-Grid for R410A evaporator simulations despite higher cell count. | แนะนำ: ใช้ O-Grid สำหรับการจำลอง evaporator R410A แม้จะมี cell count ทสูงขึ้น

### Mesh Quality Analysis | การวิเคราะห์คุณภาพ Mesh

**Why O-Grid is Better for R410A:** | ทำไม O-Grid จึงดีกว่าสำหรับ R410A:

1. **Orthogonality at Walls:** | ความเป็นฉากที่ผนัง:
   - O-Grid maintains ~90° at wall
   - Critical for accurate wall heat transfer (evaporation)
   - Quad mesh has skewed cells near wall

2. **Boundary Layer Control:** | การควบคุมชั้นเบาะเบาะ:
   - Precise control of first cell height
   - Enables y+ < 1 for liquid R410A
   - Essential for phase change modeling

3. **Aspect Ratio:** | อัตราส่วนของด้าน:
   - O-Grid: AR ≈ 3 (acceptable)
   - Quad mesh: AR ≈ 8 (problematic for vapor)

4. **Property Jump Handling:** | การจัดการการกระโดดคุณสมบัติ:
   - Better cell quality at interface
   - Reduces numerical diffusion in VOF
   - Maintains sharp interface

### Validation Test Procedure | ขั้นตอนการทดสอบการยืนยัน

**Step 1: Generate Mesh** | ขั้นตอนที่ 1: สร้าง Mesh
```bash
blockMesh
...
**Step 2: Check Mesh Quality** | ขั้นตอนที่ 2: ตรวจสอบคุณภาพ Mesh
```bash
checkMesh -allGeometry -allTopology

# Expected output:
# Mesh OK
#    Overall domain bounding box (0 0 0) (0.005 0.005 0.1)
#    Mesh has 2 topology problems
#        Number of regions: 1 (okay)
#    Mesh OK
#
#    Non-orthogonality: Max = 5.2 deg, average = 2.1 deg
#    Skewness: Max = 0.32, average = 0.12
#    Aspect ratio: Max = 3.1, average = 1.8
```

**Step 3: Verify y+** | ขั้นตอนที่ 3: ยืนยัน y+

**Run single-phase liquid flow:** | รันการไหลแบบ single-phase liquid:
```bash
# Set boundary conditions for liquid R410A
# Inlet: U = 0.167 m/s (from G = 200 kg/m²s)
# Outlet: p = 0 (relative)
# Wall: noSlip
...
# Check y+ field
# Should have y+ < 1 at wall for liquid
```

**Step 4: Test Two-Phase Flow** | ขั้นตอนที่ 4: ทดสอบการไหลแบบ Two-Phase

**Initialize interface:** | เริ่มต้น interface:
```bash
# Set alpha = 0.5 in center region
...
//     boxToCell
//     {
//         box (0.001 -0.005 -0.005) (0.004 0.005 0.105);
//         fieldValues
//         (
...
**Run interFoam:** | รัน interFoam:
```bash
interFoam

# Monitor interface sharpness
# Should maintain 2-3 cells across interface
```

**Step 5: Visual Validation** | ขั้นตอนที่ 5: การตรวจสอบด้วยสายตา
```bash
paraFoam -builtin

# Check:
# - Mesh quality (rendering)
# - Interface sharpness (alpha field)
# - Velocity profile (U field)
# - Boundary layer resolution
```

### Expected Results | ผลลัพธ์ที่คาดหวัง

**For R410A evaporator (5mm tube, G = 200 kg/m²s):** | สำหรับ evaporator R410A (ท่งอ 5mm, G = 200 kg/m²s):

| Parameter | Expected | How to Verify | พารามิเตอร์ | คาดหวัง | วิธีการตรวจสอบ |
|-----------|----------|---------------|--------------|----------|------------------|
| First cell y+ | < 1 (liquid) | yPlus utility | y+ ของเซลล์แรก | < 1 (liquid) | yPlus utility |
| Mesh quality | All checks pass | checkMesh | คุณภาพ Mesh | ผ่านทุกการตรวจสอบ | checkMesh |
| Interface thickness | 2-3 cells | ParaView visualization | ความหนาของ interface | 2-3 cells | ParaView visualization |
| Pressure drop | Matches correlation | Compare with Darcy-Weisbach | ความดันหมด | ตรงกับ correlation | Compare with Darcy-Weisbach |
| Convergence | Residuals < 1e-5 | Check log file | การลู่เข้า | Residuals < 1e-5 | Check log file |

### Troubleshooting | การแก้ไขปัญหา

**Issue: High aspect ratio (> 5)** | ปัญหา: อัตราส่วนของด้านสูง (> 5)
```cpp
// Solution: Reduce grading
// Change from:
edgeGrading (1 1 100 1 1 100 1 1 1 1 1 1)

// To:
edgeGrading (1 1 50 1 1 50 1 1 1 1 1 1)

// And increase radial cells:
// From 20 to 30
```

**Issue: Poor orthogonality** | ปัญหา: ความเป็นฉากแย่
```bash
# Check where:
checkMesh -writeSets vtk

# Visualize non-orthogonality field
# Identify problematic cells
# Adjust vertex positions locally
```

**Issue: y+ too high** | ปัญหา: y+ สูงเกินไป
```bash
# Reduce first cell thickness:
# From 2.5e-6 to 1.0e-6

# Recalculate grading:
# r = (250/1.0)^(1/20) ≈ 1.35

# Update edgeGrading:
edgeGrading (1 1 150 1 1 150 1 1 1 1 1 1)
```

### Integration with Mesh I/O System | การผสานรวมกับระบบ Mesh I/O

O-Grid mesh generation can be integrated into our MeshReader/Writer system through specialized O-grid MeshReader class:

```cpp
// Specialized O-grid MeshReader for R410A evaporator
class R410A_OGridReader : public MeshReader
{
public:
    //- Create O-grid topology for evaporator tube
    void createOGridTopology
    (
        const scalar tubeRadius,
        const scalar centerBlockRatio,
        const label radialCells,
        const label axialCells
    );

    //- Apply boundary layer grading
    void applyBoundaryLayerGrading
    (
        const scalar firstCellHeight,
        const scalar growthRatio,
        const label boundaryLayers
    );

    // Override read method to generate O-grid directly
    virtual bool read(const fileName& file) override;
};
```

This O-grid implementation demonstrates the practical application of mesh I/O and quality control principles from this day to create specialized meshes for two-phase flow applications. | Implementation นี้แสดงการประยุกต์ใช้หลักการ mesh I/O และการควบคุมคุณภาพจากวันนี้เพื่อสร้าง mesh ที่เฉพาะสำหรับแอพลิเคชันการไหลแบบ two-phase
