...
## 18.6 R410A-Specific Convergence Criteria | เกณฑ์การ Konvergence สำหรับ R410A

### 18.6.1 Grid Convergence Study for R410A Evaporator | การศึกษาการ Konvergence ของกริดสำหรับระบบเครื่องระเหย R410A

**Grid Convergence Parameters for R410A:**
เมื่อจำลองการเครื่องระเหยของ R410A ในท่อ เราต้องพิจารณาการควบคุมความถูกต้องของกริดอย่างรอบด้านเนื่องจากฟิสิกส์ที่ซับซ้อน รวมถึงการเปลี่ยนสถานะจากของเหลวเป็นไอ การแบ่งกริด (grid independence study) เป็นสิ่งสำคัญเพื่อให้มั่นใจว่าผลลัพธ์ไม่ขึ้นอยู่กับความละเอียดของกริด

**Grid Sizes Tested:**
| Grid Level | Cells | Avg. Δx (mm) | Aspect Ratio | Skewness | Purge Level |
|------------|-------|--------------|--------------|----------|-------------|
| Coarse | 50,000 | 1.0 | 3.5 | 0.80 | 85% |
| Medium | 100,000 | 0.5 | 3.2 | 0.60 | 95% |
| Fine | 200,000 | 0.25 | 3.0 | 0.35 | 98% |
| Ultra-fine | 400,000 | 0.125 | 2.8 | 0.20 | 99.5% |

**Monitored Quantities:**
- Outlet quality: x_out (vapor quality at outlet)
- Pressure drop: ΔP (bar)
- Heat transfer coefficient: h (W/m²·K)
- Interface position: Location where α = 0.5
- Mass flow rate conservation: ṁ (kg/s)

**Grid Convergence Index (GCI) Calculation:**
The Grid Convergence Index helps quantify the numerical uncertainty:
$$
GCI = \frac{1.25 |e_{21}|}{r^p - 1}
$$
where:
- $e_{21}$ is the relative difference between two consecutive grid levels
- $r$ is the refinement ratio
- $p$ is the order of convergence (typically 2 for second-order schemes)

**Results for R410A Evaporator:**
| Quantity | Coarse → Medium | Medium → Fine | Fine → Ultra-fine | GCI < 5%? |
|----------|-----------------|---------------|------------------|-----------|
| x_out | 2.3% | 0.8% | 0.2% | ✅ Yes (at Fine) |
| ΔP | 5.1% | 1.9% | 0.5% | ✅ Yes (at Fine) |
| h | 8.2% | 3.1% | 0.9% | ✅ Yes (at Ultra-fine) |

**Recommendation for R410A Simulations:**
- **Grid independence achieved at 200,000 cells**
- GCI < 5% for most quantities
- Ultra-fine grid only needed for heat transfer coefficient

### 18.6.2 Time Step Independence Analysis | การวิเคราะห์การไม่ขึ้นกับขนาดเวลา (Time Step Independence)

**Time Step Parameters:**
ในการจำลองการเครื่องระเหยของ R410A ขนาดเวลา (time step) เป็นปัจจัยที่สำคัญมากเนื่องจากมีการเปลี่ยนสถานะที่เร็วและเขตสภาพ (phase change front) ที่แหลมคม

**Time Steps Tested:**
| Δt (s) | Co_max | CPU Time (relative) | x_out Error | ΔP Error | Interface Position Error |
|--------|--------|---------------------|------------|---------|------------------------|
| 1.0×10⁻⁵ | 0.28 | 1.0× | 2.3% | 1.8% | 3.5% |
| 5.0×10⁻⁶ | 0.14 | 2.1× | 0.8% | 0.6% | 1.2% |
| 2.5×10⁻⁶ | 0.07 | 4.3× | 0.2% | 0.2% | 0.4% |

**Courant Number Requirements for R410A:**
$$Co = \frac{|U| \Delta t}{\Delta x} < 0.3$$
The stricter limit (Co < 0.3 instead of Co < 1) is used for R410A to ensure:
- Accurate interface tracking
- Stable phase change
- Proper mass transfer between phases

**Recommendations:**
- **Optimal Δt = 5×10⁻⁶ s**
- Balance between accuracy and computational cost
- Co_max = 0.14 ensures stability

### 18.6.3 Residual Convergence Targets for R410A | เป้าหมายการ Konvergence สำหรับ R410A

**Solver Settings in fvSolution:**
สำหรับการจำลองการเครื่องระเหยของ R410A การตั้งค่า solver ต้องคำนึงถึงฟิสิกส์ของกระบวนการสองสถานะและการเปลี่ยนสถานะ:
...
solvers
{
    ...
    alpha.water
    {
        nAlphaCorr      2;      // Correct alpha twice
        nAlphaSubCycles 1;
        cAlpha          1.0;
        MULESCorr       yes;
    }
...
**Iteration Count Recommendations by Region:**
| Region | Iterations | Convergence Criteria |
|--------|------------|----------------------|
| Inlet (Single-Phase Liquid) | 10-30 | p: 1e-6, U: 1e-7 |
| Two-Phase Region | 50-200 | p: 1e-7, α: 1e-8 |
| Near Outlet (Single-Phase Vapor) | 100-300 | p: 1e-7, U: 1e-8 |

### 18.6.4 Monitoring Script for R410A Convergence | สคริปต์ติดตามการ Konvergence สำหรับ R410A

```python
#!/usr/bin/env python3
"""Monitor R410A evaporator convergence and grid independence"""
...
class R410AConvergenceMonitor:
    """Class for monitoring R410A evaporator convergence"""

    def __init__(self, log_file="log.R410AEvaporator"):
    ...
    def read_log_file(self):
        """Read and parse the OpenFOAM log file"""
        try:
            # Read log file, skip header lines
            with open(self.log_file, 'r') as f:
                lines = f.readlines()

            # Find data start line (after "Time" line)
            start_idx = None
            ...
            if start_idx is None:
                raise ValueError("Could not find data start in log file")
            ...
            for line in lines[start_idx:]:
                if line.strip():
                    ...
                    if len(parts) >= 5:
                        data.append({
                            'Time': float(parts[0]),
                            'p_rgh_initial': float(parts[1]),
                            'U_initial': float(parts[2]),
                            'T_initial': float(parts[3]),
                            'alpha_initial': float(parts[4]),
                            'p_rgh_final': float(parts[5]),
                            'U_final': float(parts[6]),
                            'T_final': float(parts[7]),
                            'alpha_final': float(parts[8]),
    ...
    def plot_convergence(self, save_path="convergence_plot.png"):
        ...
        # Residuals plot
        ax1 = axes[0, 0]
        ax1.semilogy(self.data['Time'], self.data['p_rgh_initial'],
                    label='p_rgh', color='blue', linewidth=1)
        ax1.semilogy(self.data['Time'], self.data['U_initial'],
                    label='U', color='red', linewidth=1)
        ax1.semilogy(self.data['Time'], self.data['T_initial'],
                    label='T', color='green', linewidth=1)
        ax1.semilogy(self.data['Time'], self.data['alpha_initial'],
                    label='alpha', color='orange', linewidth=1)
        ...
        # Final residuals
        ax2 = axes[0, 1]
        final_residuals = self.data.iloc[-1]
        categories = ['p_rgh', 'U', 'T', 'alpha']
        values = [final_residuals['p_rgh_final'],
                 final_residuals['U_final'],
                 final_residuals['T_final'],
                 final_residuals['alpha_final']]

        bars = ax2.bar(categories, values, color=['blue', 'red', 'green', 'orange'])
        ax2.set_ylabel('Final Residual')
        ax2.set_title('Final Residuals')
        ax2.set_yscale('log')
        ...
        ax3 = axes[1, 0]
        ax3.plot(self.data['Time'], self.data['iterations'], 'o-',
                color='purple', linewidth=1, markersize=3)
        ax3.set_xlabel('Time (s)')
        ax3.set_ylabel('Iterations')
        ax3.set_title('Iterations per Time Step')
        ax3.grid(True, alpha=0.3)

        # Convergence rate
        ax4 = axes[1, 1]
        # Calculate convergence rate for last 5 time steps
        recent_data = self.data.tail(5)
        for field in ['p_rgh', 'U', 'T', 'alpha']:
            residuals = recent_data[f'{field}_initial'].values
            # Calculate log of residuals
            log_residuals = np.log(residuals)
            # Fit linear trend
            coeffs = np.polyfit(range(len(log_residuals)), log_residuals, 1)
            convergence_rate = -coeffs[0]  # Negative of slope

            ax4.bar(field, convergence_rate, color={'p_rgh':'blue', 'U':'red',
                                               'T':'green', 'alpha':'orange'}[field])

        ax4.set_ylabel('Convergence Rate')
        ax4.set_title('Linear Convergence Rate (last 5 steps)')
    ...
    def calculate_statistics(self):
        ...
        print("\n=== R410A Convergence Statistics ===")
        print(f"Total simulation time: {self.data['Time'].max():.3f} s")
        print(f"Number of time steps: {len(self.data)}")

        # Final residuals
        final = self.data.iloc[-1]
        print(f"\nFinal Residuals:")
        print(f"  p_rgh: {final['p_rgh_final']:.2e}")
        print(f"  U: {final['U_final']:.2e}")
        print(f"  T: {final['T_final']:.2e}")
        print(f"  alpha: {final['alpha_final']:.2e}")

        # Average iterations
        avg_iter = self.data['iterations'].mean()
        print(f"\nAverage iterations per time step: {avg_iter:.1f}")
    ...
    def check_convergence(self):
        ...
        criteria_met = {
            'p_rgh': final['p_rgh_final'] < 1e-7,
            'U': final['U_final'] < 1e-8,
            'T': final['T_final'] < 1e-9,
            'alpha': final['alpha_final'] < 1e-8
        }

        print("\nConvergence Check:")
        all_met = True
        for field, met in criteria_met.items():
        ...
        if all_met:
            ...
            print("\n⚠️ Some convergence criteria not met. Consider:")
            print("  - Reducing relaxation factors")
            print("  - Adjusting solver tolerances")
            print("  - Checking mesh quality")

def main():
    """Main execution function"""
    # Initialize monitor
    monitor = R410AConvergenceMonitor()
...
### 18.6.5 Validation Metrics for R410A | เมตริกการตรวจสอบความถูกต้องสำหรับ R410A

**Comparison with Empirical Correlations:**
เพื่อยืนยันความถูกต้องของการจำลองการเครื่องระเหยของ R410A เราสามารถเปรียบเทียบผลลัพธ์กับสูตรการพยากรณ์จากการทดลอง:

**1. Pressure Drop Validation:**
Darcy-Weisbach correlation for two-phase flow:
$$ \Delta P_{corr} = f \frac{L}{D} \frac{G^2}{2\rho_m} $$
where:
- $f$ is the friction factor (using Lockhart-Martinelli correlation)
- $\rho_m = \alpha \rho_v + (1-\alpha) \rho_l$ is the mixture density
- $G = \dot{m}/A$ is the mass flux

**2. Heat Transfer Validation:**
Shah correlation for evaporation:
$$ h_{corr} = h_l \left[ (1-x)^{0.8} + \frac{3.8x^{0.76}(1-x)^{0.04}}{P_r^{0.38}} \right] $$

**3. Outlet Quality Validation:**
Energy balance equation:
$$ x_{out} = x_{in} + \frac{Q}{\dot{m} h_{fg}} $$

**Acceptable Error Thresholds:**
| Quantity | Acceptable Error | Rationale |
|----------|------------------|-----------|
| Pressure drop | < 10% | Affects pumping power design |
| Heat transfer coefficient | < 15% | Critical for thermal sizing |
| Outlet quality | < 5% | Determines downstream components |
| Mass conservation | < 1% | Physical law violation |
| Energy conservation | < 1% | Physical law violation |

**Validation Script:**
```python
#!/usr/bin/env python3
"""Validate R410A simulation results against correlations"""

import numpy as np
import matplotlib.pyplot as plt
from scipy.interpolate import interp1d

class R410AValidation:
    """Validate R410A simulation results"""

    def __init__(self):
        # R410A properties at typical evaporator conditions
        self.rho_l = 1200.0  # kg/m³, liquid density
        self.rho_v = 70.0    # kg/m³, vapor density
        self.mu_l = 2.5e-4   # Pa·s, liquid viscosity
        self.mu_v = 1.2e-5   # Pa·s, vapor viscosity
        self.k_l = 0.08      # W/m·K, liquid thermal conductivity
        self.k_v = 0.02      # W/m·K, vapor thermal conductivity
        self.cp_l = 1800     # J/kg·K, liquid specific heat
        self.cp_v = 1200     # J/kg·K, vapor specific heat
        self.h_fg = 180000   # J/kg, latent heat of vaporization

    def calculate_lockhart_martinelli(self, X_tt):
        """Calculate Lockhart-Martinelli parameter"""
        # Turbulent-turbulent flow assumption
        phi_tt_squared = 1 + 20/X_tt + 1/X_tt**2
        return np.sqrt(phi_tt_squared)

    def calculate_shah_coefficient(self, x, P_r):
        """Calculate Shah heat transfer coefficient"""
        # Shah correlation for convective boiling
        h_l = 0.023 * (self.rho_l * self.mu_l / 1000) ** 0.8 \
              * (self.cp_l * 1000 / self.k_l) ** 0.4 \
              * self.k_l / 0.01  # Assuming D = 0.01m
    ...
    def validate_pressure_drop(self, x, G, D, L):
        """Validate pressure drop against correlation"""
        # Calculate two-phase multiplier
        X_tt = ((1-x)/x) ** 0.9 * (self.rho_v / self.rho_l) ** 0.5 \
               * (self.mu_l / self.mu_v) ** 0.1

        phi_tt = self.calculate_lockhart_martinelli(X_tt)

        # Single-phase pressure drop
        Re_l = G * D / self.mu_l
        f_l = 0.316 / Re_l ** 0.25  # Blasius correlation

        Delta_p_single = f_l * L / D * G ** 2 / (2 * self.rho_l)

        # Two-phase pressure drop
        Delta_p_corr = phi_tt ** 2 * Delta_p_single

        return Delta_p_corr

    def validate_heat_transfer(self, x, T_sat, T_wall, P_r):
        """Validate heat transfer coefficient"""
        h_corr = self.calculate_shah_coefficient(x, P_r)

        # Heat flux
        q = h_corr * (T_wall - T_sat)

        return h_corr, q

    def plot_validation(self, simulation_data):
        """Plot validation results"""
        fig, axes = plt.subplots(1, 3, figsize=(15, 5))

        # Pressure drop validation
        axes[0].scatter(simulation_data['x'], simulation_data['Delta_p_sim'],
                       color='red', label='Simulation', s=50)
        axes[0].plot(simulation_data['x'], simulation_data['Delta_p_corr'],
                    'b-', label='Correlation', linewidth=2)
        axes[0].set_xlabel('Vapor Quality x')
        axes[0].set_ylabel('Pressure Drop (Pa)')
        axes[0].set_title('Pressure Drop Validation')
        axes[0].legend()
        axes[0].grid(True, alpha=0.3)

        # Heat transfer validation
        axes[1].scatter(simulation_data['x'], simulation_data['h_sim'],
                       color='red', label='Simulation', s=50)
        axes[1].plot(simulation_data['x'], simulation_data['h_corr'],
                    'b-', label='Correlation', linewidth=2)
        axes[1].set_xlabel('Vapor Quality x')
        axes[1].set_ylabel('Heat Transfer Coefficient (W/m²·K)')
        axes[1].set_title('Heat Transfer Validation')
        axes[1].legend()
        axes[1].grid(True, alpha=0.3)

        # Error analysis
        error_dp = np.abs(simulation_data['Delta_p_sim'] -
                         simulation_data['Delta_p_corr']) / simulation_data['Delta_p_corr'] * 100
        error_h = np.abs(simulation_data['h_sim'] -
                        simulation_data['h_corr']) / simulation_data['h_corr'] * 100

        axes[2].fill_between(simulation_data['x'], 0, error_dp,
                           alpha=0.3, color='red', label='ΔP Error')
        axes[2].fill_between(simulation_data['x'], 0, error_h,
                           alpha=0.3, color='blue', label='h Error')
        axes[2].axhline(y=10, color='red', linestyle='--', alpha=0.5, label='ΔP Limit')
        axes[2].axhline(y=15, color='blue', linestyle='--', alpha=0.5, label='h Limit')
        axes[2].set_xlabel('Vapor Quality x')
        ...
        plt.tight_layout()
        plt.savefig('r410a_validation.png', dpi=300, bbox_inches='tight')
        plt.show()

        # Print summary
        print("\n=== R410A Validation Summary ===")
        print(f"Average ΔP error: {np.mean(error_dp):.1f}%")
        print(f"Average h error: {np.mean(error_h):.1f}%")
        print(f"Max ΔP error: {np.max(error_dp):.1f}%")
        print(f"Max h error: {np.max(error_h):.1f}%")

        success = (np.mean(error_dp) < 10) and (np.mean(error_h) < 15)
        if success:
            print("\n🎉 Simulation validated successfully!")
        else:
            print("\n⚠️ Validation failed - consider improving model")

if __name__ == "__main__":
    # Example usage
    validator = R410AValidation()
    ...
    T_sat = 5  # °C
    T_wall = 10  # °C
    P_r = 0.3  # Reduced pressure

    sim_data = {
        'x': x,
        'Delta_p_sim': validator.validate_pressure_drop(x, G, D, L) * np.random.normal(1, 0.05, len(x)),
        'Delta_p_corr': [validator.validate_pressure_drop(xi, G, D, L) for xi in x],
        'h_sim': [validator.validate_heat_transfer(xi, T_sat, T_wall, P_r)[0] * np.random.normal(1, 0.05) for xi in x],
        'h_corr': [validator.validate_heat_transfer(xi, T_sat, T_wall, P_r)[0] for xi in x]
    }

    validator.plot_validation(sim_data)
```

### 18.6.6 Best Practices for R410A Convergence | วิธีการดีที่สุดสำหรับ Konvergence ของ R410A

**Before Starting Simulation:**
1. **Mesh Quality Assessment**
   - Verify aspect ratio < 20
   - Check skewness < 0.8
   - Ensure orthogonality > 0.8
   - Use `checkMesh` command

2. **Initialization**
   - Initialize with single-phase liquid
   - Use `potentialFoam` for pressure initialization
   - Ensure all fields are bounded (0 ≤ α ≤ 1)

3. **Boundary Conditions**
   - Set appropriate inlet velocity (Re > 4000)
   - Use pressure outlet boundary
   - Set wall temperature for heat transfer

**During Simulation:**
1. **Monitoring**
   - Watch residuals every 5-10 iterations
   - Monitor mass conservation (`massFlowRate` function object)
   - Track interface evolution

2. **Adaptation**
   - Reduce relaxation factors if oscillations occur
   - Increase maximum iterations if not converging
   - Consider using `PBiCGStab` for better stability

3. **Troubleshooting**
   - If α goes outside [0,1]: use `bounded` interpolation
   - If pressure oscillates: reduce `relaxationFactors/fields/p_rgh`
   - If temperature diverges: check thermal boundary conditions
...
2. **Grid Independence**
   - Run on finer grid to confirm results
   - Calculate GCI for key quantities

3. **Documentation**
   - Save all convergence plots
   - Document solver settings used
   - Record computational time and resources

**Common Issues and Solutions:**
| Issue | Symptom | Solution |
|-------|---------|----------|
| Interface instability | α oscillates | Use sharper gradient scheme |
| Mass conservation error | Total mass changes | Use `VOF` scheme for α |
| Slow convergence | High iteration count | Use AMG preconditioner |
| Pressure oscillations | p_rgh residual jumps | Reduce relaxation factor |
| Temperature divergence | T → ±∞ | Check thermal BCs |
