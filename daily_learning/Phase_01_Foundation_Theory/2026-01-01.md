...
พลังงานแฝง (Latent Heat, $h_{fg}$) คือพลังงานที่ต้องเพิ่มหรือกำจัดออกเพื่อเปลี่ยนสถานะของสารหนึ่งหน่วยมวลโดยที่อุณหภูมิคงที่ ดังนั้น อัตราการสร้างความร้อนแฝง $\dot{q}_{latent}$ มีความสัมพันธ์โดยตรงกับอัตราการถ่ายโอนมวล $\dot{m}$ (จาก Day 11, Lee Model) ดังนี้:
...
นี่คือ **Hero Implementation** ของวันนี้ตามที่ระบุใน Bible ฟังก์ชันนี้รับพารามิเตอร์หลักคือ `fvMatrix<scalar>& pEqn` ซึ่งเป็นเมทริกซ์ของสมการ pressure Poisson ที่กำลังถูกประกอบ (`A * p = b`) ฟังก์ชันจะเพิ่มเทอม `ṁ(1/ρ_v - 1/ρ_l)` ลงใน source vector `b` ของเมทริกซ์นี้ **การทำเช่นนี้สำคัญมาก** เพราะถ้าเราแก้สมการ pressure Poisson แบบมาตรฐาน (`∇·(∇p) = ∇·(H/A)` โดยไม่มีเทอมนี้ ความเร็วที่คำนวณได้จาก `U = H/A - (1/A)∇p` จะไม่สอดคล้องกับสมการ continuity ที่มี phase change ทำให้เกิด mass imbalance และ solver จะ diverge ในทันที
...
# 4. สร้าง Library หลักของ CFD Engine
...
# 8. ส่วนของ Unit Tests (สำคัญมากสำหรับการ verify correctness)
enable_testing()
...
2.  **การตีความทางฟิสิกส์ของเทอม Expansion:**
    *   เทอม `ṁ(1/ρ_v - 1/ρ_l)` ในสมการ ∇·U = ṁ(1/ρ_v - 1/ρ_l) มีหน่วยเป็น `[1/s]` ซึ่งคือ **อัตราการขยายตัวของปริมาตรต่อหน่วยปริมาตร (Volumetric Expansion Rate)**
    *   **ส่วนประกอบ `ṁ/ρ_v`**: แสดงถึง**อัตราการสร้างปริมาตรไอใหม่** จากการระเหย
    *   **ส่วนประกอบ `-ṁ/ρ_l`**: แสดงถึง**อัตราการสูญเสียปริมาตรของเหลว** จากการระเหย
    *   เนื่องจาก `1/ρ_v > 1/ρ_l` เสมอ (เพราะ ρ_v < ρ_l) ผลลัพธ์ของเทอมนี้จึงเป็น**บวก** นั่นคือ ∇·U > 0 ซึ่งหมายความว่า **มี "แหล่งกำเนิด" ของปริมาตร (volume source) เกิดขึ้นภายใน control volume ที่มี interface** ทำให้การไหลออกทางผิวหน้าของ control volume มากกว่าการไหลเข้าในแง่ของปริมาตร
...
3.  **ผลกระทบต่อ Pressure-Velocity Coupling:**
    ...
    *   หากเราใช้สมการความต่อเนื่องแบบดั้งเดิม (∇·U = 0) เราจะได้สมการ Pressure Poisson แบบมาตรฐาน: ∇·( (1/A_p) ∇p ) = ∇·(H/A)
    *   แต่ถ้าเราใช้สมการความต่อเนื่องที่มีเทอม expansion (∇·U = S_exp) สมการ Pressure Poisson จะกลายเป็น:
        `∇·( (1/A_p) ∇p ) = ∇·(H/A) - S_exp`
    *   **เทอม `-S_exp` นี้สำคัญมาก** มันทำหน้าที่เป็น **pressure source ที่ขับเคลื่อนการไหล** เพื่อรองรับการขยายตัวของปริมาตรที่เกิดขึ้น หากไม่มีเทอมนี้ สนามความดันที่คำนวณได้จะพยายามบังคับให้ ∇·U = 0 ซึ่งเป็นไปไม่ได้ทางฟิสิกส์ในบริเวณที่มีการระเหย ส่งผลให้ Solver **Diverge ทันที** เนื่องจากเกิดความไม่สอดคล้อง (inconsistency) ระหว่างสมการ
...
3.  **ตัวอย่างการ Assemble Matrix จากสมการ Diffusion:**
    ...
    `a_P * phi_P + ∑_N a_N * phi_N = source_P`
    ...
    *   **การเชื่อมโยงระหว่างเซลล์**: ค่า `a_N` ที่เชื่อมโยงระหว่าง cell P (owner) และ cell N (neighbour) ผ่าน face f จะถูกเก็บไว้ใน `upper()[faceIndex]` (สำหรับการมีส่วนร่วมของ N ต่อสมการของ P) และใน `lower()[faceIndex]` (สำหรับการมีส่วนร่วมของ P ต่อสมการของ N)
    *   **Boundary Conditions**: Contribution จาก BCs จะถูกเพิ่มเข้าไปใน `diag()` และ `source()` ของ cell ขอบเขต (boundary cell) เท่านั้น โดยไม่สร้าง off-diagonal entries ใหม่
...
## 6.3 คำถาม: Latent Heat Source (q̇_latent) ในสมการพลังงานเชื่อมโยงกับ Mass Transfer Rate (ṁ) อย่างไร?
...
1.  **พื้นฐานทางฟิสิกส์:**
    *   การเปลี่ยนสถานะ (ระเหย/กลั่นตัว) เกิดขึ้นที่อุณหภูมิคงที่ คือ **อุณหภูมิอิ่มตัว (T_sat)** ที่ความดันระบบขณะนั้น
    *   เพื่อเปลี่ยนของเหลว 1 กิโลกรัม ให้กลายเป็นไอ ณ อุณหภูมิอิ่มตัวนั้น ต้องจ่ายพลังงานจำนวนหนึ่งให้กับของเหลวในรูปแบบของ **ความร้อนแฝงการระเหย (Latent Heat of Vaporization, h_fg)** โดยพลังงานนี้ใช้ในการสลายพันธะระหว่างโมเลกุล ไม่ได้เพิ่มอุณหภูมิ
    *   ในทางกลับกัน เมื่อไอ 1 กิโลกรัม กลั่นตัวกลับเป็นของเหลว ก็จะปล่อยพลังงาน `h_fg` ออกมาในบริเวณนั้น

2.  **การกำหนด Latent Heat Source Term:**
    *   ให้ `ṁ` เป็น **อัตราการเปลี่ยนมวลต่อหน่วยปริมาตร** (kg/m³·s) โดยมี**เครื่องหมายบวกสำหรับการระเหย** (ของเหลว→ไอ) และ**เครื่องหมายลบสำหรับการกลั่นตัว** (ไอ→ของเหลว)
    *   **อัตราการดูดกลืน/คายความร้อนแฝงต่อหน่วยปริมาตร** (`q̇_latent`) จะเป็น:
        `q̇_latent = - ṁ * h_fg`
    *   **ที่มาของเครื่องหมายลบ**: พิจารณาจากสมการพลังงานในรูป `∂(ρe)/∂t + ... = ... + q̇`
        *   **กรณีระเหย (ṁ > 0)**: ระบบ (control volume ที่มี interface) **ต้องการดูดกลืนความร้อน** จากสิ่งแวดล้อมเพื่อเปลี่ยนสถานะ ดังนั้น `q̇_latent` ควรเป็น **แหล่งความร้อนเข้าสู่ระบบ (positive source)** แต่เนื่องจาก `ṁ > 0` สมการ `q̇_latent = - ṁ * h_fg` จะให้ค่าลบ! **ข้อขัดแย้งนี้เป็นเรื่องของนิยาม** ใน OpenFOAM/CFD ส่วนใหญ่ สมการพลังงานถูกเขียนในรูปของอุณหภูมิ (T) และ `q̇` ถูกมองเป็น source term ทางด้านขวา **การระเหยทำให้ T ลดลงหากไม่มี heat supply จากที่อื่น** ดังนั้น `q̇_latent` ที่เป็นลบจึงหมายถึง **การสูญเสียพลังงานจาก sensible heat เพื่อนำไปใช้ในกระบวนการระเหย** ซึ่งจะลดค่า T ลง หากเราต้องการรักษา T ไว้ที่ T_sat เราต้องมี external heating ที่จ่าย `ṁ * h_fg` เข้ามาชดเชยพอดี
        *   **ในทางปฏิบัติ** สำหรับ model เช่น Lee Model ที่สมมติ interface อยู่ที่ T_sat เสมอ การคำนวณ `ṁ` จะขึ้นกับ `(T - T_sat)` โดยตรง และ `q̇_latent` จะถูกคำนวณตามสมการข้างต้นและใส่เป็น source term ในสมการพลังงาน การมี source term นี้จะทำให้อุณหภูมิในเซลล์ที่เกิดการเปลี่ยนสถานะมีแนวโน้มจะถูก "ดึง" กลับมาที่ T_sat

3.  **การ Implement ใน Solver:**
    *   ในขั้นตอน `solveEnergy()` หลังจากคำนวณ `ṁ` จาก model (เช่น Lee Model: `ṁ = C * α * ρ * (T - T_sat)/T_sat`) แล้ว
    *   ให้คำนวณ `q̇_latent = - ṁ * h_fg`
    *   เพิ่ม `q̇_latent` ลงใน source term ของสมการพลังงาน (`TEqn.source() += q_latent`)
    *   **สำคัญ**: ค่า `h_fg` อาจเป็นฟังก์ชันของอุณหภูมิ (หรือความดัน) ได้ ต้องคำนวณให้ถูกต้องในแต่ละ iteration

4.  **การรักษา Interface Temperature (T_sat):**
    *   การมีคู่ `ṁ` และ `q̇_latent` ที่สัมพันธ์กันผ่าน `h_fg` นี้สร้างกลไก feedback ที่ทรงพลัง
    *   หาก `T > T_sat` ในบริเวณ interface: `ṁ > 0` (ระเหย) → `q̇_latent < 0` (ดูดความร้อน) → สมการพลังงานจะลดค่า `T` ลง
    *   หาก `T < T_sat` ในบริเวณ interface: `ṁ < 0` (กลั่นตัว) → `q̇_latent > 0` (คายความร้อน) → สมการพลังงานจะเพิ่มค่า `T` ขึ้น
    *   กลไกนี้ช่วย **รักษาอุณหภูมิที่ interface ให้ใกล้เคียงกับ T_sat** โดยอัตโนมัติ ซึ่งเป็นสมมติฐานพื้นฐานของหลายๆ phase change model
