...
## 1.1 Volume Fraction Transport Equation
...
## 1.1.1 Mathematical Foundation (รากฐานทางคณิตศาสตร์)
...
## 1.1.2 The Interface Compression Term
...
## 1.1.3 Discretization Imperatives and the Role of MULES
...
## 1.1.4 Variable Definitions and Physical Constants (นิยามตัวแปรและค่าคงที่ทางฟิสิกส์)
...
## 1.1.5 Critical Warnings and Best Practices
...
## 2.1.1 Core Algorithm and Code Dissection
...
## 2.1.2 What We Do DIFFERENTLY: MULES for Phase Change
...
## 2.2.1 Core Algorithm and Code Dissection
...
## 2.2.2 What We Do DIFFERENTLY: Interface in Phase Change

| Aspect | Standard OpenFOAM (`interFoam`) | Our Evaporator Solver Implementation | Rationale |
| :--- | :--- | :--- | :--- |
| **Interface Definition** | Interface นิยามโดย `alpha` isosurface (เช่น `alpha=0.5`) เพียงอย่างเดียว | **Phase change เกิดที่ Saturation temperature isotherm** สำหรับ Pure fluid, Interface ก็คือ Isotherm ที่ `T_sat` เรามี Coupled level sets สองตัวคือ `alpha` และ `T` ใน Lee model ปัจจุบันเราสมมติว่ามันทับซ้อนกัน แต่เป็นการประมาณ | ในความเป็นจริง Interface คือขอบเขตเฟสที่ `T = T_sat` `alpha` field เป็นเพียงโครงสร้างทางตัวเลขเพื่อระบุตำแหน่ง สำหรับ High-fidelity boiling อาจต้องใช้ Sharp temperature field ที่ Interface |
...
## 2.3 Class Analysis: `twoPhaseMixture` and `incompressibleTwoPhaseMixture`
...
## 2.3.1 Core Algorithm and Code Dissection
...
## 2.3.2 What We Do DIFFERENTLY: Properties for Phase Change
...
## 3.1 Architectural Overview & Core Class Relationships
...
## 3.3 Integration with the Main Solver Loop
...
## 4.1 Header File: `AlphaTransportSolver.H`
...
## 4.2 Implementation File: `AlphaTransportSolver.C`
...
## 4.3 Header File: `InterfaceSharpener.H`
...
## 4.4 Implementation File: `InterfaceSharpener.C`
...
## 5.1.1 Project Structure and CMakeLists.txt
...
## 5.1.3 Source Directory CMakeLists.txt
...
## 5.1.4 Applications Directory CMakeLists.txt
...
## 5.1.5 Test Directory CMakeLists.txt
...
## 5.2.1 TestAlphaTransport.C - Alpha Transport Solver Unit Tests
...
## 5.2.2 TestInterfaceSharpener.C - Interface Sharpening Tests
...
## 5.2.3 TestMULESIntegration.C - MULES Integration Tests
...
## 5.3.1 TestZalesakDisk.C - Interface Preservation Test
...
## 5.3.2 TestRisingBubble.C - Two-Phase Flow Test
...
## 5.5 Continuous Integration Configuration
...
## 5.6 Verification and Validation Checklist

## 5.6.1 Code Quality Checks
...
## 5.6.2 Numerical Verification Tests
...
## 5.7 Troubleshooting Build Issues
...
## 7.1.1 Foundational VOF & Interface Capturing Papers
...
## 7.1.2 MULES & Boundedness Enforcement
...
## 7.1.3 Surface Tension & Interface Modeling
...
## 7.2.2 Forward Dependencies (การเตรียมพร้อมสู่วันข้างหน้า)
...
## 7.3 Online Resources & Community
...
## 7.4 Key Source Files for Further Study
