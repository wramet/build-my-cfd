---
title: "Day 04: Temporal Discretization: Euler, Crank-Nicolson, CFL"
date: 2026-01-04
day: 4
phase: 1
...
status: Core Theory
prerequisites:
  - "[[2026-01-03|Day 03: Spatial Discretization]]"
next_day: "[[2026-01-05|Day 05: Mesh Topology Concepts]]"
summary: >
  เรียนรู้หลักการของ Temporal Discretization และผลกระทบต่อเสถียรภาพ (Stability) ของการจำลองแบบ Transient
  เจาะลึกความแตกต่างระหว่าง Euler (Explicit/Implicit) และ Crank-Nicolson ในแง่ของความแม่นยำและ Computational Cost
  พร้อมการวิเคราะห์เงื่อนไข CFL Condition ขั้นสูงสำหรับ Two-Phase Flow ที่มี Phase Change ซึ่งเทอมการขยายตัว (Expansion) มีบทบาทสำคัญ
---
...
$$
Co_{total} = \max \left( \underbrace{\frac{|\mathbf{U}| \Delta t}{\Delta x}}_{Convection},\ \underbrace{\frac{\nu \Delta t}{(\Delta x)^2}}_{Diffusion},\ \underbrace{\frac{|\dot{m} (1/\rho_v - 1/\rho_l)| \Delta t}{\Delta x}}_{Interface\ Expansion} \right)
$$
...
classDiagram
    ...
    class TimeIntegrator {
        ...
        +TimeIntegrator(const Time& runTime, const fvMesh& mesh)
        +setTimeScheme("const word& schemeName") : void
        +calculateDT(const volVectorField& U, const volScalarField& nu, const volScalarField& alpha, const volScalarField& mDot) : scalar
        +enforceMaxDT(const scalar maxDT) : void
        +deltaT() : scalar
        +scheme() : const ddtScheme&
        +writeState(Ostream& os) : void
    }

    class CFLCalculator {
        -const fvMesh& mesh_
        +CFLCalculator(const fvMesh& mesh)
        +calculateConvectionCFL(const volVectorField& U, const scalar deltaT) : scalarField
        +calculateDiffusionCFL(const volScalarField& nu, const scalar deltaT) : scalarField
        +calculateInterfaceCFL(const volScalarField& alpha, const volScalarField& mDot, const scalar rhoV, const scalar rhoL, const scalar deltaT) : scalarField
        +getMaxCFL(const scalarField& localCFL) : scalar
    }

    class ddtScheme {
        <<abstract>>
        #ddtScheme(const fvMesh& mesh)
        +fvmDdt(GeometricField~Type, fvPatchField, volMesh~&) : tmp~fvMatrix~*
        +fvcDdt(GeometricField~Type, fvPatchField, volMesh~&) : tmp~GeometricField~*
        +type() : word*
    }
    ...
    class CrankNicolsonDdtScheme {
        -scalar psi_
        +CrankNicolsonDdtScheme(const fvMesh& mesh, const scalar psi=0.5)
        +fvmDdt(GeometricField~Type, fvPatchField, volMesh~&) : tmp~fvMatrix~
        +fvcDdt(GeometricField~Type, fvPatchField, volMesh~&) : tmp~GeometricField~
        +type() : word
    }

    TimeIntegrator --> CFLCalculator : uses
    TimeIntegrator --> ddtScheme : uses
...
```
    # Set properties for the test: it should run and pass.
    set_tests_properties(TemporalIntegration_UnitTests
        PROPERTIES
            PASS_REGULAR_EXPRESSION "All tests passed"
            FAIL_REGULAR_EXPRESSION "Test failed|ERROR|ASSERT"
            TIMEOUT 30 # Seconds
    )
endif()
...
bool testCFLCalculator_Interface()
{
    ...
    (
        dimensionedScalar("nu", dimViscosity, 1e-6), // Small kinematic viscosity
        dt
    );
```
hoV, rhoL, dt);
...
1.  **Stability Constraints:** สมการควบคุมประกอบด้วยทั้ง Convective (`U·∇U`) และ Diffusive (`ν∇²U`) Terms การทำ Discretization แบบ Explicit กำหนดขีดจำกัดความเสถียรที่ต่างกัน:
    *   **Convective CFL (Explicit):** `Δt_conv ≤ C * Δx / |U|` Time step แปรผัน **ตามเส้นตรง (linearly)** กับขนาดเซลล์ที่เล็กที่สุด `Δx`
    *   **Diffusive CFL (Explicit):** สำหรับ 1D Explicit Diffusion, ความเสถียรต้องการ `Δt_diff ≤ (Δx)² / (2ν)` Time step แปรผันตาม **กำลังสอง (square)** ของขนาดเซลล์ที่เล็กที่สุด

2.  **Mesh Refinement Impact:** ใน Evaporator ของเรา เพื่อจับภาพ Liquid Films ที่บาง, Boiling Nucleation, และ Thermal Boundary Layers เราลด `Δx_min` ลงอย่างมากในบริเวณวิกฤต สมมติว่าเรา Refine Mesh ด้วย Factor 10 (`Δx_new = Δx_old / 10`):
...
**Question:** Crank-Nicolson (CN) Scheme มีความแม่นยำลำดับที่สองและเสถียรแบบ Unconditional สำหรับปัญหาเชิงเส้น แต่สำหรับการแก้สมการ VOF Transport (`∂α/∂t + ∇·(Uα) = 0`) การใช้ CN อาจนำไปสู่ความล้มเหลวร้ายแรง จงอธิบายกลไกที่ทำให้เกิด Spurious Oscillations และทำไมมันถึงยอมรับไม่ได้อย่างยิ่งสำหรับ field `α`
...
1.  **The Nature of the Oscillation:** CN เป็น Central Scheme ในเวลา สำหรับสมการ Advection แบบสเกลาร์ง่ายๆ การอัปเดตเขียนได้เป็น `α^{n+1} = α^n - 0.5*Δt*(F^n + F^{n+1})` โดยที่ `F` คือ Spatial Flux รูปแบบนี้ไม่ได้รับประกันว่า `α^{n+1}` จะมีค่าอยู่ระหว่างค่าที่ใช้ในการสร้างมัน หากมีความไม่ต่อเนื่องที่คมชัด (เช่น Liquid-Gas Interface ที่ `α` กระโดดจาก 0 ไป 1) การเฉลี่ยทางเวลา (Temporal Averaging) สามารถสร้าง **Overshoots** (`α > 1`) และ **Undershoots** (`α < 0`) ในเซลล์ที่ติดกับ Interface เหล่านี้ไม่ใช่ Error เล็กน้อย; มันอาจมีขนาดเท่ากับ Solution เอง

2.  **Why It's Catastrophic for VOF:**
    *   **Physical Meaning Violated:** `α` คือ Volume Fraction โดยนิยามแล้วต้องเป็นไปตาม `0 ≤ α ≤ 1` ค่า `α = 1.2` หรือ `α = -0.1` นั้นไม่มีความหมายทางกายภาพ มันสื่อถึง "เหลวเกินเต็ม" หรือ "ปริมาตรติดลบ"
    *   **Material Property Corruption:** ใน Two-phase Mixture, ความหนาแน่น `ρ` และความหนืด `μ` คำนวณจาก `ρ = α*ρ_l + (1-α)*ρ_v` หาก `α` อยู่นอกช่วง [0,1] คุณสมบัติเหล่านี้จะผิดเพี้ยนไป (เช่น ความหนาแน่นติดลบ) ซึ่งจะทำให้สมการ Momentum ล่มทันที
    *   **Interface Definition Destroyed:** Interface มักจะอยู่ที่ Isosurface `α = 0.5` Spurious Oscillations จะสร้าง Contour `α = 0.5` ปลอมๆ ขึ้นมามากมาย ทำลาย Interface ที่คมชัดและนิยามได้ ทำให้การ Post-processing (เช่น การคำนวณ Curvature, Heat Transfer Coefficients) เป็นไปไม่ได้

3.  **The Link to Phase Change:** ปัญหานี้รุนแรงขึ้นเมื่อมี Phase Change Source term `ṁ` ใน Lee Model นั้นแปรผันตรงกับ `α_l` หรือ `α_v` และผลต่างอุณหภูมิ ค่า `α` ที่แกว่งไปมานำไปสู่ Mass Transfer Rates ที่แกว่งอย่างบ้าคลั่ง ทำให้เกิดความไม่เสถียรทางตัวเลขและมวลไม่คงที่ (Non-conservation of Mass)
...
**Question:** "Hero Concept" Expansion Term คือ `∇·U = ṁ (1/ρ_v - 1/ρ_l)` จงแสดงให้เห็นว่า Volumetric Source Term นี้แปลเป็น *Interface Velocity* ที่มีผลต่อ CFL Condition ได้อย่างไร และจงคำนวณขนาดทั่วไปสำหรับ R410A ที่ 5°C

**Answer & Deep Analysis:**

ข้อนี้เชื่อมโยงฟิสิกส์ของการเปลี่ยนสถานะโดยตรงกับข้อจำกัดด้านความเสถียรทางตัวเลข

1.  **From Volumetric Source to Velocity:** Divergence ของความเร็วแสดงถึงอัตราการสร้างปริมาตรต่อหน่วยปริมาตร ที่ Phase Interface ของเหลวที่ความหนาแน่น `ρ_l` ระเหยกลายเป็นไอที่ความหนาแน่น `ρ_v` อัตราการแปลงมวลต่อหน่วยพื้นที่ผิวคือ `ṁ` อัตราการผลิต *เชิงปริมาตร* ฝั่งไอคือ `ṁ / ρ_v` และอัตราการทำลายเชิงปริมาตรฝั่งของเหลวคือ `-ṁ / ρ_l` ผลสุทธิเมื่อพิจารณาในเซลล์ที่มี Interface อยู่ คือการขยายตัวของปริมาตร เราสามารถโมเดลสิ่งนี้เป็น Effective Velocity *ในแนวตั้งฉากกับ Interface* ที่ขับเคลื่อนการขยายตัว

    พิจารณาสถานการณ์ 1D ง่ายๆ ที่ Interface เคลื่อนที่ด้วยความเร็ว `U_int` อัตราการเปลี่ยนแปลงปริมาตรใน Control Cell สัมพันธ์กับความเร็วนี้ การวิเคราะห์ความเสถียรต้องการให้ Interface ไม่เคลื่อนที่เกินหนึ่งความกว้างเซลล์ `Δx` ต่อ Time Step: `|U_int| * Δt ≤ Δx` ดังนั้น `CFL_int = |U_int| Δt / Δx ≤ 1`

2.  **Estimating U_int from the Expansion Term:** Expansion Term ให้เราทราบอัตราการสร้างปริมาตร สำหรับเซลล์ที่ถูกตัดผ่านโดย Interface, Net Volumetric Source `S_vol` มีค่าประมาณ `ṁ * V_cell * (1/ρ_v - 1/ρ_l)` โดยเราสมมติว่า `ṁ` ไม่เป็นศูนย์เฉพาะใน Interface Cells การสร้างปริมาตรนี้ต้อง
ได้รับการรองรับโดยการไหลออกจากเซลล์ เชื่อมโยงสิ่งนี้กับสเกลความเร็ว: `S_vol ≈ |U_int| * A_cross` โดยที่ `A_cross` คือพื้นที่หน้าตัดที่เกี่ยวข้องสำหรับการประมาณ CFL แบบคร่าวๆ เราสามารถใช้ Characteristic Velocity เป็น:
    `U_int ~ ṁ * (1/ρ_v - 1/ρ_l) * L_char` โดยที่ `L_char` คือ Length Scale ในระดับขนาดเซลล์ `Δx` วิธีที่ Conservative และนิยมใช้ Implementation คือการนิยามโดยตรงว่า:
    ```
    CFL_interface = ( |ṁ| * |1/ρ_v - 1/ρ_l| ) * Δt / (Δx/2)
    ```
    การหารด้วย 2 (`Δx/2`) ปรากฏขึ้นเนื่องจากผลกระทบของการขยายตัวจะรู้สึกได้แรงที่สุดผ่านครึ่งเซลล์ (จาก Interface ไปยัง Cell Center)

3.  **Numerical Example for R410A:**
    *   Saturation Temperature: `T_sat = 5°C`
    *   Saturated Liquid Density, `ρ_l ≈ 1200 kg/m³`
    *   Saturated Vapor Density, `ρ_v ≈ 55 kg/m³` (Density ratio `ρ_l/ρ_v ≈ 22`)
    *   `(1/ρ_v - 1/ρ_l) ≈ (1/55 - 1/1200) ≈ 0.01818 - 0.00083 ≈ 0.01735 m³/kg`
    *   Typical Evaporation Mass Flux `ṁ` ใน High-performance Evaporator อาจอยู่ที่ `~0.1 kg/(m²s)`
    *   **Effective Expansion Velocity Scale:** `U_int ≈ ṁ * (1/ρ_v - 1/ρ_l) = 0.1 * 0.01735 ≈ 0.001735 m/s`
    *   ดูเหมือนน้อย อย่างไรก็ตาม `ṁ` อาจสูงกว่านี้มากในระดับ Local โดยเฉพาะที่ Nucleation Sites เมื่อใช้ Lee Model, `ṁ = C * α * ρ * (T - T_sat)/T_sat` ด้วย Typical Coefficient `C = 100 s⁻¹`, Superheat `5 K` และ `α*ρ ≈ ρ_l` เราจะได้ `ṁ ≈ 100 * 1200 * (5/278) ≈ 21,600 kg/(m²s)`
    *   นี่คือค่าที่สูงมากเฉพาะจุด แต่มันแสดงให้เห็นว่า **Interface CFL สามารถครอบงำ Convective CFL ได้อย่างง่ายดาย** ใน Boiling Cells หาก `Δx = 0.1 mm` ดังนั้น `U_int ≈ 21,600 * 0.01735 ≈ 375 m/s` ซึ่งนำไปสู่ CFL Number มหาศาลเว้นแต่ `Δt` จะเล็กมาก

**Implementation Imperative:** นี่คือเหตุผลที่ใน `CFLCalculator::calculateInterfaceCFL()` เราต้องคำนวณสิ่งนี้แบบ Local สำหรับแต่ละเซลล์ตาม `ṁ` และ Densities ในบริเวณนั้น Global Time Step `Δt` ต้องเป็นไปตามเงื่อนไข `max(CFL_conv, CFL_diff, CFL_interface) < CoMax`
...
1.  **Core Adaptive Logic:** เมธอด `calculateDT()` ของ Time Integrator จะถูกเรียกเมื่อจบแต่ละ Time Step ที่ประสบความสำเร็จ New Time Step จะถูกคำนวณดังนี้:
    `Δt_new = min(Δt_growth * Δt_old, Δt_max, Co_desired * (Δx_min / U_scale_max))`
    โดยที่ `U_scale_max = max(|U|, U_diff_scale, U_int_scale)` คือความเร็วสูงสุดจากเกณฑ์ CFL ทั้งหมด `Δt_growth` (เช่น 1.1) ช่วยให้เพิ่ม Time Step ได้อย่างค่อยเป็นค่อยไป ในขณะที่เงื่อนไข CFL ป้องกันความไม่เสถียร

2.  **Phase-Specific Considerations:**
    *   **Subcooled Liquid (Start-up):** การไหลอาจถูกขับเคลื่อนด้วยปั๊มหรือแรงโน้มถ่วง `U_scale_max` ถูกกำหนดโดย **Convective Velocity** `Δt` ถูกตั้งค่าโดย Convective CFL ส่วน Interface CFL เป็นศูนย์ (`ṁ=0`) Time steps สามารถมีขนาดค่อนข้างใหญ่ได้
    *   **Nucleation Onset:** ฟองแรกก่อตัวขึ้น นี่คือ Critical Instability
        *   **Spike in Interface CFL:** ใน Nucleation Cell, `T >> T_sat`,
 causing a huge local `ṁ` via the Lee model. `U_int_scale` spikes by orders of magnitude.
        *   **Strategy:** อัลกอริทึม Adaptive ต้อง **ลด `Δt` ลงอย่างมาก** เพื่อจับภาพเหตุการณ์นี้ สิ่งนี้ต้องการ Safety Factor: หาก `CFL_interface` กระโดดขึ้นมากกว่าสมมติ 50% จาก Step ก่อนหน้า ให้กระตุ้นการลดแบบ Aggressive (เช่น `Δt_new = 0.2 * Δt_old`) Solver อาจต้อง Reject Step นี้และ Retry ด้วย `Δt` ที่เล็กกว่า
    *   **Established Boiling:** ฟองอากาศเติบโต หลุดออก และรวมตัวกัน การไหลมีความโกลาหล (Chaotic) แต่คงที่ในเชิงสถิติ (Statistically Steady)
        *   **Multiple Limiting Factors:** ตอนนี้ `U_scale_max` มีสัดส่วนจาก Bulk Convection, Turbulent Eddies (high local `U`) และการขยายตัวของฟองอากาศหลายฟอง (`U_int`) Interface CFL ไม่ได้เป็นยอดแหลม (Spiky) อีกต่อไป แต่ยังคงเป็นตัวแปรสำคัญ
        *   **Strategy:** อัลกอริทึม Adaptive จะเข้าสู่ Dynamic Equilibrium `Δt` จะแกว่งตัวแต่อยู่ในช่วงที่เสถียร การตรวจสอบ *Maximum* CFL ในโดเมน (ไม่ใช่ค่าเฉลี่ย) เป็นสิ่งสำคัญ เพราะเซลล์เดียวที่จุดเดือด (Boiling Site) สามารถควบคุมความเสถียรได้

3.  **Implementation Safeguards:**
    *   **Absolute Bounds:** บังคับใช้ `Δt_min` (เพื่อความแม่นยำ) และ `Δt_max` (เกี่ยวข้องกับ Temporal Resolution ของปรากฏการณ์)
    *   **Backtracking:** หาก PISO Loop ลู่เข้าไม่สำเร็จ หรือมีการละเมิดขอบเขต `α` Time Step จะถูก Reject, `Δt` จะถูกลดลงครึ่งหนึ่ง และทำการ Retry จาก Time Level ก่อนหน้า
    *   **Smooth Growth:** จำกัดการเพิ่มขึ้นต่อ Step (`Δt_growth ≤ 1.2`) เพื่อป้องกันการกระโดดกะทันหันที่ทำให้พลาด Transient Features

กลยุทธ์ Adaptive นี้รับประกันประสิทธิภาพในช่วง "สงบ" และความทนทาน (Robustness) ในช่วงเหตุการณ์ Phase-change ที่ "รุนแรง"

---

### **Concept Check 5: Implicit vs. Explicit Treatment of the Phase Change Source Term `ṁ`**

**Question:** ในสมการพลังงาน Phase Change Source Term `ṁ * h_lv` (Latent Heat) มีความสำคัญอย่างยิ่ง Term นี้ควรถูกจัดการแบบ Explicit (ใช้ `T^n`) หรือ Implicit (ทำให้เป็นเชิงเส้นเทียบกับ `T^{n+1}`) ภายใน Time Discretization แบบ Implicit เช่น Euler? จงวิเคราะห์ผลกระทบต่อความเสถียรและการลู่เข้า

**Answer & Deep Analysis:**

นี่คือการตัดสินใจที่มีผลกระทบใหญ่หลวงต่อความทนทานของ Solver โดยเฉพาะที่อัตราการระเหยสูง

1.  **Explicit Treatment (`ṁ(T^n) * h_lv`):**
    *   **Method:** Latent Heat Source ถูกคำนวณโดยใช้อุณหภูมิจาก Time Step ก่อนหน้า และบวกโดยตรงเข้าไปใน Source Vector `b` ของระบบสมการเชิงเส้น `A·T^{n+1} = b`
    *   **Pros:** ง่าย Matrix `A` ไม่เปลี่ยนแปลง (มักจะเป็นแค่ Diagonal จาก ddt และ Diffusion) ทำให้แก้ได้ง่ายและถูกกว่า
    *   **Cons:** นำมาซึ่ง **Stability Constraint** Source Term นี้ทำหน้าที่เหมือน Reaction Term สำหรับการวิเคราะห์แบบ Linearized การจัดการแบบ Explicit กับ Stiff Source Term (`ṁ ∝ C*(T-T_sat)`) กำหนดขีดจำกัด Time Step คล้ายกับ `Δt < 2 / C` โดยที่ `C` คือ Lee Model Coefficient (มักจะ ~100 s⁻¹) นี้ให้ค่า `Δt < 0.02 s` ซึ่งอาจเข้มงวดกว่า Convective CFL การละเมิดเงื่อนไขนี้จะนำไปสู่การแกว่งของอุณหภูมิที่ Interface และการลู่ออก (Divergence)

2.  **Implicit (Linearized) Treatment:**
    *   **Method:** Source Term ถูกทำให้เป็นเชิงเส้น (Linearized) ในรูป `S(T) ≈ S(T*) + (∂S/∂T)|_T* * (T - T*)` สำหรับ Lee Model Evaporation Term `ṁ = C * α_l * ρ_l * (T - T_sat)/T_sat` เราจะได้:
        `S(T) = ṁ * h_lv = K * (T - T_sat)` โดยที่ `K = (C * α_l * ρ_l * h_lv) / T_sat`
        การทำ Linearization รอบ `T* = T^n`:
        `S(T^{n+1}) ≈ K*(T^n - T`
_sat) + K*(T^{n+1} - T^n) = K*(T^{n+1} - T_sat)`.
    *   **Implementation:** สิ่งนี้ถูกเพิ่มเข้าไปใน `fvMatrix` สำหรับ Temperature ส่วน `K*T^{n+1}` ถูกเพิ่มลงใน Diagonal ของ Matrix `A` และส่วน `-K*T_sat` ถูกเพิ่มลงใน Source Vector `b`
    ...
    *   **Cons:** ทำให้ Matrix ซับซ้อนขึ้นเล็กน้อย (Diagonal เปลี่ยนแปลงทุก Time Step เมื่อ `α_l` และ `ρ_l` เปลี่ยน) การ Linearization นี้มีความแม่นยำเพียง First-order แต่ Error นี้สอดคล้องกับ First-order Time Scheme
...
10. **`src/finiteVolume/finiteVolume/ddtSchemes/EulerDdtScheme/EulerDdtScheme.H` & `.C`**
    ...
    *   **Key Implementation Detail:** ตรวจสอบฟังก์ชัน `fvm::ddt` มันเพิ่ม Coefficient `V/Δt` ลงใน Matrix Diagonal และ `-V/Δt * ψ_old` ลงใน Source Term นี่เป็นตัวอย่าง Canonical ของ Implicit Discretization: `a_P * ψ_new = sum(a_N * ψ_N_new) + S_u` โดยที่ `S_u` มีค่า Old Time อยู่
...
*   **Day 01: Governing Equations**
    *   **Connection:** Day 01 ได้ทำการ Derive สมการ Transport แบบ Unsteady (Mass, Momentum, Energy, Phase Fraction) ซึ่งมี Time Derivatives `∂/∂t` ที่ถูก Discretize ใน Day 04 **Expansion Term** `∇·U = mDot(1/ρ_v - 1/ρ_l)` มีอิทธิพลโดยตรงต่อ `interface CFL` number ซึ่งเป็นจุดเน้นหลักของ Adaptive Time-stepping ใน Day 04
    *   **Dependency:** คุณไม่สามารถเลือก Time-integration Scheme โดยไม่ทราบ Governing PDEs ก่อน

*   **Day 02: FVM Basics**
    *   **Connection:** Day 02 สร้าง Spatial Framework—Control Volume `V_P` และ Face Flux `S_f` Time Derivative Term `∂/∂t ∫_V ρφ dV` ถูกอินทิเกรตเหนือ Control Volume นี้ ปริมาตร `V_P` ปรากฏโดยตรงใน Implicit Euler Matrix Coefficient (`V_P/Δt`) Day 04 สร้างอีกครึ่งหนึ่งของ Space-Time Discretization Puzzle (ฝั่งเวลา)
    *   **Dependency:** การเข้าใจ Spatial Summation (`∑_f`) จำเป็นก่อนที่จะเพิ่ม Temporal Increment (`Δt`)

*   **Day 03: Spatial Discretization**
    *   **Connection:** นี่คือคู่หูที่ตรงที่สุดของ Day 04 Day 03 จัดการกับการ Interpolate `φ` ไปยัง Faces (`φ_f`) สำหรับ Convection (`div`) และ Diffusion (`laplacian`) Terms Day 04 จัดการกับการ Interpolate `φ` ในเวลา (`φ^n`, `φ^{n+1}`) **TVD Limiters** (VanLeer, SuperBee) จาก Day 3 มี Temporal Analogs (เช่น Flux Limiters สำหรับ Crank-Nicolson) และทั้งคู่เกี่ยวข้องกับ Boundedness และ Monotonicity
