...
## OpenFOAM Reference (การอ้างอิง OpenFOAM)
...
) const
{
    ...
    forAll(owner, faceI)
    {
        ...
        if (nonOrthogonalCorrection_ && schemeName == "corrected")
        {
            ...
            if (theta > (maxNonOrthogonality_ * constant::mathematical::pi/180.0))
            {
                corr = 1.0 / (cosTheta + SMALL);
            }
        }
        
        // Calculate diffusion coefficient
        scalar coeff = gammaFace[faceI] * magSf[faceI] / (deltaMag + SMALL) * corr;
...
) const
{
    ...
    forAll(mesh_.boundary(), patchI)
    {
        ...
        forAll(patch, i)
        {
            ...
            // Boundary flux (positive = leaving domain)
            scalar flux = phif[faceI];
            
            // Owner cell loses mass/quantity
            divPhi[own] += flux;
        }
    }
...
## Build & Test (การบิลด์และการทดสอบ)
...
TEST_F(MatrixAssemblyTest, DiagonalDominance) {
    ...
    const Foam::scalarField& diag = matrix.diag();
    const Foam::scalarField& upper = matrix.upper();
    const Foam::scalarField& lower = matrix.lower(); // If accessible or implicitly handled
    
    // Check diagonal dominance: |diag| >= sum(|off-diag|)
    // Note: For pure diffusion, row sum is zero, so diag = -sum(off-diag)
    // Thus |diag| == sum(|off-diag|) if signs are consistent
    
    forAll(diag, cellI) {
        // Calculate sum of off-diagonals for this row (requires complex addressing traversal in test)
        // Simplified check: Diag should be positive (and negative of sum of off-diagonals)
        EXPECT_GE(diag[cellI], 0.0) << "Diagonal element must be positive";
    }
}
```

## Concept Checks (การทดสอบแนวคิด)
...
for (label faceI = 0; faceI < mesh.nInternalFaces(); ++faceI)
{
    ...
    // 1. Calculate geometric factors
    scalar magS = magSf[faceI];
    scalar d = delta[faceI];           // distance between cell centers
    scalar gamma_f = gamma[faceI];     // interpolated diffusion coeff

    // 2. Calculate matrix coefficient (simplified)
    scalar coeff = gamma_f * magS / d;

    // 3. Add to matrix (ensuring symmetry)
    // Owner contribution
    diag[own] += coeff;
    upper[faceI] -= coeff; // Off-diagonal connecting own -> nei

    // Neighbour contribution
    diag[nei] += coeff;
    lower[faceI] -= coeff; // Off-diagonal connecting nei -> own
}
```

## References & Related Days (เอกสารอ้างอิงและบทเรียนที่เกี่ยวข้อง)
