...
### คำแนะนำสำหรับ Modern C++: Composition over Inheritance

#### Composition คืออะไร?

**Composition** เป็นหลักการออกแบบ OOP ที่เน้น **"มี" (has-a)** แทนที่จะ **"เป็น" (is-a)**

| Inheritance (is-a) | Composition (has-a) |
| :--- | :--- |
| "รถยนต์ **เป็น** ยานพาหนะ" | "รถยนต์ **มี** เครื่องยนต์" |
| `class Car : public Vehicle` | `class Car { Engine engine_; }` |
| ผูกติดกับ parent class | ยืดหยุ่น เปลี่ยน/ทดสอบง่าย |

ในบริบท CFD:
- **Inheritance:** `volScalarFieldEnhanced` สืบทอดจาก `GeometricField` แล้วเพิ่ม `addExpansionSource()` เข้าไป — Field และ Expansion logic ผูกติดกัน
- **Composition:** สร้าง `ExpansionSourceCalculator` แยกต่างหาก แล้วให้มัน "ใช้" `ScalarField` เป็น input — แยกหน้าที่ชัดเจน

#### ตัวอย่างโค้ด: Composition Approach

สำหรับ **Engine ใหม่** ของคุณ ให้เน้น Composition ในจุดที่เป็นไปได้:
...
#### Move Semantics คืออะไร?

**Move Semantics** เป็นฟีเจอร์ใน C++11 ที่ให้คุณ **"ย้าย"** ทรัพยากร (เช่น memory) จาก object หนึ่งไปยังอีก object หนึ่ง แทนที่จะ **"คัดลอก"**

| Copy | Move |
| :--- | :--- |
| สร้างสำเนาใหม่ทั้งหมด | ขโมยทรัพยากรจากต้นทาง |
| ช้า (ต้อง allocate + copy ข้อมูล) | เร็ว (แค่ย้าย pointer) |
| ต้นทางยังใช้งานได้ปกติ | ต้นทางกลายเป็น "ว่างเปล่า" |

**เปรียบเทียบง่ายๆ:**
- **Copy** = ถ่ายเอกสาร (เอกสารเดิมยังอยู่, ได้สำเนาใหม่)
- **Move** = ย้ายบ้าน (ของในบ้านเดิมถูกขนไปบ้านใหม่, บ้านเดิมว่างเปล่า)
...
#### ทำไมถึงเกิดขึ้น: Two-Phase Lookup

**Two-Phase Lookup** คือกระบวนการที่ C++ compiler ใช้ในการ parse template code แบ่งเป็น 2 ขั้นตอน:

| Phase | เมื่อไหร่ | ทำอะไร |
| :--- | :--- | :--- |
| **Phase 1** | ตอนเห็น template definition | มองหาชื่อที่ **ไม่ขึ้นกับ T** (non-dependent) |
| **Phase 2** | ตอน instantiate template | มองหาชื่อที่ **ขึ้นกับ T** (dependent) |

**เปรียบเทียบง่ายๆ:**
- Phase 1 = อ่านสูตรอาหาร (ยังไม่รู้ว่าจะทำอาหารอะไร)
- Phase 2 = ทำอาหารจริง (รู้แล้วว่าทำ "ผัดไทย" ต้องหาวัตถุดิบของผัดไทย)

ปัญหาคือ: `value_` สืบทอดมาจาก `Base<T>` ซึ่ง **ขึ้นกับ T** ดังนั้น Phase 1 ยังไม่รู้ว่ามันอยู่ตรงไหน!
...
**RTTI (Run-Time Type Information)** คือความสามารถในการ **รู้ว่า object เป็น type อะไร** ขณะโปรแกรมกำลังทำงาน

| Compile-time Type | Run-time Type |
| :--- | :--- |
| รู้ตอนเขียนโค้ด | รู้ตอนโปรแกรมรัน |
| `int x;` → รู้ว่าเป็น int | `Base* ptr;` → ptr อาจชี้ไป Derived |
| ง่าย, เร็ว | ยืดหยุ่น, ช้ากว่า |

**เปรียบเทียบง่ายๆ:**
- Compile-time = รู้ว่า "คนนี้เป็นพนักงาน" ตอนจ้างงาน
- Run-time = รู้ว่า "พนักงานคนนี้เป็นวิศวกร" ตอนถามตอนทำงาน

#### ทำไม CFD ต้องการ RTTI?
