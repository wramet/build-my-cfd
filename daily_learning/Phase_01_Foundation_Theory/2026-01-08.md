...
## 1.1 Conjugate Gradient Method for Symmetric Positive Definite Matrices
...
## 1.2 Bi-Conjugate Gradient Stabilized Method for Asymmetric Matrices
...
## 1.3 Preconditioners: Diagonal, DIC, DILU
...
## 2.1 Deep Dive: The `lduMatrix::solver` Base Class
...
## 2.3 Deep Dive: The `PBiCGStab` Solver
...
## 2.4 Putting It All Together: The `fvSolution` Dictionary and Runtime Selection
...
## 3.1 Architectural Overview: The Solver Hierarchy
...
4.  **Norm Factor Calculation**: เมธอด `normFactor()` สำคัญมากสำหรับเกณฑ์การหยุดที่มีความหมาย การใช้ Absolute Residual `||r_k|| < tolerance` ไม่แข็งแกร่ง (Not Robust), เนื่องจากมันขึ้นอยู่กับการสเกลของ Source Term `b` `normFactor` ให้การสเกลที่เฉพาะเจาะจงกับปัญหา, บ่อยครั้งคือ `||b||` หรือ `||A|| * ||psi|| + ||b||`, ทำให้ `tolerance_` เป็นมาตรวัดแบบ Relative
...
## 3.3 Driver Class: `IterativeSolverDriver`
...
## 4.1 Header File: `IterativeSolverDriver.H`
...
## 4.2 Implementation File: `IterativeSolverDriver.C`
...
## 4.3 Compilation and Integration Notes
...
ข้อบกพร่องพื้นฐานในการใช้ CG กับสมการโมเมนตัมอยู่ที่ **เงื่อนไขทางคณิตศาสตร์** ของมัน อัลกอริทึม CG มาตรฐาน (ดังที่นำเสนอใน Section 1) พิสูจน์ได้ว่าจะลู่เข้าสู่คำตอบที่แม่นยำภายใน *n* รอบ (สำหรับ Matrix ขนาด *n*x*n*) ก็ต่อเมื่อ Matrix **A** เป็น **Symmetric Positive Definite (SPD)** เท่านั้น
...
*   **Loss of Theoretical Guarantees:** คุณสมบัติ Conjugacy ของทิศทางการค้นหา `p_k` ไม่สามารถรักษาไว้ได้อีกต่อไป อัลกอริทึมสูญเสียคุณสมบัติการยุติภายในจำนวนรอบที่จำกัด (finite-termination property)
...
*   **Breakdown:** การคำนวณ `α_k = (r_k^T r_k) / (p_k^T A p_k)` อาจล้มเหลวหากตัวส่วน `p_k^T A p_k` เป็นศูนย์หรือติดลบ ทำให้เกิด floating-point exception
...
**Question:** อัลกอริทึม BiCGStab มีการแนะนำ "Shadow Residual" `r̃_0` (เริ่มต้นเท่ากับ `r_0`) วัตถุประสงค์ทางคณิตศาสตร์ของเวกเตอร์ช่วยนี้คืออะไร และมันช่วยแก้ปัญหาสำคัญอะไรในวิธี BiCG ดั้งเดิม?

**Answer & Deep Explanation:**

Shadow Residual `r̃_0` เป็นสิ่งประดิษฐ์สำคัญที่ช่วยให้อัลกอริทึม BiCGStab สามารถทำงานบน **Matrix ที่ไม่สมมาตร** ได้โดยไม่ต้องใช้ Transpose ของ Matrix (`A^T`)
...
2.  **การรักษาเสถียรภาพของปัญหา Breakdown ใน BiCG:**
    อัลกอริทึม BiCG ดั้งเดิมมีแนวโน้มที่จะเกิด **Breakdown รุนแรง** เมื่อ `ρ_k = r̃_k^T r_k = 0` ทำให้การคำนวณสัมประสิทธิ์ `α_k` และ `β_k` ไม่นิยาม (หารด้วยศูนย์) นี่คือความไม่เสถียรทางตัวเลข ไม่ใช่แค่การลู่เข้าช้า
    **นวัตกรรมของ BiCGStab** คือการแทนที่ลำดับที่สอง (`r̃_k`) ด้วย **ขั้นตอนปราบเรียบ (Stabilization step) ภายใน** โดยจะเก็บไว้เพียง Shadow vector เริ่มต้น `r̃_0` เท่านั้น ค่า `ρ_k` จะถูกคำนวณเป็น `r̃_0^T r_k` จากนั้นอัลกอริทึมจะทำขั้นตอน *Minimum Residual* (เวกเตอร์ `s` และ `t`, การคำนวณ `ω_k`) ภายใน Subspace 2D ปัจจุบัน ขั้นตอน Local นี้จะช่วยลดความผันผวนของการลู่เข้า ซึ่งมักจะป้องกันการแกว่งที่รุนแรงของ BiCG และทำให้อัลกอริทึมมีความยืดหยุ่นต่อเงื่อนไข Exact breakdown มากขึ้น แม้ว่า "Near-breakdowns" ยังคงทำให้การลู่เข้าไม่สม่ำเสมอได้

**Implementation Insight:** ในเมธอด `PBiCGStab::solve` ของเรา บรรทัด `rho = rDotr0` (Dot product ของ Residual ปัจจุบันกับ Shadow residual ที่เก็บไว้) คือจุดเชื่อมโยงสำคัญสู่กระบวนการ Biorthogonalization นี้ หากการลู่เข้าหยุดชะงัก กลไก *Restart* (รีเซ็ต `r̃_0 = r_k`) สามารถช่วยกู้คืนความคืบหน้าได้โดยการเริ่มความสัมพันธ์นี้ใหม่
...
**Question:** เกณฑ์การลู่เข้าแบบง่าย `||r_k|| / ||b|| < tolerance` อาจทำให้เข้าใจผิดได้ จงอธิบายสถานการณ์สองอย่างที่เกณฑ์นี้อาจผ่าน แต่ผลลัพธ์ของ Solver ไม่เป็นที่ยอมรับทางตัวเลข และ `IterativeSolverDriver::monitorConvergence` เกรด Production ควรมีการตรวจสอบอะไรเพิ่มเติมบ้าง?
...
1.  **กับดัก "Small Right-Hand Side":**
    *   **สถานการณ์:** พิจารณากรณีที่เทอม Source `b` มีค่าเล็กมาก อาจเป็นเพราะ Boundary conditions หรือสนามการไหลที่เกือบลู่เข้าแล้วใน PISO loop เช่น `||b|| ≈ 1e-10` ค่า Tolerance `1e-6` จะผ่านเงื่อนไขหาก `||r_k|| < 1e-16` ซึ่งเป็นไปได้ยากใน Double precision หรือในทางกลับกัน ถ้า Solver หยุดทำงานหลังจาก 1 รอบด้วย Initial guess `x=0` เพราะ `||r_0|| = ||b||` และ Relative residual เป็น 1
    *   **สถานการณ์จริง:** อันตรายที่แท้จริงคือ **Absolute residual** ยังคงมีขนาดใหญ่เมื่อเทียบกับสเกลของผลเฉลย หากผลเฉลยจริง `x*` มีค่า `O(1)` แต่ Solver หยุดที่ `||r_k||=1e-9` โดยที่ `b=1e-10` Relative residual คือ 10 ซึ่งสูงมาก
...
**Question:** หัวใจหลัก (Core kernel) ของ Iterative Solver คือการคูณ Matrix-Vector `w = A * p` เมื่อกำหนดให้ `lduMatrix` เก็บ Array สัมประสิทธิ์ `diag()`, `lower()`, และ `upper()` และ `lduAddressing` ให้รายการ `owner()` และ `neighbour()` จงเขียน Pseudo-code สำหรับการทำงาน `Amul` ที่เคารพการจัดเก็บแบบอสมมาตรนี้ และทำไมการดำเนินการนี้ถึงถูกจำกัดโดย Memory Bandwidth (Memory-bandwidth bound) และประสิทธิภาพของมันส่งผลต่อการเลือก Solver อย่างไร?
...
1.  **Krylov Method Choice:** วิธีอย่าง BiCGStab ต้องการการเรียก `Amul` **สองครั้ง** ต่อรอบ (สำหรับ `v_k = A p_k` และ `t = A s`) GMRES ต้องการ `Amul` หนึ่งครั้งต่อรอบแต่เก็บหลายเวกเตอร์ ซึ่งเพิ่ม Memory pressure การเลือกจึงต้องแลกกันระหว่าง Memory bandwidth กับความเร็วในการลู่เข้า
