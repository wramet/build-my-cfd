...
1.  **เข้าใจ (Understand)** ฟิสิกส์พื้นฐานและที่มาของ **Expansion Term** ในสมการความต่อเนื่องสำหรับการไหลแบบสองสถานะ (two-phase flow) ที่มีการเปลี่ยนสถานะ (phase change) โดยเฉพาะอย่างยิ่งสามารถอธิบายความสัมพันธ์ทางคณิตศาสตร์ $\nabla \cdot \mathbf{U} = \dot{m}(1/\rho_v - 1/\rho_l)$ และเหตุผลทางกายภาพว่าทำไมเทอมนี้จึงเป็น **Critical** ต่อความเสถียรของ Solver (หากไม่มีเทอมนี้ Solver จะ diverge อย่างแน่นอน)

2.  **อธิบาย (Explain)** ความหมายทางกายภาพของเทอมต่างๆ ในสมการอนุรักษ์มวล (Continuity), โมเมนตัม (Navier-Stokes) และพลังงาน (Energy) สำหรับของไหลนิวตัน (Newtonian Fluid) และระบุได้ว่าแต่ละเทอมถูกนำไปเขียนในโปรแกรม OpenFOAM อย่างไร

3.  **วิเคราะห์ (Analyze)** สถาปัตยกรรมของคลาส `volField` (เช่น `volScalarField`, `volVectorField`) และ `fvMatrix` ใน OpenFOAM โดยเข้าใจเหตุผลเบื้องหลังการออกแบบ (Design Rationale) ที่ต้องแยกการเก็บข้อมูลสนาม (Fields) ออกจากการประกอบเมทริกซ์ (Matrix Assembly)

4.  **ออกแบบ (Design)** โครงสร้างพื้นฐานของ CFD Engine สำหรับ Evaporator โดยการกำหนดความสัมพันธ์ระหว่าง Class หลักและตัวแปรควบคุมที่จำเป็นสำหรับการคำนวณการเปลี่ยนสถานะ

5.  **สร้าง (Create)** ระบบ Build โปรเจคด้วย CMake ที่รองรับการคอมไพล์โค้ด C++ ของ OpenFOAM และสามารถจัดการ Dependency พื้นฐานได้อย่างเป็นระบบ

6.  **ระบุ (Identify)** จุดผิดพลาดที่พบบ่อย (Common Pitfalls) และข้อควรระวัง (Warnings) ในการนำสมการการอนุรักษ์ไปสู่การเขียนโปรแกรมเชิงตัวเลข โดยเฉพาะประเด็นเรื่อง **Numerical Stability** ที่เกี่ยวข้องกับ Non-linearity, Source Term Stiffness, และการรักษา **Boundedness** ของตัวแปรสนาม (เช่น เศษส่วนปริมาตร $\alpha$ ต้องอยู่ระหว่าง 0 ถึง 1 เสมอ)
...
ในวิธี Volume of Fluid (VOF) เราไม่ได้ติดตามแต่ละเฟสแยกกัน แต่ติดตาม **เศษส่วนปริมาตร (Volume Fraction)** $\alpha$ ซึ่งกำหนดให้:
*   $\alpha = 1$: เซลล์นั้นเป็นของเหลวทั้งหมด (liquid)
*   $\alpha = 0$: เซลล์นั้นเป็นไอทั้งหมด (vapor)
*   $0 < \alpha < 1$: เซลล์นั้นมีอินเทอร์เฟซ (interface) อยู่
...
1.  **ความเร็วมีไดเวอร์เจนซ์ที่ไม่เป็นศูนย์**: ในบริเวณที่เกิดการเปลี่ยนสถานะ ($\dot{m} \neq 0$) การไหลจะไม่เป็นไปตาม$\nabla \cdot \mathbf{U} = 0$อีกต่อไป
2.  **เครื่องหมายของไดเวอร์เจนซ์**: เนื่องจาก$\rho_l > \rho_v$เสมอ ดังนั้น$\left( \frac{1}{\rho_v} - \frac{1}{\rho_l} \right) > 0$
...
| Aspect                   | Standard OpenFOAM `volScalarField`        | Our Extended Implementation               | Rationale                                                | Detailed Reference                                                                            |
| :----------------------- | :---------------------------------------- | :---------------------------------------- | :------------------------------------------------------- | :-------------------------------------------------------------------------------------------- |
| **Source Term Handling** | Generic field container only              | มี method `addExpansionSource()` โดยเฉพาะ | ต้องการ interface ที่ชัดเจนสำหรับ expansion term         | [[supplementary/03_volFields_DeepDive.md#Phase-Specific Implementation (การนำไปใช้เฉพาะเฟส)\|volFields Deep Dive]] |
| **Phase Properties** | ไม่มีการเก็บข้อมูล phase-specific | มี reference ถึง `rho_l`, `rho_v` | จำเป็นสำหรับคำนวณ $\dot{m}(1/\rho_v - 1/\rho_l)$ | [[supplementary/03_volFields_DeepDive.md#Phase Properties Management (การจัดการคุณสมบัติของเฟส)\|volFields Deep Dive]] |
| **Interface Tracking** | ไม่มี built-in support | Link ไปยัง `alpha` field (VOF) | ต้องรู้ตำแหน่ง interface สำหรับการคำนวณ mass transfer | [[supplementary/03_volFields_DeepDive.md#Interface Tracking (การติดตามอินเทอร์เฟซ)\|volFields Deep Dive]] |
| **Linearization** | ไม่มี mechanism สำหรับ linearized sources | Implement `LinearizedSourceTerm` class | Phase change source terms ต้อง linearize เพื่อ stability | [[supplementary/04_fvMatrix_DeepDive.md#Source Types and Linearization (ประเภทของแหล่งกำเนิดและการทำไลเนียไรเซชัน)\|fvMatrix Deep Dive]] |
...
| Aspect | Standard `fvMatrix::addSource()` | Our Enhanced Implementation | Rationale | Detailed Reference |
| :--- | :--- | :--- | :--- | :--- |
| **Source Types** | เพิ่มค่าเข้า source vector เท่านั้น | Support ทั้ง explicit และ implicit sources | Phase change sources ต้อง linearize เป็น implicit เพื่อ stability | [[supplementary/04_fvMatrix_DeepDive.md#source-types-and-linearization-ประเภทของแหล่งกำเนิดและการทำไลเนียไรเซชัน|fvMatrix Deep Dive]] |
| **Linearization** | ไม่มี built-in linearization | มี `LinearizedSource` class ที่แยกส่วน | สามารถ handle terms like $C \cdot \alpha \cdot (T - T_{sat})$ | [[supplementary/04_fvMatrix_DeepDive.md#source-types-and-linearization-ประเภทของแหล่งกำเนิดและการทำไลเนียไรเซชัน|fvMatrix Deep Dive]] |
| **Jacobian Contribution** | ไม่คำนวณ Jacobian | เพิ่ม diagonal contribution อัตโนมัติ | จำเป็นสำหรับ strong coupling ระหว่าง fields | [[supplementary/04_fvMatrix_DeepDive.md#jacobian-contribution-ส่วนร่วมของจาโคเบียน|fvMatrix Deep Dive]] |
| **Phase-Specific Logic** | Generic สำหรับทุก field | มี logic เฉพาะสำหรับ vapor/liquid phases | การคำนวณ expansion term ต้องการความรู้ของ phase | [[supplementary/03_volFields_DeepDive.md#phase-specific-implementation-การนำไปใช้เฉพาะเฟส|volFields Deep Dive]] |
...
public:
    ...
    // สมการ: $S_p = \dot{m} \cdot (1/\rho_v - 1/\rho_l)$
    // วิธี: Linearize source term เป็นรูป $S_p \phi + S_u$ เพื่อความเสถียร
    void addExpansionSource(
        ...
        // 1. คำนวณค่าสัมประสิทธิ์ $S_p$ (Implicit part)
        ...
        // 2. คำนวณค่าสัมประสิทธิ์ $S_u$ (Explicit part)
    ...
    tmp<volVectorField> grad() const override; // คำนวณ gradient $\nabla p$
};
...
public:
    ...
    // ฟังก์ชัน utility
    tmp<volScalarField> div() const override; // คำนวณ divergence $\nabla \cdot \mathbf{U}$
    tmp<volTensorField> grad() const override; // คำนวณ gradient $\nabla \mathbf{U}$ (ได้ tensor)
};
```

**คำอธิบาย `addExpansionSource()`:**
นี่คือ **Hero Implementation** ของวันนี้ตามที่ระบุใน Bible ฟังก์ชันนี้รับพารามิเตอร์หลักคือ `fvMatrix<scalar>& pEqn` ซึ่งเป็นเมทริกซ์ของสมการ pressure Poisson ที่กำลังถูกประกอบ ($A \mathbf{x} = \mathbf{b}$) ฟังก์ชันจะเพิ่มเทอม $\dot{m}(1/\rho_v - 1/\rho_l)$ ลงใน source vector $\mathbf{b}$ ของเมทริกซ์นี้ **การทำเช่นนี้สำคัญมาก** เพราะถ้าเราแก้สมการ pressure Poisson แบบมาตรฐาน ($\nabla \cdot (\nabla p) = \nabla \cdot (\mathbf{H}/A)$) โดยไม่มีเทอมนี้ ความเร็วที่คำนวณได้จาก $\mathbf{U} = \mathbf{H}/A - (1/A)\nabla p$ จะไม่สอดคล้องกับสมการ continuity ที่มี phase change ทำให้เกิด mass imbalance และ solver จะ diverge ในทันที
### 3.2.4 คลาส `surfaceScalarField`

ฟิลด์ประเภทนี้เก็บข้อมูลที่หน้า (face) ของเซลล์ ใช้เป็นหลักสำหรับ **ฟลักซ์ (flux)** เช่น mass flux $\phi = \mathbf{U}_f \cdot \mathbf{S}_f$
```cpp
...
${EXAMPLES_DIR}/day01_evaporator_solver.C
)
...
1.  **การขยายตัวของปริมาตรมหาศาล (Massive Volume Expansion):**
    *   เมื่อของเหลว (liquid) เปลี่ยนเป็นไอ (vapor) ที่อุณหภูมิอิ่มตัวเดียวกัน **ความหนาแน่นจะลดลงอย่างรุนแรง** ($\rho_l \gg \rho_v$)
    *   สำหรับ R410A ที่ 5°C: $\rho_l \approx 1200 \, \text{kg/m}^3, \rho_v \approx 55 \, \text{kg/m}^3 \rightarrow$ **อัตราส่วนความหนาแน่น (Density Ratio) $\approx 22:1$**
...
2.  **โครงสร้างข้อมูลหลัก 4 ส่วนใน fvMatrix (LDU Storage):**
    *   **`diag()`**: เป็น `scalarField` (หรือ `Field<Type>`) ขนาดเท่ากับจำนวนเซลล์ (nCells) เก็บค่าสัมประสิทธิ์บน **เส้นทแยงมุมหลัก ($a_P$)** ของเมทริกซ์ ซึ่งมาจากการ integrate เทอมต่างๆ (เช่น เทอม temporal, diffusion) เหนือ volume ของ cell นั้นๆ
    *   **`upper()`**: เป็น `scalarField` ขนาดเท่ากับจำนวน internal faces (nInternalFaces) เก็บค่าสัมประสิทธิ์ที่เชื่อมโยง **จาก Owner ไปยัง Neighbour ($a_N$)** มักจะมาจากการ discretize เทอม diffusion (laplacian) หรือ convection แบบ implicit
    *   **`lower()`**: เป็น `scalarField` ขนาดเท่ากับจำนวน internal faces (nInternalFaces) เก็บค่าสัมประสิทธิ์ที่เชื่อมโยง **จาก Neighbour ไปยัง Owner ($a_N$ สำหรับ owner)** ในเมทริกซ์สมมาตร (symmetric) `lower()` จะเท่ากับ `upper()` เสมอ
...
## 6.3 คำถาม: Latent Heat Source ($\dot{q}_{\text{latent}}$) ในสมการพลังงานเชื่อมโยงกับ Mass Transfer Rate ($\dot{m}$) อย่างไร?

**คำตอบโดยละเอียด:**

การเชื่อมโยงระหว่าง $\dot{q}_{\text{latent}}$ และ $\dot{m}$ เป็นหัวใจของ **Energy Conservation** ในปัญหาการเปลี่ยนสถานะ หากจัดการผิดพลาดจะทำให้สมดุลพลังงานรวมของระบบผิดเพี้ยน และอาจนำไปสู่ผลลัพธ์ทางฟิสิกส์ที่ไม่ถูกต้อง เช่น อุณหภูมิที่อินเทอร์เฟซ (interface) ไม่คงที่
...
2.  **การกำหนด Latent Heat Source Term:**
    *   ให้ $\dot{m}$ เป็น **อัตราการเปลี่ยนมวลต่อหน่วยปริมาตร** (kg/m³·s) โดยมี **เครื่องหมายบวกสำหรับการระเหย** (ของเหลว $\rightarrow$ ไอ) และ **เครื่องหมายลบสำหรับการกลั่นตัว** (ไอ $\rightarrow$ ของเหลว)
    *   **อัตราการดูดกลืน/คายความร้อนแฝงต่อหน่วยปริมาตร** ($\dot{q}_{\text{latent}}$) จะเป็น:
$$ \dot{q}_{\text{latent}} = - \dot{m} \cdot h_{fg} $$
    *   **ที่มาของเครื่องหมายลบ**: พิจารณาจากสมการพลังงานในรูป $\partial(\rho e)/\partial t + \dots = \dots + \dot{q}$
        ...
        *   **ในทางปฏิบัติ**: สำหรับโมเดลเช่น Lee Model ที่สมมติ interface อยู่ที่ $T_{\text{sat}}$ เสมอ การคำนวณ $\dot{m}$ จะขึ้นกับ $(T - T_{\text{sat}})$ โดยตรง และ $\dot{q}_{\text{latent}}$ จะถูกคำนวณตามสมการข้างต้นและใส่เป็น source term ในสมการพลังงาน การมี source term นี้จะทำให้อุณหภูมิในเซลล์ที่เกิดการเปลี่ยนสถานะมีแนวโน้มจะถูก "ดึง" กลับมาที่ $T_{\text{sat}}$

3.  **การ Implement ใน Solver:**
    *   ในขั้นตอน `solveEnergy()` หลังจากคำนวณ $\dot{m}$ จากโมเดล (เช่น Lee Model: $\dot{m} = C \cdot \alpha \cdot \rho \cdot (T - T_{\text{sat}})/T_{\text{sat}}$) แล้ว
...
4.  **การรักษา Interface Temperature ($T_{\text{sat}}$):**
    *   การมีคู่ $\dot{m}$ และ $\dot{q}_{\text{latent}}$ ที่สัมพันธ์กันผ่าน $h_{fg}$ นี้สร้างกลไก feedback ที่ทรงพลัง
    *   หาก $T > T_{\text{sat}}$ ในบริเวณ interface: $\dot{m} > 0$ (ระเหย) $\rightarrow$ $\dot{q}_{\text{latent}} < 0$ (ดูดความร้อน) $\rightarrow$ สมการพลังงานจะลดค่า $T$ ลง
    *   หาก $T < T_{\text{sat}}$ ในบริเวณ interface: $\dot{m} < 0$ (กลั่นตัว) $\rightarrow$ $\dot{q}_{\text{latent}} > 0$ (คายความร้อน) $\rightarrow$ สมการพลังงานจะเพิ่มค่า $T$ ขึ้น
...
**สรุป:** $\dot{q}_{\text{latent}} = - \dot{m} \cdot h_{fg}$ เป็นสมการ coupling ที่รับประกันการอนุรักษ์พลังงานระหว่างกระบวนการเปลี่ยนสถานะ มันแปลงอัตราการเปลี่ยนมวลให้เป็นอัตราการแลกเปลี่ยนความร้อนแฝง และสร้าง feedback loop ที่สำคัญสำหรับการรักษาเงื่อนไขอุณหภูมิที่ interface

---

## 6.4 คำถาม: Non-linear Convection Term ($\nabla \cdot (\mathbf{U} \mathbf{U})$) ในสมการโมเมนตัม สร้างความท้าทายอะไรในการ Discretize และเราจัดการกับมันอย่างไร?

**คำตอบโดยละเอียด:**

เทอม $\nabla \cdot (\rho \mathbf{U} \mathbf{U})$ (หรือ $\nabla \cdot (\mathbf{U} \mathbf{U})$ สำหรับความหนาแน่นคงที่) คือเทอมการพาความร้อน (convective term) ในสมการโมเมนตัม มันเป็นแหล่งที่มาของ **Non-linearity** และ **ความไม่เสถียร (instability)** หลักในการคำนวณ CFD
...
2.  **ความท้าทายด้าน Numerical Stability:**
    *   การ discretize เทอม convective flux ที่ face ($(\mathbf{U} \mathbf{U})_f \cdot \mathbf{S}_f$) ต้องใช้ **interpolation scheme** เนื่องจาก $\mathbf{U}$ ถูกกำหนดไว้ที่ cell centers (P, N) แต่เราต้องการค่าที่ face (f)
    *   Scheme ที่ง่ายที่สุดคือ **Central Differencing (CDS)**: $\mathbf{U}_f = 0.5(\mathbf{U}_P + \mathbf{U}_N)$ มันมี accuracy สูง (second-order) แต่มีปัญหาใหญ่คือ **ไม่มีคุณสมบัติ Boundedness** (ค่าอาจจะเกินขอบเขตทางกายภาพ) และอาจเกิดการแกว่งกวัดเชิงตัวเลข (numerical oscillations) ในบริเวณที่การพามวลเข้มข้นกว่าการแพร่ (High Peclet Number)
...
4.  **Lee, W.H. (1980). *A Pressure Iteration Scheme for Two-Phase Flow Modeling*. In *Multiphase Transport: Fundamentals, Reactor Safety, Applications* (Vol. 1, pp. 407-432). Hemisphere Publishing.**
    *   **ความเกี่ยวข้อง:** เอกสารต้นทางของ **Lee Model** ซึ่งเป็น Phase Change Model ที่เราจะ implement ใน Day 11 เอกสารนี้อธิบายที่มาของสมการ Mass Transfer Rate $\dot{m} = C \cdot \alpha \cdot \rho \cdot (T - T_{\text{sat}})/T_{\text{sat}}$ และการ couple กับสมการพลังงาน
...
2.  **Hardt, S., & Wondra, F. (2008). *Evaporation model for interfacial flows based on a continuum-field representation of the source terms*. Journal of Computational Physics, 227(11), 5871-5895.**
    *   **ความเกี่ยวข้อง:** อธิบาย Phase Change Model แบบ Continuum Field (คล้าย Lee Model) สำหรับ VOF Method โดยละเอียด รวมถึงการจัดการกับ Expansion Term $\nabla \cdot \mathbf{U} = \dot{m}(1/\rho_v - 1/\rho_l)$ ซึ่งเป็น **Hero Concept** ของเรา
...
*   **Concept Flow:** $\partial/\partial t$ $\rightarrow$ จะถูกแทนที่ด้วย Time Discretization (Day 04), $\nabla \cdot (\rho \mathbf{U})$ $\rightarrow$ จะถูกคำนวณโดยใช้ Gauss's Theorem บน Control Volume ที่ Day 02 อธิบาย, $\nabla p$ และ $\nabla^2 \mathbf{U}$ $\rightarrow$ จะถูกประเมินที่ Face โดยใช้ Gradient และ Laplacian Schemes
...
*   **Connection:** Term $\nabla \cdot (\mathbf{U} \mathbf{U})$ (Convection) ใน Momentum Equation และ $\nabla \cdot (\mathbf{U} T)$ ใน Energy Equation เป็น **Nonlinear Term** ที่มีผลต่อ Stability อย่างมาก Day 03 จะสอนวิธีประมาณค่า $\mathbf{U}_f$ และ $T_f$ ที่ Face ระหว่าง Cell โดยใช้ Schemes ต่างๆ (Upwind, Central, TVD)
...
*   **Connection:** Term $\partial/\partial t$ ในทุกสมการต้องถูกแทนที่ด้วย Time-Stepping Scheme Day 04 จะสอนการแปลง PDE ให้เป็นระบบสมการพีชคณิต (Algebraic Equations) ที่สามารถ solve ได้ในแต่ละ Time Step
*   **Stability Link:** ขนาดของ Time Step $\Delta t$ ที่เลือกจะสัมพันธ์กับ $\mathbf{U}$ และ Mesh Size ผ่าน **Courant Number (Co)** ซึ่งเป็น concept สำคัญจาก Day 04
...
*   **Implementation Bridge:** การที่เรารู้ว่า Field $p$ มีค่า $p[\text{cell}_i]$ นั้นไม่เพียงพอ เราต้องรู้ว่า Cell $i$ เชื่อมต่อกับ Cell $j$ ใดบ้างเพื่อจะคำนวณ Flux $(p_j - p_i)$ ได้
...
*   **Field Completeness:** ฟิลด์ $\mathbf{U}, p, T$ ที่สร้างใน Day 01 จะไม่สมบูรณ์จนกว่า Boundary Conditions จะถูกกำหนดและจัดการอย่างถูกต้องใน Day 06

### 7.2.6 Day 07: Linear Algebra (LDU)
*   **Connection:** หลังจาก Discretize สมการจาก Day 01 แล้ว เราจะได้ระบบสมการเชิงเส้น $A \mathbf{x} = \mathbf{b}$ สำหรับแต่ละฟิลด์ Day 07 จะสอนว่า OpenFOAM เก็บ Sparse Matrix $A$ อย่างไรในรูปแบบ LDU (Lower-Diagonal-Upper) โดยใช้ Addressing จาก Mesh (Day 05)
*   **Matrix Assembly:** Term ต่างๆ ในสมการ Day 01 จะมี Contribution ที่แตกต่างกันต่อ Matrix $A$ และ Source $\mathbf{b}$ เช่น Diffusion Term ($\nu \nabla^2 \mathbf{U}$) ให้ symmetric contribution, Convection Term ($\nabla \cdot (\mathbf{U} \mathbf{U})$) ให้ asymmetric contribution

### 7.2.7 Day 08: Iterative Solvers
*   **Connection:** ระบบสมการ $A \mathbf{x} = \mathbf{b}$ ที่ได้จาก Day 07 มีขนาดใหญ่และ Sparse ต้องแก้ด้วย Iterative Solvers Day 08 จะสอน Algorithm เช่น PBiCGStab (สำหรับ Asymmetric Matrix จาก Momentum Eq.) และ PCG (สำหรับ Symmetric Matrix จาก Pressure Poisson Eq.)
...
*   **Hero Concept Integration:** **Expansion Term $\nabla \cdot \mathbf{U} = \dot{m}(1/\rho_v - 1/\rho_l)$ จาก Continuity Equation จะต้องถูก incorporate เป็น Source Term ใน Pressure Poisson Equation** ภายในขั้นตอนของ PISO/SIMPLE หากขาดเทอมนี้ไป การขยายตัวของไอจะไม่ถูก compensated โดย Pressure Field ทำให้เกิด Divergence ทันที

### 7.2.9 Day 10: Two-Phase Fundamentals (VOF)
*   **Connection:** สมการใน Day 01 เป็นสมการสำหรับ **Mixture** (ของเหลว+ไอ) Day 10 จะแนะนำสมการเพิ่มเติมสำหรับตัวแปร $\alpha$ (Volume Fraction) เพื่อบอกว่าส่วนไหนเป็นของเหลว ($\alpha=1$) หรือ ไอ ($\alpha=0$)
*   **Property Calculation:** ค่า $\rho$ และ $\nu$ ในสมการ Momentum และ Energy ของ Day 01 จะไม่ใช่ค่าคงที่อีกต่อไป แต่คำนวณจาก $\alpha$ เช่น $\rho = \alpha \rho_l + (1-\alpha) \rho_v$
*   **Interface Dynamics:** การเคลื่อนที่ของ Interface ($\alpha$ contour) ถูกควบคุมโดยสมการ VOF ซึ่งเชื่อมโยงกับ Velocity Field $\mathbf{U}$ จาก Momentum Equation

### 7.2.10 Day 11: Phase Change Theory
*   **Connection:** นี่คือวันที่ให้ **Physical Model** สำหรับ Source Term $\dot{m}$ (Mass Transfer Rate) ที่ปรากฏใน **Hero Concept** ($\nabla \cdot \mathbf{U} = \dot{m}(1/\rho_v - 1/\rho_l)$) และในสมการ Energy ($\dot{q}_{\text{latent}}$)
*   **Closing the Loop:** Day 11 จะเชื่อมโยงสมการฟิสิกส์ทั้งหมด:
    *   **Energy Eq. (Day 01)** $\rightarrow$ คำนวณ $T$
    *   **Lee Model (Day 11)** $\rightarrow$ ใช้ $T$ และ $\alpha$ คำนวณ $\dot{m}$
    *   **Continuity Eq. (Day 01)** $\rightarrow$ ใช้ $\dot{m}$ คำนวณ $\nabla \cdot \mathbf{U}$ (Expansion)
    *   **VOF Eq. (Day 10)** $\rightarrow$ ใช้ $\dot{m}$ อัพเดท $\alpha$
    *   **Momentum Eq. (Day 01)** $\rightarrow$ ได้ $\mathbf{U}$ ที่สอดคล้องกับการขยายตัว
    *   **Pressure Eq. (Day 09)** $\rightarrow$ อัพเดท $p$ เพื่อรักษา Mass Conservation
