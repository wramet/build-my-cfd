...
3.  **ออกแบบและ Implement (Design & Implement)** Custom TVD Limiter สำหรับปัญหาการจับ Interface แบบเฉียบคม (Sharp Interface Capturing) ในวิธี Volume of Fluid (VOF)
    ...
    *   **Gradient Calculation**: Implement การคำนวณ Gradient Ratio $r_f = (\phi_P - \phi_U) / (\phi_N - \phi_P)$ ซึ่งต้องการค่าจาก Upwind-upwind Cell ($\phi_U$) เพื่อประเมินความชันของ solution upstream
...
- [[#1. Theory (ทฤษฎี)|1. Theory (ทฤษฎี)]]
- [[#2. OpenFOAM Reference (การอ้างอิง OpenFOAM)|2. OpenFOAM Reference (การอ้างอิง OpenFOAM)]]
- [[#3. Class Design (การออกแบบคลาส)|3. Class Design (การออกแบบคลาส)]]
- [[#4. Implementation (การนำไปใช้)|4. Implementation (การนำไปใช้)]]
- [[#5. Build & Test (การบิลด์และการทดสอบ)|5. Build & Test (การบิลด์และการทดสอบ)]]
- [[#6. Concept Checks (การทดสอบแนวคิด)|6. Concept Checks (การทดสอบแนวคิด)]]
- [[#7. References & Related Days (เอกสารอ้างอิงและบทเรียนที่เกี่ยวข้อง)|7. References & Related Days (เอกสารอ้างอิงและบทเรียนที่เกี่ยวข้อง)]]
...
ใน Finite Volume Method (FVM) ที่เราสร้างขึ้นใน Day 02 สิ่งที่เราต้องการคือการเปลี่ยนสมการอนุพันธ์ย่อย (PDE) ให้อยู่ในรูปของสมการพีชคณิตที่สามารถแก้ได้ด้วยคอมพิวเตอร์ สำหรับ Phase 1 ของโครงการนี้ สมการแม่บทที่เราจะต้องเผชิญซ้ำแล้วซ้ำเล่าคือ **สมการการขนส่ง (Transport Equation)** รูปแบบทั่วไป

$$ \frac{\partial (\rho \phi)}{\partial t} + \nabla \cdot (\rho \mathbf{U} \phi) = \nabla \cdot (\Gamma \nabla \phi) + S_{\phi} $$

**การตีความทางฟิสิกส์ของแต่ละเทอม:**
...
เมื่อเราใช้ **Gauss's Divergence Theorem** (จาก Day 02) กับเทอม convective และ diffusive เราจะได้สมการในรูป integral สำหรับ control volume $V_P$ ใดๆ:

$$ \int_{V_P} \frac{\partial (\rho \phi)}{\partial t} dV + \sum_f \int_{f} (\rho \mathbf{U} \phi)_f \cdot d\mathbf{S}_f = \sum_f \int_{f} (\Gamma \nabla \phi)_f \cdot d\mathbf{S}_f + \int_{V_P} S_{\phi} dV $$

หากเราสมมติว่าค่าเฉลี่ยในเซลล์เป็นตัวแทนที่ดีของปริมาณภายในเซลล์ และค่าบนผิวหน้าเป็นตัวแทนที่ดีของปริมาณบนหน้าดังกล่าว เราสามารถเขียนสมการในรูป discrete ได้ดังนี้:

$$ V_P \frac{\partial (\rho_P \phi_P)}{\partial t} + \sum_f (\rho_f \mathbf{U}_f \phi_f) \cdot \mathbf{S}_f = \sum_f \Gamma_f (\nabla \phi)_f \cdot \mathbf{S}_f + V_P S_{\phi, P} $$

**จุดวิกฤต (Critical Juncture):** สมการข้างต้นทำให้เราเห็นปัญหาหลักของการ discretize ในเชิงพื้นที่: **เรามีค่า $\phi$ ที่ศูนย์กลางเซลล์ ($\phi_P$) แต่เราต้องการค่า $\phi$ บนผิวหน้า ($\phi_f$) เพื่อคำนวณ convective flux** และเราต้องการ gradient บนผิวหน้า $(\nabla \phi)_f$ เพื่อคำนวณ diffusive flux
...
* $r_f \approx 1$: ความชันสม่ำเสมอ (Smooth variation) → ควรใช้ CDS (Second-Order)
* $r_f \ll 1$ หรือ $r_f < 0$: ความชันเปลี่ยนอย่างรวดเร็วหรือเปลี่ยนเครื่องหมาย (Extremum) → ควรใช้ UDS (First-Order) เพื่อป้องกัน Oscillation
...
**พื้นที่ที่ยอมรับได้ของ TVD Limiter (TVD Region):** ฟังก์ชันลิมิตเตอร์ $\psi(r)$ ใดๆ ที่จะทำให้ scheme มีคุณสมบัติ TVD ต้องอยู่ในขอบเขตต่อไปนี้สำหรับทุก $r \ge 0$: $$0  \le  \psi(r)  \le  \min(2r, 2)$$  กราฟของ $\psi(r)$ ต้องอยู่ภายในพื้นที่สามเหลี่ยมที่ล้อมรอบด้วยเส้น $\psi=0$, $\psi=2r$, และ $\psi=2$
...
2.  **สำหรับแต่ละสนามที่ต้องการแก้ ($\mathbf{U}$, $p$, $\alpha$, $T$)**:
    *   สร้างออบเจ็กต์จัดการ scheme (เช่น `surfaceInterpolationScheme`).
    *   ผูก scheme กับสนามนั้นๆ (เช่น $\mathbf{U}$ ใช้ `TVDvanLeer`, $\alpha$ ใช้ `TVDsuperBee`).
...
## 2.1.1 Abstract Base Class: `surfaceInterpolationScheme<Type>` (คลาสพื้นฐานแบบนามธรรม)
...
## 2.1.2 First-Order Scheme: `upwind<Type>` (สกีมอันดับหนึ่ง)
...
) const
{
    ...
    forAll(owner, facei)
    {
        ...
        // คำนวณ mixture density: $\rho_{mix} = \alpha \rho_l + (1-\alpha) \rho_v$
        const scalar rhoFace = 
...
## 2.1.4 TVD Scheme Base Class: `limitedSurfaceInterpolationScheme<Type>` (คลาสพื้นฐานสกีม TVD)
...
3.  **Blending Formula**: $\phi_f = \phi_P + 0.5 \psi(r) (\phi_N - \phi_P)$ โดย $\psi(r)$ มาจาก limiter function
...
## 2.1.5 Runtime Dictionary: `fvSchemes` (พจนานุกรม Runtime)
...
## 2.1.6 Comparison Table: What We Do DIFFERENTLY (ตารางเปรียบเทียบ: สิ่งที่เราทำต่างออกไป)
...
2.  **Generic Programming**: การใช้ Template (`<Type>`) ทำให้ Code ชุดเดียวสามารถทำงานได้กับทั้ง Scalar, Vector, และ Tensor Fields ลดการซ้ำซ้อนของ Code
3.  **Optimization**: การใช้ `tmp` pointer และการ cache weights ช่วยลด Memory Footprint และเพิ่มประสิทธิภาพในการคำนวณ
...
## 3.2 Abstract Base Class: `surfaceInterpolationScheme<Type>` (คลาสพื้นฐานแบบนามธรรม)
...
## 3.2.1 Class Specification (Strict) (รายละเอียดคลาสอย่างเคร่งครัด)
...
## 3.2.2 Key Design Decisions and Rationale (การตัดสินใจออกแบบหลักและเหตุผล)
...
## 3.3 Concrete Scheme Classes (คลาส Concrete Scheme)
...
// สูตร: $\phi_f = \lambda_f \phi_P + (1-\lambda_f) \phi_N + (\nabla \phi)_c \cdot (\mathbf{x}_f - \mathbf{x}_{linear})$
// โดย (x_linear = \lambda_f*x_P + (1-\lambda_f)*x_N)
```

## 3.4 TVD Framework Class: `limitedSurfaceInterpolationScheme<Type>` (คลาสโครงสร้าง TVD)
...
## 3.5.1 vanLeerLimiter Class (คลาส vanLeerLimiter)
...
## 4.1 SpatialDiscretizer Class (คลาส SpatialDiscretizer)
...
) const
{
    ...
    // Calculate local Peclet number: $Pe = \rho U L / \Gamma$
...
## 4.2 TVDLimiter Class (คลาส TVDLimiter)
...
private:
    ...
    // Private Member Functions
        ...
        //- No copy construct
        TVDLimiter(const TVDLimiter&) = delete;

        //- No copy assignment
        void operator=(const TVDLimiter&) = delete;
};
...
## 5. Build & Test (การบิลด์และการทดสอบ)

## 5.1 CMake Build System: Creating Test Executable (ระบบ Build ด้วย CMake: การสร้าง Executable สำหรับทดสอบ Spatial Discretization)
...
## 5.1.2 CMakeLists.txt for Module Day 03 (สำหรับ Module Day 03)
...
## 5.2 Unit Testing for Spatial Discretization (การเขียน Unit Tests สำหรับ Spatial Discretization Schemes)
...
## 5.2.1 Test Infrastructure: Base Test Class (โครงสร้างพื้นฐานการทดสอบ: Base Test Class)
...
public:
    ...
    bool runTest() override
    {
        ...
        // สร้าง smooth analytical field: $\phi(x,y,z) = \sin(\pi x) \cos(\pi y)$
        ...
        // $\nabla \phi = [\pi \cos(\pi x) \cos(\pi y), -\pi \sin(\pi x) \sin(\pi y), 0]$
...
> [!CHECK]- เฉลย: คำตอบเชิงลึก (Click to expand)
> **ความแตกต่างอยู่ที่ *ทิศทางการไหลของข้อมูล (Information Propagation)* ในทางฟิสิกส์** สำหรับสมการการพา (Convection) $\nabla \cdot (\rho \mathbf{U} \phi)$ ข้อมูลจะถูกพาไปตามทิศทางของความเร็วเท่านั้น (Downstream) ไม่มีกลไกทางฟิสิกส์ที่ทำให้ข้อมูลไหลย้อนขึ้นทวนกระแส (Upstream)
>
> 1.  **Upwind Scheme (UDS):** รับรู้และเคารพหลักการนี้อย่างสมบูรณ์ โดยเลือกค่า $\phi_f$ จากเซลล์ต้นทาง (Upstream Cell) เท่านั้น กล่าวคือ
>     *   ถ้า $\mathbf{F}_f \cdot \mathbf{S}_f \ge 0$ (ลมไหลจาก P ไป N) → $\phi_f = \phi_P$
>     *   ถ้า $\mathbf{F}_f \cdot \mathbf{S}_f < 0$ (ลมไหลจาก N ไป P) → $\phi_f = \phi_N$
>     **กลไก:** มันสร้าง *Numerical Diffusion* ปลอมขึ้นมา ซึ่งมีค่าประมาณ $\frac{1}{2} |\mathbf{U}| \Delta x |\partial \phi/\partial x|$ การแพร่กระจายเทียมนี้ทำหน้าที่เหมือน "แดมเปอร์" (Damper) ที่ดูดซับการแกว่งตัว (Oscillation) ทำให้เมทริกซ์มี *Diagonal Dominance* สูง จึง **Unconditionally Stable**
>
> 2.  **Central Scheme (CDS):** ไม่สนใจทิศทางการไหลของข้อมูล โดยใช้ค่าเฉลี่ยเชิงเส้นระหว่าง $\phi_P$ และ $\phi_N$ เสมอ ($\phi_f = \lambda \phi_P + (1-\lambda) \phi_N$) **กลไก:** มันไม่มี Numerical Diffusion ในตัว ดังนั้นความเสถียรทั้งหมดต้องพึ่งพา *Physical Diffusion* ($\Gamma \nabla \phi$) เท่านั้น
>     *   เมื่อการพา (Convection) แข็งแรงกว่าการแพร่ (Diffusion) มากๆ ($Pe = \rho U \Delta x/\Gamma > 2$) สัมประสิทธิ์ในเมทริกซ์สำหรับเทอมการพาจะเปลี่ยนเครื่องหมาย ทำลาย Diagonal Dominance
...
>     *   $\phi_U$: ค่าในเซลล์ Upwind-Upstream (เซลล์ที่อยู่ต้นทางของเซลล์ต้นทางอีกที) ใช้เป็นตัววัดความชัน upstream
>     *   $\phi_P, \phi_N$: ค่าในเซลล์ Owner และ Neighbour ปกติ
>     *   **การตีความค่า $r_f$:**
>         *   $r_f \approx 1$: ความชันสม่ำเสมอ (Smooth Variation) → ควรใช้ Central Scheme
>         *   $r_f \approx 0$ หรือ $r_f < 0$: ความชันเปลี่ยนทิศทางอย่างรวดเร็ว (Extremum หรือ Sharp Gradient) → ควรใช้ Upwind Scheme เพื่อป้องกัน Overshoot
>
> 2.  **กลไกของ Limiter $\psi(r)$:**
>     Limiter จะ map ค่า $r_f$ ไปเป็นค่าสัมประสิทธิ์ระหว่าง 0 ถึง 2
>     *   **ที่ Sharp Gradient ($r_f \sim 0$):** $\psi(r) \to 0$ → สูตร $\phi_f = \phi_P + 0.5 \psi(r) (\phi_N - \phi_P)$ จะลดรูปเป็น $\phi_f = \phi_P$ (Pure Upwind) → **ป้องกัน Oscillation**
>     *   **ที่ Smooth Region ($r_f \sim 1$):** $\psi(r) \to 1$ → สูตรจะลดรูปเป็น $\phi_f = \phi_P + 0.5 (\phi_N - \phi_P) = 0.5 (\phi_P + \phi_N)$ (Pure Central) → **รักษาความแม่นยำอันดับสอง**
...
>     *   `vanAlbada`: $\psi(r) = (r^2 + r)/(r^2 + 1)$ → หลีกเลี่ยงการหารด้วยศูนย์เมื่อ $r \approx -1$ มีความเสถียรสูง
>
> **สรุปสำหรับ Implementation:** ในคลาส `limitedSurfaceInterpolationScheme` ของ OpenFOAM ขั้นตอนคือ: 1) คำนวณ field ของ $r_f$ สำหรับทุกหน้า 2) ใช้ฟังก์ชัน limiter ที่เลือก 3) คำนวณ $\phi_f$ จากสูตร TVD การคำนวณ $r_f$ ต้องอาศัย gradient field $\nabla \phi$ ซึ่งอาจต้องใช้ `fvc::grad()` ก่อน

---

## 6.3 คำถามที่ 3: จะเลือกและกำหนด Spatial Scheme อย่างไรให้เหมาะสมกับ Field ต่างๆ ใน Evaporator Simulation ($\mathbf{U}$, $p$, $\alpha$, $T$)?
...
> }
> ```
> **หมายเหตุวิกฤต:** Scheme สำหรับ `div(phi,alpha)` และ `div(phi,T)` มีผลโดยตรงต่อความถูกต้องของ Source Term ในสมการเปลี่ยนเฟส ($\dot{m}$) และ Expansion Term ($\nabla \cdot \mathbf{U}$) ใน Day 11

---

## 6.4 คำถามที่ 4: การ Implement TVD Scheme ต้องคำนวณ Gradient Ratio $r_f$ ซึ่งต้องใช้ค่า $\phi_U$ (Upwind-Upwind Cell) จะหาเซลล์นี้ได้อย่างไรใน Unstructured Mesh?
...
>     *   จาก face flux `phi` (หรือ $\mathbf{U}_f \cdot \mathbf{S}_f$) เราทราบว่า upstream cell คือ `P` (owner) หรือ `N` (neighbour)
...
>         *   ถ้า `P` เป็น owner ของ `f_in` และ `phi_in < 0` → ฟลักซ์ไหลจาก neighbour เข้า owner (`P`)
>         *   ถ้า `P` เป็น neighbour ของ `f_in` และ `phi_in > 0` → ฟลักซ์ไหลจาก owner เข้า neighbour (`P`)
>     *   Face `f_in` นี้คือ face ที่เชื่อมต่อระหว่าง cell `P` (downstream) กับ cell $U$ (upstream-upstream)
>
> 3.  **ดึงค่า $\phi_U$ จากเซลล์อีกฝั่งของ Face `f_in`:**
>     *   จาก topology ของ face `f_in`: `owner = lduAddr.owner()[f_in]`, `neighbour = lduAddr.neighbour()[f_in]`
>     *   ถ้า `owner == P` → แล้ว $\phi_U = \phi[\text{neighbour}]$
>     *   ถ้า `neighbour == P` → แล้ว $\phi_U = \phi[\text{owner}]$
>
...
> *   **Optimization ใน OpenFOAM:** OpenFOAM มักจะคำนวณและเก็บ `gradient field` (`fvc::grad(phi)`) ไว้ล่วงหน้า จากนั้นประมาณค่า $\phi_P - \phi_U$ โดยใช้ $\nabla\phi|_P \cdot d_{PU}$ (โดยที่ $d_{PU}$ เป็นเวกเตอร์จาก cell center `U$ ถึง $P$) ซึ่งเร็วกว่ามาก
...
>     *   **Physics จริง:** บริเวณ Interface ควรมีอุณหภูมิคงที่เท่ากับ $T_{sat}$ (อุณหภูมิอิ่มตัว) เนื่องจากพลังงานทั้งหมดถูกใช้ไปในการเปลี่ยนเฟส (Latent Heat)
>     *   **ผลจาก Upwind:** Numerical Diffusion จาก scheme ของสมการพลังงานจะแพร่ความร้อนจาก bulk region (ที่ $T \neq T_{sat}$) เข้าสู่ interface region
>         *   ทำให้เกิด "Artificial Superheating/Subcooling" บริเวณ interface
>         *   **ผลกระทบร้ายแรง:** Source Term $\dot{m}$ ใน Lee Model $\dot{m} \propto (T - T_{sat})$ จะถูกคำนวณจากค่า $T$ ที่ผิดนี้ → อัตราการระเหย/การกลั่นตัวผิดพลาด → มวลและพลังงานไม่ conserve → Simulation Diverges
>
> 3.  **ผลต่อ Momentum Field ($\mathbf{U}$-field):**
...
> 2.  **ใช้ Mesh ที่ละเอียดพอ:** Numerical Diffusion $\propto \Delta x$ ดังนั้นการ refine mesh บริเวณ interface ช่วยลดปัญหาได้มาก (แต่เพิ่ม computational cost)
> 3.  **ใช้ Compressive Scheme สำหรับ $\alpha$:** อย่างที่กล่าวใน Day 10, การเพิ่ม Artificial Compression Term $\nabla \cdot (\mathbf{U}_r \alpha(1-\alpha))$ ใน alpha equation ช่วยต้านทานการ smear จาก numerical diffusion โดยตรง
>
...
## 7. References & Related Days (เอกสารอ้างอิงและบทเรียนที่เกี่ยวข้อง)
...
*   **Day 09: Pressure-Velocity Coupling (SIMPLE/PISO)**
    *   **ความสัมพันธ์โดยตรง:** การ Discretize เทอม Convection ใน Momentum Equation ($\nabla \cdot (\rho \mathbf{U} \mathbf{U})$) เป็นหนึ่งในขั้นตอนที่สำคัญที่สุดใน Pressure-Velocity Coupling Algorithm
