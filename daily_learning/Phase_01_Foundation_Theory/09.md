...
2.  **วิเคราะห์ (Analyze) หลักการและที่มาของ Rhie-Chow Interpolation**
    *   หาได้ว่าสูตร Rhie-Chow Interpolation สำหรับ face velocity (`$U_f$`) หรือ face flux (`$\phi_f$`) ถูกได้มาอย่างไร จากการรวมสมการโมเมนตัมที่ discretize แล้วสำหรับ cell owner และ neighbor ที่อยู่สองข้างของ face
...
3.  **ออกแบบ (Design) ขั้นตอนอัลกอริทึม SIMPLE สำหรับ Steady-State Problems**
    ...
    *   อธิบายบทบาทและความสำคัญของ **Under-Relaxation** สำหรับทั้งสนามความเร็ว ($\alpha_U$) และสนามความดัน ($\alpha_p$) ในการรักษา stability ของ iterative process สำหรับปัญหา steady-state
...
![Pressure Checkerboard Problem](/Users/woramet/.gemini/antigravity/brain/bfc0f89f-f829-4862-bfbe-56eae508a352/pressure_checkerboard_pattern_1768238856897.png)
...
### 4.4 Header File: PressureVelocityCoupler.H
...
### 4.5 Implementation File: PressureVelocityCoupler.C
...
{
    const dictionary& solvers = fvSolution.subDict("solvers");
    
    algorithmType_ = fvSolution.subDict("PISO").lookupOrDefault<word>("algorithm", "SIMPLE");
    ...
    momentumRelaxationFactor_ = relaxationDict.lookupOrDefault<scalar>("U", 0.7);
    pressureRelaxationFactor_ = relaxationDict.lookupOrDefault<scalar>("p", 0.3);
    
    convergenceTolerance_ = pisoDict.lookupOrDefault<scalar>("tolerance", 1e-6);
    maxIterations_ = pisoDict.lookupOrDefault<label>("maxIter", 100);
}

template<class Type>
void PressureVelocityCoupler::applyMatrixRelaxation(fvMatrix<Type>& matrix, scalar alpha) const
{
    if (alpha < 1.0 - SMALL) matrix.relax(alpha);
}
...
scalar PressureVelocityCoupler::calculateContinuityError(const surfaceScalarField& phi) const
{
    ...
    forAll(mesh_.cells(), cellI)
    {
        ...
        forAll(cellFaces, faceI)
        {
            ...
            {
                if (mesh_.owner()[faceLabel] == cellI) cellFlux += phi[faceLabel];
                else cellFlux -= phi[faceLabel];
            }
        }
...
bool PressureVelocityCoupler::checkConvergence() const
{
    ...
    return (residualNorm_ < convergenceTolerance_) || (residualReduction > 0.999) || (iter_ >= maxIterations_);
}

void PressureVelocityCoupler::printIterationInfo() const
{
    Info<< "Iteration " << iter_ << ", residual = " << residualNorm_ << endl;
}

PressureVelocityCoupler::PressureVelocityCoupler(const fvMesh& mesh, const IOdictionary& fvSolution)
: mesh_(mesh), runTime_(mesh.time()), rhieChow_(mesh), algorithmType_("SIMPLE")
{
...
) const
{
    ...
    fvVectorMatrix UEqn(fvm::ddt(rho, U) + fvm::div(phi, U) - fvm::laplacian(rho*nu, U) == -fvc::grad(p));
    
    if (algorithmType_ == "SIMPLE") applyMatrixRelaxation(UEqn, momentumRelaxationFactor_);
    
    if (momentumPredictor_)
    {
        UEqn.solve();
        if (algorithmType_ == "SIMPLE") applyFieldRelaxation(U, UOld, momentumRelaxationFactor_);
    }
    U.correctBoundaryConditions();
}
...
) const
{
    ...
    fvScalarMatrix pEqn(fvm::laplacian(rhieChow_.rAP(), p) == fvc::div(phiHbyA));
    pEqn.setReference(0, 0.0);
    pEqn.solve();
    
    if (algorithmType_ == "SIMPLE") applyFieldRelaxation(p, pOld, pressureRelaxationFactor_);
...
void PressureVelocityCoupler::simpleIteration(volVectorField& U, surfaceScalarField& phi, volScalarField& p, const volScalarField& nu, const volScalarField& rho)
{
    volVectorField UOld("UOld", U);
    volScalarField pOld("pOld", p);
    rhieChow_.update(U);
    ...
    residualNorm_ = max(gMax(mag((U - UOld).internalField())), gMax(mag((p - pOld).internalField())));
    iter_++;
}

void PressureVelocityCoupler::couple(volVectorField& U, surfaceScalarField& phi, volScalarField& p, const volScalarField& nu, const volScalarField& rho)
{
    ...
    {
        rhieChow_.update(U);
        if (momentumPredictor_) solveMomentumPredictor(U, phi, p, nu, rho);
...
void PressureVelocityCoupler::reset() { iter_ = 0; residualNorm_ = GREAT; prevResidualNorm_ = GREAT; }
...
## Section 6: Concept Checks (การตรวจสอบความเข้าใจ)
...
> >
## Section 7: References & Related Days (เอกสารอ้างอิงและวันที่เกี่ยวข้อง)
...
*   **Day 02: FVM Basics**
    ...
    *   **การประยุกต์:** ใน [[Day 09: Pressure-Velocity Coupling (SIMPLE, PISO, Rhie-Chow)|Day 09]] การ derive pressure equation อาศัยการ integrate สมการความต่อเนื่อง ($\nabla \cdot \mathbf{U} = 0$) บน control volume ซึ่งได้เป็น $\sum_f (\mathbf{U}_f \cdot \mathbf{S}_f) = 0$ นี่คือจุดเริ่มต้นของ pressure-velocity coupling บน discrete level
