...
## 1.1.1 Dirichlet (Fixed Value) Condition (เงื่อนไขแบบกำหนดค่าคงที่)
...
*   **Velocity Inlet:** $\mathbf{U}_b = (U_x, 0, 0)$ กำหนด Velocity Profile ขาเข้าแบบสม่ำเสมอ
*   **Isothermal Wall:** $T_b = T_{wall}$ ตรึงอุณหภูมิผนัง
*   **Volume Fraction at Inlet:** $\alpha_b = 1$ กำหนดทางเข้าที่เป็นของเหลวล้วนในการจำลอง VOF
*   **Stagnation Point / No-Slip Wall:** $\mathbf{U}_b = (0, 0, 0)$ บังคับความเร็วเป็นศูนย์ที่ขอบเขตของแข็งสำหรับการไหลแบบหนืด
...
## 1.1.2 Neumann (Fixed Gradient) Condition (เงื่อนไขแบบกำหนดความชัน)
...
**Physical Examples and Applications:**
*   **Pressure Outlet / Symmetry Plane:** $\frac{\partial p}{\partial n} = 0$ สิ่งนี้สมมติว่าสนามความดันได้พัฒนาเต็มที่แล้วและไม่เปลี่ยนแปลงในทิศทางการไหลออก หรือการไหลมีความสมมาตร
*   **Adiabatic Wall:** $\frac{\partial T}{\partial n} = 0$ ระบุว่าไม่มี Heat Flux ผ่านผนัง (ฉนวนสมบูรณ์แบบ)
*   **Specified Shear Stress / Heat Flux:** $\mu \frac{\partial u}{\partial n} = \tau_w$ หรือ $-k \frac{\partial T}{\partial n} = q_w$ ให้เงื่อนไข Boundary Flux โดยตรง
*   **Free Surface (in some models):** $\frac{\partial u}{\partial n} = 0$ สำหรับ Tangential Velocity Component
...
## 1.1.3 Robin (Mixed) Condition (เงื่อนไขแบบผสม)
...
**Physical Examples and Applications:**
*   **Convective Heat Transfer Boundary:** $-k \frac{\partial T}{\partial n} = h (T_b - T_\infty)$ ซึ่งสามารถจัดรูปใหม่เป็น $h T_b + k \frac{\partial T}{\partial n} = h T_\infty$ ตรงกับรูปแบบ Robin
...
## 1.2.1 Fixed Velocity Inlet (ทางเข้าแบบกำหนดความเร็ว)
...
## 1.2.3 Mass Flow Inlet (ทางเข้าแบบกำหนดอัตราการไหลของมวล)
...
## 1.3 Outlet Boundary Conditions (เงื่อนไขขอบเขตทางออก)
...
## 1.3.1 Pressure Outlet (ทางออกแบบกำหนดความดัน)
...
## 1.3.2 Outflow (Zero Gradient) (ทางไหลออกแบบความชันศูนย์)
...
## 1.3.3 Convective Outlet (ทางออกแบบพาความร้อน)
...
## 1.4 Wall Boundary Conditions (เงื่อนไขขอบเขตผนัง)
...
## 1.4.1 No-Slip Wall (ผนังแบบไม่ลื่นไถล)
...
## 1.4.2 Slip Wall (ผนังแบบลื่นไถล)
...
## 1.4.3 Wall Functions for Turbulence (ฟังก์ชันผนังสำหรับความปั่นป่วน)
...
## 1.4.4 Thermal Wall Conditions (เงื่อนไขความร้อนที่ผนัง)
...
## 2.1 Core Class: `GeometricBoundaryField` - The Boundary Field Container (คลาสหลัก: GeometricBoundaryField - ตัวเก็บสนามขอบเขต)
...
## 2.2 Foundation Class: `fvPatchField` - The Abstract Boundary Condition (คลาสพื้นฐาน: fvPatchField - เงื่อนไขขอบเขตแบบนามธรรม)
...
2.  **`manipulateMatrix(fvMatrix<Type>& matrix)` - THE HEART OF BC ENFORCEMENT:**
    ...
    ```
    *   **The Challenge:** จะบังคับใช้เงื่อนไข เช่น $\phi_b = \phi_{fixed}$ (Dirichlet) ภายใน Matrix ที่สร้างสำหรับ Interior Cells ได้อย่างไร? คำตอบคือการแก้ไขสมการสำหรับ Cell ที่เป็นเจ้าของ Boundary Faces
    *   **OpenFOAM's Pattern:** สำหรับ Cell `P` ที่ติดกับ Boundary Face `f`:
        ...
        3.  **Set the boundary value** $\phi_b$ ใน Boundary Slice ของ Field
...
## 2.3 Concrete Implementations: `fixedValue`, `zeroGradient`, and `mixed` (การนำไปใช้งานจริง: fixedValue, zeroGradient และ mixed)
...
**2. `zeroGradientFvPatchField` (Homogeneous Neumann, $\partial \phi / \partial n = 0$):**
```cpp
...
**3. `mixedFvPatchField` (Robin: $a \phi + b \partial \phi / \partial n = c$):**
...
*   **`evaluate`:** คำนวณ Boundary Value โดยใช้สูตร `phi_b = valueFraction_*refValue_ + (1-valueFraction_)*(phi_P + refGrad_*delta)`, โดยที่ `delta` คือระยะทางจาก Cell Center ถึง Face Center
...
## 2.4 The `fvPatch` Class: The Geometric Backbone (คลาส fvPatch: กระดูกสันหลังทางเรขาคณิต)
...
## 3.1 Architectural Philosophy and Core Requirements (ปรัชญาทางสถาปัตยกรรมและข้อกำหนดหลัก)
...
## 3.3 Concrete Condition Class: `FixedValueCondition` (Example) (คลาสเงื่อนไขที่เป็นรูปธรรม: FixedValueCondition (ตัวอย่าง))
...
**Purpose:** เพื่อบังคับใช้ Dirichlet Condition $\phi_b = \phi_{ref}$ บน Patch และ Field ที่ระบุ
...
1.  **Identify Contribution:** หา Geometric หรือ Discretization Coefficient $a_f$ ที่ปกติจะคูณกับ Neighbor Value $\phi_N$ (Non-existent) ในสมการสำหรับ Cell $P$ สำหรับ Diffusion Term สิ่งนี้คือ `gamma_f * |S_f| / |d_CN|`
...
3.  **Modify Source:** ลบ $a_f \cdot \phi_{ref}$ ออกจาก Source Term $b_P$ สำหรับ Cell $P$ สิ่งนี้ย้าย Known Boundary Value Contribution ไปยังด้านขวามือ
...
Method `applyToField` นั้นตรงไปตรงมาแต่จำเป็น: หลังจากแก้ระบบสมการเชิงเส้นแล้ว Interior Cells $\phi_P$ จะคำนึงถึง Boundary Condition โดยนัย แต่ค่า Boundary Face $\phi_b$ ที่เก็บใน Patch Field ของ `GeometricField` ต้องถูกตั้งค่าเป็น $\phi_{ref}$ อย่างชัดแจ้งสำหรับการ Visualization, Flux Calculations, และ Time Steps ถัดไป

ตัวอย่างรูปธรรมนี้ให้พิมพ์เขียวสำหรับการ Implement `ZeroGradientCondition` (ซึ่งแก้ไข Coefficients เพื่อบังคับ $(\phi_P - \phi_b)/\Delta n = 0$), `FixedGradientCondition`, และ `MixedCondition` ที่ซับซ้อนกว่าซึ่งผสมผสานทั้งสองแนวทาง `WallFunctionCondition` จะมี `applyToMatrix` ที่ซับซ้อนกว่า ซึ่งคำนวณ Effective Wall Shear Stress ตาม Log-law และแก้ไข Source Term ของ Momentum Equation ตามนั้น
...
## 4.1 Core Class: `BoundaryCondition` (Abstract Base) (คลาสหลัก: BoundaryCondition (ฐานนามธรรม))
...
## 4.2 Concrete Implementation: `FixedValueBoundaryCondition` (การนำไปใช้งานจริง: FixedValueBoundaryCondition)

Class นี้ Implement Dirichlet Boundary Condition: $\phi_b = \text{fixedValue}$ มันบังคับใช้ Boundary Value อย่างเข้มงวด (Strongly Enforces) โดยการแก้ไข Matrix Diagonal และ Source Term
...
## 4.3 Concrete Implementation: `ZeroGradientBoundaryCondition` (การนำไปใช้งานจริง: ZeroGradientBoundaryCondition)
...
## 4.4 Manager Class: `BoundaryConditionManager` (คลาสจัดการ: BoundaryConditionManager)
...
## 5.1 CMake Configuration for Boundary Condition Framework (การกำหนดค่า CMake สำหรับเฟรมเวิร์กเงื่อนไขขอบเขต)
...
## 5.1.1 Root `CMakeLists.txt` Additions (สิ่งที่เพิ่มเติมใน Root CMakeLists.txt)
...
## 5.1.2 Module-Level `CMakeLists.txt` (CMakeLists.txt ระดับโมดูล)
...
## 5.1.3 Unit Test CMake Configuration (การกำหนดค่า CMake สำหรับ Unit Test)
...
## 6.2 Unit Test Implementation (การนำ Unit Test ไปใช้)

## 6.2.1 Test Fixture for Mesh Creation (Test Fixture สำหรับการสร้าง Mesh)
...
## 6.2.2 Test 1: Basic Boundary Condition Types (Test 1: ประเภทเงื่อนไขขอบเขตพื้นฐาน)
...
## 6.2.3 Test 2: Matrix Modification for Boundary Conditions (Test 2: การแก้ไข Matrix สำหรับเงื่อนไขขอบเขต)
...
## 6.2.4 Test 3: BoundaryConditionManager Integration (Test 3: การรวม BoundaryConditionManager)
...
## 6.2.5 Test 4: Physical Boundary Conditions (Test 4: เงื่อนไขขอบเขตทางกายภาพ)
...
## 6.2.6 Test 5: Wall Function Implementation Details (Test 5: รายละเอียดการนำฟังก์ชันผนังไปใช้)
...
1.  **Mathematical Indeterminacy of the Poisson Equation:** สมการความดันสำหรับการไหลแบบ Incompressible (เช่น ที่มาจาก PISO algorithm) เป็นสมการ Poisson ในรูป $\nabla^2 p = f$ ผลเฉลยของสมการนี้มีความUnique เพียงระดับค่าคงที่ที่บวกเพิ่มเข้าไปเท่านั้น (Additive Constant) หากไม่มีเงื่อนไข Dirichlet (fixed value) ที่จุดใดจุดหนึ่ง ระบบ Matrix $A p = b$ จะกลายเป็น Singular Matrix—โดยเฉพาะอย่างยิ่ง Matrix `A` จะมี Zero Eigenvalue ที่สอดคล้องกับ "Constant Pressure Mode" ในทางตัวเลข สิ่งนี้จะแสดงออกโดย Iterative Solver (เช่น PCG) ล้มเหลวในการ Converge เนื่องจาก Residual ไม่สามารถถูกลดค่าลงได้สำหรับ Mode นี้
...
2.  **Computational Savings Mechanism:** การประหยัดมีสองเท่า:
    *   **Vertical Resolution:** การ Resolve Viscous Sublayer ต้องการความสูง Cell แรก $\Delta y$ ที่เล็กมาก เพื่อให้ $y^+ \approx 1$ ซึ่งมักต้องการ 10-20 Cell Layers ภายใน 1% แรกของ Boundary Layer Thickness การใช้ Wall Functions อนุญาตให้วาง Cell แรกที่ $y^+ \approx 50$ ลดจำนวน Cells ในทิศทางตั้งฉากกับผนังลงได้เป็น Order of Magnitude
...
3.  **BC Modification - From Value to Flux:** นี่คือการเปลี่ยนแปลงหลักของการ Implementation
    ...
    *   **Wall Function:** เป็นเงื่อนไขแบบ Neumann/Robin แสดงในรูปของ **Shear Stress Flux** มันไม่ได้ระบุ Velocity แต่ให้ Source Term กับ Momentum Equation สำหรับ Cell `P` ที่ติดผนัง Shear Stress `τ_w` ถูกคำนวณจาก Log-Law:
        $\tau_w = \rho u_\tau^2 = \rho (U_{t,P} / U^+)^2$, โดยที่ $U^+ = (1/\kappa) \ln(E y^+)$
...
**Question:** Generic Robin condition คือ $a \phi_b + b (\partial \phi / \partial n) = c$ จงอธิบายว่าคุณจะ Implement สิ่งนี้สำหรับ Scalar Field `T` ที่ Wall ที่มีการถ่ายเทความร้อนแบบพา (Convective Heat Transfer) อย่างไร, โดยที่เงื่อนไขคือ $-k (\partial T / \partial n) = h (T_{ext} - T_b)$ จง Map เทอม `a, b, c` เข้ากับพารามิเตอร์ทางฟิสิกส์ จากนั้นร่าง Matrix Modifications (`diag`, `source`) เฉพาะเจาะจงสำหรับ Cell `P` ที่ติดผนังที่จำเป็นเพื่อกำหนดเงื่อนไขนี้แบบ Implicit

**Answer Key Points & Deep Explanation:**

1.  **Mathematical Mapping:**
    *   เริ่มจากฟิสิกส์: $-k (\partial T / \partial n) = h (T_{ext} - T_b)$
    *   จัดรูปเป็น Robin form: $h T_b + k (\partial T / \partial n) = h T_{ext}$
    *   ดังนั้น:
        *   $a = h$ (coefficient บน boundary value)
        *   $b = k$ (coefficient บน normal gradient)
        *   $c = h T_{ext}$ (external driving force)

2.  **Discretization and Matrix Modification:** เราต้องเขียน $\phi_b$ (ในที่นี้คือ $T_b$) และ Gradient ของมันในรูปของค่า Interior Cell $T_P$ โดยใช้ First-Order One-Sided Difference สำหรับ Normal Gradient ที่ Boundary Face $f$:
    $(\partial T / \partial n)_f \approx (T_b - T_P) / \Delta n$, โดยที่ $\Delta n$ คือระยะทางจาก Cell Center `P` ถึง Boundary Face Center
    *   แทนค่าลงในเงื่อนไข Robin:
        $a T_b + b ((T_b - T_P) / \Delta n) = c$
    *   แก้หาค่า Boundary Value $T_b$:
        $T_b (a + b/\Delta n) = c + (b/\Delta n) T_P$
        $T_b = [c + (b/\Delta n) T_P] / (a + b/\Delta n)$
...
3.  **Integration into the Linear System:** Flux ของ `T` เนื่องจากการแพร่ (Diffusion) ข้าม Boundary Face คือ `F_f = -k (∂T/∂n)_f * A_f = -b * ((T_b - T_P)/Δn) * A_f` แทนค่าสมการสำหรับ `T_b`:
    $F_f = -b A_f/\Delta n ( [c + (b/\Delta n) T_P]/(a + b/\Delta n) - T_P )$
    ...
    **Final Matrix Modifications in `applyToMatrix()`:**
    ...
    Let `weight` $= (b A_f) / (\Delta n (a + b/\Delta n))$.
    *   **Add to diag[P]:** `diag[P] += weight * (b /` $\Delta n$`)`. นี่คือ Coefficient ที่คูณกับ Implicit $T_P$
    *   **Add to source[P]:** `source[P] += weight * c`. นี่คือ Constant Part จาก $T_{ext}$
...
1.  **Why Standard Method Breaks Symmetry:** Laplacian Operator ($\nabla \cdot (\Gamma \nabla \phi)$) ให้ Symmetric Matrix: Coefficient $a_{PN}$ ที่เชื่อม Cell P กับ Neighbor N จะเหมือนกันกับ $a_{NP}$ การกำหนดค่า $\phi_b = V$ อย่างเข้มงวดแบบมาตรฐานสำหรับ Boundary Cell `P` จะทำดังนี้:
...
2.  **Alternative: Symmetric Modification (Penalty/Lagrange Multiplier Approach):**
    ...
    *   **Resulting Equation:** สมการสำหรับ Cell `P` กลายเป็น:
        $(a_{PP} + K) \phi_P + \sum a_{PN} \phi_N = b_P + K V$
...
## 7.1 Core Reference Texts & Papers (ตำราเอกสารอ้างอิงหลัก)
...
## 7.2 Related Days in This Curriculum (เนื้อหาที่เกี่ยวข้องในหลักสูตรนี้)

*   **Day 02: FVM Basics**
    *   **Connection:** Day 02 แนะนำ Gauss Divergence Theorem ซึ่งเป็นรากฐานสำคัญสำหรับการประเมิน Fluxes ข้าม Boundary Faces การคำนวณ $\sum_f \phi_f \cdot \mathbf{S}_f$ สำหรับ Boundary Cell รวม Contributions จาก Boundary Faces ของมัน, โดยที่ $\phi_f$ ถูกกำหนดโดย Boundary Condition (เช่น $\phi_f = \phi_{\text{fixed}}$ สำหรับ Dirichlet, หรือ $\phi_f = \phi_P + (\nabla \phi)_P \cdot \mathbf{d}_f$ สำหรับ zeroGradient) `surfaceScalarField` Type ที่สร้างใน Day 2 เป็นตัวพาหลักสำหรับ Boundary Fluxes เหล่านี้
...
## 7.3 Recommended Verification & Validation Cases (กรณีตรวจสอบความถูกต้องที่แนะนำ)
