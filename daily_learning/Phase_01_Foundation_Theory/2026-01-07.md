...
4.  **วิเคราะห์ (Analyze)** และ **จัดการ (Manage)** ผลกระทบของ **Boundary Conditions ต่อโครงสร้างเมทริกซ์** โดยสามารถอธิบายและเขียนโค้ดสำหรับการปรับเปลี่ยน matrix coefficients (`diag`) และ source vector (`b`) สำหรับเงื่อนไขขอบเขตประเภทต่างๆ ได้ เช่น:
    *   **FixedValue (Dirichlet)**: การเพิ่มค่ามหาศาล (large value) ลงใน `diag[P]` และการเพิ่ม `value * largeValue` ลงใน `source[P]` เพื่อ "บังคับ" (enforce) ให้คำตอบ $\phi_P$ อยู่ใกล้กับค่าที่กำหนด
...
## 7.4 Deep Dive: `lduMatrix` Class

### 7.4.1 Header Analysis (`lduMatrix.H`)
...
### 7.4.2 Implementation Analysis (`lduMatrix.C`)
...
### 7.4.3 What We Do DIFFERENTLY: `lduMatrix` Implementation
...
## 7.5 Deep Dive: `lduAddressing` Class

### 7.5.1 Header Analysis (`lduAddressing.H`)
...
### 7.5.2 Implementation Analysis (`lduAddressing.C`)
...
## 7.6 ภาพรวมสถาปัตยกรรม (Architecture Overview)
...
## 7.7 Class 1: `LduMatrixAssembler`
...
### 7.7.1 รายละเอียดคลาส (Class Specification)
...
public:
     ...
     * @param gammaFace Field of diffusion coefficients at faces (e.g., $\Gamma_f$).
     * @param SfMag Magnitude of face area vectors ($|S_f|$).
     * @param delta Cell-to-cell distance ($d_{PN}$) for each face.
     *
     * For a face f between owner P and neighbor N:
     *   Face coefficient $a_f = \Gamma_f |S_f| / d_{PN}$
     ...
     * @param phiFace Mass flux at faces ($F_f = \rho_f \mathbf{U}_f \cdot \mathbf{S}_f$). Sign matters.
     *
     ...
     *     **Wait:** Upwind implies $\phi_f = \phi_P$. Thus, the flux contributes to
     *     the equation for cell P as $+\phi_f$, and to cell N as $-\phi_f$.
     *     However, in matrix form for cell P: coeff * $\phi_P$.
     *     So we add phiFace[f] to diag[P]? INCORRECT.
     *
     *   Correct upwind assembly:
     *     Let flux $F = \text{phiFace}[f]$.
     *     For owner cell P equation: term = $F \phi_{upwind}$.
     *     If $F > 0$ (P→N): $\phi_{upwind} = \phi_P$ → adds F to diag[P] coefficient of $\phi_P$.
     *                     Also, the flux OUT of P appears as $-F\phi_P$ in P's equation? Let's derive properly.
     *
     *   Derivation from integrated divergence:
     *     $\int_{CV} \nabla \cdot (\mathbf{U} \phi) dV = \sum_f F_f \phi_f$.
     *     For upwind: $\phi_f = \phi_P$ if $F_f > 0$, else $\phi_N$.
     *     Therefore, contribution to equation for cell P:
     *       $\sum_{f \in \text{faces}(P)} F_f \cdot (\text{upwind selection})$
     *     This results in contributions to both diagonal and off-diagonal.
     *
     *   Example: Face f where P is owner, N is neighbor, $F_f > 0$.
     *     For cell P: term = $+F_f \phi_P$  (flux out of P is positive in divergence? Check sign convention).
     *     For cell N: term = $-F_f \phi_P$  (flux into N is negative contribution? Actually, for cell N, face f is an incoming flux, so it's $+F_f \phi_P$? Wait, the normal for cell N is opposite).
     *
     *   Sign Convention Critical: In OpenFOAM, face area vector $\mathbf{S}_f$ points from owner to neighbor.
     *     Mass flux $F_f = \rho_f (\mathbf{U}_f \cdot \mathbf{S}_f)$. So:
     *       $F_f > 0$ => flow from owner to neighbor.
     *       $F_f < 0$ => flow from neighbor to owner.
     *
     *   Upwind rule based on $F_f$:
     *       $\phi_f = \phi_{owner}$ if $F_f > 0$, else $\phi_{neighbor}$.
     *
     *   Contribution to owner cell P equation from face f:
     *       $+ F_f \phi_f$  (because divergence term is integrated and added to cell's equation).
     *
     *   Contribution to neighbor cell N equation from face f:
     *       $- F_f \phi_f$  (because the same face contributes with opposite sign to neighbor due to $\mathbf{S}_f$ direction).
     *
     *   Therefore, matrix contributions for face f:
     *     If $F_f > 0$: $\phi_f = \phi_P$.
     *       => To equation P: $+F_f \phi_P$ → add $F_f$ to diag[P].
     *       => To equation N: $-F_f \phi_P$ → add $-F_f$ to upper[f] (coefficient linking P to N in N's equation? Wait, upper[f] is for P→N in owner-based indexing).
     *          Actually, for cell N, the term $-F_f \phi_P$ is an off-diagonal contribution from variable $\phi_P$.
     *          In LDU, upper[f] stores coefficient for connection P→N (i.e., effect of $\phi_N$ on equation of P).
     *          Here we need effect of $\phi_P$ on equation of N, which is the lower coefficient? Let's check.
     *
     *     Let's define:
     ...
     *
     *     For $F_f > 0$:
     *       $a_{PN} = 0$ (owner equation uses $\phi_P$, not $\phi_N$).
     *       $a_{NP} = -F_f$ (neighbor equation uses $\phi_P$ with coefficient $-F_f$).
     *       So: lower[f] += $-F_f$.
     *
     *     Additionally, diagonal contributions:
     *       diag[P] += $F_f$   (from owner equation)
     *       diag[N] += 0? Wait, neighbor equation has no diagonal contribution from this face because it uses $\phi_P$.
     *
...
### 7.7.2 การออกแบบโครงสร้างข้อมูลภายใน (Internal Data Structure Design)
...
### 7.7.3 อัลกอริทึมการประกอบเมทริกซ์ขั้นตอนละเอียด (Detailed Matrix Assembly Algorithm)
...
## 7.8 Header File: `LduMatrixAssembler.H`
...
## 7.9 Implementation File: `LduMatrixAssembler.C`
...
)
{
    ...
    // => owner cell ใช้ค่า $\phi_{own}$, neighbor cell ใช้ค่า $\phi_{own}$
    // => สมการ Owner: เทอม +flux*$\phi_{own}$ => เพิ่ม diagonal ของ owner
    // => สมการ Neighbor: เทอม -flux*$\phi_{own}$ => เพิ่ม coeff ของ owner ในสมการ neighbor (lower)
    if (flux > 0.0)
    {
        ...
        // เทอม -flux * $\phi_{own}$ ย้ายไปอยู่ LHS หรือ RHS?
        // ใน A*x = b, เราต้องการ A_NP * $\phi_P$
        // เทอมคือ (-flux)*$\phi_P$ ดังนั้น A_NP += -flux
    ...
    else  // flux <= 0 (ไหลจาก neighbor ไป owner)
    {
        ...
        // Owner cell contribution
        // เทอม +flux * $\phi_{nei}$ (flux is negative)
        // เทอมคือ (+flux)*$\phi_{nei}$ ดังนั้น A_PN += +flux
...
## 7.10 การตั้งค่า CMake และโครงสร้างโปรเจค
...
### 7.10.1 โครงสร้างไดเรกทอรีของโปรเจค Day07
...
### 7.10.2 ไฟล์ CMakeLists.txt หลัก (Root CMakeLists.txt)
...
### 7.10.3 CMakeLists.txt สำหรับ Source Directory (src/)
...
## 7.11 การเขียน Unit Tests แบบครอบคลุม
...
### 7.11.1 Test Framework Structure
...
### 7.11.2 Unit Test สำหรับ LduMatrixAssembler
...
private:
    ...
    void testDiagonalDominance() {
        ...
        forAll(matrix.diag(), celli) {
            scalar diagonalValue = matrix.diag()[celli];
            scalar offDiagSum = 0.0;
            // ... (diagonal dominance calculation)
            if (diagonalValue < offDiagSum * 0.99) {
                weakCells++;
            }
        }
        
        assertTrue(weakCells == 0,
                  "Found " + std::to_string(weakCells) + " cells with weak diagonal dominance");
    }
    
    void testBoundaryConditions() {
        Info << "  Testing boundary condition application..." << endl;
        // ... (boundary condition test implementation)
    }
};
...
int main(int argc, char *argv[])
{
    ...
    Time runTime(args);
    fvMesh mesh(readMesh(runTime));
    
    TestRunner runner;
    runner.addTest(new TestMatrixAssembly(mesh));
    return runner.run();
}
```
...
1.  **Physical Setup:** สมมติเรามี patch face ที่มีค่า $\phi = \phi_{\text{fixed}}$ ที่ face center เซลล์ owner ของ face นั้น (เซลล์ที่อยู่ติดกับ boundary) คือเซลล์ $P$

2.  **Discretization Contribution:** เมื่อ discretize เทอมต่างๆ (เช่น diffusion) ที่ boundary face เราจะได้ contribution ที่ควรจะไปอยู่ในสมการของเซลล์ $P$ Contribution นี้มักอยู่ในรูป $a_{\text{bnd}} \phi_{\text{bnd}}$ โดยที่ $\phi_{\text{bnd}}$ คือค่าที่ face ซึ่งเรารู้แล้วว่าเท่ากับ $\phi_{\text{fixed}}$

3.  **Matrix Manipulation Strategy (Penalty Method):**
    *   **เพิ่ม Source Term:** นำ contribution $a_{\text{bnd}} \phi_{\text{fixed}}$ ไปบวกเข้าไปใน **source term** ($b_P$) ของเซลล์ $P$ นี่คือการย้ายค่าที่รู้แล้วไปไว้ฝั่งขวาของสมการ
...
1.  **เหตุผลที่ต้องแปลงจาก LDU เป็น CSR:**
    *   **External Solver Libraries:** Libraries อย่าง PETSc, Intel MKL, CUDA cuSPARSE, หรือ AMGCL ใช้ CSR/COO format เป็นมาตรฐาน หากต้องการใช้ solvers ที่มีประสิทธิภาพสูงจาก libraries เหล่านี้ (เช่น Algebraic Multigrid) การแปลง format เป็นสิ่งจำเป็น
    *   **GPU Computing:** CUDA และ OpenCL sparse matrix libraries ส่วนใหญ่รองรับ CSR format เท่านั้น การคำนวณบน GPU ต้องการ data layout ที่เหมาะสมกับ parallel execution
    *   **Interoperability:** เมื่อต้อง coupling CFD solver ของเรากับ software อื่นๆ (เช่น structural mechanics, electromagnetics) การใช้ format มาตรฐานช่วยลดความซับซ้อนในการ interface

2.  **การจัดการความสมมาตรของข้อมูล:**
    *   **Symmetric Matrix (Pure Diffusion):** เมื่อ `upper[f] == lower[f]` สำหรับทุก face สามารถเก็บแค่ครึ่งเดียวของ matrix ได้ใน CSR symmetric format ซึ่งลด memory และ computation ครึ่งหนึ่ง
    *   **Non-Symmetric Matrix (With Convection):** เมื่อ `upper[f] != lower[f]` ต้องเก็บทั้ง upper และ lower triangular parts ซึ่งต้องระวังการ map index:
        ```cpp
        // สำหรับ face f ระหว่าง owner P และ neighbor N:
        // CSR format: A[P][N] = upper[f], A[N][P] = lower[f]
        // ต้องหา row pointer และ column index ที่ถูกต้อง
        ```
    *   **Storage Overhead:** CSR format ต้องเก็บ row_ptr (size: nCells+1), col_idx (size: 2*nFaces + nCells), และ values (size: 2*nFaces + nCells) ในขณะที่ LDU เก็บแค่ diag (nCells), upper (nFaces), lower (nFaces) ดังนั้น CSR ใช้ memory มากกว่า ~30-50%

3.  **Algorithm สำหรับแปลง LDU → CSR:**
    ```cpp
    void convertLduToCsr(const lduMatrix& ldu, CsrMatrix& csr) {
        // Step 1: Count non-zeros per row
        for (int f = 0; f < nFaces; f++) {
            int own = lowerAddr[f];
            int nei = upperAddr[f];
            rowCount[own]++;  // Upper entry
            rowCount[nei]++;  // Lower entry
        }
        
        // Step 2: Build row_ptr from counts
        csr.row_ptr[0] = 0;
        for (int i = 0; i < nCells; i++) {
            csr.row_ptr[i+1] = csr.row_ptr[i] + rowCount[i] + 1; // +1 for diagonal
        }
        ...
        for (int f = 0; f < nFaces; f++) {
            ...
            // Entry (own, nei) = upper[f]
            insertEntry(csr, own, nei, ldu.upper()[f]);
            
            // Entry (nei, own) = lower[f]  
            insertEntry(csr, nei, own, ldu.lower()[f]);
        }
        
        // Step 4: Insert diagonal entries
        for (int i = 0; i < nCells; i++) {
            insertEntry(csr, i, i, ldu.diag()[i]);
        }
        
        // Step 5: Sort columns within each row (required by some libraries)
        sortColumnsPerRow(csr);
    }
    ```

4.  **Best Practices:**
    *   ทำการแปลง **ครั้งเดียว** ตอน initialization และ reuse CSR structure (เพราะ sparsity pattern ไม่เปลี่ยน)
    *   หลีกเลี่ยงการแปลงทุก iteration - ให้ update เฉพาะ **values** ใน CSR โดยไม่ต้อง rebuild structure
    *   พิจารณาใช้ **Matrix-Free Methods** ถ้า conversion overhead สูงกว่า solver time

---

# 8. Section 7: References & Related Days
