...
## 1. Section 1: Theory (ส่วนที่ 1: ทฤษฎี)

## 11.1 Lee Model for Mass Transfer Rate (แบบจำลอง Lee สำหรับอัตราการถ่ายเทมวล)
...
## 11.1.1 Governing Equations of the Lee Model (สมการกำกับของแบบจำลอง Lee)
...
## 11.1.2 Net Source Terms for Phase Continuity (เทอมแหล่งกำเนิดสุทธิสำหรับความต่อเนื่องของเฟส)
...
## 11.1.3 Physical Interpretation and Model Parameters (การตีความทางกายภาพและพารามิเตอร์ของแบบจำลอง)
...
## 11.1.4 Critical Analysis and Limitations (การวิเคราะห์เชิงวิพากษ์และข้อจำกัด)
...
## 11.2 Source Term Linearization (S_u, S_p) (การทำให้เทอมแหล่งกำเนิดเป็นเชิงเส้น)
...
## 11.2.1 The Linearized Form (รูปแบบที่ทำให้เป็นเชิงเส้นแล้ว)
...
## 11.2.2 Linearization for the Volume Fraction Equation (การทำให้สมการสัดส่วนปริมาตรเป็นเชิงเส้น)
...
## 11.2.3 Linearization for the Energy Equation (การทำให้สมการพลังงานเป็นเชิงเส้น)
...
## 11.2.4 The Golden Rule: $S_p \leq 0$ (กฎทอง: $S_p \leq 0$)
...
## 11.3 Energy Equation Coupling (การเชื่อมโยงสมการพลังงาน)
...
## 11.3.1 The Mixture Energy Equation (สมการพลังงานของผสม)
...
## 11.3.2 The Latent Heat Source Term ($S_{energy}$) (เทอมแหล่งกำเนิดความร้อนแฝง $S_{energy}$)
...
## 11.3.3 Enthalpy Formulation: An Alternative Approach (สูตรเอนทัลปี: ทางเลือกอื่น)
...
## 11.3.4 Coupling Strategy and Stability (กลยุทธ์การเชื่อมโยงและความเสถียร)
...
## 11.4 Pressure Equation with Phase Change (สมการความดันกับการเปลี่ยนเฟส)
...
## 11.4.1 The Volume Expansion Source Term (เทอมแหล่งกำเนิดจากการขยายตัวของปริมาตร)
...
## 11.4.2 Modified Pressure Poisson Equation (สมการปัวซองความดันที่ได้รับการปรับปรุง)
...
## 11.4.3 Linearization of the Volume Expansion Source (การทำให้แหล่งกำเนิดการขยายตัวของปริมาตรเป็นเชิงเส้น)
...
## 11.4.4 Impact on Momentum and Velocity Correction (ผลกระทบต่อโมเมนตัมและการแก้ไขความเร็ว)
...
## 11.5 OpenFOAM Reference (อ้างอิง OpenFOAM)

## 11.5.1 Core Class: phaseChangeTwoPhaseMixture (คลาสหลัก: phaseChangeTwoPhaseMixture)

## 11.5.1.1 Class Declaration (phaseChangeTwoPhaseMixture.H) (การประกาศคลาส phaseChangeTwoPhaseMixture.H)
...
## 11.5.1.2 Implementation Skeleton (phaseChangeTwoPhaseMixture.C) (โครงสร้างการนำไปใช้งาน phaseChangeTwoPhaseMixture.C)
...
## 11.5.1.3 Derived Class: LeeModel (LeeModel.H and .C) (คลาสสืบทอด: LeeModel (LeeModel.H และ .C))
...
## 11.5.2 Matrix Class: fvScalarMatrix and Source Integration (คลาสเมทริกซ์: fvScalarMatrix และการรวมแหล่งกำเนิด)

## 11.5.2.1 Role in Equation Assembly (บทบาทในการประกอบสมการ)
...
## 11.5.2.2 Integration into Solver Equations (การรวมเข้ากับสมการ Solver)
...
## 11.5.3 Thermodynamic Class: twoPhaseMixtureThermo (คลาสอุณหพลศาสตร์: twoPhaseMixtureThermo)

## 11.5.3.1 Architecture and Role (สถาปัตยกรรมและบทบาท)
...
## 11.5.3.2 Integration in Solvers (การรวมใน Solver)
...
## 11.5.4 Summary and Best Practices for Implementation (สรุปและแนวปฏิบัติที่ดีที่สุดสำหรับการนำไปใช้งาน)
...
## 11.6 Class Design (การออกแบบคลาส)
...
## 11.6.1 Core Class Architecture (สถาปัตยกรรมคลาสหลัก)
...
## 11.6.2 Class Specifications (รายละเอียดจำเพาะของคลาส)

## 11.6.2.1 Primary Class: PhaseChangeSolver (คลาสหลัก: PhaseChangeSolver)
...
*   **Key Method Specifications:**
    ...
    6.  **`correct()`**
        *   **Process:** Final correction step เรียก `thermalCoupler_->enforceInterfaceTemperature` เพื่อ Clamp อุณหภูมิใน Interface region เข้าหา $T_{sat}$

## 11.6.2.2 Support Class: ThermalCoupler (คลาสสนับสนุน: ThermalCoupler)
...
## 11.6.2.3 Utility Class: LinearizedSourceManager (คลาสอรรถประโยชน์: LinearizedSourceManager)
...
## 11.6.3 Integration into Solver Loop (การรวมเข้ากับลูปรวม Solver)
...
## 11.7 Implementation (การนำไปใช้งาน)
...
## 11.7.1 Header File: phaseChangeLeeModel.H (ไฟล์ส่วนหัว: phaseChangeLeeModel.H)
...
Description
    ...
    - Evaporation: $\dot{m}_{lv} = r_{coeff} \alpha_l \rho_l \frac{\max(T - T_{sat}, 0)}{T_{sat}}$
    - Condensation: $\dot{m}_{vl} = r_{coeff} \alpha_v \rho_v \frac{\max(T_{sat} - T, 0)}{T_{sat}}$
...
## 11.7.2 Implementation File: phaseChangeLeeModel.C (ไฟล์การนำไปใช้งาน: phaseChangeLeeModel.C)
...
## 11.7.3 Usage Example in Solver: alphaEqn.H Integration (ตัวอย่างการใช้งานใน Solver: การรวม alphaEqn.H)
...
## 11.7.4 Usage Example in Solver: TEqn.H Integration (ตัวอย่างการใช้งานใน Solver: การรวม TEqn.H)
...
## 11.7.5 Usage Example in Solver: pEqn.H Integration (ตัวอย่างการใช้งานใน Solver: การรวม pEqn.H)
...
## 11.7.6 Dictionary Configuration: thermophysicalProperties (การกำหนดค่าดิกชันนารี: thermophysicalProperties)
...
## 11.8 Build & Test (การ Build และทดสอบ)
...
## 11.8.1 CMake Build System Configuration (การกำหนดค่าระบบ Build CMake)
...
## 11.8.1.1 Core CMakeLists.txt (CMakeLists.txt หลัก)
...
# ------------------------------------------------------------
```
## 11.8.1.2 Unit Test Configuration (Optional but Critical) (การกำหนดค่า Unit Test)
...
## 11.8.1.3 Unit Test Subdirectory test/CMakeLists.txt (ไดเรกทอรีย่อย Unit Test test/CMakeLists.txt)
...
## 11.8.2 Comprehensive Unit Tests (Unit Test ที่ครอบคลุม)
...
## 11.8.2.1 Test 1: Lee Model Mass Transfer Calculation (การทดสอบ 1: การคำนวณการถ่ายเทมวลของ Lee Model)
...
## 11.8.2.2 Test 2: Source Term Linearization (Su/Sp) (การทดสอบ 2: การทำให้แหล่งกำเนิดเป็นเชิงเส้น Su/Sp)
...
## 11.8.2.3 Test 3: Pressure Expansion Term (การทดสอบ 3: เทอมการขยายตัวของความดัน)
...
## 11.8.3 Build and Test Execution Commands (คำสั่งดำเนินการ Build และทดสอบ)

## 11.8.3.1 Building the Module (การ Build โมดูล)
...
## 11.8.3.2 Running Unit Tests (การรัน Unit Test)
...
## 11.8.3.3 Verification Test Case (กรณีทดสอบการทวนสอบ)
...
## 11.8.4 Performance Profiling and Validation (การทำ Profiling ประสิทธิภาพและการตรวจสอบความสมเหตุสมผล)

## 11.8.4.1 Profiling Build Configuration (การกำหนดค่า Profiling Build)
...
## 11.8.4.2 Validation Metrics (เมตริกการตรวจสอบความสมเหตุสมผล)
...
## 11.9 Concept Checks (ตรวจสอบความเข้าใจ)
...
## 11.10 References & Related Days (เอกสารอ้างอิงและบทเรียนที่เกี่ยวข้อง)

## 11.10.1 Core Academic & Technical References (เอกสารอ้างอิงทางวิชาการและเทคนิคหลัก)

## 11.10.1.1 Primary Literature on the Lee Model (วรรณกรรมหลักเกี่ยวกับ Lee Model)
...
## 11.10.1.2 Foundational Texts on Numerical Methods for Phase Change (ตำราพื้นฐานเกี่ยวกับระเบียบวิธีเชิงตัวเลขสำหรับการเปลี่ยนเฟส)
...
## 11.10.1.3 Practical Implementation Guides (คู่มือการนำไปใช้งานจริง)
...
## 11.10.2 Related Days in the Curriculum (บทเรียนที่เกี่ยวข้องในหลักสูตร)

## 11.10.2.1 Prerequisite Days (พื้นฐานที่จำเป็น)
...
## 11.10.2.2 Successive Days (การต่อยอดจากพื้นฐานนี้)
...
## 11.10.3 Links to External Modules (ลิงก์ไปยังโมดูลภายนอก)
