...
### B. R410A Solver Selection Guide ⭐

For **R410A two-phase flow simulations**, solver selection is **critical** due to:
- Large density ratio (17:1)
- Property discontinuities at interface
- Strong coupling between equations
- Boundedness requirements for VOF

### Recommended Solvers
| Variable | Solver | Preconditioner | Why |
|----------|--------|----------------|-----|
| **p_rgh** (pressure) | GAMG | DIC | Fast for elliptic pressure equation |
| **alpha.water** (VOF) | PBiCGStab | DILU | Handles strong convection |
| **U** (momentum) | smoothSolver | DIC | Robust for high density ratio |
| **h** (energy) | PBiCGStab | DILU | Handles source term coupling |

**⭐ Key Recommendations:**

1. **Pressure equation:**
   - Use **GAMG** with **DIC smoother**
   - Set `nCellsInCoarsestLevel 50` for good coarse grid
   - Typical iterations: 20-40 (vs. 200+ with PCG)

2. **Alpha equation:**
   - Use **PBiCGStab** with **DILU** for better convection handling
   - Ensure boundedness with `MULESCorr yes`
   - Add `nAlphaCorr 2` for stability

3. **Momentum equation:**
   - Use **smoothSolver** with **DIC** for high density ratio stability
   - Handles strong convection and property jumps
   - Typical iterations: 10-30

**Performance comparison** (R410A evaporator, 200K cells):

| Configuration | Pressure Iters | Total Time | Stability |
|---------------|----------------|------------|-----------|
| PCG + DIC | 150-250 | 2.5× slower | ⚠️ Marginal |
| PBiCGStab + DILU (p) | 80-120 | 2× slower | ⚠️ Marginal |
| **GAMG + DIC** | **20-40** | **Fastest** | ✅ Stable |

### C. Complete fvSolution for R410A ⭐
...
// ===== SOLVERS FOR R410A TWO-PHASE EVAPORATOR =====
solvers
{
    // === PRESSURE: GAMG MULTIGRID ===
    // Most critical solver - handles density ratio 17:1
    p_rgh
    {
        solver          GAMG;
        tolerance       1e-07;
    ...
    // === MOMENTUM: PBiCGStab ===
    // Handles convection and property jumps
    U
    {
        $p_rgh;          // Use same config as pressure
        tolerance       1e-06;
        relTol          0.1;
        minIter         1;
        maxIter         50;
...
### D. Troubleshooting Solver Issues for R410A

| Problem | Symptom | Cause | Solution |
|---------|---------|-------|----------|
| **Pressure diverges** | Residuals increase, NaN | Poor initial guess | Reduce `deltaT`, set `nCorrectors 3` |
| **Alpha unbounded** | α < 0 or α > 1 | Insufficient MULES | Set `MULESCorr yes`, `nLimiterIter 5` |
| **Slow convergence** | > 100 iterations | Wrong preconditioner | Switch pressure to GAMG |
| **Mass imbalance** | Inlet ≠ outlet flux | Flux reconstruction | Set `correctPhi yes` |
| **Vapor quality wrong** | x ≠ expected | Energy equation issues | Check `h` boundary conditions |
