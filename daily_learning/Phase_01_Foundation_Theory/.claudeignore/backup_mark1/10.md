...
## Part 5: R410A VOF Implementation Guide ⭐

### 5.1 Complete R410A VOF Equation ⭐

For **R410A evaporator simulations**, the VOF equation includes **phase change source terms**:

$$\frac{\partial \alpha}{\partial t} + \nabla \cdot (\alpha \mathbf{U}) + \nabla \cdot [\alpha(1-\alpha)\mathbf{U}_r] = \frac{\dot{m}}{\rho_l}$$

Where:
- $\alpha$ = Liquid volume fraction (α = 1: liquid, α = 0: vapor)
- $\mathbf{U}$ = Mixture velocity
- $\mathbf{U}_r$ = Relative velocity between phases (m/s)
- $\dot{m}$ = Phase change mass transfer rate (kg/m³s)
- $\rho_l$ = Liquid density of R410A (kg/m³)

**⭐ R410A-specific considerations:**

1. **Density ratio impact:** $\rho_l/\rho_v \approx 17$ at 10°C
2. **Interface sharpness:** Critical for accurate evaporation prediction
3. **Mass conservation:** Refrigerant mass must be conserved precisely (< 0.1% error)
4. **Temperature dependency:** Saturation temperature varies with pressure
5. **Surface tension effects:** σ = 0.008-0.012 N/m for R410A

### 5.2 OpenFOAM Implementation for R410A ⭐

**File:** `alphaEqnSubCycle.H` (R410A-specific with MULES)

```cpp
{
    // === R410A INTERFACE COMPRESSION ===
    // Calculate compression flux
    surfaceScalarField phic(mixture.cAlpha()*mag(phi/mesh.magSf()));

    // Limit compression flux
    phic = min(interface.cAlpha()*phic, max(phic));

    // Compression velocity at faces (normal to interface)
    surfaceScalarField phir(phic*interface.nHatf());

    // === MULES EXPLICIT SOLVE FOR R410A ===
    for (int gCorr=0; gCorr<nAlphaCorr; gCorr++)
    {
        // Create bounded alpha field
        volScalarField alpha1("alpha1", alpha1);
        ...
        // === R410A PHASE CHANGE SOURCE ===
        // Add mass transfer term to alpha equation
        if (phaseChange->modelsMassTransfer())
        {
            const volScalarField& mDotLiquid = phaseChange->mDotLiquid();
            const volScalarField& mDotVapor = phaseChange->mDotVapor();

            // Source term: S = ṁ/ρ
            // Liquid → vapor: alpha decreases
            alphaPhi += fvc::flux(phi) * (mDotLiquid / rho1 - mDotVapor / rho2);
        }
        ...
        // === UPDATE FLUXES FOR R410A ===
        // Mass flux: ρφ = α*ρ_l*φ + (1-α)*ρ_v*φ
        rhoPhi = alphaPhi*(rho1 - rho2) + phi*rho2;
...
### 5.3 MULES Configuration for R410A ⭐
...
PIMPLE
{
    ...
    nAlphaSubCycles 2;           // ✅ Sub-cycling for Co > 0.3
    cAlpha          1;           // ✅ Compression factor
...
**⭐ Key MULES parameters for R410A:**

| Parameter | Range | Recommended for R410A | Effect |
|-----------|-------|---------------------|--------|
| `nAlphaCorr` | 1-5 | **2** | More iterations = sharper interface |
| `nAlphaSubCycles` | 1-4 | **2** | Sub-cycling for high Co numbers |
| `cAlpha` | 0.0-1.0 | **1** | Interface compression strength |
| `nLimiterIter` | 1-10 | **3** | Boundedness enforcement |

**Effect of cAlpha on R410A interface:**

| cAlpha | Interface Thickness | Stability | Use Case |
|--------|---------------------|-----------|----------|
| 0.0 | > 10 cells (very diffused) | Very stable | Initial testing only |
| 0.5 | 4-5 cells | Stable | Moderate mass flux |
| **1.0** | **2-3 cells** | **Stable** | ✅ **Recommended for R410A** |

### 5.4 Interface Thickness for R410A ⭐

The **interface thickness** depends on:

1. **Mesh resolution** (cell size Δx)
2. **Compression factor** (cAlpha)
3. **Numerical scheme** (limiter type)
4. **Courant number** (Co)

**Theoretical estimate:**

$$ \delta \approx 3 \Delta x $$

Where $N_{cells}$ = number of cells across interface ($0.1 < α < 0.9$)

**For R410A evaporator (5mm tube):**

| Mesh Size | Δx (mm) | cAlpha | Interface Thickness | Cells Across Interface |
|-----------|---------|--------|---------------------|------------------------|
| 50K | 0.10 | 0.5 | 0.5 mm | 5 cells (diffuse) |
| 100K | 0.08 | 1.0 | 0.24 mm | 3 cells (sharp) ✅ |
| 200K | 0.06 | 1.0 | 0.18 mm | 3 cells (very sharp) ✅ |

**For 5mm tube with 100 cells: δ ≈ 0.15 mm**

**⭐ Recommendation:** Use **cAlpha = 1.0** with **mesh resolution ≤ 0.1mm** for R410A evaporators.

### 5.5 Validation Test Cases for R410A ⭐

#### Test 1: Static Drop

**Setup:**
- Liquid R410A droplet (α = 1) in vapor (α = 0)
- No gravity, no flow
- Should remain static

**Validation criteria:**
- Mass conservation: $\Delta m < 10^{-6}$
- Shape preservation: Interface area change < 1%
- No spurious currents: $\max|\mathbf{U}| < 10^{-8}$ m/s

#### Test 2: Linear Advection

**Setup:**
- Interface advected by uniform velocity $\mathbf{U} = 0.1$ m/s
- No phase change
- Periodic boundaries

**Validation criteria:**
- Interface shape preserved after translation
- Mass error < 0.1%
- No numerical diffusion beyond expected TVD behavior

#### Test 3: Zalesak Disk

**Setup:**
- Slotted disk in rotating flow field
...
#### Test 4: R410A Evaporation

**Setup:**
- Heated wall at bottom
- Liquid R410A initially at bottom
- Vapor phase at top
- Phase change: liquid → vapor

**Validation criteria:**
- Mass conserved (liquid mass decrease = vapor mass increase)
- Energy balance: $Q_{in} = \dot{m} h_{lv}$
- Interface moves at physically correct rate

### 5.6 Common Pitfalls and Solutions for R410A

| Pitfall | Symptom | Cause | Solution |
|---------|---------|-------|----------|
| **Interface smearing** | α not 0 or 1 | cAlpha too low | Increase cAlpha to 1.0 |
| **Mass loss** | $\int α dV$ decreases | MULES disabled | Set `MULESCorr yes` |
| **Unbounded α** | α < 0 or α > 1 | Co > 0.5 | Reduce `maxAlphaCo` to 0.3 |
| **Spurious currents** | Velocity in quiescent region | Poor curvature | Refine mesh at interface |
| **Oscillations** | Wiggles near interface | Wrong limiter | Switch from superbee to van Leer |

## Task #5 Verification Summary

✅ **Complete VOF Equation with R410A phase change term**
- Equation includes relative velocity term: $\nabla \cdot [\alpha(1-\alpha)\mathbf{U}_r]$
- Mass transfer source term: $\frac{\dot{m}}{\rho_l}$

✅ **OpenFOAM Implementation**
- MULES explicit solve implementation added
- Code example with boundedness enforcement

✅ **MULES Configuration**
- Settings for R410A: nAlphaCorr=2, nAlphaSubCycles=2, cAlpha=1
- Complete configuration block provided

✅ **Interface Thickness Calculation**
- Theoretical formula: $\delta \approx 3 \Delta x$
- Example for 5mm tube with 100 cells: δ ≈ 0.15 mm

✅ **Validation Test Cases**
- Static Drop test
- Linear Advection test
- Zalesak Disk test
- R410A Evaporation test

✅ **Common Pitfalls and Solutions**
- Interface smearing, mass loss, unbounded α issues
- Solutions provided for each pitfall
