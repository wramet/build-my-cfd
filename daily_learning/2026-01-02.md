...
> [!INFO] **สรุป Section 1**
> Section 1 introduces the Finite Volume Method (FVM) as the foundation for CFD simulations, explaining how conservation equations are discretized on computational meshes. The design decisions highlight critical considerations for two-phase evaporator flow, including the mandatory expansion source term in the pressure equation to account for phase change density differences, turbulence modeling requirements for Reynolds numbers above 2300, and the importance of proper thermodynamic property handling for R410A/R32 refrigerants.
...
> [!INFO] **สรุป Section 4**
> Section 4 provides the complete C++ implementation of the FVM solver, including the FVMSolver orchestrator class that manages pressure-velocity coupling via SIMPLE/PISO algorithm, CSRMatrix for efficient sparse matrix storage, CG and BiCGStab linear solvers for symmetric and non-symmetric systems respectively, TwoPhaseSystem with Lee model for mass transfer calculations, and CoolPropWrapper for tabulated thermodynamic property lookup. The implementation emphasizes stability mechanisms including under-relaxation for pressure (0.3) and velocity (0.5), bounded volume fraction clamping, CFL-based time step control, and the critical expansion source term that prevents divergence in two-phase evaporator simulations with large density ratios.
...
### Question 1: Why does the pressure equation in two-phase evaporator flow require an expansion source term, while single-phase flow assumes $\nabla \cdot \mathbf{U} = 0$?

> **Answer:** In single-phase incompressible flow, mass conservation simplifies to $\nabla \cdot \mathbf{U} = 0$ because density is constant. However, in two-phase flow with evaporation, mass transfer between phases creates a **volume source** that cannot be ignored.
> 
> When liquid evaporates to vapor, the density decreases dramatically (R410A: $\rho_l \approx 1000$ kg/m³, $\rho_v \approx 50$ kg/m³). The same mass occupies 20x more volume! This expansion must be accounted for in the continuity equation:
> 
> $$
> \nabla \cdot \mathbf{U} = \dot{m}''' \left(\frac{1}{\rho_v} - \frac{1}{\rho_l}\right)
> $$
> 
> For R410A at evaporator conditions, the expansion factor $(1/50 - 1/1000) \approx 0.019$ m³/kg is **significant**. If this term is omitted from the pressure equation, the solver will violate mass conservation and typically **diverge within 10-20 iterations** due to the large density ratio.
> 
> In your implementation, this is why `TwoPhaseSystem::getExpansionTerm()` is called in `constructPressureEquation()` - the returned `Srho` field is added directly to the right-hand side vector `b`.

### Question 2: Why use CSR (Compressed Sparse Row) matrix format instead of OpenFOAM's LDU (Lower-Diagonal-Upper) format?

> **Answer:** This is a **trade-off between generality and simplicity**:
> 
> **LDU (OpenFOAM's choice):**
> - Optimized for finite volume meshes where coefficients are naturally organized by faces
> - Owner-neighbor relationship maps directly to lower-upper entries
> - More memory efficient for highly unstructured meshes
> - Complex to implement and debug
> 
> **CSR (Your engine's choice):**
> - Industry-standard format supported by all numerical libraries (Eigen, PETSc, etc.)
> - Simpler to implement: row pointers, column indices, values
> - Easier to debug - matrix structure is visible
> - Better documentation and community support
> - Slightly more memory for structured grids (but still $O(N)$)
> 
> For your evaporator simulation using **structured cylindrical meshes**, CSR is perfectly adequate. The 7-point stencil (for 3D structured grids) produces a predictable sparsity pattern that CSR handles efficiently. You can always switch to LDU later if you need unstructured mesh support for complex geometries.
> 
> **Implementation note:** Your `CSRMatrix::multiply()` method accesses memory sequentially, which is cache-friendly and enables SIMD optimizations - critical for performance in the solver loop.

### Question 3: Why does the momentum equation use BiCGStab while the pressure equation uses CG (Conjugate Gradient)?

> **Answer:** This choice is dictated by the **mathematical properties** of the matrices:
> 
> **Pressure Equation → CG Solver:**
> - The pressure Poisson equation $\nabla \cdot (rAU \nabla p) = \text{RHS}$ produces a **symmetric positive-definite (SPD)** matrix
> - CG is guaranteed to converge for SPD matrices
> - CG requires only 3 vector operations per iteration (very efficient)
> - With Jacobi preconditioning, convergence is typically achieved in 20-50 iterations
> 
> **Momentum Equation → BiCGStab Solver:**
> - The momentum equation includes **convection terms** ($\mathbf{U} \cdot \nabla \mathbf{U}$) that make the matrix **non-symmetric**
> - CG **cannot** solve non-symmetric systems (will fail or diverge)
> - BiCGStab handles non-symmetric matrices robustly
> - Requires more vector operations per iteration (7 vectors) but is necessary
> - More sensitive to initial guess and preconditioning quality
> 
> **Key insight:** You could use BiCGStab for both equations, but CG is **2-3x faster** for pressure. The solver selection in your `FVMSolver` class reflects this:
> ```cpp
> // Pressure: symmetric → CG
> linearSolver_ = std::make_unique<CGSolver>();
> 
> // Momentum: non-symmetric → BiCGStab
> BiCGStabSolver bicg;  // Used in solveMomentum()
> ```
> 
> **Warning:** If you add turbulence models or buoyancy terms, the pressure equation may become slightly non-symmetric. Monitor convergence - if CG starts failing, switch to BiCGStab or GMRES for pressure too.

### Question 4: Why is under-relaxation necessary for pressure and velocity, but not for volume fraction (alpha)?

> **Answer:** Under-relaxation is a **stability mechanism** for tightly coupled equations, while alpha is bounded by physics:
> 
> **Pressure and Velocity (SIMPLE coupling):**
> - Pressure and velocity are **strongly coupled** through the momentum and continuity equations
> - A change in velocity immediately affects pressure, which immediately affects velocity
> - Without under-relaxation, this feedback loop causes **oscillation and divergence**
> - Typical values: $\alpha_p = 0.3$ (pressure), $\alpha_U = 0.5$ (velocity)
> - The update is: $\phi_{new} = \alpha \phi_{calculated} + (1-\alpha) \phi_{old}$
> 
> **Volume Fraction (VOF equation):**
> - Alpha is **bounded by physics**: $0 \leq \alpha \leq 1$
> - The VOF equation is **hyperbolic** (advection-dominated), not elliptic
> - Under-relaxation would smear the interface and cause **numerical diffusion**
> - Instead of relaxation, you use **boundedness**: clamp alpha to $[10^{-6}, 1-10^{-6}]$
> - The MULES (Multidimensional Universal Limiter with Explicit Solution) approach in OpenFOAM does this implicitly
> 
> **Implementation in your code:**
> ```cpp
> // Pressure: under-relaxed
> pressure_[i] = pressureRelax_ * p_vec[i] + (1.0 - pressureRelax_) * pressure_[i];
> 
> // Alpha: bounded, not relaxed
> if (alpha_[i] < MIN_ALPHA) alpha_[i] = MIN_ALPHA;
> if (alpha_[i] > MAX_ALPHA) alpha_[i] = MAX_ALPHA;
> ```
> 
> **Rule of thumb:** Use under-relaxation for elliptic equations (pressure, velocity in steady-state). Use bounding for hyperbolic equations (VOF, scalars).

### Question 5: Why pre-generate CoolProp tables instead of calling CoolProp directly in the solver loop?

> **Answer:** This is a **performance optimization** that yields 100-1000x speedup:
> 
> **Direct CoolProp calls (slow):**
> - Each property lookup requires: equation of state evaluation → iterative solver → property calculation
> - Typical time: **0.1-1 ms per call**
> - For 1M cells × 10 properties × 1000 timesteps = **10^9 calls**
> - Total time: **10^6 - 10^7 seconds** (days to weeks!)
> 
> **Tabulated lookup (fast):**
> - Pre-generate table once at startup (1-10 seconds one-time cost)
> - Each lookup: **bilinear interpolation** (4 memory accesses + arithmetic)
> - Typical time: **0.1-1 μs per lookup**
> - Same problem: **10^3 - 10^4 seconds** (minutes to hours)
> 
> **Speedup factor:** 100-1000x
> 
> **Implementation strategy:**
> 1. At startup, call `CoolPropWrapper::generateTable()` for R410A over expected range:
>    - Temperature: 250-320 K (covers evaporator conditions)
>    - Pressure: 1-20 bar (covers operating range)
>    - Resolution: 50×50 = 2500 points (sufficient for bilinear accuracy)
> 2. In solver loop, use `CoolPropWrapper::lookup(T, p)` - O(1) time
> 3. Error analysis: bilinear interpolation error < 0.1% with sufficient resolution
> 
> **Memory trade-off:**
> - Table size: 2500 points × 5 properties × 8 bytes = **100 KB**
> - Negligible compared to field data (1M cells × 4 fields × 8 bytes = **32 MB**)
> 
> **When to use direct calls:** Only for **validation** or **offline analysis**. Never in the solver loop!
...
- Previous: [[2026-01-01]] - CFD Engine Project Setup and Development Environment
- Next: [[2026-01-03]] - Mesh Generation and Structured Grid Implementation
- See also: [[90_day_roadmap]], [[2026-01-10]] - Thermodynamics and CoolProp Integration, [[2026-01-15]] - Pressure-Velocity Coupling Algorithms
