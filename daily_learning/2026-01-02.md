...
3. **ออกแบบ (Design)** - Owner-Neighbor addressing system สำหรับ internal faces
   ...
   - รับประกัน conservation: $F_f^{owner} = -F_f^{neighbor}$
...
| $f_x$          | Interpolation Factor | น้ำหนักสำหรับ linear interpolation: $f_x = \frac{                 | \mathbf{C}_N - \mathbf{C}_f | }{  | \mathbf{C}_N - \mathbf{C}_P | }$  | dimensionless |
...
- Flux ที่เข้า neighbor: $F_f^{neighbor} = \mathbf{F}_f \cdot (-\mathbf{S}_f) = -F_f$

> [!IMPORTANT] **Conservation Property**
> เนื่องจาก $F_f^{owner} = -F_f^{neighbor}$ การ loop over faces หนึ่งครั้งคำนวณ flux เพียงครั้งเดียวแต่เพิ่มให้ owner และลบให้ neighbor ทำให้ net flux ของ domain เป็นศูนย์สำหรับ closed system
...
classDiagram
    class FiniteVolumeMesh {
        ...
        +getOwnerNeighbour() (const std::vector~int~&, const std::vector~int~&)
...
| Incorrect interpolation factor | 2nd order accuracy หาย | ใช้ geometric weight: $f_x = |C_N - C_f| / |C_N - C_P|$ |
...
> > > 
> > > 5. **Parallel communication ที่มีประสิทธิภาพ**: ใน parallel computation ต้องรู้ว่า哪些 faces เป็น internal (local), processor boundary (ต้อง communicate) หรือ true boundary
> > > 
