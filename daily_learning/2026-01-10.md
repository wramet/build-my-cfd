...
| ค่า C$ \alpha $ | ผลกระทบต่อ Interface | คำแนะนำการใช้งาน |
|--------|---------------------|-----------------|
| **C$ \alpha $ = 0** | ไม่มี compression term; interface จะหนาจาก numerical diffusion | ใช้สำหรับ debugging เท่านั้น |
| **C$ \alpha $ = 0.5** | Compression อ่อน; interface ยังค่อนข้างหนา (2-3 cells) | ใช้เมื่อต้องการ numerical stability สูง |
| **C$ \alpha $ = 1.0** | Compression ปกติ; interface คมชัด (1-2 cells) | ค่า default สำหรับส่วนใหญ่ |
| **C$ \alpha $ = 2.0** | Compression แรง; interface แคบมาก (1 cell) | ใช้เมื่อต้องการ interface ที่ sharp มาก |
| **C$ \alpha $ > 2.0** | Compression แรงเกินไป; อาจทำให้ interface unstable | หลีกเลี่ยง มักทำให้เกิด numerical instability |
...
2. **Time Step Restriction:**
   - เนื่องจากใช้ explicit discretization สำหรับ compression term จึงมี CFL condition เพิ่มเติม:
     \$text{CFL}_\alpha = \frac{|\mathbf{U}_r| \Delta t}{\Delta x} \leq \text{CFL}_{\text{max}}
     $$
...
4. **Limiter Application:** เรียกฟังก์ชัน `limit()` เพื่อ enforce bounds 0$ \le $ψ$ \le $1
...
|--------|---------------------|-------------------|----------------|
| **Normal Calculation** | n = ∇$ \alpha $/|∇$ \alpha $| (unstable near |∇$ \alpha $|=0) | n = ∇$ \alpha $/(|∇$ \alpha $| + **$ \delta $**) ด้วย **stabilization** $ \delta $ | ป้องกัน division by zero; $ \delta $ เป็น small number (~1e-8) ที่ stabilize calculation |
...
)
{
    ...
    // ∂$ \alpha $/∂t + $ \nabla $·(φ$ \alpha $) = 0 ใน discrete form
...
) const
{
    ...
    // Calculate compression flux: $ \phi $_r = C_$ \alpha $ n_f·S_f min(|U|, max|U|)
    ...
    // Compression flux ควรมีค่าสูงสุดที่ interface ($ \alpha $≈0.5)
...
Description
    ...
    1. Solve alpha transport equation: ∂$ \alpha $/∂t + $ \nabla $·(U$ \alpha $) + $ \nabla $·(U_r $ \alpha $(1-$ \alpha $)) = 0
    2. Calculate interface compression flux ตามสมการ: $ \phi $_r = C_$ \alpha $ n_f·S_f min(|$ \phi $|/|S_f|, maxVel)
    3. Apply MULES limiter เพื่อ enforce bounds ($ 0 \le \alpha \le 1 $)
    4. Update mixture properties: $ \rho $ = $ \alpha \rho_1 $ + (1-$ \alpha $)$ \rho_2 $, $ \mu $ = $ \alpha \mu_1 $ + (1-$ \alpha $)$ \mu_2 $
    5. Calculate surface tension force (optional): F_st = $ \sigma \kappa \nabla \alpha $

    Critical Implementation Details:
    - Compression term ต้องใช้ explicit discretization (fvc::) เท่านั้น
    - MULES ต้องใช้ explicit time integration
    - Interface normal คำนวณจาก ∇$ \alpha $ ด้วย stabilization $ \delta $
...
void AlphaTransportSolver::calculateInterfaceNormal()
{
    ...
    สมการ: n_f = (∇$ \alpha $)_f / (|(∇$ \alpha $)_f| + $ \delta $)
...
void AlphaTransportSolver::calculateCompressionFlux()
{
    ...
    $ \phi $_r = C_$ \alpha $ n_f·S_f min(|$ \phi $|/|S_f|, maxCompressionVel_)
    
    โดย:
    - C_$ \alpha $ คือ compression factor (typically 1.0)
    ...
    */
    ...
    if (debug)
    {
        ...
        Info<< "  C_$ \alpha $ = " << cAlpha_ << endl;
        Info<< "  max|$ \phi $|/|S| = " << maxMagPhiOverMagSf << endl;
        Info<< "  compressionVelScale = " << compressionVelScale << endl;
        Info<< "  min|$ \phi $_r| = " << minPhiComp << endl;
        Info<< "  max|$ \phi $_r| = " << maxPhiComp << endl;
    }
...
void AlphaTransportSolver::solveWithMULES()
{
    ...
    with Explicit Solution) เพื่อ enforce boundedness ($ 0 \le \alpha \le 1 $)
    
    สมการ: ∂$ \alpha $/∂t + $ \nabla $·(U$ \alpha $) + $ \nabla $·(U_r $ \alpha $(1-$ \alpha $)) = 0
    
    MULES algorithm steps:
    1. Calculate total flux: $ \phi $_{\text{total}} = $ \phi $ + $ \phi $_comp
    ...
    */
    ...
    // รวม convective flux ($ \phi $) และ compression flux ($ \phi $_comp)
    ...
    if (debug)
    {
        ...
        Info<< "  max($ \alpha $_new - $ \alpha $_old) = " << maxAlphaChange << endl;
...
{
    /*
    Enforce strict bounds ($ 0 \le \alpha \le 1 $) หลังจากการ solve
    ใช้ tolerance $ \alpha $_tol เพื่อป้องกัน numerical issues
    */
...
void AlphaTransportSolver::updateMixtureProperties()
{
    ...
    - Density: $ \rho $ = $ \alpha \rho_1 $ + (1-$ \alpha $)$ \rho_2 $
    - Viscosity: $ \mu $ = $ \alpha \mu_1 $ + (1-$ \alpha $)$ \mu_2 $
    */
...
*   **Day 11: Phase Change Theory (Evaporation/Condensation):** วันนี้จะขยาย VOF framework โดยเพิ่ม source term `$ \dot{m} $` (mass transfer rate) ลงในสมการ `alpha` และที่สำคัญที่สุดคือ **expansion term** ($ \nabla $·U = $ \dot{m} $(1/$ \rho $_v - 1/$ \rho $_l)) ลงใน pressure equation ซึ่งได้กล่าวถึงใน Hero Concept ของ Phase 1 การเข้าใจกลไกการขนส่งและ boundedness ของ `alpha` จาก [[Day 10: Two-Phase Fundamentals (VOF & MULES)|Day 10]] เป็น prerequisite ที่ขาดไม่ได้สำหรับการ implement phase change model (เช่น Lee model) ที่ต้องรักษา boundedness ของ `alpha` ภายใต้การมี source term
...
*   **Phase 2 (Geometry & Mesh):** คุณภาพของ interface ใน VOF ขึ้นอยู่กับ mesh quality เป็นอย่างมาก Mesh ที่หยาบหรือมี skewness สูงจะทำให้การคำนวณ ∇$ \alpha $ สำหรับหา interface normal และ curvature มี error สูง นำไปสู่ interface ที่แตกหรือมีแรง surface tension ที่ไม่ถูกต้อง (parasitic currents)
```
