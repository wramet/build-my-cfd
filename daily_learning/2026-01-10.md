...
// ************************************************************************* //
```
### 4.2 ไฟล์ Implementation: AlphaTransportSolver.C
...
> [!QUESTION] 6.1 คำถามที่ 1: ทำไม Interface Compression Term ใน VOF ถึงต้อง Discretize แบบ Explicit เท่านั้น?
>
> > [!SUCCESS]- เฉลย (คลิกเพื่ออ่านคำตอบ)
> > **คำตอบเชิงลึก:**
> > Interface Compression Term $\nabla \cdot (\mathbf{U}_r \alpha (1-\alpha))$ มีลักษณะเป็น **Negative Diffusion** (Anti-diffusion) ซึ่งทำหน้าที่บีบ Interface ให้แคบลง หากเรา Discretize แบบ Implicit (เอาไปใส่ใน Matrix Diagonal) จะทำให้ Matrix ขาดคุณสมบัติ Diagonal Dominance อย่างรุนแรง และทำให้ Solver ลู่ออก (Unbounded)
> > การทำ Explicit Discretization ช่วยให้เราควบคุม Flux นี้ได้โดยตรง และสามารถนำไปรวมกับ MULES Limiter เพื่อรับประกันว่าค่า $\alpha$ จะไม่หลุดช่วง $[0, 1]$

> [!QUESTION] 6.2 คำถามที่ 2: MULES แตกต่างจาก Flux Limiters ทั่วไป (เช่น Minmod, SuperBee) อย่างไร?
>
> > [!SUCCESS]- เฉลย (คลิกเพื่ออ่านคำตอบ)
> > **คำตอบเชิงลึก:**
> > Flux Limiters ทั่วไป (TVD schemes) ทำงานโดยการจำกัด Flux ที่หน้า Face *ก่อน* การแก้สมการ เพื่อลด Oscillations แต่ไม่สามารถรับประกัน Global Boundedness ได้ 100% ในทุกกรณี (โดยเฉพาะเมื่อมี Source Terms หรือบน Mesh ที่บิดเบี้ยวมาก)
> > MULES (Multidimensional Universal Limiter with Explicit Solution) ใช้วิธีการ **Global Optimization** โดยแก้สมการหาค่า Limiter $\lambda$ สำหรับทุก Face พร้อมกัน เพื่อให้มั่นใจว่าค่า $\alpha$ ในทุก Cell จะอยู่ในขอบเขตที่กำหนดอย่างเคร่งครัด (Strictly Bounded) แม้ในกรณีที่ยากลำบาก
...
2.  **Rusche, H. (2002).** *Computational fluid dynamics of dispersed two-phase flows at high phase fractions*. PhD Thesis, Imperial College London. (ต้นกำเนิดของ MULES ใน OpenFOAM)
3.  **Berberović, E., et al. (2009).** Drop impact onto a liquid layer of finite thickness: Dynamics of the cavity evolution. *Physical Review E*, 79(3), 036306. (อธิบาย Interface Compression Term)

### 7.2 ความเชื่อมโยงกับวันอื่น
*   **[[Day 09: Pressure-Velocity Coupling (SIMPLE, PISO, Rhie-Chow)|Day 09]]**: VOF Solver (interFoam) ใช้ PISO loop จาก Day 09 ในการแก้ Momentum และ Pressure
*   **[[Day 11: Phase Change Theory (Lee Model & Linearization)|Day 11]]**: ความเข้าใจเรื่อง $\alpha$ transport เป็นพื้นฐานสำคัญในการคำนวณ Mass Transfer ($\dot{m}$) ระหว่าง Phase ใน Day 11
