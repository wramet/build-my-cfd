...
*   **รายละเอียดเชิงลึก:** เข้าใจว่า VOF เป็น Eulerian method ที่ใช้ volume fraction ($\alpha$) ซึ่งเป็น scalar bounded field (0 ≤ α ≤ 1) เป็นตัวแปรหลักในการแสดงสถานะของ phase โดย α=0 แทน phase หนึ่ง (เช่น gas), α=1 แทนอีก phase (เช่น liquid) และค่า 0<α<1 บ่งชี้ถึงการมีอยู่ของ interface ภายใน cell นั้น สมการพื้นฐานเป็นสมการการขนส่ง (advection) ทั่วไป แต่ความท้าทายหลักคือการรักษา interface ให้ "sharp" (มีความหนาเชิงตัวเลขเพียง 1-2 เซลล์) และป้องกันไม่ให้ α หลุดออกจากขอบเขต [0, 1] เนื่องจาก numerical diffusion และ discretization errors
...
*   **รายละเอียดเชิงลึก:** ออกแบบกลไกสองชั้นเพื่อแก้ไขสองปัญหาหลักของ VOF: (1) **Interface Sharpening:** เพิ่ม compression term ที่เป็น nonlinear artificial flux ซึ่งมีค่าสูงสุดบริเวณ α=0.5 และเป็นศูนย์ที่ α=0 และ 1 เท่านั้น Flux นี้ทำงานในทิศทางของ interface normal ($\mathbf{n}_f$) เพื่อ "บีบ" interface ให้แคบลง โดยความแรงถูกควบคุมด้วย compression factor $C_\alpha$ (2) **Boundedness Enforcement:** ออกแบบและใช้ MULES algorithm ซึ่งเป็น explicit solver พิเศษที่ผสมผสาน high-resolution scheme (เช่น TVD) กับ limiter function (เช่น Sweby limiter) และ corrective step เพื่อบังคับให้ผลลัพธ์ $\alpha^{n+1}$ อยู่ในขอบเขต [0, 1] อย่างเคร่งครัดในทุกเซลล์ หลังการคำนวณ flux
*   **ความสำคัญ:** โดยปราศจากสองกลไกนี้ การจำลอง VOF จะล้มเหลว: interface จะกระจายหนาเหมือนหมอก และค่า α จะผิดพลาดจนทำลายความคงตัวของการคำนวณ (numerical stability)
...
*   **ความสำคัญ:** ไม่มี "ค่าที่ดีที่สุด" สำหรับทุกกรณี การเป็นผู้เชี่ยวชาญ VOF หมายถึงความสามารถในการวินิจฉัยปัญหา (เช่น interface หนาเกินไป, α หลุด bounds, interface distortion) และปรับ parameters ได้อย่างเหมาะสมเพื่อให้ได้สมดุลระหว่างความ sharp ของ interface, ความคงตัวเชิงตัวเลข, และความถูกต้องทางฟิสิกส์
...
## 10.1 สมการการขนส่ง Volume Fraction (Volume Fraction Transport Equation)
...
### 10.1.1 นิยามของ Volume Fraction (α)
...
### 10.1.2 สมการการอนุรักษ์สำหรับ Volume Fraction
...
### 10.1.3 Interface Compression Term
...
### 10.1.4 การคำนวณ Interface Normal Vector
...
### 10.1.5 Compression Factor (Cα) และการเลือกค่า

$C_\alpha$ เป็นพารามิเตอร์ที่สำคัญที่สุดตัวหนึ่งใน VOF method ซึ่งควบคุมความแรงของการบีบ interface:

| ค่า Cα | ผลกระทบต่อ Interface | คำแนะนำการใช้งาน |
|--------|---------------------|-----------------|
| **Cα = 0** | ไม่มี compression term; interface จะหนาจาก numerical diffusion | ใช้สำหรับ debugging เท่านั้น |
| **Cα = 0.5** | Compression อ่อน; interface ยังค่อนข้างหนา (2-3 cells) | ใช้เมื่อต้องการ numerical stability สูง |
| **Cα = 1.0** | Compression ปกติ; interface คมชัด (1-2 cells) | ค่า default สำหรับส่วนใหญ่ |
| **Cα = 2.0** | Compression แรง; interface แคบมาก (1 cell) | ใช้เมื่อต้องการ interface ที่ sharp มาก |
| **Cα > 2.0** | Compression แรงเกินไป; อาจทำให้ interface unstable | หลีกเลี่ยง มักทำให้เกิด numerical instability |
...
### 10.1.6 สมการเต็มรูปแบบสำหรับ VOF
...
### 10.1.7 เงื่อนไขขอบเขต (Boundary Conditions) สำหรับ α
...
### 10.1.8 ข้อควรระวังและคำเตือนสำคัญ
...
3. **Initial Condition:**
   - Initial $\alpha$ field ต้องเป็น bounded (0 ≤ α ≤ 1) เสมอ
...
### 10.1.9 สรุปสมการและตัวแปรสำคัญ
...
### 10.1.10 การเชื่อมโยงกับ Concepts ก่อนหน้า
...
## 3.1 Class Analysis: `MULES` (Multidimensional Universal Limiter with Explicit Solution)

คลาส `MULES` เป็นหัวใจของ bounded scalar transport ใน OpenFOAM โดยเฉพาะสำหรับสมการ volume fraction `α` มันไม่ใช่คลาสแบบดั้งเดิมที่เราสร้างออบเจ็กต์ แต่เป็น **namespace** ที่เต็มไปด้วยฟังก์ชัน static template functions ซึ่งเป็น design pattern ที่ชาญฉลาดสำหรับ algorithm ที่ต้องการ flexibility สูง

### 3.1.1 Header File Analysis (`src/finiteVolume/fvMatrices/fvMatrix/fvMatrix.H`)
...
### 3.1.2 Source File Analysis (`src/finiteVolume/fvMatrices/fvMatrix/MULES.C`)
...
**Algorithm Breakdown:**
1. **Time Scale Calculation:** คำนวณ `rDeltaT = 1/Δt` สำหรับใช้ใน limiter function
...
4. **Limiter Application:** เรียกฟังก์ชัน `limit()` เพื่อ enforce bounds 0≤ψ≤1

### 3.1.3 The `limit()` Function - Core of MULES
...
2. **Available Space Concept:** คำนวณว่าในแต่ละ cell มี "ที่ว่าง" สำหรับค่า ψ เพิ่มได้อีกเท่าไรก่อนจะเกิน bounds
...
### 3.1.4 What We Do DIFFERENTLY: MULES Implementation

| Aspect | Standard TVD Schemes | OpenFOAM's MULES | Why It Matters |
|--------|---------------------|------------------|----------------|
| **Boundedness Enforcement** | Enforce via flux limiters (may still have small violations) | **Strict enforcement** via iterative correction algorithm | สำหรับ volume fraction α, violation เล็กน้อย (α<0 หรือ α>1) ทำให้ properties คำนวณผิดและ solver diverge |
...
## 3.2 Class Analysis: `interfaceProperties`
...
### 3.2.1 Header File Analysis (`src/transportModels/interfaceProperties/interfaceProperties.H`)
...
### 3.2.2 Source File Analysis (`src/transportModels/interfaceProperties/interfaceProperties.C`)
...
### 3.2.3 Surface Tension Force Calculation
...
### 3.2.4 What We Do DIFFERENTLY: Interface Properties Calculation

| Aspect | Naive Implementation | OpenFOAM's Approach | Why It Matters |
|--------|---------------------|-------------------|----------------|
| **Normal Calculation** | n = ∇α/|∇α| (unstable near |∇α|=0) | n = ∇α/(|∇α| + **δ**) ด้วย **stabilization** δ | ป้องกัน division by zero; δ เป็น small number (~1e-8) ที่ stabilize calculation |
...
| **Surface Tension Model** | May implement different models | **CSF model only** (Continuum Surface Force) | CSF ง่ายและทำงานได้ดีสำหรับ大部分ปัญหา; models อื่นๆ (เช่น CSS) ต้องการ implementation เพิ่ม |

**Numerical Stability Trick:** การเพิ่ม δ ใน normalization ของ interface normal ไม่ใช่แค่ป้องกัน division by zero แต่ยังช่วย regularize calculation ใน regions ที่ ∇α ≈ 0 (away from interface) ซึ่งทำให้ numerical noise ลดลง
## 3.3 Class Analysis: `twoPhaseMixture`
...
ในส่วนนี้เราจะเจาะลึกการออกแบบคลาสหลักสำหรับการนำ VOF method ไปใช้ใน OpenFOAM framework การออกแบบต้องคำนึงถึงหลักการสำคัญสามประการ: **Boundedness (0≤α≤1)**, **Interface Sharpness**, และ **Conservative Property Transport**
...
classDiagram
    ...
    volScalarField <|-- twoPhaseMixture : contains
...
```text
## 4.2 Class Specifications รายละเอียด

### 4.2.1 Class: `AlphaTransportSolver`

คลาสนี้เป็น **Orchestrator** หลักที่ควบคุมการแก้สมการ transport ของ α โดยรวมการทำงานของ MULES, interface compression, และ boundedness enforcement เข้าด้วยกัน
...
) const
{
    ...
    // Calculate compression flux: φ_r = C_α n_f·S_f min(|U|, max|U|)
    ...
    // Compression flux ควรมีค่าสูงสุดที่ interface (α≈0.5)
...
### 4.2.2 Class: `InterfaceSharpener`
...
### 4.2.3 Class: `MULESHandler` (Wrapper Class)
...
## 5.1 ไฟล์ Header: `AlphaTransportSolver.H`
...
Description
    ...
    1. Solve alpha transport equation: ∂α/∂t + ∇·(Uα) + ∇·(U_r α(1-α)) = 0
    2. Calculate interface compression flux ตามสมการ: φ_r = C_α n_f·S_f min(|φ|/|S_f|, maxVel)
    3. Apply MULES limiter เพื่อ enforce bounds (0 ≤ α ≤ 1)
    4. Update mixture properties: ρ = αρ₁ + (1-α)ρ₂, μ = αμ₁ + (1-α)μ₂
    5. Calculate surface tension force (optional): F_st = σκ∇α

    Critical Implementation Details:
    - Compression term ต้องใช้ explicit discretization (fvc::) เท่านั้น
    - MULES ต้องใช้ explicit time integration
    - Interface normal คำนวณจาก ∇α ด้วย stabilization δ
...
public:
    ...
    // Member Functions
        ...
        //- Calculate interface thickness (จำนวนเซลล์ที่มี 0<α<1)
...
void AlphaTransportSolver::calculateInterfaceNormal()
{
    ...
    สมการ: n_f = (∇α)_f / (|(∇α)_f| + δ)
    
    โดย δ คือ small stabilization factor เพื่อป้องกัน division by zero
    */
    ...
    if (debug)
    {
        ...
        Info<< "  stabilization δ = " << delta_ << endl;
    }
...
void AlphaTransportSolver::calculateCompressionFlux()
{
    ...
    φ_r = C_α n_f·S_f min(|φ|/|S_f|, maxCompressionVel_)
    
    โดย:
    - C_α คือ compression factor (typically 1.0)
    - n_f คือ interface normal vector ที่ face
    - S_f คือ face area vector
    - φ คือ face flux จาก velocity field
    ...
    */
    ...
    if (debug)
    {
        ...
        Info<< "  C_α = " << cAlpha_ << endl;
        Info<< "  max|φ|/|S| = " << maxMagPhiOverMagSf << endl;
        Info<< "  compressionVelScale = " << compressionVelScale << endl;
        Info<< "  min|φ_r| = " << minPhiComp << endl;
        Info<< "  max|φ_r| = " << maxPhiComp << endl;
    }
...
void AlphaTransportSolver::solveWithMULES()
{
    ...
    with Explicit Solution) เพื่อ enforce boundedness (0 ≤ α ≤ 1)
    
    สมการ: ∂α/∂t + ∇·(Uα) + ∇·(U_r α(1-α)) = 0
    
    MULES algorithm steps:
    1. Calculate total flux: φ_{\text{total}} = φ + φ_comp
    ...
    */
    ...
    // รวม convective flux (φ) และ compression flux (φ_comp)
    ...
    if (debug)
    {
        ...
        Info<< "  max(α_new - α_old) = " << maxAlphaChange << endl;
        Info<< "  α error = " << alphaError << endl;
        Info<< "  min(α) = " << gMin(alphaNew) << endl;
        Info<< "  max(α) = " << gMax(alphaNew) << endl;
    }
...
{
    /*
    Enforce strict bounds (0 ≤ α ≤ 1) หลังจากการ solve
    ใช้ tolerance α_tol เพื่อป้องกัน numerical issues
    */
    ...
    */
    ...
    if (mixturePtr_.valid())
    {
        ...
        if (debug)
        {
            ...
            Info<< "  min(ρ) = " << gMin(rho.internalField()) 
                << " kg/m³" << endl;
            Info<< "  max(ρ) = " << gMax(rho.internalField()) 
                << " kg/m³" << endl;
            Info<< "  min(μ) = " << gMin(mu.internalField()) 
                << " Pa·s" << endl;
            Info<< "  max(μ) = " << gMax(mu.internalField()) 
                << " Pa·s" << endl;
        }
    }
}
...
```cmake# Day10_TwoPhaseFundamentals/CMakeLists.txt
...
```cmake# Day10_TwoPhaseFundamentals/src/CMakeLists.txt# สร้าง static library สำหรับ two-phase fundamentals
...
```cmake# Day10_TwoPhaseFundamentals/tests/CMakeLists.txt# ตั้งค่า Google Test (ถ้าใช้) หรือสร้าง custom test framework
...
> [!QUESTION] Concept Check 1: คำถามที่ 1: หลักการพื้นฐานของ VOF Method
>
...
> >
> > **คำตอบเชิงลึก:**
> > VOF method เป็น **Interface-Capturing Method** ที่ใช้ **Volume Fraction (α)** เป็นตัวแปรหลักในการระบุตำแหน่งของ interface โดย α มีค่าเป็น 0 หรือ 1 ใน bulk region ของแต่ละ phase และมีค่าระหว่าง 0 ถึง 1 ในเซลล์ที่ประกอบด้วย interface (mixed cells) หลักการทางคณิตศาสตร์อยู่บนพื้นฐานของ **Scalar Advection Equation**:
> >
> > $$
> > \frac{\partial \alpha}{\partial t} + \nabla \cdot (\mathbf{U} \alpha) = 0
> > $$
...
> > 2.  **vs. Level Set:** Level Set ใช้ **Signed Distance Function (φ)** ซึ่งมี gradient magnitude เป็น 1 เสมอ ทำให้คำนวณ curvature และ normal vector ได้แม่นยำกว่า อย่างไรก็ตาม Level Set ไม่รักษา volume conservation โดยธรรมชาติ ต้องมีขั้นตอน re-initialization ในขณะที่ VOF รักษา mass/volume ได้โดยตรงจากการ integrate α
> > 3.  **ข้อดีของ VOF:** 
> >     *   **Mass Conservation:** เนื่องจาก α แสดงถึง volume fraction โดยตรง การ integrate α เหนือ volume ให้ total volume ของ phase นั้นๆ
...
> >     *   **Interface Reconstruction:** การคำนวณ curvature และ normal vector จาก ∇α มี noise สูง ต้องใช้ smoothing
> >     *   **Boundedness Requirement:** ต้องรักษา 0 ≤ α ≤ 1 เสมอ ซึ่งเป็นข้อจำกัดทาง numerical
> >
...
>
> > [!SUCCESS]- เฉลย (คลิกเพื่ออ่านคำตอบ)
> > **คำถาม:** Interface Compression Term `∇·(U_r α(1-α))` ทำงานอย่างไรในการรักษา sharpness ของ interface? ทำไม term นี้ถึงเป็นศูนย์ที่ α=0 และ α=1? และการเลือกค่า Compression Factor `C_α` ส่งผลต่อ simulation อย่างไร?
> >
> > **คำตอบเชิงลึก:**
...
> >
> > 1.  **Mathematical Form:** Term อยู่ในรูป `∇·(U_r α(1-α))` โดย `U_r` คือ compression velocity ที่ชี้ในทิศทาง normal ของ interface (`n_f`) การคูณด้วย `α(1-α)` ทำให้ term นี้มีลักษณะเป็น **quadratic function** ของ α
> > 2.  **Why Zero at α=0 and α=1:** 
> >     *   ที่ α=0 (pure phase 2): `α(1-α) = 0*(1-0) = 0`
> >     *   ที่ α=1 (pure phase 1): `α(1-α) = 1*(1-1) = 0`
> >     *   ที่ α=0.5 (center of interface): `α(1-α) = 0.5*0.5 = 0.25` (ค่าสูงสุด)
> >     *   **Physical Interpretation:** Term นี้ถูกออกแบบมาให้มีผล **เฉพาะในบริเวณ interface เท่านั้น** (ที่ 0<α<1) โดยไม่รบกวน bulk region ที่เป็น pure phase ซึ่งเป็นคุณสมบัติที่สำคัญเพื่อป้องกันไม่ให้เกิด artificial mixing ใน region ที่ไม่ควรมี interface
> > 3.  **Compression Velocity (U_r):** คำนวณจาก `U_r = n_f * min(C_α * |φ|/|S_f|, max(|φ|/|S_f|))` โดย:
> >     *   `n_f = (∇α)_f / |(∇α)_f + δ|` คือ unit normal vector ของ interface ที่ face
> >     *   `|φ|/|S_f|` คือ characteristic velocity ที่ face
> >     *   `C_α` คือ compression factor ที่ควบคุมความแรงของการบีบ
> > 4.  **Effect of C_α:**
> >     *   `C_α = 0`: ไม่มี compression, interface จะกระจายตัวเนื่องจาก numerical diffusion
> >     *   `C_α = 1`: Standard compression, interface มีความหนาประมาณ 2-3 cells
> >     *   `C_α > 1` (เช่น 1.5-2.0): Stronger compression, interface แคบลงแต่เสี่ยงต่อ numerical instability และ interface distortion
> >     *   `C_α < 1`: Weak compression, interface หนาแต่ stable มากกว่า
> > 5.  **Discretization Requirement:** Term นี้ต้อง discretize แบบ **Explicit (ใช้ `fvc::`)** เท่านั้น เนื่องจากถ้าใช้ implicit discretization จะทำลาย boundedness property ของสมการ และอาจทำให้ α มีค่านอกขอบเขต (α<0 หรือ α>1)
> >
...
>
> > [!SUCCESS]- เฉลย (คลิกเพื่ออ่านคำตอบ)
> > **คำถาม:** MULES (Multidimensional Universal Limiter with Explicit Solution) ทำงานอย่างไรเพื่อรักษา boundedness (0≤α≤1)? ทำไม MULES ถึงต้องใช้ Explicit Time Integration เท่านั้น และไม่สามารถใช้กับ Implicit Schemes ได้?
> >
> > **คำตอบเชิงลึก:**
...
> >
> > 1.  **Flux Limiting with Sweby Limiter:** MULES ใช้ high-resolution scheme (เช่น `interfaceCompression`) ร่วมกับ flux limiter `ψ(θ)` ที่เป็น function ของ gradient ratio `θ`:
> >     $$
> >     \psi(\theta) = \max[0, \min(1, 2\theta), \min(2, \theta)]
> >     $$
> >     Limiter นี้รับประกันว่า face value `α_f` จะอยู่ระหว่าง upwind value และ high-order value โดยไม่เกิด oscillations
> >
...
> >     $$
> >     โดย `α_f^{n+1/2}` คือ face value ที่คำนวณจาก limiter ในขั้นตอน predictor
> >
> > 3.  **Bound Enforcement via Lambda Corrector:** หลังได้ `α^{n+1}` แล้ว MULES ตรวจสอบว่าแต่ละเซลล์มีค่าใน bounds หรือไม่ ถ้าไม่อยู่ใน bounds จะคำนวณ correction factors `λ` สำหรับแต่ละเซลล์และปรับ fluxes ใหม่เพื่อให้ `α` กลับมาอยู่ใน bounds
> >
...
> > 2.  **Limiter Dependency:** Flux limiter `ψ(θ)` ขึ้นอยู่กับ gradient ratio `θ` ซึ่งคำนวณจาก solution ที่ time step n สำหรับ explicit scheme แต่สำหรับ implicit scheme จะต้องใช้ solution ที่ time step n+1 ซึ่งยังไม่รู้ค่า ทำให้เกิด nonlinear coupling ที่แก้ไขยาก
...
>
> > [!SUCCESS]- เฉลย (คลิกเพื่ออ่านคำตอบ)
> > **คำถาม:** Continuum Surface Force (CSF) model `F_st = σκ∇α` ถูกนำไปใช้ใน momentum equation อย่างไร? อะไรคือความท้าทายในการคำนวณ interface curvature `κ` จาก volume fraction field `α`?
> >
> > **คำตอบเชิงลึก:**
...
> >     $$
> >     โดย `F_st = σκ∇α` เป็น **Volumetric Force** (หน่วย N/m³) ที่กระจายอยู่ใน interface region
> >
> > 2.  **Curvature Calculation:** Curvature `κ` คำนวณจาก divergence ของ interface normal:
> >     $$
> >     \kappa = -\nabla \cdot \mathbf{n} = -\nabla \cdot \left( \frac{\nabla \alpha}{|\nabla \alpha| + \delta} \right)
> >     $$
> >     โดย `δ` คือ small number เพื่อป้องกัน division by zero
> >
> > 3.  **ความท้าทายในการคำนวณ κ:**
> >     *   **Numerical Noise:** เนื่องจาก `α` เปลี่ยนจาก 0 เป็น 1 ภายใน 2-3 เซลล์เท่านั้น `∇α` จึงมีค่าเปลี่ยนแปลงอย่างรวดเร็ว ทำให้การคำนวณ `n = ∇α/|∇α|` มี noise สูง
> >     *   **Magnitude Variation:** `|∇α|` มีค่าสูงเฉพาะใน interface region และเกือบเป็นศูนย์ใน bulk region ทำให้ `n` ไม่ได้เป็น unit vector ที่ดีใน region ที่ห่างจาก interface
> >     *   **Curvature Sensitivity:** `κ` เกี่ยวข้องกับ second derivative ของ `α` ซึ่งไวต่อ numerical noise เป็นอย่างมาก
> >
> > 4.  **Techniques to Improve Curvature Calculation:**
> >     *   **Smoothing:** ใช้ smoothing กับ `α` field ก่อนคำนวณ gradient เช่น ใช้ `fvc::average` หรือ Gaussian filter
> >     *   **Height Function Method:** สำหรับ structured grid, ใช้ integration ของ `α` ในทิศทาง normal เพื่อ reconstruct interface height แล้วคำนวณ curvature จาก height function
...
> >
> > 5.  **OpenFOAM Implementation:** ใน class `interfaceProperties`, curvature ถูกคำนวณใน method `correct()` และ surface tension force ถูกคำนวณใน method `sigmaK()` ซึ่ง return `σκ∇α` เป็น `volVectorField`
> >
...
>
> > [!SUCCESS]- เฉลย (คลิกเพื่ออ่านคำตอบ)
> > **คำถาม:** Mixture properties (ρ, μ) คำนวณจาก α อย่างไร? การเปลี่ยนแปลงอย่างรวดเร็วของ properties ณ interface ส่งผลต่อ numerical stability ของ pressure-velocity coupling (PISO/SIMPLE) อย่างไร?
> >
> > **คำตอบเชิงลึก:**
...
> >     *   **Density:** `ρ = αρ₁ + (1-α)ρ₂` (arithmetic mean)
> >     *   **Viscosity:** มีหลายวิธี:
> >         *   Arithmetic mean: `μ = αμ₁ + (1-α)μ₂` (ใช้ในหลาย solver)
> >         *   Harmonic mean: `1/μ = α/μ₁ + (1-α)/μ₂` (เหมาะสำหรับ large viscosity ratios)
...
> >     $$
> >     โดย `A_p` เป็น diagonal coefficients จาก momentum equation ซึ่งขึ้นอยู่กับ `ρ` และ `μ`
> >
> > 3.  **Numerical Challenges:**
> >     *   **Large Property Ratios:** ถ้า `ρ₁/ρ₂` หรือ `μ₁/μ₂` มีค่าสูง (เช่น air-water: ρ_ratio ≈ 1000, μ_ratio ≈ 100) จะทำให้ coefficients ใน pressure equation เปลี่ยนแปลงอย่างรวดเร็วที่ interface
> >     *   **Interface Movement:** เมื่อ interface เคลื่อนที่ผ่าน cell, properties ใน cell นั้นเปลี่ยนจาก `ρ₂` เป็น `ρ₁` ทันที ทำให้ pressure equation มี coefficients ที่ไม่ต่อเนื่อง
...
> >     *   **Face Interpolation of Properties:** แทนที่จะใช้ cell-center properties ในการคำนวณ fluxes, ใช้ face-interpolated properties (`fvc::interpolate(ρ)`) เพื่อให้มีความต่อเนื่องมากขึ้น
> >     *   **Time Step Control:** ใช้ CFL condition ที่ conservative โดยพิจารณา both velocity และ interface movement
> >
> > 5.  **OpenFOAM Implementation:** ใน `twoPhaseMixture` class, method `rho()` และ `mu()` return property fields ที่คำนวณจาก `α` ปัจจุบัน ซึ่งถูกเรียกใช้ใน momentum equation และ pressure equation ในแต่ละ iteration
> >
...
> >     *   **หลักการ:** ในแต่ละ mixed cell (0<α<1) ทำการ reconstruct explicit interface เป็นเส้นตรง (2D) หรือระนาบ (3D) ที่มี orientation ตาม normal vector และ position ถูกกำหนดจาก α value
> >     *   **ขั้นตอน:** 
> >         1.  คำนวณ interface normal `n` จาก gradient ของ α หรือวิธีอื่นๆ
> >         2.  หา plane constant เพื่อให้ volume ใต้ plane เท่ากับ αV_cell
...
> >     *   **หลักการ:** ใช้ high-order spatial discretization (เช่น 5th-order WENO) ร่วมกับ artificial compression terms ที่ซับซ้อนกว่า `∇·(U_rα(1-α))`
...
### 8.1 เอกสารอ้างอิงหลัก (Primary References)
...
### 8.2 Source Code References (OpenFOAM v2312)
...
*   **Day 06 (Boundary Conditions):** `alpha` field ต้องมี boundary conditions ที่เหมาะสม เช่น `inlet` อาจใช้ `fixedValue` (α=1 สำหรับของเหลว), `outlet` ใช้ `inletOutlet` หรือ `zeroGradient` และ `walls` ใช้ `constantAlphaContactAngle` สำหรับ modeling contact angle
...
### 8.4 การเตรียมตัวสำหรับวันต่อไป (Preparation for Next Days)

*   **Day 11: Phase Change Theory (Evaporation/Condensation):** วันนี้จะขยาย VOF framework โดยเพิ่ม source term `ṁ` (mass transfer rate) ลงในสมการ `alpha` และที่สำคัญที่สุดคือ **expansion term** (∇·U = ṁ(1/ρ_v - 1/ρ_l)) ลงใน pressure equation ซึ่งได้กล่าวถึงใน Hero Concept ของ Phase 1 การเข้าใจกลไกการขนส่งและ boundedness ของ `alpha` จาก [[Day 10: Two-Phase Fundamentals (VOF & MULES)|Day 10]] เป็น prerequisite ที่ขาดไม่ได้สำหรับการ implement phase change model (เช่น Lee model) ที่ต้องรักษา boundedness ของ `alpha` ภายใต้การมี source term
...
*   **Phase 2 (Geometry & Mesh):** คุณภาพของ interface ใน VOF ขึ้นอยู่กับ mesh quality เป็นอย่างมาก Mesh ที่หยาบหรือมี skewness สูงจะทำให้การคำนวณ ∇α สำหรับหา interface normal และ curvature มี error สูง นำไปสู่ interface ที่แตกหรือมีแรง surface tension ที่ไม่ถูกต้อง (parasitic currents)
```
