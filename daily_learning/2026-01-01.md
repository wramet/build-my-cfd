...
[← Back to Top](#governing-equations--openfoam-implementation) | [Class Hierarchy →](#2-openfoam-class-hierarchy--implementation)
...
**คำอธิบายพจน์:**
- $\rho$ (rho) = **ความหนาแน่น** (density) [kg/m³] - มวลต่อหน่วยปริมาตร
- $\mathbf{U}$ = **สนามความเร็ว** (velocity field) [m/s] - เวกเตอร์ความเร็วทุกจุดในกาลเวลา
- $\nabla \cdot$ = **ตัวดำเนินการไดเวอร์เจนซ์** (divergence operator) - คำนวณการไหลออกจากปริมาตรควบคุม
- $t$ = **เวลา** (time) [s]

สำหรับ **การไหลแบบไม่อัดตัว (incompressible flow)**: $\nabla \cdot \mathbf{U} = 0$  
**ความหมาย:** ปริมาตรของของไหลที่ไหลเข้าและออกจากจุดใดๆ ต้องเท่ากัน (ไม่มีการสะสมปริมาตร)
...
**คำอธิบายพจน์:**
- $p$ = **ความดัน** (pressure) [Pa] - แรงดันตั้งฉากต่อผิวหน่วยพื้นที่
- $\boldsymbol{\tau}$ (tau) = **เทนเซอร์ความเค้น** (stress tensor) [Pa] - ความเค้นภายในของไหลจากความหนืด
- $\mathbf{g}$ = **ความเร่งเนื่องจากแรงโน้มถ่วง** (gravitational acceleration) [m/s²] = 9.81 m/s²
- $\rho \mathbf{U} \mathbf{U}$ = **flux ของโมเมนตัม** (momentum flux) - การไหลของโมเมนตัมตามความเร็ว
...
**คำอธิบายพจน์:**
- $e$ = **พลังงานภายในต่อหน่วยมวล** (specific internal energy) [J/kg] - พลังงานจลน์ + พลังงานโมเลกุล
- $\mathbf{q}$ = **flux ของความร้อน** (heat flux) [W/m²] - อัตราการถ่ายเทความร้อนต่อหน่วยพื้นที่
- $S_h$ = **แหล่งกำเนิดความร้อน** (heat source) [W/m³] - ความร้อนที่เพิ่มเข้ามาในระบบ
- $\boldsymbol{\tau} : \nabla \mathbf{U}$ = **การกระจายพลังงานเนื่องจากความเค้น** (viscous dissipation) - พลังงานที่สูญเสียจากความหนืด
...
**Summary:** This section covered the fundamental governing equations of fluid dynamics—continuity, momentum, energy, and general transport equations. These form the physical foundation that OpenFOAM implements numerically. Understanding these equations is crucial for interpreting solver behavior and debugging simulation results.

---

## 2. OpenFOAM Class Hierarchy & Implementation

[← Theory](#1-theory-core-equations--physics) | [Code Walkthrough →](#3-code-walkthrough)
...
**Summary:** This section detailed OpenFOAM's core class architecture, from field management (`GeometricField`, `fvMesh`) to equation solving (`fvMatrix`, linear solvers). The class hierarchy shows how OpenFOAM's object-oriented design enables flexible numerical scheme implementation while maintaining computational efficiency through sparse matrix storage and automatic memory management.

---

## 3. Code Walkthrough

[← Class Hierarchy](#2-openfoam-class-hierarchy--implementation) | [Dictionary Analysis →](#4-dictionary-analysis--configuration)
...
**Summary:** The code walkthrough examined three critical files in `simpleFoam`: `createFields.H` (initialization), `UEqn.H` (momentum equation assembly), and `pEqn.H` (pressure-velocity coupling via PISO algorithm). Understanding these files reveals how OpenFOAM translates mathematical equations into sparse matrix systems and solves them iteratively while enforcing mass conservation.

---

## 4. Dictionary Analysis & Configuration

[← Code Walkthrough](#3-code-walkthrough) | [Hands-on Tasks →](#5-hands-on-practical-tasks--coding)
...
solvers
{
    ...
    {
        solver          PBiCGStab;
        preconditionerUSER: Replace '' with:
...
I need to replace the `` comment in the daily_learning/2026-01-01.md file with a comprehensive analysis of OpenFOAM dictionary settings. Let me create a detailed section covering fvSchemes and fvSolution with Thai explanations.
...
**Summary:** This section analyzed OpenFOAM's configuration dictionaries (`fvSchemes` and `fvSolution`), which control numerical discretization and linear solver behavior. Key takeaways: (1) Scheme selection balances accuracy vs. stability—upwind for stability, central/linear for accuracy; (2) Solver choice (GAMG, PCG, PBiCGStab) depends on matrix symmetry; (3) Relaxation factors prevent divergence in steady-state problems; (4) Non-orthogonal corrections improve accuracy on skewed meshes.

---

## 5. Hands-on: Practical Tasks & Coding

[← Dictionary Analysis](#4-dictionary-analysis--configuration) | [Concept Checks →](#6-concept-checks)
...
**Summary:** This section provided practical implementations of custom OpenFOAM functionality: (1) A blended convection scheme balancing stability and accuracy based on local Courant number; (2) A time-varying pressure boundary condition for pulsatile flow simulation; (3) An artificial compressibility solver for steady-state problems. These examples demonstrate how to extend OpenFOAM's capabilities by inheriting from base classes and leveraging the existing infrastructure.

---

## 6. Concept Checks

[← Hands-on Tasks](#5-hands-on-practical-tasks--coding) | [Recommended Reading →](#recommended-reading)
...
**Summary:** The concept checks reinforced key understanding: (1) Continuity equation (∇·U = 0) is enforced through pressure Poisson equation; (2) `fvm` builds implicit matrices while `fvc` computes explicit fields; (3) Pressure requires lower relaxation factors (0.2-0.4) than velocity (0.6-0.8) due to non-linear coupling; (4) PISO is for transient flow, SIMPLE for steady-state, controlled by `nOuterCorrectors`; (5) Non-orthogonal corrections are essential for accuracy on skewed meshes. Mastering these concepts is critical for effective CFD simulation with OpenFOAM.
...
[← Back to Table of Contents](#governing-equations--openfoam-implementation)
