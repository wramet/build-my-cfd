...
> **สรุป Section 1:** This section establishes the mathematical foundation for two-phase refrigerant flow, emphasizing the critical expansion source term in the continuity equation that accounts for volume change during phase change. We examined design trade-offs between accuracy and simplicity, and identified key pitfalls that cause solver divergence in boiling flows.
...
> **สรุป Section 4:** This section provides complete C++ implementation code for the CFD engine, including the Solver class with segregated PIMPLE algorithm, pressure equation with expansion term, Lee phase change model, and Mixing Length turbulence model. The implementation emphasizes under-relaxation for stability, pre-tabulated CoolProp properties for performance, and adaptive time stepping to handle the rapid dynamics of boiling flows.

---

## 6. Concept Checks

### Question 1: Why does the pressure equation for boiling flow include an expansion source term, and what happens if you omit it?

> **Answer:** 
> 
> In single-phase incompressible flow, the continuity equation is $\nabla \cdot U = 0$, which leads to a simple pressure equation: $\nabla^2 p = \nabla \cdot U^*$.
> 
> However, in two-phase flow with phase change, mass is **created or destroyed** at the interface. When liquid evaporates to vapor, the volume expands dramatically because $\rho_v \ll \rho_l$ (for R410A at 25°C: $\rho_l \approx 1000$ kg/m³, $\rho_v \approx 50$ kg/m³).
> 
> The correct continuity equation becomes:
> $$ \nabla \cdot U = \dot{m} \left( \frac{1}{\rho_v} - \frac{1}{\rho_l} \right) $$
> 
...
> - The pressure equation assumes $\nabla \cdot U = 0$ (incompressible)
> - When evaporation starts, volume expands but pressure equation doesn't account for it
> - Mass conservation is violated
> - **Solver will diverge within 10-20 iterations**
> 
> This is the #1 reason why boiling flow solvers fail. The expansion term is NOT optional—it's a fundamental physical requirement.

---

### Question 2: Why use a segregated solver (solve equations sequentially) instead of a fully coupled approach?

> **Answer:** 
> 
> **Segregated solver** (SIMPLE/PIMPLE):
> - Solve momentum → pressure → energy → alpha **sequentially**
> - Each equation is solved independently
> - Requires multiple outer iterations to couple equations
> - **Pros:** Easier to implement, debug, and test each equation independently; less memory usage
> - **Cons:** Slower convergence; requires under-relaxation for stability
> 
> **Fully coupled solver**:
> - Solve all equations **simultaneously** as one large matrix system
> - **Pros:** Faster convergence (fewer iterations); more stable for difficult problems
> - **Cons:** Extremely complex to implement; requires sophisticated linear solvers; difficult to debug; high memory usage
> 
> **For a learning engine:**
> - Segregated approach is the right choice
> - You can test and validate each equation before moving to the next
> - Start with single-phase flow, validate, then add phase change
> - Under-relaxation factors (0.3-0.7) compensate for the weaker coupling
> 
> OpenFOAM uses PIMPLE (merged PISO-SIMPLE) which is a segregated approach with multiple outer iterations—this is production-proven technology.
...
> - Each call takes ~100-1000 microseconds
> - For a 100k cell mesh, solving 100 time steps: $10^5 \times 10^2 = 10^7$ calls
> - Total time: **~1000-10000 seconds** (17-167 minutes) just for property lookups!
> 
> **Pre-tabulated lookup with bilinear interpolation:**
> - Generate table once at initialization (~1-10 MB memory)
> - Each interpolation takes ~0.1-1 microsecond
> - Same problem: **~1-10 seconds** for property lookups
> - **Speedup: 100-1000x**
> 
> **Trade-offs:**
> - **Memory vs Speed:** Table uses ~1-10 MB but makes lookups 1000x faster
> - **Accuracy:** Bilinear interpolation error < 1% with 100×100 grid
> - **Flexibility:** Pre-tabulation assumes fixed refrigerant (R410A or R32)
> 
> **Implementation strategy:**
> 1. At initialization: generate 2D tables (p, T) → (ρ, μ, h, k) for liquid and vapor
> 2. In solver loop: use bilinear interpolation to get properties
> 3. Table size: 100×100 = 10,000 points per property (negligible memory)
> 
> This is a no-brainer optimization for a production solver.

---

### Question 4: Why use the Mixing Length turbulence model instead of more advanced models like k-epsilon or k-omega SST?

> **Answer:** 
> 
> **Mixing Length model:**
> - Formula: $\nu_t = l_m^2 |\partial U/\partial y|$
> - **Pros:** Extremely simple to implement (1 line of code); no additional transport equations; computationally cheap; sufficient for pipe/annular flows
> - **Cons:** Not accurate for separated flows, complex geometries, or rapid changes in strain rate
> 
> **k-epsilon / k-omega SST:**
> - Solve 2 additional transport equations (turbulent kinetic energy k and dissipation ε or specific dissipation ω)
> - **Pros:** More accurate for complex flows; industry-standard for many applications
> - **Cons:** Requires solving 2 extra PDEs; more complex boundary conditions; stability issues near walls; requires wall functions or low-Re models
> 
> **For evaporator tube flow:**
> - Flow regime is primarily **annular** (well-predicted by Mixing Length)
> - Geometry is simple (straight tube)
> - Primary goal is predicting heat transfer coefficient (HTC)
> - Mixing Length with $l_m = 0.41y$ (near wall) gives reasonable HTC predictions
> - **Trade-off:** Accept 10-20% error in HTC for 10x simpler implementation
> 
> **Future enhancement:** Once the engine works, you can replace Mixing Length with k-epsilon as a drop-in module.

---

### Question 5: Why does the Lee model use temperature to drive phase change instead of pressure or superheat?

> **Answer:** 
> 
> **Lee model:**
> $$ \dot{m} = r \cdot \alpha \cdot \rho \cdot \frac{|T - T_{sat}|}{T_{sat}} $$
> - Phase change is driven by **temperature difference** from saturation
> - If $T > T_{sat}$: evaporation (liquid → vapor)
> - If $T < T_{sat}$: condensation (vapor → liquid)
> 
> **Why temperature-driven?**
> - **Simplicity:** Temperature is already solved in energy equation
> - **Physical intuition:** Superheated liquid evaporates; subcooled vapor condenses
> - **No extra equations:** Doesn't require solving bubble radius transport (like Schnerr-Sauer)
> 
> **Alternative models:**
> - **Schnerr-Sauer / Merkle:** Pressure-driven, based on Rayleigh-Plesset equation for bubble dynamics
> - More physically accurate for cavitating flows
> - Require solving additional transport equation for bubble radius or void fraction
> - More complex to implement and tune
> 
> **Trade-offs:**
> - **Lee model pros:** Simple; easy to implement; works well for boiling flows where temperature is the driving force
> - **Lee model cons:** Can cause spurious currents (interface instability) if relaxation coefficient $r$ is too large; requires careful tuning
> - **Pitfall:** Must clamp mass transfer to avoid creating more mass than exists in a cell
> 
> **For evaporator simulation:**
> - Wall temperature is fixed or heat flux is specified
> - Temperature gradient drives evaporation
> - Lee model is appropriate and widely used in boiling flow simulations
> - **Implementation tip:** Start with $r = 0.1$ s⁻¹ and increase gradually while monitoring stability
...
- Next: [[2026-01-02]] - Mesh Generation and Finite Volume Discretization
- See also: [[90_day_roadmap]], [[CFD_Fundamentals]], [[Thermodynamics_Properties]]
