...
```mermaid
flowchart LR
    A[∂ ρφ / ∂t<br/>Time Derivative] --> B[∇ · ρφU<br/>Convection]
    B --> C[∇ · Γ∇φ<br/>Diffusion]
    C --> D[Sφ<br/>Source Term]
    ...
    E[Rate of Change<br/>of φ in CV] --> F[Net Flux Out<br/>by Convection]
    F --> G[Net Flux In<br/>by Diffusion]
    G --> H[Source<br/>Generation]
    
    subgraph Balance
        I[Accumulation] --> J[Out - In + Source]
    end
```
...
```mermaid
flowchart TD
    A[Field to Solve] --> B{Matrix Type?}
    
    B -->|Symmetric| C{Problem Size?}
    B -->|Non-Symmetric| D{Problem Size?}
    
    C -->|Small| E[PCG<br/>Preconditioned Conjugate Gradient]
    C -->|Large| F[GAMG<br/>Geometric-Algebraic Multigrid]
    
    D -->|Small| G[PBiCGStab<br/>Preconditioned Bi-CG Stabilized]
    D -->|Large| H[smoothSolver<br/>Smoothing-based Solver]
    ...
    I -->|DIC| K[Diagonal Incomplete Cholesky]
    I -->|FDIC| L[Fast DIC for GAMG]
    I -->|none| M[No Preconditioning]
    
    J -->|DILU| N[Diagonal Incomplete LU]
    J -->|none| M
    ...
    O --> P{Converged?}
    P -->|No| Q{Max Iterations?}
    Q -->|No| O
    Q -->|Yes| R[Return Solution]
    P -->|Yes| R
...
```mermaid
flowchart TD
    subgraph PISO["PISO Algorithm nOuterCorrectors=1"]
        A1[Start Time Step] --> B1[Solve Momentum Eq]
        B1 --> C1{nCorrectors<br/>Loop}
        C1 --> D1[Solve Pressure Eq]
        D1 --> E1[Correct Flux]
        E1 --> F1{More<br/>Correctors?}
        F1 -->|Yes| D1
        F1 -->|No| G1[Correct Velocity]
        G1 --> H1[Update Turbulence]
        H1 --> I1[End Time Step]
    end
    
    subgraph PIMPLE["PIMPLE Algorithm nOuterCorrectors>1"]
        A2[Start Time Step] --> B2{nOuterCorrectors<br/>Loop}
        B2 --> C2[Solve Momentum Eq]
        C2 --> D2{nCorrectors<br/>Loop}
        D2 --> E2[Solve Pressure Eq]
        E2 --> F2[Correct Flux]
        F2 --> G2{More<br/>Correctors?}
        G2 -->|Yes| E2
        G2 -->|No| H2[Correct Velocity]
        H2 --> I2{More<br/>Outer Iters?}
        I2 -->|Yes| C2
        I2 -->|No| J2[Update Turbulence]
        J2 --> K2[End Time Step]
    end
    
    style PISO fill:#e3f2fd
    style PIMPLE fill:#f3e5f5
```
