...
)
{
    ...
    // => coefficient ไปที่ diagonal ของ owner และ off-diagonal จาก owner ไป neighbor
    if (flux > 0.0)
    {
        // Owner cell contribution
        diag[own] += flux;  // ϕ_own * flux
        
        // Neighbor cell contribution (implicit)
        // จากสมการ: flux * ϕ_own = flux * ϕ_own + 0 * ϕ_nei
        // ดังนั้นไม่มี contribution ไปที่ diagonal ของ neighbor
        // แต่มี negative contribution ใน off-diagonal
        upper[faceI] -= flux;  // negative เพราะอยู่ฝั่งขวาของสมการ
    }
    else  // flux <= 0 (ไหลจาก neighbor ไป owner)
    {
        // Neighbor cell contribution
        diag[nei] -= flux;  // ϕ_nei * (-flux) = positive contribution
        
        // Owner cell contribution (implicit)
        lower[faceI] += flux;  // positive เพราะ
...
*   **Day 08 (Iterative Solvers - CG, BiCGStab):**
    ...
    *   **Performance Consideration:** ประสิทธิภาพของ solver (ความเร็วในการลู่เข้า) ขึ้นอยู่กับคุณสมบัติของเมทริกซ์ที่เราสร้างในวันนี้เป็นอย่างมาก การที่เมทริกซ์มี condition number ที่ดี (ไม่แย่เกินไป) และเป็น diagonal dominant จะช่วยให้ solver ทำงานได้稳定และรวดเร็ว
...
3.  **Advanced Topic: Matrix-Free Methods:**
    *   **แนวคิด:** ในปัญหาที่ซับซ้อนมากๆ หรือมี mesh ที่เปลี่ยนแปลง动态 (dynamic mesh) การเก็บเมทริกซ์ `A` ไว้ใน memory แบบเต็มรูปแบบ (แม้จะเป็น sparse) อาจไม่ใช่ทางเลือกที่ดีที่สุด
