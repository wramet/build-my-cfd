...
1.  **การใช้ `static_assert` ใน `dimensionedSum` มีจุดประสงค์เพื่ออะไร?**
    <details>
    <summary>เฉลย</summary>
    เพื่อ **หยุดการ Compile ทันที** ถ้ามิติของตัวแปรสองตัวที่นำมาบวกกันไม่ตรงกัน มันเป็นการบังคับใช้กฏฟิสิกส์ในระดับ Compiler ทำให้แน่ใจว่าไม่มีทางที่จะเกิด executable file ที่คำนวณผิดตรรกะทางฟิสิกส์ได้
    </details>

2.  **ถ้าเราต้องการสร้างโมเดล Turbulence ใหม่ที่มีหน่วยแปลกๆ เราต้องไปแก้ Source Code ของ OpenFOAM หรือไม่?**
    <details>
    <summary>เฉลย</summary>
    **ไม่ต้อง** เพราะ OpenFOAM ออกแบบมาให้ขยายได้ (Extensible) เราสามารถสร้างคลาสใหม่ที่สืบทอด (Inherit) จาก `dimensionedType` หรือสร้าง plugin ที่มี `dimensionSet` ของตัวเองได้เลย โดยยังคงใช้ระบบตรวจสอบมิติเดิมของ OpenFOAM ได้
    </details>

3.  **ทำไมการใช้ `Expression Templates` ถึงช่วยให้โค้ดเร็วขึ้น?**
    <details>
    <summary>เฉลย</summary>
    เพราะมันทำช่วย **กำจัด Temporary Objects** (เช่น `temp = a + b; result = temp + c;`) และทำ **Lazy Evaluation** (ยังไม่บวกจริงจนกว่าจะถูกเรียกใช้ค่า) ทำให้ลดการจองหน่วยความจำและการ Copy ข้อมูลซ้ำซ้อน ซึ่งเป็นคอขวดสำคัญในการคำนวณ CFD ขนาดใหญ่
    </details>
