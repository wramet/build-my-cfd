...
## 🔍 แนวคิดระดับสูง: The Strict Doorman (คนเฝ้าประตูสุดเนี๊ยบ)

> [!TIP] **Physical Analogy: The Strict Doorman (คนเฝ้าประตูสุดเนี๊ยบ)**
>
> ลองจินตนาการถึง **คลับหรูระดับ High-end** (OpenFOAM Compiler) ที่มีกฏการแต่งกายเคร่งครัดมาก:
>
> -   **DimensionSet** คือ **"Dress Code"**: ผู้เข้างานต้องใส่สูท-ผูกไทเท่านั้น ห้ามใส่ชุดนอน (ห้ามหน่วยผิด)
> -   **Compiler Error** คือ **"คนเฝ้าประตู" (Bouncer)**: ถ้าคุณใส่รองเท้าแตะ (เอา Pressure + Velocity) คนเฝ้าประตูจะ **"ถีบ"** คุณออกมาทันทีตั้งแต่หน้าประตู (Compile-time Error) ไม่ยอมให้เข้าไปนั่งกินข้าวแล้วค่อยไล่ออก (Runtime Error)
> -   **Zero Overhead** คือ **"การคัดกรองแค่หน้าประตู"**: เมื่อเข้าไปในคลับแล้ว ทุกคนจะสนุกกันเต็มที่โดยไม่มีคนเฝ้าประตูมาเดินตามจ้องจับผิดตลอดเวลา (No Runtime Check) ทำให้ปาร์ตี้ (Simulation) ลื่นไหลไม่สะดุด
>
> ในขณะที่โค้ด CFD ทั่วไปเหมือน "ตลาดนัด" ที่ใครจะใส่อะไรมาก็ได้ (double + double) ซึ่งเสี่ยงมากที่จะเจอตีกันกลางงาน (Physics Mismatch Crashes)
...
1.  **ทำไม OpenFOAM ถึงเลือกเช็คหน่วย (Dimensions) ตอน Compile มากกว่าตอน Run โปรแกรม?**
    <details>
    <summary>เฉลย</summary>
    เพื่อให้ได้ **ประสิทธิภาพสูงสุด (Performance)** เพราะถ้าเช็คตอน Run โปรแกรม ทุกครั้งที่มีการบวกเลข คอมพิวเตอร์ต้องเสียเวลามานั่งเช็คหน่วย ทำให้ช้าลงมหาศาล แต่ถ้าเช็คตอน Compile มันจบตั้งแต่ตอนสร้างโปรแกรม ตอนรันจริงก็ใส่เต็มที่ได้เลย (Zero Overhead)
    </details>

2.  **`dimensionSet(1, -1, -2, 0, 0, 0, 0)` คือหน่วยของปริมาณอะไร?**
    <details>
    <summary>เฉลย</summary>
    คือ **ความดัน (Pressure)** หรือหน่วย Pascal ($Pa = N/m^2 = kg \cdot m/s^2 \cdot 1/m^2 = kg \cdot m^{-1} \cdot s^{-2}$)
    - Mass (M) = 1
    - Length (L) = -1
    - Time (T) = -2
    </details>

3.  **ฟังก์ชัน `pow(a, b)` ใน OpenFOAM มีข้อจำกัดเรื่องมิติอย่างไร?**
    <details>
    <summary>เฉลย</summary>
    ตัวเลขชี้กำลัง `b` ต้องเป็น **scalar ธรรมดา (ไร้มิติ)** เท่านั้น ส่วนฐาน `a` สามารถมีมิติได้ และผลลัพธ์จะมีมิติเป็น $Dimensions(a)^b$
    *แต่ถ้าเป็นฟังก์ชัน transcendental อื่นๆ เช่น `exp`, `sin`, `log` ทั้ง input และ output ต้องไร้มิติเท่านั้น*
    </details>