...
> [!TIP] **Physical Analogy: The Multi-Kitchen Banque (งานเลี้ยงโต๊ะจีนหลายครัว)**
>
> ลองจินตนาการงานเลี้ยงขนาดใหญ่ที่มีโต๊ะจีน 1,000 โต๊ะ (1 Million Cells):
>
> 1.  **Serial Processing**: มีครัวเดียว พ่อครัวคนเดียวทำทุกอย่าง ตั้งแต่หั่นผักยันเสิร์ฟ (ช้ามาก)
> 2.  **Parallel Processing**: แบ่งงานให้ 4 ครัว (Processors) ดูแลคนละ 250 โต๊ะ
> 3.  **Domain Decomposition**: การแบ่งโซนที่นั่ง ไม่ใช่สุ่มมั่ว แต่ต้องแบ่งให้แต่ละครัวเดินส่งอาหารใกล้ที่สุด (Minimize Interface) และจำนวนโต๊ะเท่ากัน (Load Balance)
> 4.  **Ghost Cells (Halo Data)**: คือ **"โต๊ะริมเขตแดน"** ที่ครัว A ต้องคอยมองว่าโต๊ะของครัว B ที่ติดกันกินเสร็จหรือยัง เพื่อจะได้เสิร์ฟจานต่อไปพร้อมกัน
> 5.  **MPI Communication**: คือเสียงตะโกนระหว่างหัวหน้าครัว "เฮ้ย! โต๊ะ 250 ขอเติมน้ำซุปหน่อย!" (Data Exchange)
...
1.  **Ghost Cells (หรือ Halo Cells) คืออะไร และทำไมเราถึงต้องการมัน?**
    <details>
    <summary>เฉลย</summary>
    Ghost Cells คือสำเนาข้อมูลของ Cells ที่อยู่ติดกับขอบเขตของ Processor เพื่อนบ้าน เราต้องการมันเพื่อคำนวณ Gradient หรือ Flux ที่รอยต่อ (Processor Boundary) โดยไม่ต้องส่งข้อมูลข้ามเครื่องตลอดเวลา (ลด Communication cost)
    </details>

2.  **ทำไมการสื่อสารแบบ Non-blocking (`MPI_Isend`, `MPI_Irecv`) ถึงมีประสิทธิภาพดีกว่าแบบ Blocking?**
    <details>
    <summary>เฉลย</summary>
    เพราะมันช่วยให้ CPU สามารถ **"ทำงานไปพร้อมกับการส่งของ"** (Computation-Communication Overlap) ได้ แทนที่จะยืนรอจนกว่าของจะส่งถึงมือผู้รับ ทำให้ลดเวลาว่าง (Idle time) ของ CPU ลงได้อย่างมาก
    </details>

3.  **ถ้าเราแบ่ง Mesh ให้ Processor หนึ่งมี 10,000 Cells แต่อีกตัวหนึ่งมี 100,000 Cells (Load Imbalance) จะเกิดผลเสียอย่างไร?**
    <details>
    <summary>เฉลย</summary>
    ความเร็วรวมของการจำลองจะถูกจำกัดด้วย **Processor ที่ช้าที่สุด** (ตัวที่มี 100,000 Cells) ส่วนตัวที่ทำงานเสร็จเร็วก็จะต้องนั่งรอ (Wait state) ทำให้เสียทรัพยากรไปโดยเปล่าประโยชน์
    </details>

---

**Next:** Review [[06_Common_Pitfalls|Common Pitfalls]] to avoid parallel programming mistakes.
