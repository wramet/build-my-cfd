...
> [!TIP] **Physical Analogy: The Train Compartments (ขบวนรถไฟ)**
>
> จินตนาการว่า `List` คือขบวนรถไฟ:
>
> 1.  **Contiguous Memory:** ตู้รถไฟเชื่อมต่อกันยาวเป็นขบวนเดียว (หน่วยความจำต่อเนื่อง) คุณสามารถเดินจากตู้แรกไปตู้สุดท้ายได้รวดเร็วโดยไม่ต้องกระโดดไปมา
> 2.  **Random Access:** ถ้าคุณรู้ว่าตู้เสบียงอยู่ตู้ที่ 5 (`list[4]`) คุณเดินตรงไปที่ตู้นั้นได้เลย ไม่ต้องเริ่มนับใหม่จากหัวขบวน (ต่างจาก Linked List)
> 3.  **Resize:** การเพิ่มตู้รถไฟต้องทำด้วยความระมัดระวัง (Expensive) คุณต้องปลดล็อกหัวรถจักร และอาจต้องย้ายผู้โดยสาร (Copy data) ไปขบวนใหม่ที่ยาวกว่าเดิมถ้าสถานี (Memory Block) ปัจจุบันไม่พอ
>
> ดังนั้น OpenFOAM จึงออกแบบให้เรา "จองตั๋ว" (Reserve) ขนาดขบวนให้พอดีตั้งแต่แรก เพื่อไม่ต้องเสียเวลาย้ายขบวนบ่อยๆ
...
1.  **ทำไม `List` ถึงเร็วกว่า `std::vector` ในบางกรณีของ OpenFOAM?**
    <details>
    <summary>เฉลย</summary>
    เพราะ `List` ของ OpenFOAM ถูกออกแบบมาให้เรียบง่าย (Lightweight) และทำงานร่วมกับระบบ IO และ Parallel Processing ของ OpenFOAM ได้โดยตรง ทำให้มี Overhead น้อยกว่า และมีการจัดการ Memory Alignment ที่เหมาะสมกับการคำนวณทางวิทยาศาสตร์
    </details>

2.  **ถ้าเรารู้จำนวน Elements ล่วงหน้า (เช่น จำนวนเซลล์) เราควรทำอย่างไรเพื่อให้ประสิทธิภาพสูงสุด?**
    <details>
    <summary>เฉลย</summary>
    เราควรใช้ `List(size)` หรือ `setSize(size)` ตั้งแต่ต้น (pre-allocation) แทนที่จะใช้ `append()` ทีละตัว เพราะจะช่วยลดการ Reallocate หน่วยความจำและการ Copy ข้อมูลซ้ำซ้อน ซึ่งเป็นกระบวนการที่กินเวลามาก
    </details>

3.  **`UList` ต่างจาก `List` อย่างไร?**
    <details>
    <summary>เฉลย</summary>
    `UList` (Unique List) **ไม่ได้เป็นเจ้าของ** หน่วยความจำที่มันชี้อยู่ มันทำหน้าที่เป็นเพียง "หน้าต่าง" (View) เพื่อเข้าไปดูหรือแก้ไขข้อมูลที่มีอยู่แล้วเท่านั้น ในขณะที่ `List` เป็นเจ้าของหน่วยความจำและรับผิดชอบในการสร้าง/ทำลาย (Allocate/Deallocate)
    </details>