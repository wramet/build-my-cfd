...
```cpp
// สร้าง dimensioned scalar (ความหนาแน่น)
dimensionedScalar rho
(
    "rho",                          // ชื่อ
    dimensionSet(1, -3, 0, 0, 0, 0, 0),  // มิติ: กก./ม³
    1.225                           // ค่า: 1.225 กก./ม³
);

// สร้าง dimensioned scalar (ความเร็ว)
dimensionedScalar U
(
    "U",
    dimensionSet(0, 1, -1, 0, 0, 0, 0),  // มิติ: ม./วินาที
    10.0                          // ค่า: 10 ม./วินาที
);
```
...
```cpp
// ถูกต้อง - ทั้งสองมีมิติเดียวกัน (ความดัน)
dimensionedScalar p1("p1", dimPressure, 101325.0);
dimensionedScalar p2("p2", dimPressure, 100000.0);
dimensionedScalar pTotal = p1 + p2;  // โอเค: ทั้งสองเป็นความดัน

// ข้อผิดพลาด - มิติไม่ตรงกัน
dimensionedScalar velocity("U", dimVelocity, 10.0);
dimensionedScalar pressure("p", dimPressure, 101325.0);
dimensionedScalar wrong = velocity + pressure;  // ข้อผิดพลาดเวลาคอมไพล์หรือรันไทม์
```
...
class dimensionSet
{
    // เลขชี้กำลังสำหรับ 7 มิติพื้นฐาน
...
{
    word name_;            // ชื่อปริมาณทางกายภาพ
...
```cpp
// ผิด - สมมติว่าค่ามีหน่วยเดียวกัน
scalar force1 = 10.0;  // จริงคือ 10 N
scalar force2 = 5.0;   // จริงคือ 5 lbf
scalar totalForce = force1 + force2;  // ไม่มีความหมายทางกายภาพ!

// ถูกต้อง - ใช้ dimensionedScalar
dimensionedScalar force1("force1", dimForce, 10.0);      // 10 N
dimensionedScalar force2("force2", dimForce, 22.241);    // 5 lbf แปลงเป็น N
dimensionedScalar totalForce = force1 + force2;          // ถูกต้อง: 32.241 N
```
...
```cpp
// ผิด - ความดันควรเป็น kg/(m·s²) ไม่ใช่ kg/m³
dimensionedScalar pWrong("p", dimensionSet(1, -3, 0, 0, 0, 0, 0), 101325.0);

// ถูกต้อง - ใช้ค่าคงที่มิติที่กำหนดไว้ล่วงหน้า
...
```cpp
// ผิด - เพียงตัวเลขโดยไม่มีหน่วย
scalar gravity = 9.81;

// ถูกต้อง - มีมิติกับหน่วย
dimensionedScalar g("g", dimAcceleration, 9.81);  // 9.81 ม./วินาที²
```
...
// การคำนวณเรย์โนลด์ส์
dimensionedScalar Re = rho * U * L / mu;  // ผลลัพธ์ไม่มีมิติ
```

ระบบ `dimensionedType` ทำให้มั่นใจว่า `Re` ไม่มีมิติจริง จับข้อผิดพลาดหากใช้ตัวแปรผิด
...
```cpp
// การคำนวณไกรเดียนท์ความดันยังคงความสอดคล้องทางมิติ
volVectorField gradP = fvc::grad(p);  // p [Pa] → gradP [Pa/m]

// สมการโมเมนตัมกับการตรวจสอบมิติ
fvVectorMatrix UEqn
(
    fvm::ddt(rho, U)                 // [kg/(m³·s)] × [m/s] = [Pa/m]
  + fvm::div(phi, U)                 // [kg/(m²·s)] × [m/s] = [Pa/m]
 ==
    fvc::div(tau)                    // [Pa/m] - การแพร่กระจายความเครียด
  + rho * g                          // [kg/m³] × [m/s²] = [Pa/m]
);
```
...
```cpp
// ทางเข้าความเร็วด้วยการตรวจสอบทางมิติ
...
// อนุพันธ์ตามเวลายังคงความสอดคล้องทางมิติ
fvScalarMatrix TEqn
(
    fvm::ddt(T)                   // [K/s]
  + fvm::div(phi, T)             // [K/s]
...
// การคำนวณการไหลพร้อมความสอดคล้องทางมิติ
