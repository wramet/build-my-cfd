...
> [!TIP] **Physical Analogy: The Mail Delivery Options (ตัวเลือกการส่งไปรษณีย์)**
>
> ลองนึกภาพคุณสั่งของออนไลน์ (`Reading`) หรือส่งของ (`Writing`):
>
> -   **MUST_READ:** เหมือน **"จดหมายลงทะเบียนที่สำคัญมาก"** (เช่น ใบแจ้งหนี้) ถ้าไปรษณีย์หาไม่เจอ (File not found) คุณจะเดือดร้อนทันที (Simulation Crash).
> -   **READ_IF_PRESENT:** เหมือน **"ใบปลิวโฆษณา"** ถ้ามีก็อ่าน ถ้าไม่มีก็ไม่เป็นไร ใช้ชีวิตต่อได้.
> -   **NO_READ:** เหมือน **"กระดาษเปล่า"** อย่าไปสนใจของเก่า เริ่มเขียนใหม่เลย.
> -   **AUTO_WRITE:** เหมือน **"สมัครสมาชิกนิตยสารรายเดือน"** ถึงเวลาก็ส่งมาให้เองอัตโนมัติ (Write every output time).
> -   **NO_WRITE:** เหมือน **"ไดอารี่ลับ"** เขียนไว้ดูคนเดียวใน Memory ไม่ต้องตีพิมพ์ส่งใคร.
...
1.  **ทำไมเราถึงไม่ใช้ `int` หรือ `double` ตรงๆ ใน OpenFOAM?**
    <details>
    <summary>เฉลย</summary>
    เพื่อให้โค้ดสามารถพกพาไปใช้ได้ทุกเครื่อง (Portability) และควบคุมความแม่นยำได้ตามต้องการ (Precision Control) โดยไม่ต้องแก้โค้ด เพียงแค่ Compile ใหม่เท่านั้น
    </details>

2.  **ถ้าเราพยายามนำ `velocity` บวกกับ `temperature` จะเกิดอะไรขึ้น?**
    <details>
    <summary>เฉลย</summary>
    จะเกิด **Compiler Error** ทันที เพราะระบบเช็ค Dimensions ของ OpenFOAM จะเห็นว่าหน่วยไม่ตรงกัน ($L/T$ vs $\Theta$) ซึ่งผิดกฎฟิสิกส์
    </details>

3.  **`IOobject` มีหน้าที่หลักคืออะไร?**
    <details>
    <summary>เฉลย</summary>
    เป็นเสมือน "ป้ายกำกับ" ให้กับ Object เพื่อบอกระบบว่าจะต้องอ่านไฟล์นี้หรือไม่ (Read option), ต้องเขียนลงไฟล์ไหม (Write option), และไฟล์นี้อยู่ที่ไหน (Path/Registry)
    </details>