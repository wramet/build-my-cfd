...
> [!INFO] **Overview**
> ตัวดำเนินการ Divergence1$\nabla \cdot1เป็นกระบวนการ fundamental ที่วัดปริมาณ **flux สุทธิ** ของสนามเวกเตอร์ผ่านพื้นผิวปิด ในบริบทของ CFD ตัวดำเนินการนี้ปรากฏอย่างกว้างขวางในสมการการอนุรักษ์ที่ควบคุมพลศาสตร์ของไหล
...
ตัวดำเนินการ divergence1$\nabla \cdot1เปลี่ยนสนามเวกเตอร์เป็นสนามสเกลาร์:

$$\nabla \cdot \mathbf{F} = \frac{\partial F_x}{\partial x} + \frac{\partial F_y}{\partial y} + \frac{\partial F_z}{\partial z}$$

โดยที่:
-1$\mathbf{F}1= สนามเวกเตอร์ (vector field)
-1$F_x, F_y, F_z1= องค์ประกอบของสนามเวกเตอร์
-1$\nabla \cdot1= divergence operator
...
-1$\rho1= ความหนาแน่นของของไหล
-1$\mathbf{u}1= เวกเตอร์ความเร็วของของไหล

**สำหรับการไหลแบบอินคอมเพรสซิเบิลที่1$\rho1คงที่:**
...
-1$p1= ความดัน
-1$\boldsymbol{\tau}1= เทนเซอร์ความเครียดของเชื้อเพลิง
-1$\mathbf{g}1= เวกเตอร์ความโน้มถ่วง
...
โดยที่:
-1$V1= ปริมาตรของควบคุม (control volume)
-1$A1= พื้นผิวขอบเขตของปริมาตรควบคุม
-1$\mathbf{n}1= เวกเตอร์หน่วยที่ตั้งฉากกับพื้นผิว
-1$\mathbf{S}_f = \mathbf{n}_f A_f1= เวกเตอร์พื้นที่หน้าที่ชี้ออกจากเซลล์
-1$\mathbf{U}_f \cdot \mathbf{S}_f1= ฟลักซ์ (Flux) ที่ไหลผ่านหน้าเซลล์1$f$
...
> - **การคำนาณ:**1$\sum_f \mathbf{U}_f \cdot \mathbf{S}_f1คือการรวม flux ทั้งหมดของหน้าเซลล์ทุกหน้า
...
สำหรับเซลล์การคำนาณที่มีปริมาตร1$V1และ1$N1หน้า:
...
> - การคำนาณภายในใช้ Gauss theorem:1$\sum_f \phi_f \cdot S_f / V$
...
> - **Flux Field:** `phi` คือ surfaceScalarField เก็บค่า1$\mathbf{U} \cdot \mathbf{S}_f1สำหรับทุกหน้าเซลล์
> - **Location:** สร้างใน solver createFields.H หรือคำนาณใน main loop
> - **Common Variables:**
>   - `phi` - volume flux ($\mathbf{U} \cdot \mathbf{S}_f$, หน่วย1$m^3/s$)
>   - `rhoPhi` - mass flux ($\rho \mathbf{U} \cdot \mathbf{S}_f$, หน่วย1$kg/s$)
> - **Usage:** Flux ถูกส่งผ่านระหว่างสมการต่างๆ เพื่อรักษา conservation

Surface flux1$\phi_f1สามารถคำนาณได้หลายวิธีขึ้นอยู่กับบริบททางฟิสิกส์:

| **ชนิด Flux** | **สมการ** | **การใช้งาน** |
|--------------|------------|----------------|
| **Mass Flux** |1$\phi_f = (\rho \mathbf{u})_f \cdot \mathbf{S}_f1| การอนุรักษ์มวล |
| **Momentum Flux** |1$\phi_f^{\text{mom}} = (\rho \mathbf{u} \mathbf{u})_f \cdot \mathbf{S}_f1| การอนุรักษ์โมเมนตัม |
| **Heat Flux** |1$q_f = (-k \nabla T)_f \cdot \mathbf{S}_f1| การถ่ายเทความร้อน |
...
|-----------|-----------|------------------|-------------------|
| **Dirichlet** |1$\phi = \phi_01| `fixedValue` | Flux ถูกกำหนดโดยค่า |
| **Neumann** |1$\frac{\partial \phi}{\partial n} = q_01| `fixedGradient` | Normal flux ถูกกำหนด |
| **Zero Gradient** |1$\frac{\partial \phi}{\partial n} = 01| `zeroGradient` | Normal flux = 0 |
...
|------------------|------------|----------------------------|
| **Mass Conservation** |1$\nabla \cdot \mathbf{U} = 01| `fvc::div(phi)` |
| **Compressible Flow** |1$\frac{\partial \rho}{\partial t} + \nabla \cdot (\rho \mathbf{U}) = 01| `fvc::div(rhoPhi)` |
| **Momentum Convection** |1$\nabla \cdot (\mathbf{U}\mathbf{U})1| `fvc::div(UU)` |
| **Stress Divergence** |1$\nabla \cdot \boldsymbol{\tau}1| `fvc::div(tau)` |
| **Heat Flux** |1$\nabla \cdot \mathbf{q}1| `fvc::div(q)` |
...
<details>
<summary><b>1. ทำไม1$\nabla \cdot U = 01จึงสำคัญสำหรับ incompressible flow?</b></summary>

$\nabla \cdot U = 01คือ **continuity equation** สำหรับ incompressible flow:
