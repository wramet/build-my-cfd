...
> - **การคำนวณ:** $\sum_f \mathbf{U}_f \cdot \mathbf{S}_f$ คือการรวม flux ทั้งหมดของหน้าเซลล์ทุกหน้า
...
// การคำนวณ divergence ของสนามเวกเตอร์ ได้ผลลัพธ์เป็นสนามสเกลาร์
volScalarField divU = fvc::div(U);

// Divergence of flux field (area-weighted)
// การคำนวณ divergence ของสนามฟลักซ์ (ถ่วงน้ำหนักด้วยพื้นที่)
volScalarField divPhi = fvc::div(phi);

// Divergence of convective flux with a scalar field
// การคำนวณ divergence ของฟลักซ์ convection ร่วมกับสนามสเกลาร์
...
> 1. `fvc::div(U)` - คำนวณ divergence ของสนามความเร็ว ได้ผลลัพธ์เป็นสนามสเกลาร์
> 2. `fvc::div(phi)` - คำนวณ divergence ของ surface flux field (มีหน่วย [m³/s])
> 3. `fvc::div(phi, T)` - คำนวณเทอม convection สำหรับสมการขนส่งสเกลาร์
>
> **🔑 Key Concepts:**
> - `fvc::div()` เป็น explicit operation คำนวณค่าโดยตรงจากสนามที่มีอยู่
> - ผลลัพธ์เป็น `volScalarField` สำหรับทุก cell ใน mesh
> - การคำนวณภายในใช้ Gauss theorem: $\sum_f \phi_f \cdot S_f / V$
...
// การคำนวณ divergence พร้อมระบุ interpolation scheme แบบ custom
volScalarField divCustom = fvc::div(
    upwind<scalar>(mesh, phi) & mesh.Sf()
);

// Divergence of tensor field (e.g., stress tensor)
// การคำนวณ divergence ของสนามเทนเซอร์ (เช่น เทนเซอร์ความเค้น)
...
> - Interpolation scheme ควบคุมวิธีคำนวณค่าที่ face centers
> - Divergence ของเทนเซอร์ใช้ในการคำนวณแรงจากความเค้น (stress forces)
...
// คำนวณ divergence แบบ explicit จากความเร็วปัจจุบัน
volScalarField divU = fvc::div(U);
...
> - **Explicit (`fvc::div`)**: คำนวณค่า divergence โดยตรง ใช้สำหรับ source terms หรือ post-processing
...
> - **Location:** สร้างใน solver createFields.H หรือคำนวณใน main loop
> - **Common Variables:**
...
> - `fvc::div(U)` คำนวณ divergence ของความเร็ว ซึ่งควรมีค่าเป็นศูนย์สำหรับ incompressible flow
...
> - ค่า divergence ที่ไม่เป็นศูนย์บ่งชี้ถึงความผิดพลาดในการคำนวณ
...
> - `sum(fvc::div(U) * mesh.V())` คำนวณผลรวมของ divergence × volume ในทุก cell
...
> - **Density equation**: `fvc::div(rhoPhi)` คำนวณ mass flux divergence
>
> **🔑 Key Concepts:**
...
> - **Best Practice:** ตรวจสอบ `.dimensions()` ก่อนการคำนวณ

### 10.1 ข้อผิดพลาดทั่วไปและวิธีแก้ไข

```cpp
// ERROR: Attempting divergence of a scalar field
// ข้อผิดพลาด: พยายามคำนวณ divergence ของสนามสเกลาร์
...
> - Monitor convergence ของ `max(divPhi)` ระหว่างการคำนวณ
...
> - **Flux Caching:** ใช้ `rhoPhi` ที่คำนวณไว้แล้ว ไม่ใช่คำนวณใหม่ทุกครั้ง
...
// ใช้ face fluxes ที่แคชไว้ (หลีกเลี่ยงการคำนวณซ้ำ)
volScalarField divRhoU = fvc::div(rhoPhi, U);
...
> - **Pre-calculate fluxes**: คำนวณ `phiNew` ทีเดียว ใช้ซ้ำหลายครั้ง
> - **Use cached fluxes**: `rhoPhi` มักถูกคำนวณแล้วใน solver step ก่อนหน้า
> - **Avoid redundant calculations**: เก็บผลลัพธ์ `divU` แทนการคำนวณซ้ำ
>
> **🔑 Key Concepts:**
