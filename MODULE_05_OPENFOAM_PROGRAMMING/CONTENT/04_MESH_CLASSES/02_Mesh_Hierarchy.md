...
> [!TIP] **Physical Analogy: The Human Body System (ระบบร่างกายมนุษย์)**
>
> เปรียบเทียบ 3 ชั้นนี้กับระบบในร่างกาย:
>
> 1.  **primitiveMesh (โครงกระดูก - Skeleton)**:
>     -   เป็นโครงสร้างพื้นฐานที่บอกรูปทรง (Points) และขนาด (Volumes, Areas)
>     -   รับรู้แค่ทางกายภาพว่ากระดูกชิ้นนี้ยาวเท่าไหร่ อยู่ตรงไหน
>
> 2.  **polyMesh (ระบบเส้นเอ็นและข้อต่อ - Connective Tissue)**:
>     -   เชื่อมกระดูกเข้าด้วยกัน (Connectivity) บอกว่ากระดูก A ต่อกับกระดูก B
>     -   กำหนดขอบเขตผิวหนัง (Boundaries) ว่าส่วนไหนคือ "ภายใน" ส่วนไหนคือ "ภายนอก"
>
> 3.  **fvMesh (ระบบหมุนเวียนเลือด - Circulatory System)**:
>     -   นำเลือด (Fields like U, p, T) ไหลเวียนไปตามเซลล์ต่างๆ
>     -   คำนวณการแลกเปลี่ยนสาร (Fluxes) ผ่านผนังเซลล์
>     -   นี่คือชั้นที่มี "ชีวิต" (Physics) เกิดขึ้นจริงๆ
...
1.  **ทำไม OpenFOAM ถึงออกแบบให้ `fvMesh` สืบทอดมาจาก `polyMesh` และ `polyMesh` สืบทอดมาจาก `primitiveMesh`? (Inheritance Direction)**
    <details>
    <summary>เฉลย</summary>
    เพราะเป็นความสัมพันธ์แบบ "is-a" ที่เพิ่มความเฉพาะเจาะจงขึ้นเรื่อยๆ:
    - `polyMesh` **เป็น** `primitiveMesh` ที่เพิ่มข้อมูล Connectivity และ Boundary
    - `fvMesh` **เป็น** `polyMesh` ที่เพิ่มความสามารถในการเก็บ Fields และคำนวณ FVM
    ทิศทางนี้ทำให้ Layer บน (fvMesh) เรียกใช้ความสามารถของ Layer ล่างได้ทั้งหมด แต่ Layer ล่างไม่ต้องรู้จัก Layer บน
    </details>

2.  **ถ้าเราเขียน Solver ใหม่ แล้วเผลอไปเรียกฟังก์ชัน `mesh.cellVolumes()` (จาก primitiveMesh) บ่อยๆ ในทุกๆ Time Step จะเกิดปัญหา Performance หรือไม่?**
    <details>
    <summary>เฉลย</summary>
    **ไม่เกิดปัญหา** เพราะ OpenFOAM ใช้ระบบ **Caching/Lazy Evaluation** ค่า Volume จะถูกคำนวณครั้งแรกครั้งเดียวและเก็บไว้ใน Cache การเรียกครั้งต่อๆ ไปจะเป็นการอ่านค่าจาก Memory เท่านั้น (เร็วมาก) ยกเว้นแต่ว่า Mesh จะมีการขยับ (Mesh Motion) ซึ่งจะทำให้ Cache ถูกล้าง
    </details>

3.  **ความหมายของ "Bottom-Up Dependency" ในบริบทนี้คืออะไร?**
    <details>
    <summary>เฉลย</summary>
    หมายถึง Layer ล่างสุด (`primitiveMesh`) เป็นรากฐานที่ไม่ขึ้นกับใครเลย ส่วน Layer บน (`fvMesh`) ต้องพึ่งพา Layer ล่างร100% ถ้า `primitiveMesh` พัง ทุกอย่างพัง แต่ถ้าเราแก้ `fvMesh` จะไม่กระทบ `primitiveMesh`
    </details>
