...
> [!TIP] **มุมมองเปรียบเทียบ: ห้องฝึกนักบิน (The Flight Simulator Analogy)**
> 
> การทำแบบฝึกหัดเหล่านี้เปรียบเสมือนการเข้า **"ห้องฝึกบินจำลอง (Flight Simulator)"**:
> *   **ทฤษฎี (Manual):** คุณอ่านมาแล้วว่าเครื่องบินบินได้อย่างไร (Navier-Stokes, Energy Eq.)
> *   **แบบฝึกหัด (Simulator):** นี่คือเวลาที่คุณจะได้จับคันบังคับจริงๆ
>     *   **CHT Exercise:** เหมือนการฝึกคุมเครื่องยนต์ไม่ให้ร้อนเกินไป (Thermal Management)
>     *   **FSI Exercise:** เหมือนการบินฝ่าพายุที่ปีกเครื่องบินสั่นไหว (Aeroelasticity)
> *   **Crash (Divergence):** ถ้าเครื่องตก (Simulation Error) ไม่ต้องตกใจ! มันคือส่วนหนึ่งของการเรียนรู้ ให้ดู Log file ว่าพลาดตรงไหนแล้วบินใหม่
...
<details>
<summary><b>1. ในแบบฝึกหัด CHT, ถ้าเราเปลี่ยนวัสดุจาก "อลูมิเนียม" เป็น "ไม้" เราต้องปรับค่าอะไรใน blockMesh หรือไม่?</b></summary>

**คำตอบ:** ไม่ต้องปรับ `blockMesh` แต่ต้องปรับ:
1.  `constant/solid/thermophysicalProperties`: เปลี่ยนค่า $k$ (Thermal Conductivity) และ $Cp$ (Heat Capacity)
2.  **Timestep ($\Delta t$):** ไม้นำความร้อนช้ากว่าอลูมิเนียมมาก อาจต้องปรับ time step หรือ relaxation factor ถ้าการเปลี่ยนแปลงอุณหภูมิช้าลงหรือเร็วขึ้นผิดปกติจนกระทบเสถียรภาพ
</details>

<details>
<summary><b>2. ในแบบฝึกหัด FSI ธงสะบัด, "Flutter Frequency" ขึ้นอยู่กับอะไรบ้าง?</b></summary>

**คำตอบ:** ขึ้นอยู่กับ:
1.  **Fluid Velocity ($U$):** ลมแรง สะบัดเร็ว
2.  **Flag Stiffness ($E$):** ธงแข็ง สะบัดยากหรือถี่
3.  **Flag Length ($L$):** ธงยาว ความถี่ต่ำ
4.  **Density Ratio ($\rho_f/\rho_s$):** อัตราส่วนความหนาแน่นส่งผลต่อ Added Mass และความเสถียรของการสั่น
</details>

<details>
<summary><b>3. ถ้า Simulation FSI ของเรา "ระเบิด" (Diverge) ทันทีที่ธงเริ่มขยับ เราควรแก้ที่ไหนก่อน?</b></summary>

**คำตอบ:**
1.  **ลด Time Step ($\Delta t$):** FSI ต้องการ time step ที่เล็กมากเพื่อจับ interaction
2.  **เพิ่ม Under-Relaxation:** ลดค่า relaxation factor ของ displacement หรือ interface force
3.  **ตรวจสอบ Mesh Quality:** ถ้า Mesh บิดเบี้ยวมากตอนธงขยับ Solver จะคำนวณผิดพลาด (อาจต้องใช้ Dynamic Mesh ที่ดีขึ้น)
</details>
