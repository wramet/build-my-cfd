...
> [!TIP] **มุมมองเปรียบเทียบ: การตรวจสอบบัญชี (The Accounting Audit Analogy)**
> 
> การทำ CFD Simulation โดยไม่มีการ Validation ก็เหมือนการ **"ทำธุรกิจโดยไม่มีผู้ตรวจสอบบัญชี"**:
> *   **Conservation Checks (Mass/Energy Balance):** คือการทำ **"งบดุล (Balance Sheet)"** เงินเข้าต้องเท่ากับเงินออกบวกเงินที่เก็บไว้ (Input = Output + Accumulation) ถ้าเงินหายไปไหนไม่รู้ แสดงว่ามีการทุจริต (Numerical Error)!
> *   **Analytical Benchmark:** คือการ **"เทียบกับทฤษฎีเศรษฐศาสตร์"** (Exact Solution) ว่าโมเดลธุรกิจเราเป็นไปตามกฎพื้นฐานที่พิสูจน์แล้วหรือไม่
> *   **Cross-Validation (StarCCM+):** คือการ **"เทียบกับคู่แข่งรายใหญ่"** (Competitor Analysis) ว่าผลกำไร (Result) เราใกล้เคียงกับมาตรฐานอุตสาหกรรมที่คนอื่นยอมรับหรือไม่
> *   **Grid Convergence:** คือการ **"ตรวจละเอียดขึ้นเรื่อยๆ"** ถ้าจ้างนักบัญชี 1 คน, 10 คน, 100 คน แล้วผลลัพธ์เริ่มนิ่งเท่ากัน แสดงว่าตัวเลขนั้นเชื่อถือได้
...
>   - `dimensionedScalar`: Tolerances ที่มี units (เช่น kg, J) ทำให้การเปรียบเทียบมีความหมายทางฟิสิกส์
> - **Virtual Methods:** `execute()` และ `write()` เป็น standard interface ของ function objects
...
<details>
<summary><b>1. ทำไมเราต้องเช็ค "Mass Conservation" แยกแต่ละ Phase ใน Multiphase Flow?</b></summary>

**คำตอบ:** เพราะ **Global Continuity (div(U) = 0) อย่างเดียวไม่พอ**
*   Global Continuity บอกแค่ว่าปริมาตรรวมคงที่ (สำหรับ incompressible)
*   แต่ Phase Conservation บอกว่า **"น้ำไม่หายไปไหน"** และ **"อากาศไม่หายไปไหน"**
*   ถ้าน้ำหายไปกลายเป็นอากาศโดยไม่มี Boiler (Phase change model) แสดงว่าเกิด Numerical Drift หรือ Simulation ผิดพลาด
</details>

<details>
<summary><b>2. Richardson Extrapolation ใช้ทำอะไรในการ Validation?</b></summary>

**คำตอบ:** ใช้เพื่อ **"เดาคำตอบที่แม่นยำที่สุด (Exact Solution)"** จากผลการรัน Grid หยาบๆ หลายระดับ
*   ช่วยให้เราประเมิน **Discretization Error** ได้โดยไม่ต้องรัน Grid ที่ละเอียดระดับ Infinity
*   เป็นการยืนยันว่า Solver ของเรามี Order of Accuracy (เช่น 2nd Order) ตามทฤษฎีจริงหรือไม่
</details>

<details>
<summary><b>3. L2 Norm Error บอกอะไรเราที่ดีกว่าดูค่า Max Error?</b></summary>

**คำตอบ:** บอก **"ความผิดพลาดโดยเฉลี่ยทั้งโดเมน"**
*   **Max Error:** อาจเกิดจากจุดเดียวที่ Mesh ไม่ดี (Outlier) ทำให้เราตกใจเกินเหตุ
*   **L2 Norm:** เอา Error ทุกจุดมายกกำลังสองรวมกันแล้วถอดราก ทำให้เห็นภาพรวมว่า "โดยรวมแล้ว Simulation นี้แม่นยำแค่ไหน"
</details>
