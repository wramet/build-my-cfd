...
> [!TIP] **มุมมองเปรียบเทียบ: คอนโดมิเนียมและการจัดการผู้พักอาศัย (The Condominium Analogy)**
> 
> ลองจินตนาการว่า OpenFOAM คือคอนโดมิเนียมขนาดใหญ่:
> *   **Global Registry (Time):** คือ "นิติบุคคล" ส่วนกลางที่คุมทุกอย่าง
> *   **Sub-Registries (Regions):** คือ "ห้องชุด" แต่ละห้อง (ห้อง `fluid`, ห้อง `solid`)
> *   **Fields (U, T, p):** คือ "ผู้พักอาศัย" ในห้องนั้นๆ
> *   **The Problem:** ทั้งห้อง `fluid` และห้อง `solid` ต่างก็มีคนชื่อ "สมชาย" (Field `T`) อาศัยอยู่
> *   **The Solution:** นิติบุคคลแยกแยะด้วยเลขห้อง -> `fluid::T` (สมชายห้องของไหล) และ `solid::T` (สมชายห้องของแข็ง) เป็นคนละคนกัน อยู่คนละที่ แต่ชื่อเหมือนกันได้โดยไม่สับสน!
...
<details>
<summary><b>1. "Namespace" ใน C++ กับ "Region Name" ใน OpenFOAM ต่างกันอย่างไร?</b></summary>

**คำตอบ:**
*   **C++ Namespace:** เป็นฟีเจอร์ของภาษา (Compiler-level) ใช้แยกขอบเขตของตัวแปรในโค้ด (เช่น `std::vector`)
*   **Region Name ใน OpenFOAM:** เป็นแค่ **String** (Runtime-level) ที่เอามาแปะหน้าชื่อตัวแปรเฉยๆ (เช่น "fluid::T") ตัวแปรจริงๆ คือ Object ที่เก็บใน Hash Table ไม่ใช่ตัวแปรที่ Compiler รู้จักในแง่ของ Scope
</details>

<details>
<summary><b>2. ทำไมเราต้อง Register Object เข้า Registry? ไม่ Register ได้ไหม?</b></summary>

**คำตอบ:**
*   **ได้แต่ไม่แนะนำ** ถ้าคุณสร้างตัวแปร `volScalarField` แบบไม่ Register (`IOobject::NO_READ, IOobject::NO_WRITE`) มันก็ใช้งานได้
*   **แต่** คุณจะเสียฟีเจอร์สำคัญ:
    1.  **Automatic Write:** มันจะไม่ถูกเขียนลงไฟล์เมื่อถึงเวลา save
    2.  **Lookup:** Solver อื่นหรือ Function Object จะหาตัวแปรนี้ไม่เจอ (เพราะไม่อยู่ในสมุดรายชื่อ)
</details>

<details>
<summary><b>3. ถ้าเรา lookupObject แล้วหาไม่เจอ จะเกิดอะไรขึ้น?</b></summary>

**คำตอบ:**
*   โปรแกรมจะ **Crash (Fatal Error)** ทันที!
*   **วิธีป้องกัน:** ใช้ `foundObject<Type>("name")` เพื่อเช็คก่อนว่ามีของไหม หรือใช้ `lookupObjectRef(..., true)` เพื่อปิด Error message
</details>
