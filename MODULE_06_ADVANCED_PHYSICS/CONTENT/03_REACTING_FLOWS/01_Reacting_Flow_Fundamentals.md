...
In OpenFOAM, reacting flow simulations require solving coupled systems of partial differential equations that describe the evolution of chemical species, temperature, and flow fields simultaneously.
 ...
 > [!TIP] **Analogy: The Dance Floor (ฟลอร์เต้นรำ)**
 >
 > Imagine a reacting flow as a **dance party**:
 > *   **Species ($Y_i$):** Dancers (Red shirt, Blue shirt) needed to form a couple.
 > *   **Reaction:** Two dancers meeting and starting to dance together (Product).
 > *   **Chemical Kinetics:** How fast they decide to dance once they meet (Are they shy or eager?).
 > *   **Turbulence (Mixing):** How fast the crowd moves and shuffles people around.
 >
 > Even if the dancers are eager (**Fast Kinetics**), if the crowd is stagnant (**Poor Mixing**), they will never meet! This is **Mixing-Controlled Combustion**. Conversely, if the crowd shuffles fast but dancers are shy, it's **Kinetics-Controlled**.
...
## 🧠 Concept Check: ทดสอบความเข้าใจ
 ...
 <details>
 <summary><b>1. Damköhler Number (Da) บอกอะไรเราเกี่ยวกับการควบคุมการเผาไหม้?</b></summary>
 
 **คำตอบ:**
 *   **Da << 1:** (Reaction is slow) -> **Kinetics Controlled**. เคมีช้ากว่าการผสม สารผสมกันทั่วแล้วค่อยๆ เกิดปฏิกิริยา (เหมือนเตาอบ)
 *   **Da >> 1:** (Reaction is fast) -> **Mixing Controlled**. ปฏิกิริยาพร้อมเกิดทันทีที่สารเจอกัน (เหมือนเปลวไฟหัวฉีด) การผสม (Turbulence) คือตัวกำหนดอัตราการเผาไหม้
 </details>
 
 <details>
 <summary><b>2. สมการการขนส่งสปีชีส์ (Species Transport) ต่างจากสมการ Passive Scalar อย่างไร?</b></summary>
 
 **คำตอบ:** ต่างกันที่ **Source Term ($\dot{\omega}_i$)**
 *   Passive Scalar (เช่น สีหยดในน้ำ) ไม่มี Source Term (หรือมีค่าคงที่)
 *   Species Transport มี **Reaction Rate** ที่ผันผวนอย่างรุนแรงและขึ้นกับอุณหภูมิแบบ Exponential (Arrhenius) ทำให้สมการมีความแข็ง (Stiff) และรุนแรงกว่ามาก
 </details>
 
 <details>
 <summary><b>3. ทำไม Low Mach Number Solver (reactingFoam) ถึงประหยัดกว่า Compressible Solver (rhoReactingFoam)?</b></summary>
 
 **คำตอบ:** เพราะ `reactingFoam` ตัด **Acoustic Waves** (คลื่นเสียง) ออกไป
 *   ไม่ต้องใช้ Time Step เล็กๆ เพื่อจับความเร็วเสียง (Courant Condition Based on Speed of Sound)
 *   สามารถใช้ Time Step ที่ใหญ่ขึ้นได้มาก (ขึ้นกับความเร็วลม Flow Velocity เท่านั้น) เหมาะสำหรับการเผาไหม้ Deflagration ทั่วไป
 </details>
