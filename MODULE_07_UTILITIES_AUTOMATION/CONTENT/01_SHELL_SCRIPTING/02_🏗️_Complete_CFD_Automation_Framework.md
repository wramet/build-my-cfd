...
> [!TIP] **เปรียบเทียบกับไซต์ก่อสร้าง (Construction Analogy)**
> ให้จินตนาการว่าเฟรมเวิร์กนี้คือ **ผู้จัดการโครงการก่อสร้าง (Site Manager)**:
> - **Geometry/MeshConfig** คือ "สถาปนิกและวิศวกรโครงสร้าง" ที่เตรียมแบบแปลน (Blueprint)
> - **SolverConfig** คือ "หัวหน้าคนงาน" ที่คอยกำกับดูแลคนงาน (Processor Cores) ให้ทำงานก่อสร้าง (Calculation) ตามแบบ
> - **PhysicsConfig** คือ "กฎฟิสิกส์และวัสดุศาสตร์" ที่กำหนดว่าปูน (Fluid) จะแข็งตัวอย่างไร
> - **PostProcessingConfig** คือ "ผู้ตรวจสอบคุณภาพ (Inspector)" ที่มาตรวจรับงานและออกใบรับรอง (Report)
> ระบบอัตโนมัติช่วยให้ผู้จัดการไม่ต้องลงไปสั่งคนงานทีละคน แต่สั่งผ่านระบบจัดการกลางได้เลย
...
> **📊 ผลลัพธ์ที่คาดหวัง (Visual Output):**
> หน้าต่างโต้ตอบ 3 มิติ (Interactive 3D Window) ที่แสดงรูปทรงเรขาคณิตของชิ้นงาน พร้อมเฉดสี (Contour Color Map) แสดงค่า Magnitude ของความเร็ว โดยสีแดงแสดงความเร็วสูงและสีน้ำเงินแสดงความเร็วต่ำ ผู้ใช้สามารถหมุน (Rotate), ซูม (Zoom) หรือเลื่อน (Pan) เพื่อดูรายละเอียดได้
...
1. **ถาม:** หาก `checkMesh` รายงานว่ามี **Highly Skewed Cells** เราควรปรับแก้ที่ส่วนใดของกระบวนการอัตโนมัติ?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b> ควรกลับไปปรับที่ขั้นตอน **Meshing Configuration** โดยอาจจะเพิ่ม `nCellsBetweenLevels` ใน `snappyHexMeshDict` เพื่อให้การเปลี่ยนความละเอียดเมชนุ่มนวลขึ้น หรือลดขนาด Mesh ในตำแหน่งที่มีความโค้งสูง
   </details>

2. **ถาม:** ในการตั้งค่า `forceCoeffs` ทำไมการระบุ **CofR (Center of Rotation)** ให้ถูกต้องจึงมีความสำคัญ?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b> เพราะ CofR ใช้เป็นจุดหมุนในการคำนวณ **โมเมนต์ (Moment Coefficients)** หากระบุตำแหน่งผิด ค่าสัมประสิทธิ์โมเมนต์ ($C_m$) จะผิดเพี้ยนไปจากความจริง ทำให้การวิเคราะห์เสถียรภาพ (Stability Analysis) ผิดพลาดได้
   </details>

3. **ถาม:** ในระบบ Adaptive Control เหตุใดจึงต้อง **ลด** ค่า Relaxation Factors เมื่อค่า Residuals เริ่มสูงขึ้น?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b> เพื่อเพิ่ม **ความเสถียร (Stability)** ของการคำนวณ การลด Relaxation Factor (Under-relaxation) จะทำให้การอัปเดตค่าตัวแปรในแต่ละรอบการคำนวณเปลี่ยนแปลงน้อยลง ช่วยป้องกันไม่ให้ Solver เกิดการลู่เข้าหาค่าอนันต์ (Divergence) หรือแกว่งตัวรุนแรง
   </details>
