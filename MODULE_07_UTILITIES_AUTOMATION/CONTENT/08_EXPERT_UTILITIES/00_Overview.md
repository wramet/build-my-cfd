...
// Base class for all OpenFOAM applications
class Foam::className
:
    public Foam::argList  // Inherits command-line argument handling
{
    // Constructor - Initialize with command line arguments
    className(int argc, char* argv[])
    :
        argList(argc, argv)
    {
        // Read settings from dictionaries
        // Setup models
...
<div style="background: #f8f9fa; border-left: 4px solid #007bff; padding: 1rem; margin: 1rem 0; border-radius: 4px;">
  <strong>📂 Source:</strong> OpenFOAM Core Architecture (src/OpenFOAM/applications)<br>
  <strong>💡 Explanation:</strong> โครงสร้างพื้นฐานของ Utility ทุกตัวใน OpenFOAM สืบทอดจากคลาส <code>argList</code> ซึ่งจัดการ arguments จาก command line และการอ่าน configuration files<br>
  <strong>🔑 Key Concepts:</strong>
  <ul>
    <li><strong>argList:</strong> Base class สำหรับจัดการ command-line arguments</li>
    <li><strong>Dictionaries:</strong> ไฟล์配置 ในรูปแบบ OpenFOAM dictionary format</li>
    <li><strong>Constructor:</strong> ฟังก์ชันเริ่มต้นที่รับ argc และ argv</li>
  </ul>
</div>
...
(
    ...
    (100 0 0)         // Point 1: x-direction
    (100 50 0)        // Point 2: xy-plane corner
    (0 50 0)          // Point 3: y-direction
    (0 0 20)          // Point 4: z-direction
    (100 0 20)        // Point 5: xz-plane corner
    (100 50 20)       // Point 6: opposite corner
    (0 50 20)         // Point 7: yz-plane corner
);

// Define hexahedral blocks
blocks
(
    // hex (vertices) (cells x y z) grading
...
edges
(
    // Use arc, spline, or polyline for curved boundaries
);
...
<div style="background: #f8f9fa; border-left: 4px solid #007bff; padding: 1rem; margin: 1rem 0; border-radius: 4px;">
  <strong>📂 Source:</strong> OpenFOAM Tutorials / mesh/snappyHexMesh (similar structure)<br>
  <strong>💡 Explanation:</strong> blockMeshDict กำหนดพิกัดจุดยอด (vertices) ของบล็อก การเชื่อมต่อระหว่างจุดยอดเป็น hexahedral cells และการแบ่งชนิดของ boundaries แต่ละชนิด<br>
  <strong>🔑 Key Concepts:</strong>
  <ul>
    <li><strong>convertToMeters:</strong> ตัวคูณแปลงหน่วยจากหน่วยใน dictionary เป็นเมตร</li>
    <li><strong>vertices:</strong> พิกัด x,y,z ของแต่ละจุดยอดบล็อก</li>
    <li><strong>blocks:</strong> การนิยาม hexahedral block และจำนวน cell divisions</li>
    <li><strong>simpleGrading:</strong> อัตราส่วนการขยาย cell ในแต่ละทิศทาง</li>
    <li><strong>boundary types:</strong> patch, wall, symmetryPlane, cyclic</li>
  </ul>
</div>
...
# Additional options:
# -allTopology: Check all topology quality metrics
# -allGeometry: Check all geometry quality metrics
# -time: Specify time directory to check
```
...
// Enable mesh generation stages
castellatedMesh true;
snap            true;
addLayers       false;

// Define geometry for meshing
geometry
{
    geometry.stl
    {
...
<div style="background: #f8f9fa; border-left: 4px solid #007bff; padding: 1rem; margin: 1rem 0; border-radius: 4px;">
  <strong>📂 Source:</strong> OpenFOAM Tutorials / mesh/snappyHexMesh/snappyHexMeshDict<br>
  <strong>💡 Explanation:</strong> snappyHexMesh สร้างเมชอัตโนมัติจาก STL surface ผ่าน 3 ขั้นตอน: castellated mesh (สร้างเมชพื้นฐาน), snap (ปรับให้แนบชิดผิว), และ addLayers (เพิ่ม boundary layers)<br>
  <strong>🔑 Key Concepts:</strong>
  <ul>
    <li><strong>castellatedMesh:</strong> ขั้นตอนสร้างเมช hexahedral พื้นฐาน</li>
    <li><strong>snap:</strong> การปรับ node ให้แนบชิดกับ surface geometry</li>
    <li><strong>addLayers:</strong> การเพิ่ม prism layers บน boundaries</li>
    <li><strong>refinementSurfaces:</strong> ระดับการละเอียดบนแต่ละ surface</li>
    <li><strong>nCellsBetweenLevels:</strong> จำนวน cell ระหว่างระดับการละเอียด</li>
  </ul>
</div>
...
numberOfSubdomains 16; // Depends on core count

simpleCoeffs
{
    n           (4 4 1); // For method = simple
    delta       0.001;
}
...
// Manual decomposition data file
// manualCoeffs
// {
...
<div style="background: #f8f9fa; border-left: 4px solid #007bff; padding: 1rem; margin: 1rem 0; border-radius: 4px;">
  <strong>📂 Source:</strong> OpenFOAM Tutorials / multiphase/interFoam/laminar/decomposeParDict<br>
  <strong>💡 Explanation:</strong> decomposeParDict กำหนดวิธีการแบ่งโดเมนสำหรับ parallel processing โดยแต่ละวิธีมีข้อดีและข้อเสะท้อนต่อ load balancing และ communication overhead<br>
  <strong>🔑 Key Concepts:</strong>
  <ul>
    <li><strong>method:</strong> วิธีการแบ่งโดเมน (scotch, metis, simple, hierarchical)</li>
    <li><strong>numberOfSubdomains:</strong> จำนวน subdomains ที่ต้องการแบ่ง</li>
    <li><strong>simpleCoeffs:</strong> การตั้งค่าสำหรับ simple method (แบ่งตามแกน x,y,z)</li>
    <li><strong>delta:</strong> ค่า tolerance สำหรับการปรับสมดุล load</li>
  </ul>
</div>
...
decomposePar                    # Decompose domain
mpirun -np 16 solver -parallel  # Run parallel
...
reconstructPar -latestTime      # Reconstruct latest time only
reconstructPar -time 1000       # Reconstruct specific time
```
...
foamToVTK                       # Convert all time steps
foamToVTK -latestTime           # Convert latest time only
foamToVTK -time 1000            # Convert specific time
foamToVTK -ascii                # Convert to ASCII (larger but readable)
foamToVTK -fields "(p U k epsilon)"  # Specify fields
```
...
functions
{
    ...
    forces
    {
        ...
        rho         rhoInf;      // or rho rho; for compressible
    ...
    // Calculate statistics on surface
    surfaceInterpolate
    {
        type        surfaces;
        libs        ("libsampling.so");
...
<div style="background: #f8f9fa; border-left: 4px solid #007bff; padding: 1rem; margin: 1rem 0; border-radius: 4px;">
  <strong>📂 Source:</strong> OpenFOAM Tutorials / incompressible/simpleFoam/airFoil2D/system/controlDict (similar structure)<br>
  <strong>💡 Explanation:</strong> controlDict สามารถเพิ่ม function objects สำหรับ post-processing ระหว่าง simulation หรือหลัง simulation ผ่าน postProcess utility<br>
  <strong>🔑 Key Concepts:</strong>
  <ul>
    <li><strong>forces:</strong> คำนวณ drag/lift forces บน patches ที่กำหนด</li>
    <li><strong>surfaceInterpolate:</strong> สุ่มค่าบน surfaces (planes, boxes)</li>
    <li><strong>patchAverage:</strong> คำนวณค่าเฉลี่ยของ fields บน patches</li>
    <li><strong>writeControl:</strong> ควบคุมความถี่ในการบันทึกข้อมูล</li>
  </ul>
</div>
...
postProcess -func "forces"                           # Calculate forces
postProcess -func "surfaceInterpolate"               # Sample surface
postProcess -func "patchAverage"                     # Patch average
postProcess -func "mag(U)"                           # Calculate magnitude
postProcess -func "vorticity"                        # Calculate vorticity
```
...
<div style="background: #f8f9fa; border-left: 4px solid #007bff; padding: 1rem; margin: 1rem 0; border-radius: 4px;">
  <strong>📂 Source:</strong> OpenFOAM Tutorials / compressible/rhoPimpleFoam/RAS/angledDuctLift/system/sampleDict<br>
  <strong>💡 Explanation:</strong> sampleDict ใช้สำหรับสุ่มข้อมูลจาก fields บน sets (เส้น), surfaces (ระนาบ), หรือ clouds (จุดกระจาย)<br>
  <strong>🔑 Key Concepts:</strong>
  <ul>
    <li><strong>sets:</strong> สุ่มข้อมูลตามเส้น (uniform, face, cloud)</li>
    <li><strong>surfaces:</strong> สุ่มข้อมูลบน surfaces (plane, box)</li>
    <li><strong>interpolate:</strong> การปักแต่งค่าจาก cell centers ไปยัง sampling points</li>
  </ul>
</div>
...
<div style="background: #f8f9fa; border-left: 4px solid #007bff; padding: 1rem; margin: 1rem 0; border-radius: 4px;">
  <strong>📂 Source:</strong> OpenFOAM Tutorials / multiphase/interFoam/laminar/damBreak/system/setFieldsDict<br>
  <strong>💡 Explanation:</strong> setFieldsDict ใช้กำหนดค่าเริ่มต้นของ fields ในบริเวณเฉพาะ เช่น กล่อง ทรงกลม หรือจาก surface geometry<br>
  <strong>🔑 Key Concepts:</strong>
  <ul>
    <li><strong>defaultFieldValues:</strong> ค่าเริ่มต้นทั่วทั้งโดเมน</li>
    <li><strong>boxToCell:</strong> เลือก cells ในกล่องสี่เหลี่ยม</li>
    <li><strong>sphereToCell:</strong> เลือก cells ในทรงกลม</li>
    <li><strong>surfaceToCell:</strong> เลือก cells ภายใน surface</li>
  </ul>
</div>
...
setFields -time 0                 # Set values at time 0
setFields -dict system/setFieldsDict  # Use custom dictionary
```
...
mapMethod     cellVolumeWeight; // or nearestCell, direct
```

<div style="background: #f8f9fa; border-left: 4px solid #007bff; padding: 1rem; margin: 1rem 0; border-radius: 4px;">
  <strong>📂 Source:</strong> .applications/test/fieldMapping/pipe1D (similar concept)<br>
  <strong>💡 Explanation:</strong> mapFields ใช้สำหรับถ่ายโอน field data ระหว่างเมชที่แตกต่างกัน ซึ่งมีประโยชน์สำหรับ mesh refinement studies<br>
  <strong>🔑 Key Concepts:</strong>
  <ul>
    <li><strong>sourceCase:</strong> path ไปยัง case ต้นทาง</li>
    <li><strong>sourceTime:</strong> time directory ที่ต้องการแมป</li>
    <li><strong>mapMethod:</strong> วิธีการแมป (cellVolumeWeight = แม่นยำสูงสุด)</li>
  </ul>
</div>
...
thermoType
{
    ...
    transport       sutherland;       // or const, polynomial
    thermo          hConst;           // or hPolynomial
...
mixture
{
    specie
    {
        molWeight   28.96;           // [kg/kmol] for air
    }
...
<div style="background: #f8f9fa; border-left: 4px solid #007bff; padding: 1rem; margin: 1rem 0; border-radius: 4px;">
  <strong>📂 Source:</strong> OpenFOAM Tutorials / heatTransfer/chtMultiRegionFoam/heatExchanger/0/air/thermophysicalProperties<br>
  <strong>💡 Explanation:</strong> thermophysicalProperties กำหนดคุณสมบัติทางกายภาพของ fluid รวมทั้ง transport properties, thermodynamic properties, และ equation of state<br>
  <strong>🔑 Key Concepts:</strong>
  <ul>
    <li><strong>thermoType:</strong> ประเภทของ thermophysical model</li>
    <li><strong>transport:</strong> โมเดลความหนืด (sutherland, const, polynomial)</li>
    <li><strong>equationOfState:</strong> สมการสถานะ (perfectGas, incompressiblePerfectGas)</li>
    <li><strong>mu:</strong> ความหนืด [Pa·s]</li>
    <li><strong>Pr:</strong> Prandtl number</li>
  </ul>
</div>
...
fluxScheme    Kurganov;             // or Tadmor, central
...
<div style="background: #f8f9fa; border-left: 4px solid #007bff; padding: 1rem; margin: 1rem 0; border-radius: 4px;">
  <strong>📂 Source:</strong> OpenFOAM Tutorials / compressible/rhoCentralFoam/biconic25-55Run/system/fvSchemes<br>
  <strong>💡 Explanation:</strong> rhoCentralFoam ใช้ flux schemes พิเศษสำหรับ compressible flows ที่มี shock waves<br>
  <strong>🔑 Key Concepts:</strong>
  <ul>
    <li><strong>fluxScheme:</strong> วิธีการคำนวณ flux (Kurganov, Tadmor)</li>
    <li><strong>Riemann solvers:</strong> การแก้ปัญหา Riemann สำหรับ compressible flows</li>
  </ul>
</div>
...
foamDebugInfo mesh -case .         # Display mesh information
foamDebugInfo libs                 # Display loaded libraries
foamDebugInfo decomposition        # Display decomposition info
```
...
testMesh -blockMesh -case cavity   # Create test mesh
```
...
foamListTimes                      # List all time directories
foamListTimes -latestTime          # Show latest time only
foamListTimes -time 1000           # Check if time 1000 exists
```
...
reconstructParMesh -mergeTols 1e-6 # Merge mesh from processor*
reconstructParMesh -constant       # Merge constant/polyMesh only
```
...
decomposePar -nproc 16             # Decompose to 16 cores
mpirun -np 16 solver -parallel     # Run
reconstructPar -latestTime         # Reconstruct results
decomposePar -nproc 32 -force      # Redecompose to 32 cores
mpirun -np 32 solver -parallel     # Continue run
```
...
<div style="background: #f8f9fa; border-left: 4px solid #007bff; padding: 1rem; margin: 1rem 0; border-radius: 4px;">
  <strong>📂 Source:</strong> .applications/test/fieldMapping/pipe1D/system/fvSolution<br>
  <strong>💡 Explanation:</strong> fvSolution กำหนด solver algorithms, tolerances, และ convergence criteria สำหรับแต่ละ field<br>
  <strong>🔑 Key Concepts:</strong>
  <ul>
    <li><strong>solver:</strong> ชนิดของ linear solver (GAMG, smoothSolver, PCG)</li>
    <li><strong>tolerance:</strong> absolute tolerance สำหรับการลู่ตัว</li>
    <li><strong>relTol:</strong> relative tolerance สำหรับแต่ละ iteration</li>
    <li><strong>residualControl:</strong> criteria สำหรับการหยุด simulation</li>
  </ul>
</div>
...
solver > log.solver &
foamMonitor -l log.solver &     # Plot residuals in real-time
```
...
solvers
{
    p
    {
        tolerance       1e-4;     // Start with looser tolerance
        relTol          0.1;
    }
}
```
...
<div style="background: #f8f9fa; border-left: 4px solid #007bff; padding: 1rem; margin: 1rem 0; border-radius: 4px;">
  <strong>📂 Source:</strong> OpenFOAM Documentation / Control Dict OptimisationSwitches<br>
  <strong>💡 Explanation:</strong> OptimisationSwitches ใช้ปรับแต่ง performance parameters ของ OpenFOAM เช่น I/O handling, memory limits<br>
  <strong>🔑 Key Concepts:</strong>
  <ul>
    <li><strong>fileHandler:</strong> วิธีการจัดการ I/O (collated, collatedThreads)</li>
    <li><strong>fileBufferSize:</strong> ขนาด buffer สำหรับ file I/O</li>
    <li><strong>maxTemporaryDirectorySize:</strong> ขีดจำกัด disk space สำหรับ temporary files</li>
  </ul>
</div>
...
# Export from Blender: File → Export → STL
...
paraFoam -builtin                    # Use builtin reader
paraFoam -batch -script script.py   # Run in batch mode
```
