...
> [!TIP] **เปรียบเทียบ Python Plotting (Analogy)**
> ให้คิดว่า **ParaView** คือ **"กล้องถ่ายวิดีโอ"** ที่บันทึกภาพเคลื่อนไหว 3 มิติได้สวยงามและเห็นภาพรวมได้ดีเยี่ยม
> แต่ **Python (Matplotlib/Pandas)** คือ **"กล้องจุลทรรศน์และเครื่องชั่งน้ำหนักดิจิทัล"**
> - มันไม่ได้ให้ภาพรวมที่หวือหวา 3 มิติ แต่ให้ **"ตัวเลขที่แม่นยำ"** (Quantitative Data)
> - มันสามารถ "ชั่ง ตวง วัด" ค่าต่างๆ เช่น $C_d$, $C_l$, หรือ $y^+$ ออกมาเป็นกราฟที่ละเอียดและแม่นยำ
> - มันเหมือน **"นักบัญชีผู้ละเอียดรอบคอบ"** ที่สรุปผลการไหลทั้งหมดออกมาเป็นรายงานสถิติที่เชื่อถือได้ ในขณะที่ ParaView เหมือน **"ช่างภาพศิลปะ"**
...
> 1.  **Lift Coefficient ($C_L$)**: กราฟแสดงการสั่นแบบ Periodic ที่ชัดเจน โดยค่าเฉลี่ยเป็น 0 แต่มี Amplitude คงที่ (Limit Cycle Oscillation)
> 2.  **Drag Coefficient ($C_D$)**: กราฟแสดงค่าเฉลี่ยที่เป็นบวก (เช่น $C_D \approx 1.2 - 1.4$) และสั่นด้วยความถี่ 2 เท่าของ $C_L$
> 3.  **Power Spectral Density (PSD)**: กราฟ FFT ของสัญญาณ $C_L$ จะแสดง Peak ที่โดดเด่นเพียงค่าเดียว (Dominant Frequency) ซึ่งสอดคล้องกับ Strouhal Number $St \approx 0.2$
...
> **ผลลัพธ์ที่คาดหวัง (Expected Results):**
> 1.  **Mean Velocity Profile ($u^+$ vs $y^+$)**: กราฟ Log-law ที่สมบูรณ์แบบ
>     - **Viscous Sublayer ($y^+ < 5$)**: ตามสมการ $u^+ = y^+$ (เส้นตรง Slope 1)
>     - **Log-law Region ($30 < y^+ < 100$)**: ตามสมการ $u^+ = \frac{1}{\kappa} \ln(y^+) + B$ (เส้นตรงในกราฟ Semi-log)
> 2.  **Reynolds Stresses**: กราฟ $\overline{u'v'}$ จะมีค่าเป็นลบและแปรผันเป็นเส้นตรง near-wall และลดลงเป็นศูนย์ที่กึ่งกลาง Channel
> 3.  **Turbulent Kinetic Energy ($k$)**: จะมีค่าสูงสุด (Peak) ที่บริเวณ Buffer Layer ($y^+ \approx 15-20$) ซึ่งเป็นที่ที่มีการผลิต Turbulence สูงสุด
...
1. **ถาม:** ทำไมในกราฟ Velocity Profile ของ Turbulent Boundary Layer เราจึงนิยมพล็อตแกน X เป็น $y^+$ (แบบ Log scale) และแกน Y เป็น $u^+$ แทนที่จะพล็อตระยะทาง $y$ กับความเร็ว $U$ ความจริง?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b> เพราะการใช้ตัวแปรไร้มิติ (Dimensionless Variables) $y^+$ และ $u^+$ ทำให้เราสามารถเปรียบเทียบผลการทดลองหรือการจำลองที่มี Reynolds Number ต่างกันได้ (Universal Law of the Wall) และการใช้ Log scale ช่วยให้เรามองเห็นพฤติกรรมใน Viscous Sublayer (ที่เล็กมาก) และ Log-law Region ได้ชัดเจนในกราฟเดียวกัน
   </details>

2. **ถาม:** ในการคำนวณ FFT (Fast Fourier Transform) ของค่า Lift Coefficient เพื่อหา Vortex Shedding Frequency สิ่งสำคัญที่สุดที่ต้องระวังเกี่ยวกับข้อมูล Time Series คืออะไร?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b> ข้อมูลต้องมี **Sampling Rate ที่สม่ำเสมอ** (Constant Time Step) และต้องมีความละเอียดเพียงพอ (ตาม Nyquist Theorem ความถี่ sampling ต้องมากกว่า 2 เท่าของความถี่สูงสุดที่สนใจ) นอกจากนี้ข้อมูลควรจะตัดช่วง Transient (ช่วงเริ่มต้นที่ยังไม่คงที่) ออกไปก่อน
   </details>

3. **ถาม:** ถ้าเราพล็อต Time Series ของ Residuals แล้วเห็นว่ากราฟลดลงเรื่อยๆ แต่เริ่ม "แกว่ง" (Oscillate) และไม่ลดลงไปต่ำกว่า $10^{-3}$ อีกเลย แปลว่า Simulation ของเราล้มเหลวหรือไม่?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b> ไม่เสมอไป การที่ Residuals แกว่งและไม่ลดลงต่ออาจเกิดจากธรรมชาติของการไหลที่เป็น Unsteady (เช่น Vortex Shedding) ซึ่ง solver แบบ Steady State พยายามหาคำตอบนิ่งๆ แต่หาไม่ได้ ในกรณีนี้ควรเปลี่ยนไปใช้ Solver แบบ Transient (เช่น `pimpleFoam`) เพื่อจับพฤติกรรมจริง หรือตรวจสอบว่าการแกว่งนั้นยอมรับได้ในระดับความแม่นยำที่ต้องการหรือไม่
   </details>
