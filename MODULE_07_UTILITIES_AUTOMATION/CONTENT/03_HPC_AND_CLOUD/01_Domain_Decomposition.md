...
> [!TIP] **เปรียบเทียบการย่อยโดเมน (Pizza Analogy)**
> ให้จินตนาการว่าโดเมนการคำนวณคือ **"พิซซ่าถาดใหญ่"** และโปรเซสเซอร์ (CPU Cores) คือ **"คนกินพิซซ่า 4 คน"**:
> - **Domain Decomposition** คือการตัดแบ่งพิซซ่าให้แต่ละคน
> - **Simple Method** คือการตัด "แบบตาราง" (ตัดเส้นตรง X, Y) ซึ่งอาจทำให้บางชิ้นมีแต่แป้ง บางชิ้นมีหน้าเยอะ (Load Imbalance) ถ้าพิซซ่า (Geometry) ไม่ใช่รูปสี่เหลี่ยม
> - **Scotch Method** คือการตัด "แบบอัจฉริยะ" ที่พยายามแบ่งให้ทุกคนได้ปริมาณเท่ากันเป๊ะๆ แม้พิซซ่าจะมีรูปร่างแปลกประหลาด
> - **Interface** คือ "ขอบรอยตัด" ที่ต้องแชร์ซอสกัน (Communication) ยิ่งรอยตัดยาว ยิ่งเลอะเทอะและเสียเวลาส่งซอส

---

## 1. รากฐานคณิตศาสตร์ของ Parallel CFD
...
(keep existing content until step 5 log output)
...
ไฟล์ `log.solver` ควรมีข้อมูลดังนี้:

```
// ตัวอย่างบรรทัดที่แสดงว่ารันแบบ Parallel สำเร็จ
// Example lines indicating successful parallel run
Calculating potential flow
Parallel processing using 8 processors // ยืนยันว่าใช้ 8 cores
...
Time = 1
Courant Number mean: 0.1234 max: 0.5678
ExecutionTime = 12.34 s  ClockTime = 12.50 s
// ExecutionTime ควรน้อยกว่าแบบ Serial อย่างเห็นได้ชัด (Speedup)
```
...
...
1. **ถาม:** ถ้าเรามีจำนวน cells ทั้งหมด 2,000,000 cells ควรใช้จำนวน CPU Cores ประมาณกี่ cores จึงจะเหมาะสมที่สุดตามกฎ Best Practice?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b> ประมาณ 4 ถึง 20 cores (คำนวณจากกฎ 100,000 - 500,000 cells/core) การใช้น้อยกว่านี้อาจช้าเกินไป และการใช้มากกว่า 20 cores อาจไม่คุ้มค่าเนื่องจาก Communication Overhead
   </details>

2. **ถาม:** ในกรณีที่ Geometry มีความซับซ้อนสูง (เช่น ห้องเครื่องยนต์ หรือรูปทรงอากาศพลศาสตร์) ทำไมวิธี **Scotch** จึงดีกว่า **Simple**?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b> เพราะ **Simple** แบ่งตามแกน X, Y, Z ตรงๆ ซึ่งอาจทำให้บางโดเมนว่างเปล่า (ไม่มี cells) หรือมี cells น้อยมากในขณะที่โดเมนอื่นมีเยอะ (Major Load Imbalance) ส่วน **Scotch** ใช้ Graph partitioning algorithm ที่ฉลาดกว่าในการเฉลี่ยจำนวน cells ให้เท่ากันโดยไม่สนใจแกนพิกัด
   </details>

3. **ถาม:** ค่า **Efficiency** และ **Speedup** แตกต่างกันอย่างไรในบริบทของ Parallel Computing?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b> **Speedup** บอกว่า "เร็วขึ้นกี่เท่า" เทียบกับ 1 core (ค่าอุดมคติคือเท่ากับจำนวน cores) ส่วน **Efficiency** บอกว่า "คุ้มค่าแค่ไหน" หรือเราใช้ศักยภาพของ hardware ได้กี่เปอร์เซ็นต์ (เช่น Speedup 16 เท่าจาก 20 cores = Efficiency 80%)
   </details>
