...
- **อธิบาย (Explain)** - แตกต่างระหว่าง **utilities** กับ **solvers** ใน OpenFOAM
- **จัดหมวดหมู่ (Categorize)** - แบ่ง utilities ตาม **workflow phases** ของงาน CFD
- **เลือกใช้ (Select)** - ใช้ utilities ที่เหมาะสมกับแต่ละขั้นตอนการทำงาน
- **ประยุกต์ใช้ (Apply)** - เรียงลำดับ utilities อย่างถูกต้องตาม workflow
- **วิเคราะห์ (Analyze)** - ตรวจสอบผลลัพธ์จาก utilities แต่ละตัว
...
### ❓ Why: ทำไมต้องเข้าใจการจัดหมวดหมู่?

1. **เลือกเครื่องมือที่ถูกต้อง** - ไม่ต้องลองผิดลองถูกกับ 200+ utilities
2. **ประหยัดเวลา** - รู้ว่าต้องใช้อะไรตอนไหนใน workflow
3. **หลีกเลี่ยงข้อผิดพลาด** - เรียงลำดับใช้งานอย่างถูกต้อง
4. **แก้ปัญหาได้เร็ว** - รู้ว่าควรใช้ utility ไหนช่วย debug

### 🎯 How: เรียนรู้อย่างไร?

- **จัดหมวดหมู่ตาม workflow** - เรียงจาก pre-processing → post-processing
- **เรียนรู้ syntax พื้นฐาน** - วิธีเรียกใช้ utilities และดู output
- **ฝึกด้วยตัวอย่างจริง** - ลองรัน utilities พร้อมคำอธิบาย
- **แบบฝึกหัด** - ทดสอบความเข้าใจในทุกระดับ
...
### 🔍 What: แต่ละ phase ทำอะไร?

| Phase | หน้าที่หลัก | Utilities ตัวอย่าง |
|:------|:-------------|:-------------------|
| **Pre-Processing** | เตรียมข้อมูล, สร้าง mesh | blockMesh, snappyHexMesh, setFields |
| **Mesh Manipulation** | ปรับ mesh, ตรวจสอบ | transformPoints, checkMesh, refineMesh |
| **Running/Parallel** | เตรียมและรัน parallel | decomposePar, reconstructPar |
| **Post-Processing** | วิเคราะห์ผลลัพธ์ | postProcess, sample, probes |
| **Data Conversion** | แปลงรูปแบบข้อมูล | foamToVTK, foamToEnsight |
...
| หมวดหมู่ | Utilities หลัก | หน้าที่ | Output สำคัญ |
|:---------|:---------------|:---------|:-------------|
| **Pre-Processing** | blockMesh, snappyHexMesh, setFields | สร้าง mesh, กำหนดค่าเริ่มต้น | `polyMesh/`, `0/` |
| **Mesh Manipulation** | transformPoints, refineMesh, checkMesh | แปลง/ตรวจสอบ mesh | `log.checkMesh`, modified mesh |
| **Running/Parallel** | decomposePar, reconstructPar | แบ่ง/รวม parallel mesh | `processor*/`, reconstructed results |
| **Post-Processing** | postProcess, sample, probes | วิเคราะห์ผลลัพธ์ | `postProcessing/`, `sets/` |
| **Data Conversion** | foamToVTK, foamToEnsight | แปลงรูปแบบข้อมูล | `VTK/`, `*.case` |

### 🎯 Points สำคัญที่ต้องจำ

1. **200+ utilities** แต่เราไม่ต้องจำทุกตัว — เข้าใจ **หมวดหมู่** และ **workflow** ก็พอ
2. **Utilities vs Solvers** — utilities = เครื่องมือช่วย, solvers = โปรแกรมคำนวณ
3. **5 workflow phases** — Pre → Mesh → Running → Post → Convert
4. **เรียงลำดับสำคัญ** — ใช้ utilities ตามลำดับที่ถูกต้อง
5. **ตรวจสอบทุกขั้นตอน** — ใช้ log files และ `checkMesh` ตรวจสอบ
6. **postProcess ทรงพลัง** — ใช้ command line แทน GUI สำหรับ automation
