...
> [!TIP] **ทำไมการมีส่วนร่วมกับชุมชน (Community) ถึงสำคัญ?**
> เนื้อหาบทนี้เกี่ยวข้องกับ **Domain E: Coding/Customization** และ **Professional Practice** ซึ่งจะพาคุณไปเรียนรู้วิธีการมีส่วนร่วมกับชุมชน OpenFOAM อย่างมืออาชีพ ไม่ว่าจะเป็นการรายงานบั๊ก (Bug Reporting), การปรับปรุงเอกสาร (Documentation), หรือการส่งโค้ดกลับเข้าสู่โครงการหลัก (Code Contribution)
>
> **ทำไมถึงสำคัญต่อ Simulation?**
> - **ความยั่งยืน (Sustainability)**: OpenFOAM เติบโตได้เพราะชุมชน การมีส่วนร่วมช่วยให้ซอฟต์แวร์นี้ดีขึ้นและอยู่ได้นาน
> - **ความน่าเชื่อถือ (Reliability)**: การรายงานบั๊กและทดสอบเวอร์ชันใหม่ช่วยให้ Solver ที่คุณใช้ทำงานได้อย่างเสถียร
> - **การเรียนรู้ (Learning)**: Code Review จากผู้เชี่ยวชาญจะช่วยปรับปรุงทักษะการเขียนโค้ด C++ ของคุณ ซึ่งสะท้อนกลับมาที่การทำ Simulation ที่ดีขึ้น
...
> ส่วนนี้เกี่ยวข้องกับ **Domain E: Coding/Customization** และ **Professional Practice** โดยเฉพาะการทำงานกับ:
> - **Source Code Structure**: `src/` directory (เช่น `src/finiteVolume`, `src/transportModels`)
> - **Tutorials**: `tutorials/` directory (ใช้สำหรับสร้าง Case ตัวอย่าง)
> - **Documentation**: `doc/` directory หรือ Wiki ของโครงการ
> - **Git Repository**: GitLab/GitHub ของ OpenFOAM (เช่น [OpenFOAM-git](https://develop.openfoam.com/Development/openfoam))
> - **Issue Tracker**: ส่วนรายงาน Bug (Mantis, GitLab Issues, หรือ GitHub Issues)
...
> [!NOTE] **📂 OpenFOAM Context**
> ส่วนนี้เกี่ยวข้องกับ **Domain E: Coding/Customization** โดยเฉพาะ:
> - **Source Files**: `src/` directory (เช่น `src/finiteVolume/fields/fvPatchFields/`)
> - **Header Files (`.H`)**: ประกาศ Class และ Interface
> - **Implementation Files (`.C`)**: การทำงานจริงของโค้ด
> - **Make Files**: `Make/files` (ระบุไฟล์ที่ต้อง Compile), `Make/options` (ระบุ Include path และ Library)
> - **Coding Guidelines**: `doc/Guidelines` ใน Source Tree หรือ [OpenFOAM Coding Style Guide](https://develop.openfoam.com/Development/openfoam/-/blob/master/doc/Guidelines.md)
...
> [!NOTE] **📂 OpenFOAM Context**
> ส่วนนี้เกี่ยวข้องกับ **Domain E: Coding/Customization** และ **Version Control**:
> - **Git Workflow**: การใช้ Git สำหรับจัดการ Source Code
> - **Branching**: Feature Branch (เช่น `fix/turbulence-bug`, `feature/new-bc`)
> - **Remote Repository**: GitLab ([develop.openfoam.com](https://develop.openfoam.com)) หรือ GitHub (สำหรับ Community Forks เช่น [OpenFOAM/OpenFOAM-dev](https://github.com/OpenFOAM/OpenFOAM-dev))
> - **Merge Request (MR)**: หน้าจอ Review โค้ดบน GitLab/GitHub
> - **Continuous Integration (CI)**: Automated Testing ที่รันเมื่อส่ง PR (เช่น ` foamInstallationTest`, `foamRunTutorials`)
...
> [!NOTE] **📂 OpenFOAM Context**
> ส่วนนี้เกี่ยวข้องกับ **Legal & Professional Practice**:
> - **License Files**: `COPYING` file ใน root ของ OpenFOAM Source Tree
> - **Source Code Headers**: ทุกไฟล์ `.C` และ `.H` จะมี License Header แนบอยู่
> - **Third-Party Libraries**: `etc/config.sh/` ระบุ Libraries ที่ใช้ (เช่น MPI, Metis, Scotch)
> - **GPL Compliance**: สำคัญมากสำหรับการ Distribute ซอฟต์แวร์ที่พัฒนาจาก OpenFOAM
...
> [!NOTE] **📂 OpenFOAM Context**
> ส่วนนี้เกี่ยวข้องกับ **Bug Reporting & Debugging** โดยเฉพาะ:
> - **Case Structure**: ไฟล์ Case แบบย่อ (เช่น `0/`, `constant/`, `system/`)
> - **Dictionary Files**: `system/blockMeshDict`, `system/controlDict`, `constant/turbulenceProperties`
> - **Tutorials**: `$FOAM_TUTORIALS` directory ใช้เป็นฐานสำหรับสร้าง MWE
> - **Log Files**: `log.*` files สำคัญสำหรับ Traceback ของ Bug
> - **Allrun Script**: สคริปต์自动化การรัน Case สำหรับ Reproduce ปัญหา
