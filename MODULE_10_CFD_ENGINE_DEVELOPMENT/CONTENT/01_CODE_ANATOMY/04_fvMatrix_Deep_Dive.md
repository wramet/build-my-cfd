...
Prompt: "fvMatrix Structure and LDU Storage. **Left Panel (Class Structure):** UML-like class diagram showing fvMatrix with members: 'GeometricField& psi_' (unknown field), 'scalarField diag_' (diagonal coefficients), 'scalarField upper_/'lower_' (off-diagonal), 'Field source_' (RHS), 'FieldField internalCoeffs_/'boundaryCoeffs_' (boundary). **Center Panel (3-Cell Mesh Example):** Three cells arranged horizontally: Cell 0 (owner), Cell 1 (owner/neighbour), Cell 2 (neighbour). Face 0 between 0-1, Face 1 between 1-2. Each cell labeled 'P' with value φᵢ. Faces labeled with coefficient a_f. **Right Panel (LDU Arrays):** Three arrays shown visually: diag = [a₀₀, a₁₁, a₂₂] (3 values), upper = [a₀₁, a₁₂] (2 values), lower = [a₁₀, a₂₁] (2 values). Below: owner = [0, 1] (for each face), neighbour = [1, 2]. **Bottom Equation:** Stencil showing 'a_PP φ_P + Σ_a_PN φ_N = b_P'. **Style:** CFD data structure visualization, clear cell-to-coefficient mapping, color-coded arrays, technical illustration."
-->
...
Prompt: "LDU vs CSR Storage Format Comparison. **Top Panel (Standard Matrix):** 5×5 sparse matrix shown with mostly zeros. Non-zero elements highlighted in blue. Diagonal entries (5), off-diagonal entries (8). **Left Bottom Panel (CSR - Compressed Sparse Row):** Three arrays: values = [4, -1, -1, 4, -1, -1, 4, -1, 4, -1, -1, 4, 4] (13 nonzeros), col_ind = [0, 1, 0, 1, 2, 1, 2, 3, 2, 3, 4, 3, 4] (column indices), row_ptr = [0, 2, 5, 8, 10, 13] (row pointers). **Right Bottom Panel (LDU - OpenFOAM):** Three arrays: diag = [4, 4, 4, 4, 4] (5 values), upper = [-1, -1, -1, -1] (4 values), lower = [-1, -1, -1, -1] (4 values). Two connectivity arrays: owner = [0, 1, 2, 3], neighbour = [1, 2, 3, 4]. **Annotations:** CSR: 'General sparse, stores column indices', LDU: 'FVM-specific, uses mesh connectivity, 50% less memory'. **Memory usage bar chart:** CSR = 30 units, LDU = 15 units. **Style:** Side-by-side comparison, color-coded arrays, memory efficiency visualization, technical data structure diagram."
-->
...
Prompt: "Face-to-Cell Connectivity and BC Implementation. **Main Mesh (Center):** 2D structured grid 4×4 cells (16 cells total). Cells labeled with indices. Internal faces shown as gray lines. **Zoom Panel (Left):** Single cell P highlighted with its 4 faces. Each face labeled: f_N (north), f_S (south), f_E (east), f_W (west). Arrows showing face normal vectors S_f. Owner-neighbour pairs shown: 'If face f is between cells P and N, then owner[f] = P, neighbour[f] = N'. **Boundary Panel (Right):** Boundary cell at corner showing 2 boundary faces. For boundary face: 'internalCoeffs contributes to diagonal', 'boundaryCoeffs contributes to source'. FixedValue BC example: 'φ_b = 100 → internalCoeffs = a_f, boundaryCoeffs = a_f × 100'. FixedGradient BC example: '∂φ/∂n = 0 → internalCoeffs = 0, boundaryCoeffs = 0'. **Bottom:** owner[] and neighbour[] arrays mapping: 'face 5: owner=3, neighbour=7', 'face 20: owner=15, neighbour=-1 (boundary)'. **Style:** Unstructured mesh visualization, clear face labeling, color-coded boundary types, connectivity arrows, technical CFD illustration."
-->
