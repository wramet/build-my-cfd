...
> [!TIP]
> **Mental Model: `fvm` vs `fvc` - อดีต ปัจจุบัน และอนาคต**
>
> จำง่ายๆ เพื่อการเขียนโค้ดที่ถูกต้อง:
>
> 1.  **`fvm::` (Implicit / Future):** คือการถามหา **"ค่าในอนาคต ($n+1$)"** ที่เรา **กำลังจะหาคำตอบ**
>     *   มันจะสร้าง **Coefficient Matrix [A]** เพราะมันต้องแก้สมการถึงจะรู้ค่า
>     *   *ใช้กับ:* ตัวแปรที่เรากำลัง Solve (เช่น `U` ใน `UEqn`)
>
> 2.  **`fvc::` (Explicit / Past & Present):** คือการใช้ **"ค่าปัจจุบัน ($n$) หรืออดีต ($n-1$)"** ที่เรา **รู้อยู่แล้ว**
>     *   มันจะคำนวณออกมาเป็นตัวเลขตรงๆ เลย แล้วเอาไปใส่ใน **Source Vector [b]**
>     *   *ใช้กับ:* ตัวแปรที่เรารู้ค่าแล้ว หรือค่าที่ต้องการ Fix ไว้ (เช่น grad(p) ใน Momentum Eq)
>
> **กฎเหล็ก:** ห้ามใช้ `fvc::` กับเทอมหลักที่คุณต้องการ Solve ไม่งั้นสมการจะไม่มีคำตอบ! (เพราะไม่มีตัวแปร Unknown ใน Matrix)
...
<details>
<summary><b>1. จะเกิดอะไรขึ้นถ้าเราเขียนโค้ดผิดเป็น <code>fvc::div(phi, U)</code> แทนที่จะเป็น <code>fvm::div(phi, U)</code> ในสมการโมเมนตัม?</b></summary>

**คำตอบ:** <b>หายนะ (Disaster)!</b>
`fvc::div` จะคำนวณค่า Divergence จากความเร็วเก่า (Old Time) แล้วส่งกลับมาเป็น Field "ค่าคงที่" ตัวหนึ่ง ไม่ได้สร้าง Matrix Coefficient สำหรับตัวแปร `U` ใหม่
ผลคือ **สมการจะไม่เห็นว่า `U` เป็นตัวแปรที่ต้องหาคำตอบ** (Matrix [A] จะว่างเปล่าหรือผิดเพี้ยน) และ Solver จะแก้สมการไม่ได้ หรือได้คำตอบที่ผิดมหันต์ (เพราะเทอม Convection หายไปจากการเป็น Unknown)
</details>

<details>
<summary><b>2. <code>volScalarField</code> กับ <code>surfaceScalarField</code> ต่างกันอย่างไร และใช้อันไหนเมื่อไหร่?</b></summary>

**คำตอบ:**
*   **volScalarField:** ข้อมูลอยู่ที่ **Core/Center ของ Cell** ใช้เก็บตัวแปรหลักทางฟิสิกส์ เช่น $p, T, \rho$
*   **surfaceScalarField:** ข้อมูลอยู่ที่ **Face ของ Cell** ใช้เก็บ **Flux** ($\phi$)
*   FVM ต้องใช้ `surfaceScalarField` (Flux) ในการคำนวณการขนส่งระหว่างเซลล์ ดังนั้นเรามักจะต้อง Interp `volField` ไปเป็น `surfaceField` ก่อนคำนวณเสมอ
</details>

<details>
<summary><b>3. ทำไมเราต้องมี <code>UEqn.relax()</code> ก่อนที่จะ <code>solve()</code>?</b></summary>

**คำตอบ:** เพื่อ **ความเสถียร (Stability)**
ในระบบสมการที่ซับซ้อน (Non-linear coupled system) การเปลี่ยนแปลงค่าตัวแปรแบบ "ก้าวกระโดด" เกินไปใน Iteration เดียวอาจทำให้ระบบคำนวณระเบิด (Diverge) ได้
การ `relax()` (Under-relaxation) จะช่วย "เบรก" การเปลี่ยนแปลงให้ค่อยเป็นค่อยไป ทำให้ Solver เดินหน้าไปสู่คำตอบที่ถูกต้องได้อย่างมั่นคง
</details>