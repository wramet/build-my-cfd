...
> [!TIP] เปรียบเทียบ: การตรวจรับคอนโดมิเนียม (Condo Inspection Concept)
> - **TestAssertion (การตรวจจุดเล็ก)**: เหมือนการเดินไปเช็ค "ก๊อกน้ำห้องน้ำ" ว่าน้ำไหลไหม, "ปลั๊กไฟหัวเตียง" ว่ามีไฟเข้าไหม (เช็คจุดเดียว เฉพาะเรื่อง)
> - **TestCase (การตรวจทั้งห้อง)**: เหมือนการรวมรายการตรวจทั้งหมดใน "ห้องนอน Master" (เช็คไฟ, แอร์, พื้น, ผนัง)
> - **TestSuite (การตรวจทั้งตึก)**: เหมือนการรวมผลการตรวจทุกห้อง (ห้องนอน, ห้องครัว, ระเบียง) เพื่อสรุปว่า "คอนโดนี้ผ่านการตรวจรับหรือไม่"
...
1. **ถาม:** TestAssertion แตกต่างจาก TestCase อย่างไร?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b> TestAssertion เป็นหน่วยการตรวจสอบที่เล็กที่สุด (เช่น A == B หรือไม่) ส่วน TestCase คือ "กลุ่ม" ของ Assertions ที่รวมตัวกันเพื่อทดสอบฟีเจอร์หรือสถานการณ์หนึ่งๆ (เช่น การทดสอบ Boundary Condition หนึ่ง อาจประกอบด้วย Assertion หลายตัวเพื่อเช็คค่าที่ทุก Face)
   </details>

2. **ถาม:** ทำไมใน Unit Test เราถึงปิดการทำงานแบบ Parallel (`argList::noParallel()`)?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b> เพราะ Unit Test มักทดสอบฟังก์ชันพื้นฐานที่ไม่ขึ้นกับการสื่อสารระหว่าง Processor (เช่น การบวกเลข, การจัดการ Dictionary) การรันแบบ Parallel จะเพิ่มความซับซ้อนและ overhead โดยไม่จำเป็น เว้นแต่จะทดสอบฟีเจอร์ Parallel โดยเฉพาะ
   </details>

3. **ถาม:** หน้าที่ของไฟล์ `Make/options` คืออะไรในการคอมไพล์?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b> กำหนด "ตำแหน่ง" ของ Header files (`-I...`) และ "ชื่อ" ของ Library (`-l...`) ที่ Compiler และ Linker ต้องใช้ เพื่อให้โค้ดของเรารู้จักคลาสและฟังก์ชันต่างๆ ของ OpenFOAM
   </details>
