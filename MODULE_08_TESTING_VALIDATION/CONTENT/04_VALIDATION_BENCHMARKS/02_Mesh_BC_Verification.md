...
> [!TIP] เปรียบเทียบ: รากฐานของบ้าน (House Foundation Analogy)
> - **Mesh Quality**: เปรียบเหมือน "เสาเข็มและรากฐาน" ของบ้าน ถ้าเสาเข็มเอียง (Non-orthogonality) หรือบางเกินไป (Aspect Ratio) บ้านก็เสี่ยงถล่ม (Divergence) หรือเอียง (Inaccurate)
> - **Boundary Conditions**: เปรียบเหมือน "กำแพงและหน้าต่าง" ถ้าเราติดหน้าต่างผิด (กำหนด BC ผิด) เช่น เอากำแพงไปกั้นทางเข้า ลมก็พัดเข้าไม่ได้ (Simulation ไม่เกิดการไหล)
...
> **[ตัวอย่างผลลัพธ์ checkMesh]**:
> ```
> Checking geometry...
>     Overall domain bounding box (-1 -1 -1) (1 1 1)
>     Mesh has 3 geometric determinations: 3D
>     Boundary openness (1.2e-16 1.5e-17 0) OK.
>     Max cell openness = 1.25e-16 OK.
>     Max non-orthogonality = 42.5 OK.
>     Max skewness = 0.35 OK.
> Mesh OK.
> ```

> **[กราฟความแม่นยำ vs คุณภาพเมช]**:
> - **Skewness > 0.8**: ความคลาดเคลื่อนพุ่งสูงขึ้น (>10%) และ Solver มีโอกาส Diverge
> - **Non-ortho > 70°**: Residuals ลู่เข้าช้าลง และอาจเกิด Artifacts ในผลลัพธ์บริเวณนั้น
...
1. **ถาม:** ทำไม **Non-orthogonality** (ความไม่ตั้งฉาก) ถึงส่งผลเสียต่อการคำนวณใน FVM?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b> เพราะ FVM คำนวณ Flux ผ่านหน้าตัดเซลล์ ($\mathbf{S}_f$) โดยสมมติว่า Gradient ของค่าตัวแปร ($\nabla \phi$) ตั้งฉากกับหน้าตัด หากเวกเตอร์เชื่อมศูนย์กลางเซลล์ ($\mathbf{d}$) ไม่ขนานกับเวกเตอร์พื้นที่หน้าตัด ($\mathbf{S}_f$) (เกิดมุม $\theta$) จะทำให้เกิด error ในการประมาณค่า Gradient ซึ่งต้องแก้ด้วย Correction terms ที่อาจทำให้สมการไม่เสถียร
   </details>

2. **ถาม:** ในกรณีใดที่เรายอมรับ **High Aspect Ratio** (เช่น > 100) ได้?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b> ในกรณีของ **Boundary Layer Mesh** ที่ติดผนัง ซึ่งการไหลเปลี่ยนแปลงรวดเร็วในทิศตั้งฉากกับผนัง (Normal direction) แต่เปลี่ยนแปลงช้ามากในทิศขนานกับผนัง (Streamwise direction) การใช้เซลล์แบนยาว (High AR) ช่วยลดจำนวนเซลล์โดยรวมลงได้โดยไม่เสียความแม่นยำมากนัก ตราบใดที่ Flow aligned กับเซลล์
   </details>

3. **ถาม:** ทำไมเราต้องเช็ค **Gradient Consistency** ที่ Boundary แบบ ZeroGradient?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b> เพื่อยืนยันว่าการตั้งค่า Boundary Condition นั้นถูกต้องทางคณิตศาสตร์และไม่มี Discontinuity ที่ผิดปกติเกิดขึ้น โดยทฤษฎีแล้ว ZeroGradient หมายถึง $\frac{\partial \phi}{\partial n} = 0$ ซึ่งแปลว่าค่าที่ Face ควรเท่ากับค่าที่ Cell Center ข้างเคียง หากค่ากระโดดแสดงว่า code หรือ mesh อาจมีปัญหา
   </details>