...
> [!TIP] เปรียบเทียบ: สถานีครัวระดับมิชลิน (Michelin Kitchen Station Analogy)
> - **Isolation**: เชฟแต่ละคนมี "เขียงและมีดส่วนตัว" (Clean Slate) ไม่ใช้ปนกัน เพื่อไม่ให้รสชาติหรือเชื้อโรคปนเปื้อน (Cross-contamination)
> - **Reproducibility**: สูตรอาหารต้องระบุเป็นกรัมและวินาที (Deterministic) ไม่ใช่ "กะเอา" เพื่อให้รสชาติเหมือนเดิมทุกจาน ไม่ว่าจะเสิร์ฟวันไหน
> - **Error Handling**: ถ้าวัตถุดิบเน่า (Error Input) เชฟต้องทิ้งทันที (Catch Exception) ไม่ใช่ฝืนปรุงต่อจนลูกค้าท้องเสีย (System Crash)
...
1. **ถาม:** ทำไมเราควรใช้ **Random Seed** (เช่น `randomSeed 12345`) ใน `controlDict` เมื่อทำการทดสอบ?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b> เพื่อให้ผลลัพธ์มีความสามารถในการผลิตซ้ำ (Reproducibility) โดยเฉพาะเมื่อใช้ Turbulence Models หรือ Initialization ที่มีองค์ประกอบของการสุ่ม หากไม่กำหนด Seed การรันแต่ละครั้งอาจให้ค่าเริ่มต้นต่างกันเล็กน้อย ซึ่งจะขยายผลไปสู่ความแตกต่างของผลลัพธ์สุดท้าย ทำให้ Debug ยาก
   </details>

2. **ถาม:** อะไรคือความแตกต่างระหว่าง **Relative Tolerance** และ **Absolute Tolerance** และเมื่อไหร่ควรใช้แบบไหน?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b>
   - **Relative Tolerance**: เช็ค error เทียบกับขนาดของค่าจริง (เช่น error 1%) ใช้กับค่าทั่วไปที่มีขนาดใหญ่
   - **Absolute Tolerance**: เช็ค error เทียบกับค่าคงที่ (เช่น error < 1e-6) ใช้กับค่าที่มีขนาดเล็กมากหรือใกล้ศูนย์ (ซึ่ง Relative error อาจมหาศาลได้แม้ความต่างจริงจะน้อยนิด)
   - **Best Practice**: ควรใช้ Combined Tolerance คือยอมรับถ้าผ่านเกณฑ์อย่างใดอย่างหนึ่ง
   </details>

3. **ถาม:** ทำไม **Unit Test** ถึงควรใช้เวลาแค่ "ระดับวินาที" (Seconds)?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b> เพราะ Unit Test ต้องถูกรันบ่อยครั้ง (เช่น ทุกครั้งที่ Save ไฟล์หรือ Compile) เพื่อให้ Feedback loop ของนักพัฒนาสั้นที่สุด หาก Unit Test รันนาน นักพัฒนาจะเลิกรันมันบ่อยๆ ทำให้บั๊กหลุดรอดไปได้นานขึ้น
   </details>