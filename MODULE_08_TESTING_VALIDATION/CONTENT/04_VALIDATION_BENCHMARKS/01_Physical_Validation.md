...
> [!TIP] เปรียบเทียบ: การถ่ายเทความร้อนผ่านผนังบ้าน (Heat Loss Analogy)
> - **1D Conduction**: เปรียบเสมือนความร้อนที่ไหลจากภายในบ้าน (แอร์เย็น) ออกสู่ภายนอก (อากาศร้อน) ผ่านผนังปูนหนา
> - **Analytical Solution**: คำนวณได้ง่ายๆ จากกฎของฟูริเยร์ (Fourier's Law) ว่าอุณหภูมิจะลดลงอย่างเป็นเส้นตรงจากผิวนอกสู่ผิวใน
> - **CFD Simulation**: เราแบ่งผนังเป็นชิ้นเล็กๆ (Cells) แล้วคำนวณสมดุลความร้อนในแต่ละชิ้น ซึ่งผลรวมต้องได้กราฟเส้นตรงเหมือนกันเป๊ะๆ
...
### 📉 กราฟเปรียบเทียบ Velocity Profile
> ![Velocity Profile Comparison: Analytical vs OpenFOAM]
> - **analytical**: $u(y) = \frac{1}{2\mu} \frac{dp}{dx} (y^2 - hy)$
> - **OpenFOAM**: ผลลัพธ์จาก `simpleFoam` บนเมช 50 เซลล์
> - **Error**: สูงสุด < 0.1% บริเวณกึ่งกลางท่อ
...
### 📊 ผลเปรียบเทียบกับการทดลอง (Experimental Validation)
> ![Validation Graph: $C_p$ distribution on NACA 0012]
> - **Experiment**: NASA Langley (1982)
> - **CFD**: `simpleFoam` (Spalart-Allmaras turbulence model)
> - **Matching**: สอดคล้องกันอย่างดีเยี่ยม โดยเฉพาะบริเวณ Leading Edge ที่มี Gradient สูง
...
> **ตารางบันทึกผลจริง (Example Real Results):**
> 
> | Mesh Level | Cells ($N$) | $C_D$ | $\Delta C_D$ (%) | GCI (%) |
> |------------|-------------|-------|------------------|---------|
> | M1 (Coarse)| 50,000      | 0.421 | -                | -       |
> | M2 (Medium)| 100,000     | 0.408 | 3.09%            | 4.2%    |
> | M3 (Fine)  | 200,000     | 0.405 | 0.73%            | 1.1%    |
> 
> *วิเคราะห์: ค่า $C_D$ เริ่มลู่เข้าเมื่อจำนวนเซลล์เพิ่มขึ้น และ GCI ลดลงเหลือ 1.1% ในเมชระดับ Fine แสดงว่าผลลัพธ์มีความน่าเชื่อถือสูง*
...
1. **ถาม:** ในการเปรียบเทียบกับข้อมูลการทดลอง ทำไมเราถึงต้องจับคู่ **Dimensionless Parameters** (เช่น $Re, Ma$) แทนที่จะจับคู่ค่าทางกายภาพ (เช่น $U, L, \nu$) โดยตรง?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b> เพื่อให้มั่นใจถึง **Dynamic Similarity** คือพฤติกรรมการไหลจะเหมือนกันถ้า Dimensionless Parameters ตรงกัน แม้ว่าขนาดจริง ($L$) หรือความเร็ว ($U$) หรือชนิดของไหล ($\nu$) จะต่างกัน (เช่น ทดสอบโมเดลเครื่องบินขนาดเล็กในอุโมงค์ลม แต่พฤติกรรมเหมือนเครื่องบินจริง)
   </details>

2. **ถาม:** ค่า **GCI (Grid Convergence Index)** บอกอะไรเราเกี่ยวกับการจำลอง?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b> GCI บอกถึง "ช่วงความเชื่อมั่น" (Error Band) ของผลลัพธ์ว่าห่างจาก "ค่าที่แท้จริงทางคณิตศาสตร์" (Asymptotic value) มากที่สุดเท่าไหร่ หาก GCI ต่ำแดงว่าผลลัพธ์นั้น Stable และ Independent จาก Mesh แล้ว
   </details>

3. **ถาม:** ทำไมในการ Validation เคส 1D Heat Conduction เราถึงปิดเทอม `divSchemes` (ตั้งเป็น `none`)?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b> เพราะสมการ 1D Steady Heat Conduction ($\nabla^2 T = 0$) ไม่มีเทอม Convection (การพาความร้อนเนื่องจากการไหล, $\nabla \cdot (\mathbf{U} T)$) มีแต่เทอม Diffusion (การแพร่อุณหภูมิ) ดังนั้นจึงไม่ต้องใช้ Divergence Scheme
   </details>