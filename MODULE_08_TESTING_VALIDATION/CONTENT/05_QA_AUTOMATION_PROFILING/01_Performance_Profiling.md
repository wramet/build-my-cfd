...
> [!TIP] เปรียบเทียบ: การขยายถนน (Traffic Analogy)
> - **Strong Scaling**: เปรียบเหมือนการเพิ่มจำนวนเลนบนถนนเดิม (เพิ่ม CPU) เพื่อระบายรถจำนวนคงที่ (Fixed Problem) ให้เร็วขึ้น แรกๆ รถวิ่งคล่องตัวมาก (Linear Speedup) แต่พอเพิ่มเลนไปเรื่อยๆ รถเริ่มเบียดกันแย่งเลนเอง (Communication Overhead) จนการเพิ่มเลนไม่ช่วยอะไร
> - **Weak Scaling**: เปรียบเหมือนการสร้างเมืองใหม่ที่ใหญ่ขึ้น (Increased Problem) พร้อมกับสร้างถนนให้กว้างขึ้นตามสัดส่วน (Increased CPU) ถ้าระบบจัดการจราจรดี (Efficient Algorithm) การจราจรในเมืองใหม่ควรจะคล่องตัวเท่ากับเมืองเก่า
> - **Amdahl's Law**: เปรียบเหมือนด่านเก็บเงินทางด่วน (Serial Bottleneck) ต่อให้ถนนกว้าง 100 เลน แต่ถ้าทุกคนต้องมาต่อคิวที่ด่านเก็บเงินช่องเดียว รถก็ติดอยู่ดี
...
### 💹 กราฟตัวอย่างและข้อมูลวิเคราะห์ (Example Data)
> 1. **Strong Scaling**: กราฟแสดง Speedup ที่ลู่เข้าสู่ค่าคงที่เมื่อจำนวน CPU มากขึ้น (เนื่องจาก Communication Overhead)
> 2. **Memory Profiling**: กราฟจาก Massif แสดงการจองหน่วยความจำแบบคงที่ (Stable) ตลอดการรัน (ไม่มี Leak)
> 3. **Scheme Comparison**:
>    | Scheme | Computational Time (s) | Accuracy |
>    | :--- | :--- | :--- |
>    | Upwind | 850 | Low |
>    | LinearUpwind | 1200 | High |
>    | LimitedLinear | 1050 | Medium-High |
> 4. **Parallel Heatmap**: แสดงว่า Master Processor ใช้เวลาทำ I/O นานกว่า Worker Processors (Bottleneck)
...
1. **ถาม:** ถ้าเราใช้ **Strong Scaling** แล้วพบว่า Efficiency ลดลงเหลือ 50% อย่างรวดเร็ว สาเหตุที่น่าจะเป็นไปได้มากที่สุดคืออะไร?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b> สาเหตุหลักมักมาจาก **Communication Overhead** ที่สูงเกินไปเมื่อเทียบกับ Computation (อัตราส่วน Surface-to-Volume ของ Sub-domain สูง) หรือมี **Serial Part** ที่สำคัญ (เช่น I/O หรือ Global Reduction) ตามกฎ Amdahl's Law
   </details>

2. **ถาม:** ทำไม **GAMG (Geometric-Algebraic Multi-Grid)** Solver ถึงมักจะให้ประสิทธิภาพดีกว่า PCG ในการแก้สมการ Pressure สำหรับปัญหาขนาดใหญ่?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b> เพราะ GAMG ใช้เทคนิค Multi-grid ที่แก้สมการบน Mesh หยาบเพื่อกำจัด Low-frequency error ได้รวดเร็ว แล้วค่อย map กลับมา Mesh ละเอียด ทำให้สามารถลู่เข้า (Converge) ได้เร็วกว่า PCG ที่ทำงานบน Fine Mesh เพียงอย่างเดียว ซึ่งต้องใช้จำนวน Iteration มากกว่าเมื่อ Mesh ละเอียดขึ้น
   </details>

3. **ถาม:** **Memory Leaks** แตกต่างจาก **High Memory Usage** อย่างไร?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b> **High Memory Usage** คือโปรแกรมจำเป็นต้องใช้ RAM เยอะแต่จัดการคืนเมื่อเลิกใช้ (เป็นพฤติกรรมปกติถ้างานใหญ่) ส่วน **Memory Leaks** คือความผิดปกติที่โปรแกรมจองหน่วยความจำแล้ว "ลืมคืน" (ไม่เรียก delete หรือ free) ทำให้ RAM ถูกกินไปเรื่อยๆ จนหมดเครื่องแม้ขนาดงานจะเท่าเดิม
   </details>