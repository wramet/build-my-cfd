...
> [!TIP] เปรียบเทียบ: หุ่นทดสอบการชน (Crash Test Dummy Analogy)
> - **Reference Result**: เหมือนกับ "หุ่นทดสอบมาตรฐาน" ที่ผ่านการสอบเทียบ (Calibration) แล้วว่าตอบสนองต่อแรงกระแทกเหมือนคนจริง
> - **Regression Test**: เมื่อเราออกแบบรถรุ่นใหม่ (New Code) เราต้องเอาหุ่นตัวเดิมมาทดสอบชนอีกรอบ เพื่อดูว่าถุงลมนิรภัยยังทำงานถูกต้องเหมือนรุ่นก่อนไหม ถ้าหุ่นหัวหลุด (Crash) หรือเซนเซอร์ขานค่าผิด (Incorrect Value) แสดงว่าการออกแบบใหม่มีปัญหา (Regression)
> - **Tolerance**: เราไม่ได้คาดหวังให้แขนหุ่นสะบัดไปที่องศาเดิมเป๊ะๆ (Exact Match) แต่ค่าแรงกระแทกที่หัว (HIC Score) ต้องไม่เกินเกณฑ์ที่ปลอดภัย (Statistical Tolerance)
...
### 📉 กราฟวิเคราะห์ Regression
> - **Error Evolution**: กราฟแสดงแนวโน้ม Error ระหว่างเวอร์ชัน v1.0 ถึง v2.0
> - **Outlier Detection**: จุดสีแดงบนกราฟที่กระโดดออกจากกลุ่ม (บ่งบอกว่าบาง Cell มีความปกติ)
> - **L2 Norm Log**: ตารางบันทึกค่า L2 Norm สำหรับทุกๆ 50 Timesteps เพื่อเทียบความนิ่งของผลลัพธ์ระหว่างเวอร์ชัน
```
...
1. **ถาม:** เมื่อไหร่ควรใช้ **Statistical Match** แทนที่จะใช้ **Exact Match** ในการทดสอบ CFD?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b> เมื่อเราเปลี่ยนสภาพแวดล้อมการคำนวณ เช่น เปลี่ยน Compiler Version, เปลี่ยนระดับ Optimization (-O2 vs -O3), หรือเปลี่ยนจำนวน Processor ในการรันขนาน ซึ่งมักจะทำให้ลำดับการคำนวณ Floating Point เปลี่ยนไป ส่งผลให้ผลลัพธ์ท้ายๆ เปลี่ยนแปลงเล็กน้อย (Machine Epsilon level) แต่ยังถูกต้องทางฟิสิกส์
   </details>

2. **ถาม:** **CI/CD Pipeline** ช่วยลดปัญหา "It works on my machine" ได้อย่างไร?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b> เพราะ CI/CD จะรันการทดสอบในสภาพแวดล้อมกลาง (Clean Environment) ที่ถูกกำหนดไว้ชัดเจน (เช่น Docker Container) ซึ่งเป็นกลางและเหมือนกันทุกครั้ง ทำให้มั่นใจได้ว่าโค้ดที่ผ่านการทดสอบจะทำงานได้จริงๆ ไม่ใช่แค่เพราะมีการตั้งค่าเฉพาะในเครื่องของผู้พัฒนา
   </details>

3. **ถาม:** ทำไมเราต้องแยก **Reference Database** ตามเวอร์ชันของ OpenFOAM?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b> เพราะ OpenFOAM เวอร์ชันต่างกันอาจมีการปรับปรุง Numerical Schemes, Physical Models หรือ Solver Algorithms ซึ่งอาจทำให้ผลลัพธ์ที่ "ถูกต้อง" เปลี่ยนไปเล็กน้อย การใช้ Reference ข้ามเวอร์ชันอาจทำให้เกิด False Failure ได้
   </details>