...
> [!TIP] เปรียบเทียบ: การตรวจสุขภาพ (Medical Checkup Analogy)
> - **Validation Scripts**: เปรียบเหมือน "เครื่องวัดสัญญาณชีพ" (Pulse Oximeter, เครื่องวัดความดัน) ที่คอย Monitor สุขภาพของ Solver ตลอดเวลา
> - **Key Parameters (Drag, Lift, Mass Balance)**: เปรียบเหมือน "ชีพจร, ความดัน, อุณหภูมิร่างกาย" ถ้าค่าไหนผิดปกติ (เกิน Tolerance) หมอ (User) จะรู้ทันทีและวินิจฉัยโรคได้ถูกจุด
...
### 📉 ผลลัพธ์ตัวอย่าง (Sample Results):
> - **Velocity Profile**: กราฟเปรียบเทียบระหว่าง `U` จาก Simulation (จุดสีน้ำเงิน) และ Analytical Solution (เส้นสีแดง) พบว่ามีความคลาดเคลื่อนเฉลี่ย 1.5%
> - **Convergence**: กราฟแสดง Residuals ลดลงต่ำกว่า $10^{-6}$ ภายใน 200 รอบการคำนวณ
> - **Error Table**:
>   | Mesh Level | $u_{max}$ Error | $u_{mean}$ Error | Status |
>   | :--- | :--- | :--- | :--- |
>   | Coarse | 4.2% | 3.5% | [FAIL] |
>   | Medium | 1.8% | 1.2% | [PASS] |
>   | Fine | 0.8% | 0.5% | [PASS] |
...
1. **ถาม:** ในการ Validation "Channel Flow" ทำไมเราต้องกำหนด Tolerance สำหรับ Mean Velocity ($u_{mean}$) และ Maximum Velocity ($u_{max}$) แยกกันหรือทั้งคู่?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b> การตรวจสอบทั้งคู่ช่วยให้มั่นใจว่า Profile ของความเร็วถูกต้องทั้งรูปร่าง ($u_{max}$ เช็คจุดสูงสุด) และปริมาณอัตราการไหลรวม ($u_{mean}$ สัมพันธ์กับ mass flow rate) บางครั้ง Profile อาจผิดรูปแต่มี Mean เท่าเดิมก็เป็นได้
   </details>

2. **ถาม:** ทำไมเราต้องตรวจสอบ Mass Balance โดยใช้ Relative Imbalance?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b> เพราะค่า Net Flux (FluxIn - FluxOut) เพียงอย่างเดียวบอกไม่ได้ว่า Errors มากหรือน้อยถ้าไม่เทียบกับ Total Flux ที่ไหลเข้า เช่น Error 1 kg/s อาจจะเยอะมากถ้าไหลเข้าแค่ 10 kg/s (10%) แต่จะน้อยมากถ้าไหลเข้า 1,000,000 kg/s (0.0001%)
   </details>

3. **ถาม:** ฟังก์ชัน `lookupOrDefault` ใน OpenFOAM Dictionary มีประโยชน์อย่างไรในการเขียน Validation Scripts?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b> ช่วยให้โค้ดมีความยืดหยุ่น (Robust) หาก User ลืมระบุค่าบางค่าในไฟล์ Dictionary โปรแกรมจะยังทำงานต่อได้โดยใช้ค่า Default ที่ตั้งไว้ แทนที่จะ Crash หรือ Error
   </details>
2. **Verification and Validation in Computational Science and Engineering** by Oberkampf & Trucano
3. **CFD Online**: https://www.cfd-online.com/Wiki/Best_practice_guidelines_for_CFD