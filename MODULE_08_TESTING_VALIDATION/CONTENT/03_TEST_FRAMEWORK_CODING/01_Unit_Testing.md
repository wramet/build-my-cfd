...
> [!TIP] เปรียบเทียบ: การทดสอบรถยนต์ (Car Testing Analogy)
> - **Unit Testing**: เหมือนการทดสอบ "ชิ้นส่วน" แยกกัน (ทดสอบหัวเทียน, ทดสอบเบรกบนแท่น, ทดสอบเกียร์) ก่อนจะประกอบเข้าด้วยกัน
> - **Simulation/Full Application**: เหมือนการ "Test Drive" ขับรถจริงบนถนน
> ถ้าเราไม่ทำ Unit Test แล้วประกอบรถเลย หากรถสตาร์ทไม่ติด เราจะไม่รู้เลยว่าเป็นที่หัวเทียน แบตเตอรี่ หรือระบบน้ำมัน!
...
1. **ถาม:** ทำไมเราถึงใช้ $\max(\epsilon_{abs}, \epsilon_{rel} \cdot |A|)$ ใน Combined Tolerance?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b> เพื่อให้ครอบคลุมทั้ง 2 กรณี: (1) เมื่อค่า $A$ มีขนาดใหญ่มาก เราต้องการตรวจสอบความผิดพลาดเป็น **%** (Relative) (2) เมื่อค่า $A$ ใกล้ศูนย์ (เช่น Residuals) ค่า Relative Error จะคำนวณไม่ได้ (หารด้วยศูนย์) เราจึงต้องใช้ Absolute Tolerance มากำกับ
   </details>

2. **ถาม:** การทดสอบ `fvc::div(U)` โดยกำหนดให้ `U` เป็นค่าคงที่ (Constant Velocity) มีวัตถุประสงค์เพื่ออะไร?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b> เพื่อตรวจสอบว่า Operator คำนวณค่า Divergence ได้ถูกต้องตามทฤษฎีหรือไม่ ซึ่งสำหรับสนามความเร็วคงที่ Divergence ต้องเป็น **ศูนย์** เสมอ ($\nabla \cdot \mathbf{const} = 0$) เป็นการเช็คความ Consistency ของ Scheme
   </details>

3. **ถาม:** "Test Isolation" มีความสำคัญอย่างไรใน Unit Testing ของ OpenFOAM?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b> ป้องกัน **Side Effects** (ผลข้างเคียง) ระหว่างการทดสอบ เช่น ถ้า Test หนึ่งเปลี่ยนแปลงค่าใน Mesh หรือ Boundary Condition แล้วไม่ได้คืนค่าเดิม Test ถัดไปที่ใช้ Mesh นั้นอาจพัง (Fail) โดยไม่ใช่ความผิดของมันเอง การแยก Environment (เช่น สร้าง Mesh ใหม่ทุกครั้ง) ช่วยแก้ปัญหานี้
   </details>