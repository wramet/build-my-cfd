...
> [!TIP] เปรียบเทียบ: ระบบสายพานการผลิตรถยนต์ (Factory Assembly Line Analogy)
> - **Alltest/Automation**: เปรียบเหมือน "สายพานการผลิตอัตโนมัติ" ที่มีหุ่นยนต์ QC ตรวจสอบทุกขั้นตอน
> - **Manual Testing**: เปรียบเหมือน "ช่างฝีมือ" ที่ตรวจรถทีละคันด้วยมือ ซึ่งช้าและอาจพลาดจุดเล็กๆ น้อยๆ ได้
> - **CI/CD**: เปรียบเหมือน "เซนเซอร์แจ้งเตือนทันที" เมื่อมีชิ้นส่วนไหนไม่ได้มาตรฐาน สายพานจะหยุดและแจ้งเตือนวิศวกรทันที
...
1. **ถาม:** ทำไมใน `Alltest` script ถึงต้องมีการตรวจสอบ Exit Code ของการรัน `Allrun`?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b> เพื่อให้ Master Script รู้สถานะความสำเร็จของแต่ละ Test Case ว่าผ่าน (Exit Code 0) หรือไม่ผ่าน (Exit Code Non-zero) และสามารถสรุปผลรวม (Pass/Fail Count) ได้อย่างถูกต้อง หากไม่เช็ค Exit Code เราจะไม่รู้เลยว่า Solver รันจบสมบูรณ์หรือ Crash กลางคัน
   </details>

2. **ถาม:** ประโยชน์หลักของการแยก `Allrun` script ไว้ในแต่ละโฟลเดอร์ Test Case คืออะไร?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b> ทำให้แต่ละ Test Case มีความ **Atomic** และ **Self-contained** คือมีขั้นตอนการรันเฉพาะตัว (เช่น บางเคสต้องรัน Mesh พิเศษ, บางเคสต้องรัน Parallel) โดยไม่กระทบกับเคสอื่น และง่ายต่อการ Debug แยกต่างหาก
   </details>

3. **ถาม:** ในการทำ CI/CD, "Regression Testing" มีความหมายว่าอย่างไร?
   <details>
   <summary>เฉลย</summary>
   <b>ตอบ:</b> คือการทดสอบเพื่อยืนยันว่า "ของเก่าไม่พัง" เมื่อมีการแก้โค้ดใหม่ (เช่น แก้ Bug A แล้วไปทำให้ฟีเจอร์ B พัง) หรือผลลัพธ์การคำนวณยังคงเดิม (ภายใน Tolerance) เมื่อเทียบกับผลลัพธ์ที่เคยถูกต้องในอดีต (Baseline)
   </details>
...
### 📑 กรณีศึกษา: ระบบ CI/CD สำหรับ MySolver
> 1. **Automated Run**: ทุกครั้งที่มีการแก้ไข `UEqn.H`, สคริปต์ `Alltest` จะรันกรณีศึกษา `pitzDaily` ทันที
> 2. **Result Check**: เทียบค่า `k` และ `epsilon` กับค่า Baseline
> 3. **Summary Table**:
>    | Version | Alltest Status | Wall Time (s) | Memory (MB) |
>    | :--- | :--- | :--- | :--- |
>    | v1.0 (Ref) | PASS | 120 | 450 |
>    | v1.1 (Modified)| PASS | 105 (-12%) | 452 (+0.4%) |
>    | v1.2 (Latest) | FAIL | N/A | N/A |
> *วิเคราะห์: เวอร์ชัน 1.1 ทำงานเร็วขึ้นโดยไม่เสียความแม่นยำ ส่วนเวอร์ชัน 1.2 เกิด Crash (Divergent) ต้องตรวจสอบแก้ไข*