...
def log_info(msg):
    ...
    if _logger_enabled:
        _logger.log_task_start(msg)

def log_complete(msg, result=None):
    """Log completion message if logger is available."""
    if _logger_enabled:
        _logger.log_task_complete(msg, result)
...
def main():
    ...
    if args.mode == 'hierarchy':
        log_info(f"Gate 1: Ground Truth Extraction - {args.path}")
        ...
        log_complete(f"Gate 1: Ground Truth Extraction", f"Extracted {len(hierarchy)} classes from {args.path}")
